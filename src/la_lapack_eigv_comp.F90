...
module la_lapack_eigv_comp
...
#ifdef LA_WITH_XDP
     !> XGGBAK: forms the right or left eigenvectors of a real generalized
     !> eigenvalue problem A*x = lambda*B*x, by backward transformation on
     !> the computed eigenvectors of the balanced pair of matrices output by
     !> XGGBAL.

     pure subroutine la_xggbak(job,side,n,ilo,ihi,lscale,rscale,m,v,ldv,info)
        ...
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ...
           integer(ilp),intent(out) :: info
           ! Array Arguments
           real(xdp),intent(in) :: lscale(*),rscale(*)
        ...
        ! =====================================================================
           ...
           if (la_lsame(job,'S') .or. la_lsame(job,'B')) then
              ...
              if (rightv) then
                 do i = ilo,ihi
                    call la_xscal(m,rscale(i),v(i,1),ldv)
                 end do
              end if
              ! backward transformation on left eigenvectors
              if (leftv) then
                 do i = ilo,ihi
                    call la_xscal(m,lscale(i),v(i,1),ldv)
                 end do
              end if
           end if
           ! backward permutation
...
#ifdef LA_WITH_XDP
     !> XGGBAL: balances a pair of general real matrices (A,B).  This
     !> involves, first, permuting A and B by similarity transformations to
     !> isolate eigenvalues in the first 1 to ILO$-$1 and last IHI+1 to N
     !> elements on the diagonal; and second, applying a diagonal similarity
     !> transformation to rows and columns ILO to IHI to make the rows
     ...
     pure subroutine la_xggbal(job,n,a,lda,b,ldb,ilo,ihi,lscale,rscale,work,info)
        ...
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ...
           real(xdp),intent(out) :: lscale(*),rscale(*),work(*)
        ! =====================================================================
           ! Parameters
           real(xdp),parameter :: sclfac = 1.0e+1_xdp

           ! Local Scalars
           integer(ilp) :: i,icab,iflow,ip1,ir,irab,it,j,jc,jp1,k,kount,l,lcab,lm1, &
                     lrab,lsfmax,lsfmin,m,nr,nrp2
           real(xdp) :: alpha,basl,beta,cab,cmax,coef,coef2,coef5,cor,ew,ewc,gamma, &
                     pgamma,rab,sfmax,sfmin,sum,t,ta,tb,tc
           ! Intrinsic Functions
           intrinsic :: abs,real,int,log10,max,min,sign
           ! Executable Statements
           ...
           lscale(m) = i
           if (i == m) go to 170
           call la_xswap(n - k + 1,a(i,k),lda,a(m,k),lda)
           call la_xswap(n - k + 1,b(i,k),ldb,b(m,k),ldb)
           ! permute columns m and j
           170 continue
           rscale(m) = j
           if (j == m) go to 180
           call la_xswap(l,a(1,j),1,a(1,m),1)
           call la_xswap(l,b(1,j),1,b(1,m),1)
           180 continue
           go to(20,90) iflow
           190 continue
           ilo = k
           ...
           ! start generalized conjugate gradient iteration
           250 continue
           gamma = la_xdot(nr,work(ilo + 4*n),1,work(ilo + 4*n),1) + la_xdot(nr, &
                     work(ilo + 5*n),1,work(ilo + 5*n),1)
           ew = zero
           ewc = zero
           do i = ilo,ihi
           ...
           t = coef5*(ewc - three*ew)
           tc = coef5*(ew - three*ewc)
           call la_xscal(nr,beta,work(ilo),1)
           call la_xscal(nr,beta,work(ilo + n),1)
           call la_xaxpy(nr,coef,work(ilo + 4*n),1,work(ilo + n),1)
           call la_xaxpy(nr,coef,work(ilo + 5*n),1,work(ilo),1)
           do i = ilo,ihi
              work(i) = work(i) + tc
              work(i + n) = work(i + n) + t
           end do
           ...
           do i = ilo,ihi
              ...
              work(i + 2*n) = real(kount,KIND=xdp)*work(i + n) + sum
           end do
           do j = ilo,ihi
              kount = 0
              sum = zero
              ...
              work(j + 3*n) = real(kount,KIND=xdp)*work(j) + sum
           end do
           sum = la_xdot(nr,work(ilo + n),1,work(ilo + 2*n),1) + la_xdot(nr,work( &
                     ilo),1,work(ilo + 3*n),1)
           alpha = gamma/sum
           ! determine correction to current iteration
           cmax = zero
           ...
           if (cmax < half) go to 350
           call la_xaxpy(nr,-alpha,work(ilo + 2*n),1,work(ilo + 4*n),1)
           call la_xaxpy(nr,-alpha,work(ilo + 3*n),1,work(ilo + 5*n),1)
           pgamma = gamma
           it = it + 1
           if (it <= nrp2) go to 250
           ! end generalized conjugate gradient iteration
           ...
           lsfmax = int(log10(sfmax)/basl,KIND=ilp)
           do i = ilo,ihi
              irab = la_ixamax(n - ilo + 1,a(i,ilo),lda)
              rab = abs(a(i,irab + ilo - 1))
              irab = la_ixamax(n - ilo + 1,b(i,ilo),ldb)
              rab = max(rab,abs(b(i,irab + ilo - 1)))
              lrab = int(log10(rab + sfmin)/basl + one,KIND=ilp)
              ir = int(lscale(i) + sign(half,lscale(i)),KIND=ilp)
              ir = min(max(ir,lsfmin),lsfmax,lsfmax - lrab)
              lscale(i) = sclfac**ir
              icab = la_ixamax(ihi,a(1,i),1)
              cab = abs(a(icab,i))
              icab = la_ixamax(ihi,b(1,i),1)
              cab = max(cab,abs(b(icab,i)))
              lcab = int(log10(cab + sfmin)/basl + one,KIND=ilp)
              jc = int(rscale(i) + sign(half,rscale(i)),KIND=ilp)
              jc = min(max(jc,lsfmin),lsfmax,lsfmax - lcab)
           ...
           ! row scaling of matrices a and b
           do i = ilo,ihi
              call la_xscal(n - ilo + 1,lscale(i),a(i,ilo),lda)
              call la_xscal(n - ilo + 1,lscale(i),b(i,ilo),ldb)
           end do
           ! column scaling of matrices a and b
           do j = ilo,ihi
              call la_xscal(ihi,rscale(j),a(1,j),1)
              call la_xscal(ihi,rscale(j),b(1,j),1)
           end do
           return
     end subroutine la_xggbal
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
     !> XGGHRD: reduces a pair of real matrices (A,B) to generalized upper
     !> Hessenberg form using orthogonal transformations, where A is a
     !> general matrix and B is upper triangular.  The form of the
     !> generalized eigenvalue problem is
     !> A*x = lambda*B*x,
     ...
     pure subroutine la_xgghrd(compq,compz,n,ilo,ihi,a,lda,b,ldb,q,ldq,z,ldz, &
               info)
        use la_constants_xdp
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ...
        ! =====================================================================
           ...
           if (info /= 0) then
              call la_xerbla('XGGHRD',-info)
              return
           end if
           ! initialize q and z if desired.
           if (icompq == 3) call la_xlaset('FULL',n,n,zero,one,q,ldq)
           if (icompz == 3) call la_xlaset('FULL',n,n,zero,one,z,ldz)
           ! quick return if possible
           if (n <= 1) return
           ! zero out lower triangle of b
           do jcol = 1,n - 1
           ...
           do jcol = ilo,ihi - 2
              do jrow = ihi,jcol + 2,-1
                 ! step 1: rotate rows jrow-1, jrow to kill a(jrow,jcol)
                 temp = a(jrow - 1,jcol)
                 call la_xlartg(temp,a(jrow,jcol),c,s,a(jrow - 1,jcol))
                 a(jrow,jcol) = zero
                 call la_xrot(n - jcol,a(jrow - 1,jcol + 1),lda,a(jrow,jcol + 1),lda,c,s)

                 call la_xrot(n + 2 - jrow,b(jrow - 1,jrow - 1),ldb,b(jrow,jrow - 1),ldb,c, &
                           s)
                 if (ilq) call la_xrot(n,q(1,jrow - 1),1,q(1,jrow),1,c,s)
                 ! step 2: rotate columns jrow, jrow-1 to kill b(jrow,jrow-1)
                 temp = b(jrow,jrow)
                 call la_xlartg(temp,b(jrow,jrow - 1),c,s,b(jrow,jrow))
                 b(jrow,jrow - 1) = zero
                 call la_xrot(ihi,a(1,jrow),1,a(1,jrow - 1),1,c,s)
                 call la_xrot(jrow - 1,b(1,jrow),1,b(1,jrow - 1),1,c,s)
                 if (ilz) call la_xrot(n,z(1,jrow),1,z(1,jrow - 1),1,c,s)
              end do
           end do
           return
     end subroutine la_xgghrd
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
     !> XGGHD3: reduces a pair of real matrices (A,B) to generalized upper
     !> Hessenberg form using orthogonal transformations, where A is a
     !> general matrix and B is upper triangular.  The form of the
     !> generalized eigenvalue problem is
     !> A*x = lambda*B*x,
     ...
     !> Q1 * B * Z1**T = (Q1*Q) * T * (Z1*Z)**T
     !> If Q1 is the orthogonal matrix from the QR factorization of B in the
     !> original equation A*x = lambda*B*x, then XGGHD3 reduces the original
     !> problem to generalized Hessenberg form.
     !> This is a blocked variant of XGGHRD, using matrix-matrix
     !> multiplications for parts of the computation to enhance performance.

     pure subroutine la_xgghd3(compq,compz,n,ilo,ihi,a,lda,b,ldb,q,ldq,z,ldz, &
               work,lwork,info)
        use la_constants_xdp
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
       ...
       ! =====================================================================
           ...
           integer(ilp) :: cola,i,ierr,j,j0,jcol,jj,jrow,k,kacc22,len,lwkopt,n2nb,nb, &
                      nblst,nbmin,nh,nnb,nx,ppw,ppwo,pw,top,topq
           real(xdp) :: c,c1,c2,s,s1,s2,temp,temp1,temp2,temp3
           ! Intrinsic Functions
           intrinsic :: real,max
           ! Executable Statements
           ! decode and test the input parameters.
           info = 0
           nb = la_ilaenv(1,'XGGHD3',' ',n,ilo,ihi,-1)
           lwkopt = max(6*n*nb,1)
           ...
           if (info /= 0) then
              call la_xerbla('XGGHD3',-info)
              return
           else if (lquery) then
              return
           end if
           ! initialize q and z if desired.
           if (initq) call la_xlaset('ALL',n,n,zero,one,q,ldq)
           if (initz) call la_xlaset('ALL',n,n,zero,one,z,ldz)
           ! zero out lower triangle of b.
           if (n > 1) call la_xlaset('LOWER',n - 1,n - 1,zero,zero,b(2,1),ldb)
           ! quick return if possible
           nh = ihi - ilo + 1
           if (nh <= 1) then
              work(1) = one
           ...
           ! determine the blocksize.
           nbmin = la_ilaenv(2,'XGGHD3',' ',n,ilo,ihi,-1)
           if (nb > 1 .and. nb < nh) then
              ! determine when to use unblocked instead of blocked code.
              nx = max(nb,la_ilaenv(3,'XGGHD3',' ',n,ilo,ihi,-1))
              if (nx < nh) then
                 ! determine if workspace is large enough for blocked code.
                 if (lwork < lwkopt) then
                    ! not enough workspace to use optimal nb:  determine the
                    ! minimum value of nb, and reduce nb or force use of
                    ! unblocked code.
                    nbmin = max(2,la_ilaenv(2,'XGGHD3',' ',n,ilo,ihi,-1))
                    if (lwork >= 6*n*nbmin) then
                       nb = lwork/(6*n)
                    else
                       nb = 1
           ...
           if (nb < nbmin .or. nb >= nh) then
              ...
              ! use blocked code
              kacc22 = la_ilaenv(16,'XGGHD3',' ',n,ilo,ihi,-1)
              blk22 = kacc22 == 2
              do jcol = ilo,ihi - 2,nb
                 nnb = min(nb,ihi - jcol - 1)
                 ! initialize small orthogonal factors that will hold the
                 ...
                 n2nb = (ihi - jcol - 1)/nnb - 1
                 nblst = ihi - jcol - n2nb*nnb
                 call la_xlaset('ALL',nblst,nblst,zero,one,work,nblst)
                 pw = nblst*nblst + 1
                 do i = 1,n2nb
                    call la_xlaset('ALL',2*nnb,2*nnb,zero,one,work(pw),2*nnb)
                    pw = pw + 4*nnb*nnb
                 end do
                 ! reduce columns jcol:jcol+nnb-1 of a to hessenberg form.
                 do j = jcol,jcol + nnb - 1
                    ...
                    do i = ihi,j + 2,-1
                       temp = a(i - 1,j)
                       call la_xlartg(temp,a(i,j),c,s,a(i - 1,j))
                       a(i,j) = c
                       b(i,j) = s
                    end do
                    ! accumulate givens rotations into workspace array.
                    ...
                    do jj = n,j + 1,-1
                       ...
                       if (jj < ihi) then
                          temp = b(jj + 1,jj + 1)
                          call la_xlartg(temp,b(jj + 1,jj),c,s,b(jj + 1,jj + 1))
                          b(jj + 1,jj) = zero
                          call la_xrot(jj - top,b(top + 1,jj + 1),1,b(top + 1,jj),1,c,s)
                    ...
                    ! update a by transformations from right.
                    ! explicit loop unrolling provides better performance
                    ! compared to la_xlasr.
                     ! call la_xlasr( 'right', 'variable', 'backward', ihi-top,
           ! $                     ihi-j, a( j+2, j ), b( j+2, j ),
           ! $                     a( top+1, j+1 ), lda )
                    jj = mod(ihi - j - 1,3)
                    do i = ihi - j - 3,jj + 1,-3
                    ...
                    if (jj > 0) then
                       do i = jj,1,-1
                          call la_xrot(ihi - top,a(top + 1,j + i + 1),1,a(top + 1,j + i),1,a( &
                                    j + 1 + i,j),-b(j + 1 + i,j))
                       end do
                    end if
                    ! update (j+1)th column of a by transformations from left.
                    if (j < jcol + nnb - 1) then
                       ...
                       ! triangular.
                       jrow = ihi - nblst + 1
                       call la_xgemv('TRANSPOSE',nblst,len,one,work,nblst,a(jrow,j + 1) &
                                 ,1,zero,work(pw),1)
                       ppw = pw + len
                       do i = jrow,jrow + nblst - len - 1
                          work(ppw) = a(i,j + 1)
                       ...
                       call la_xtrmv('LOWER','TRANSPOSE','NON-UNIT',nblst - len,work( &
                                 len*nblst + 1),nblst,work(pw + len),1)
                       call la_xgemv('TRANSPOSE',len,nblst - len,one,work((len + 1)*nblst - &
                       len + 1),nblst,a(jrow + nblst - len,j + 1),1,one,work(pw + len),1)
                       ...
                       do jrow = j0,jcol + 1,-nnb
                          ...
                          call la_xtrmv('UPPER','TRANSPOSE','NON-UNIT',len,work(ppwo + &
                                    nnb),2*nnb,work(pw),1)
                          call la_xtrmv('LOWER','TRANSPOSE','NON-UNIT',nnb,work(ppwo + &
                                    2*len*nnb),2*nnb,work(pw + len),1)
                          call la_xgemv('TRANSPOSE',nnb,len,one,work(ppwo),2*nnb,a( &
                                    jrow,j + 1),1,one,work(pw),1)
                          call la_xgemv('TRANSPOSE',len,nnb,one,work(ppwo + 2*len*nnb + &
                                    nnb),2*nnb,a(jrow + nnb,j + 1),1,one,work(pw + len),1)
                          ppw = pw
                          do i = jrow,jrow + len + nnb - 1
                             a(i,j + 1) = work(ppw)
                 ...
                 cola = n - jcol - nnb + 1
                 j = ihi - nblst + 1
                 call la_xgemm('TRANSPOSE','NO TRANSPOSE',nblst,cola,nblst,one,work, &
                           nblst,a(j,jcol + nnb),lda,zero,work(pw),nblst)
                 call la_xlacpy('ALL',nblst,cola,work(pw),nblst,a(j,jcol + nnb),lda)
                 ...
                 do j = j0,jcol + 1,-nnb
                    if (blk22) then
                       ...
                       ! where all blocks are nnb-by-nnb, u21 is upper
                       ! triangular and u12 is lower triangular.
                       call la_xorm22('LEFT','TRANSPOSE',2*nnb,cola,nnb,nnb,work(ppwo) &
                                 ,2*nnb,a(j,jcol + nnb),lda,work(pw),lwork - pw + 1,ierr)
                    else
                       ! ignore the structure of u.
                       call la_xgemm('TRANSPOSE','NO TRANSPOSE',2*nnb,cola,2*nnb,one, &
                                 work(ppwo),2*nnb,a(j,jcol + nnb),lda,zero,work(pw),2*nnb)
                       call la_xlacpy('ALL',2*nnb,cola,work(pw),2*nnb,a(j,jcol + nnb), &
                                  lda)
                    end if
                    ppwo = ppwo + 4*nnb*nnb
                 end do
                 ...
                 if (wantq) then
                    ...
                    call la_xgemm('NO TRANSPOSE','NO TRANSPOSE',nh,nblst,nblst,one,q( &
                              topq,j),ldq,work,nblst,zero,work(pw),nh)
                    call la_xlacpy('ALL',nh,nblst,work(pw),nh,q(topq,j),ldq)
                    ...
                    do j = j0,jcol + 1,-nnb
                       ...
                       if (blk22) then
                          ! exploit the structure of u.
                          call la_xorm22('RIGHT','NO TRANSPOSE',nh,2*nnb,nnb,nnb,work( &
                                    ppwo),2*nnb,q(topq,j),ldq,work(pw),lwork - pw + 1,ierr)
                       else
                          ! ignore the structure of u.
                          call la_xgemm('NO TRANSPOSE','NO TRANSPOSE',nh,2*nnb,2*nnb,one, &
                                     q(topq,j),ldq,work(ppwo),2*nnb,zero,work(pw),nh)
                          call la_xlacpy('ALL',nh,2*nnb,work(pw),nh,q(topq,j),ldq)
                 ...
                 if (wantz .or. top > 0) then
                    ...
                    call la_xlaset('LOWER',ihi - jcol - 1,nnb,zero,zero,a(jcol + 2, &
                              jcol),lda)
                    call la_xlaset('LOWER',ihi - jcol - 1,nnb,zero,zero,b(jcol + 2, &
                              jcol),ldb)
                 end if
                 ! apply accumulated orthogonal matrices to a and b.
                 if (top > 0) then
                    j = ihi - nblst + 1
                    call la_xgemm('NO TRANSPOSE','NO TRANSPOSE',top,nblst,nblst,one,a( &
                              1,j),lda,work,nblst,zero,work(pw),top)
                    call la_xlacpy('ALL',top,nblst,work(pw),top,a(1,j),lda)
                    ...
                    do j = j0,jcol + 1,-nnb
                       if (blk22) then
                          ! exploit the structure of u.
                          call la_xorm22('RIGHT','NO TRANSPOSE',top,2*nnb,nnb,nnb,work( &
                                    ppwo),2*nnb,a(1,j),lda,work(pw),lwork - pw + 1,ierr)
                       else
                          ! ignore the structure of u.
                          call la_xgemm('NO TRANSPOSE','NO TRANSPOSE',top,2*nnb,2*nnb, &
                                    one,a(1,j),lda,work(ppwo),2*nnb,zero,work(pw),top)
                          call la_xlacpy('ALL',top,2*nnb,work(pw),top,a(1,j),lda)

                       end if
                       ppwo = ppwo + 4*nnb*nnb
                    end do
                    j = ihi - nblst + 1
                    call la_xgemm('NO TRANSPOSE','NO TRANSPOSE',top,nblst,nblst,one,b( &
                              1,j),ldb,work,nblst,zero,work(pw),top)
                    call la_xlacpy('ALL',top,nblst,work(pw),top,b(1,j),ldb)
                    ...
                    do j = j0,jcol + 1,-nnb
                       if (blk22) then
                          ...
                          call la_xgemm('NO TRANSPOSE','NO TRANSPOSE',top,2*nnb,2*nnb, &
                                    one,b(1,j),ldb,work(ppwo),2*nnb,zero,work(pw),top)
                          call la_xlacpy('ALL',top,2*nnb,work(pw),top,b(1,j),ldb)
                 ...
                 if (wantz) then
                    ...
                    call la_xgemm('NO TRANSPOSE','NO TRANSPOSE',nh,nblst,nblst,one,z( &
                              topq,j),ldz,work,nblst,zero,work(pw),nh)
                    call la_xlacpy('ALL',nh,nblst,work(pw),nh,z(topq,j),ldz)
                    ...
                    do j = j0,jcol + 1,-nnb
                       ...
                       if (blk22) then
                          ...
                          call la_xgemm('NO TRANSPOSE','NO TRANSPOSE',nh,2*nnb,2*nnb,one, &
                                     z(topq,j),ldz,work(ppwo),2*nnb,zero,work(pw),nh)
                          call la_xlacpy('ALL',nh,2*nnb,work(pw),nh,z(topq,j),ldz)
           ...
           if (jcol < ihi) call la_xgghrd(compq2,compz2,n,jcol,ihi,a,lda,b,ldb,q,ldq, &
                      z,ldz,ierr)
           work(1) = real(lwkopt,KIND=xdp)
           return
     end subroutine la_xgghd3
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
     !> XHGEQZ: computes the eigenvalues of a real matrix pair (H,T),
     !> where H is an upper Hessenberg matrix and T is upper triangular,
     !> using the double-shift QZ method.
     !> Matrix pairs of this type are produced by the reduction to
     !> generalized upper Hessenberg form of a real matrix pair (A,B):
     !> A = Q1*H*Z1**T,  B = Q1*T*Z1**T,
     !> as computed by XGGHRD.
     !> If JOB='S', then the Hessenberg-triangular pair (H,T) is
     !> also reduced to generalized Schur form,
     !> H = Q*S*Z**T,  T = Q*P*Z**T,
     !> where Q and Z are orthogonal matrices, P is an upper triangular
     ...
     !> factorization may be postmultiplied into an input matrix Q1, and the
     !> orthogonal matrix Z may be postmultiplied into an input matrix Z1.
     !> If Q1 and Z1 are the orthogonal matrices from XGGHRD that reduced
     !> the matrix pair (A,B) to generalized upper Hessenberg form, then the
     !> output matrices Q1*Q and Z1*Z are the orthogonal factors from the
     !> generalized Schur factorization of (A,B):
     !> A = (Q1*Q)*S*(Z1*Z)**T,  B = (Q1*Q)*P*(Z1*Z)**T.
     ...
     subroutine la_xhgeqz(job,compq,compz,n,ilo,ihi,h,ldh,t,ldt,alphar,alphai, &
               beta,q,ldq,z,ldz,work,lwork,info)
        ...
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ...
           integer(ilp),intent(out) :: info
           ! Array Arguments
           real(xdp),intent(out) :: alphai(*),alphar(*),beta(*),work(*)
           real(xdp),intent(inout) :: h(ldh,*),q(ldq,*),t(ldt,*),z(ldz,*)
        ! =====================================================================
           ! Parameters
          ...
          ! $                     safety = one )
           ...
           integer(ilp) :: icompq,icompz,ifirst,ifrstm,iiter,ilast,ilastm,in,ischur, &
                     istart,j,jc,jch,jiter,jr,maxit
           real(xdp) :: a11,a12,a1i,a1r,a21,a22,a2i,a2r,ad11,ad11l,ad12,ad12l,ad21, &
           ad21l,ad22,ad22l,ad32l,an,anorm,ascale,atol,b11,b1a,b1i,b1r,b22,b2a,b2i, &
           b2r,bn,bnorm,bscale,btol,c,c11i,c11r,c12,c21,c22i,c22r,cl,cq,cr,cz, &
           eshift,s,s1,s1inv,s2,safmax,safmin,scale,sl,sqi,sqr,sr,szi,szr,t1,tau, &
           temp,temp2,tempi,tempr,u1,u12,u12l,u2,ulp,vs,w11,w12,w21,w22,wabs,wi, &
           ...
           safmin = la_xlamch('S')
           safmax = one/safmin
           ulp = la_xlamch('E')*la_xlamch('B')
           anorm = la_xlanhs('F',in,h(ilo,ilo),ldh,work)
           bnorm = la_xlanhs('F',in,t(ilo,ilo),ldt,work)
           atol = max(safmin,ulp*anorm)
           btol = max(safmin,ulp*bnorm)
           ascale = one/max(safmin,anorm)
           bscale = one/max(safmin,bnorm)
           ...
           loop_360: do jiter = 1,maxit
              ...
              loop_60: do j = ilast - 1,ilo,-1
                 ...
                 if (abs(t(j,j)) < max(safmin,ulp*temp)) then
                    ...
                    if (ilazro .or. ilazr2) then
                       do jch = j,ilast - 1
                          temp = h(jch,jch)
                          call la_xlartg(temp,h(jch + 1,jch),c,s,h(jch,jch))
                          h(jch + 1,jch) = zero
                          call la_xrot(ilastm - jch,h(jch,jch + 1),ldh,h(jch + 1,jch + 1), &
                                    ldh,c,s)
                          call la_xrot(ilastm - jch,t(jch,jch + 1),ldt,t(jch + 1,jch + 1), &
                                    ldt,c,s)
                          if (ilq) call la_xrot(n,q(1,jch),1,q(1,jch + 1),1,c,s)
                       ...
                       do jch = j,ilast - 1
                          temp = t(jch,jch + 1)
                          call la_xlartg(temp,t(jch + 1,jch + 1),c,s,t(jch,jch + 1))

                          t(jch + 1,jch + 1) = zero
                          if (jch < ilastm - 1) call la_xrot(ilastm - jch - 1,t(jch,jch + 2),ldt, &
                                    t(jch + 1,jch + 2),ldt,c,s)
                          call la_xrot(ilastm - jch + 2,h(jch,jch - 1),ldh,h(jch + 1,jch - 1), &
                                    ldh,c,s)
                          if (ilq) call la_xrot(n,q(1,jch),1,q(1,jch + 1),1,c,s)

                          temp = h(jch + 1,jch)
                          call la_xlartg(temp,h(jch + 1,jch - 1),c,s,h(jch + 1,jch))

                          h(jch + 1,jch - 1) = zero
                          call la_xrot(jch + 1 - ifrstm,h(ifrstm,jch),1,h(ifrstm,jch - 1), &
                                    1,c,s)
                          call la_xrot(jch - ifrstm,t(ifrstm,jch),1,t(ifrstm,jch - 1),1, &
                                     c,s)
                          if (ilz) call la_xrot(n,z(1,jch),1,z(1,jch - 1),1,c,s)
              ...
              70 continue
              temp = h(ilast,ilast)
              call la_xlartg(temp,h(ilast,ilast - 1),c,s,h(ilast,ilast))
              h(ilast,ilast - 1) = zero
              call la_xrot(ilast - ifrstm,h(ifrstm,ilast),1,h(ifrstm,ilast - 1),1,c,s &
                        )
              call la_xrot(ilast - ifrstm,t(ifrstm,ilast),1,t(ifrstm,ilast - 1),1,c,s &
                        )
              if (ilz) call la_xrot(n,z(1,ilast),1,z(1,ilast - 1),1,c,s)
              ! h(ilast,ilast-1)=0 -- standardize b, set alphar, alphai,
                                    ! and beta
                                    80 continue
              if (t(ilast,ilast) < zero) then
              ...
              if ((iiter/10)*10 == iiter) then
                 ! exceptional shift.  chosen for no particularly good reason.
                 ! (single shift only.)
                 if ((real(maxit,KIND=xdp)*safmin)*abs(h(ilast,ilast - 1)) < abs(t(ilast - 1, &
                           ilast - 1))) then
                    eshift = h(ilast,ilast - 1)/t(ilast - 1,ilast - 1)
                 else
                    eshift = eshift + one/(safmin*real(maxit,KIND=xdp))
                 end if
                 s1 = one
                 wr = eshift
              else
                 ! shifts based on the generalized eigenvalues of the
                 ! bottom-right 2x2 block of a and b. the first eigenvalue
                 ! returned by la_xlag2 is the wilkinson shift (aep p.512_xdp),
                 call la_xlag2(h(ilast - 1,ilast - 1),ldh,t(ilast - 1,ilast - 1),ldt, &
                           safmin*safety,s1,s2,wr,wr2,wi)
                 if (abs((wr/s1)*t(ilast,ilast) - h(ilast,ilast)) > abs((wr2/s2)*t( &
                           ilast,ilast) - h(ilast,ilast))) then
                    temp = wr
              ...
              temp = s1*h(istart,istart) - wr*t(istart,istart)
              temp2 = s1*h(istart + 1,istart)
              call la_xlartg(temp,temp2,c,s,tempr)
              ! sweep
              loop_190: do j = istart,ilast - 1
                 if (j > istart) then
                    temp = h(j,j - 1)
                    call la_xlartg(temp,h(j + 1,j - 1),c,s,h(j,j - 1))
                    h(j + 1,j - 1) = zero
                 end if
                 do jc = j,ilastm
                    temp = c*h(j,jc) + s*h(j + 1,jc)
                 ...
                 temp = t(j + 1,j + 1)
                 call la_xlartg(temp,t(j + 1,j),c,s,t(j + 1,j + 1))
                 t(j + 1,j) = zero
                 do jr = ifrstm,min(j + 2,ilast)
                    temp = c*h(jr,j + 1) + s*h(jr,j)
                    h(jr,j) = -s*h(jr,j + 1) + c*h(jr,j)
              ...
              if (ifirst + 1 == ilast) then
                 ...
                 ! step 1: standardize, that is, rotate so that
                         ...
                         ! b = (         )  with b11 non-negative.
                             ! (  0  b22 )
                 call la_xlasv2(t(ilast - 1,ilast - 1),t(ilast - 1,ilast),t(ilast,ilast), &
                            b22,b11,sr,cr,sl,cl)
                 if (b11 < zero) then
                    cr = -cr
                    sr = -sr
                 ...
                 call la_xrot(ilastm + 1 - ifirst,h(ilast - 1,ilast - 1),ldh,h(ilast,ilast - 1) &
                           ,ldh,cl,sl)
                 call la_xrot(ilast + 1 - ifrstm,h(ifrstm,ilast - 1),1,h(ifrstm,ilast),1, &
                           cr,sr)
                 if (ilast < ilastm) call la_xrot(ilastm - ilast,t(ilast - 1,ilast + 1),ldt,t( &
                           ilast,ilast + 1),ldt,cl,sl)
                 if (ifrstm < ilast - 1) call la_xrot(ifirst - ifrstm,t(ifrstm,ilast - 1),1,t( &
                           ifrstm,ilast),1,cr,sr)
                 if (ilq) call la_xrot(n,q(1,ilast - 1),1,q(1,ilast),1,cl,sl)

                 if (ilz) call la_xrot(n,z(1,ilast - 1),1,z(1,ilast),1,cr,sr)
                 ...
                 if (abs(c11r) + abs(c11i) + abs(c12) > abs(c21) + abs(c22r) + abs(c22i)) &
                           then
                    t1 = la_xlapy3(c12,c11r,c11i)
                    cz = c12/t1
                    szr = -c11r/t1
                    szi = -c11i/t1
                 else
                    cz = la_xlapy2(c22r,c22i)
                    if (cz <= safmin) then
                       cz = zero
                       szr = one
                       szi = zero
                 ...
                 b1r = cq*cz*b11 + tempr*b22
                 b1i = tempi*b22
                 b1a = la_xlapy2(b1r,b1i)
                 b2r = cq*cz*b22 + tempr*b11
                 b2i = -tempi*b11
                 b2a = la_xlapy2(b2r,b2i)
                 ! normalize so beta > 0, and im( alpha1 ) > 0
                 beta(ilast - 1) = b1a
                 beta(ilast) = b2a
                 alphar(ilast - 1) = (wr*b1a)*s1inv
                 ...
                 v(3) = ad32l*ad21l
                 istart = ifirst
                 call la_xlarfg(3,v(1),v(2),1,tau)
                 v(1) = one
                 ! sweep
                 loop_290: do j = istart,ilast - 2
                    ! all but last elements: use 3x3 householder transforms.
                    ...
                    if (j > istart) then
                       ...
                       v(2) = h(j + 1,j - 1)
                       v(3) = h(j + 2,j - 1)
                       call la_xlarfg(3,h(j,j - 1),v(2),1,tau)
                       v(1) = one
                       h(j + 1,j - 1) = zero
                       h(j + 2,j - 1) = zero
                    end if
                    ...
                    ! zero j-th column of b (see xlagbc for details)
                    ! swap rows to pivot
                    ilpivt = .false.
                    temp = max(abs(t(j + 1,j + 1)),abs(t(j + 1,j + 2)))
                    temp2 = max(abs(t(j + 2,j + 1)),abs(t(j + 2,j + 2)))
           ...
           ! exit (other than argument error) -- return optimal workspace size
           420 continue
           work(1) = real(n,KIND=xdp)
           return
     end subroutine la_xhgeqz
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
     !> YGGBAK: forms the right or left eigenvectors of a complex generalized
     !> eigenvalue problem A*x = lambda*B*x, by backward transformation on
     !> the computed eigenvectors of the balanced pair of matrices output by
     !> YGGBAL.

     pure subroutine la_yggbak(job,side,n,ilo,ihi,lscale,rscale,m,v,ldv,info)
        ...
        ! =====================================================================
           ...
           if (la_lsame(job,'S') .or. la_lsame(job,'B')) then
              ...
              if (rightv) then
                 do i = ilo,ihi
                    call la_yxscal(m,rscale(i),v(i,1),ldv)
                 end do
              end if
              ! backward transformation on left eigenvectors
              if (leftv) then
                 do i = ilo,ihi
                    call la_yxscal(m,lscale(i),v(i,1),ldv)
                 end do
              end if
           end if
           ! backward permutation
...
#ifdef LA_WITH_XDP
     !> YGGBAL: balances a pair of general complex matrices (A,B).  This
     !> involves, first, permuting A and B by similarity transformations to
     !> isolate eigenvalues in the first 1 to ILO$-$1 and last IHI+1 to N
     !> elements on the diagonal; and second, applying a diagonal similarity
     !> transformation to rows and columns ILO to IHI to make the rows
     ...
     pure subroutine la_yggbal(job,n,a,lda,b,ldb,ilo,ihi,lscale,rscale,work,info)
        use la_constants_xdp
        ...
        ! =====================================================================
           ...
           lscale(m) = i
           if (i == m) go to 170
           call la_yswap(n - k + 1,a(i,k),lda,a(m,k),lda)
           call la_yswap(n - k + 1,b(i,k),ldb,b(m,k),ldb)
           ! permute columns m and j
           170 continue
           rscale(m) = j
           if (j == m) go to 180
           call la_yswap(l,a(1,j),1,a(1,m),1)
           call la_yswap(l,b(1,j),1,b(1,m),1)
           180 continue
           go to(20,90) iflow
           190 continue
           ilo = k
           ...
           lsfmax = int(log10(sfmax)/basl,KIND=ilp)
           do i = ilo,ihi
              irab = la_iyamax(n - ilo + 1,a(i,ilo),lda)
              rab = abs(a(i,irab + ilo - 1))
              irab = la_iyamax(n - ilo + 1,b(i,ilo),ldb)
              rab = max(rab,abs(b(i,irab + ilo - 1)))
              lrab = int(log10(rab + sfmin)/basl + one,KIND=ilp)
              ir = int(lscale(i) + sign(half,lscale(i)),KIND=ilp)
              ir = min(max(ir,lsfmin),lsfmax,lsfmax - lrab)
              lscale(i) = sclfac**ir
              icab = la_iyamax(ihi,a(1,i),1)
              cab = abs(a(icab,i))
              icab = la_iyamax(ihi,b(1,i),1)
              cab = max(cab,abs(b(icab,i)))
              lcab = int(log10(cab + sfmin)/basl + one,KIND=ilp)
              jc = int(rscale(i) + sign(half,rscale(i)),KIND=ilp)
              jc = min(max(jc,lsfmin),lsfmax,lsfmax - lcab)
           ...
           ! row scaling of matrices a and b
           do i = ilo,ihi
              call la_yxscal(n - ilo + 1,lscale(i),a(i,ilo),lda)
              call la_yxscal(n - ilo + 1,lscale(i),b(i,ilo),ldb)
           end do
           ! column scaling of matrices a and b
           do j = ilo,ihi
              call la_yxscal(ihi,rscale(j),a(1,j),1)
              call la_yxscal(ihi,rscale(j),b(1,j),1)
           end do
           return
     end subroutine la_yggbal
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
     !> YGGHRD: reduces a pair of complex matrices (A,B) to generalized upper
     !> Hessenberg form using unitary transformations, where A is a
     !> general matrix and B is upper triangular.  The form of the
     !> generalized eigenvalue problem is
     !> A*x = lambda*B*x,
     ...
     !> Q1 * B * Z1**H = (Q1*Q) * T * (Z1*Z)**H
     !> If Q1 is the unitary matrix from the QR factorization of B in the
     !> original equation A*x = lambda*B*x, then YGGHRD reduces the original
     !> problem to generalized Hessenberg form.

     pure subroutine la_ygghrd(compq,compz,n,ilo,ihi,a,lda,b,ldb,q,ldq,z,ldz, &
               info)
        use la_constants_xdp
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ...
        ! =====================================================================
           ...
           if (info /= 0) then
              call la_xerbla('YGGHRD',-info)
              return
           end if
           ! initialize q and z if desired.
           if (icompq == 3) call la_ylaset('FULL',n,n,czero,cone,q,ldq)
           if (icompz == 3) call la_ylaset('FULL',n,n,czero,cone,z,ldz)
           ! quick return if possible
           if (n <= 1) return
           ! zero out lower triangle of b
           do jcol = 1,n - 1
           ...
           do jcol = ilo,ihi - 2
              do jrow = ihi,jcol + 2,-1
                 ! step 1: rotate rows jrow-1, jrow to kill a(jrow,jcol)
                 ctemp = a(jrow - 1,jcol)
                 call la_ylartg(ctemp,a(jrow,jcol),c,s,a(jrow - 1,jcol))
                 a(jrow,jcol) = czero
                 call la_yrot(n - jcol,a(jrow - 1,jcol + 1),lda,a(jrow,jcol + 1),lda,c,s)

                 call la_yrot(n + 2 - jrow,b(jrow - 1,jrow - 1),ldb,b(jrow,jrow - 1),ldb,c, &
                           s)
                 if (ilq) call la_yrot(n,q(1,jrow - 1),1,q(1,jrow),1,c,conjg(s))

                 ! step 2: rotate columns jrow, jrow-1 to kill b(jrow,jrow-1)
                 ctemp = b(jrow,jrow)
                 call la_ylartg(ctemp,b(jrow,jrow - 1),c,s,b(jrow,jrow))
                 b(jrow,jrow - 1) = czero
                 call la_yrot(ihi,a(1,jrow),1,a(1,jrow - 1),1,c,s)
                 call la_yrot(jrow - 1,b(1,jrow),1,b(1,jrow - 1),1,c,s)
                 if (ilz) call la_yrot(n,z(1,jrow),1,z(1,jrow - 1),1,c,s)
              end do
           end do
           return
     end subroutine la_ygghrd
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
     !> YGGHD3: reduces a pair of complex matrices (A,B) to generalized upper
     !> Hessenberg form using unitary transformations, where A is a
     !> general matrix and B is upper triangular.  The form of the
     !> generalized eigenvalue problem is
     !> A*x = lambda*B*x,
     ...
     !> Q1 * B * Z1**H = (Q1*Q) * T * (Z1*Z)**H
     !> If Q1 is the unitary matrix from the QR factorization of B in the
     !> original equation A*x = lambda*B*x, then YGGHD3 reduces the original
     !> problem to generalized Hessenberg form.
     !> This is a blocked variant of ZGGHRD, using matrix-matrix
     !> multiplications for parts of the computation to enhance performance.

     pure subroutine la_ygghd3(compq,compz,n,ilo,ihi,a,lda,b,ldb,q,ldq,z,ldz, &
               work,lwork,info)
        use la_constants_xdp
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ...
        ! =====================================================================
           ...
           integer(ilp) :: cola,i,ierr,j,j0,jcol,jj,jrow,k,kacc22,len,lwkopt,n2nb,nb, &
                      nblst,nbmin,nh,nnb,nx,ppw,ppwo,pw,top,topq
           real(xdp) :: c
           complex(xdp) :: c1,c2,ctemp,s,s1,s2,temp,temp1,temp2,temp3
           ! Intrinsic Functions
           intrinsic :: real,cmplx,conjg,max
           ! Executable Statements
           ! decode and test the input parameters.
           info = 0
           nb = la_ilaenv(1,'YGGHD3',' ',n,ilo,ihi,-1)
           lwkopt = max(6*n*nb,1)
           ...
           if (info /= 0) then
              call la_xerbla('YGGHD3',-info)
              return
           else if (lquery) then
              return
           end if
           ! initialize q and z if desired.
           if (initq) call la_ylaset('ALL',n,n,czero,cone,q,ldq)
           if (initz) call la_ylaset('ALL',n,n,czero,cone,z,ldz)
           ! zero out lower triangle of b.
           if (n > 1) call la_ylaset('LOWER',n - 1,n - 1,czero,czero,b(2,1),ldb)
           ! quick return if possible
           nh = ihi - ilo + 1
           if (nh <= 1) then
              work(1) = cone
           ...
           ! determine the blocksize.
           nbmin = la_ilaenv(2,'YGGHD3',' ',n,ilo,ihi,-1)
           if (nb > 1 .and. nb < nh) then
              ! determine when to use unblocked instead of blocked code.
              nx = max(nb,la_ilaenv(3,'YGGHD3',' ',n,ilo,ihi,-1))
              if (nx < nh) then
                 ! determine if workspace is large enough for blocked code.
                 if (lwork < lwkopt) then
                    ! not enough workspace to use optimal nb:  determine the
                    ! minimum value of nb, and reduce nb or force use of
                    ! unblocked code.
                    nbmin = max(2,la_ilaenv(2,'YGGHD3',' ',n,ilo,ihi,-1))
                    if (lwork >= 6*n*nbmin) then
                       nb = lwork/(6*n)
                    else
                       nb = 1
           ...
           if (nb < nbmin .or. nb >= nh) then
              ...
              ! use blocked code
              kacc22 = la_ilaenv(16,'YGGHD3',' ',n,ilo,ihi,-1)
              blk22 = kacc22 == 2
              do jcol = ilo,ihi - 2,nb
                 nnb = min(nb,ihi - jcol - 1)
                 ! initialize small unitary factors that will hold the
                 ...
                 n2nb = (ihi - jcol - 1)/nnb - 1
                 nblst = ihi - jcol - n2nb*nnb
                 call la_ylaset('ALL',nblst,nblst,czero,cone,work,nblst)
                 pw = nblst*nblst + 1
                 do i = 1,n2nb
                    call la_ylaset('ALL',2*nnb,2*nnb,czero,cone,work(pw),2*nnb)
                 ...
                 do j = jcol,jcol + nnb - 1
                    ...
                    do i = ihi,j + 2,-1
                       temp = a(i - 1,j)
                       call la_ylartg(temp,a(i,j),c,s,a(i - 1,j))
                       a(i,j) = cmplx(c,KIND=xdp)
                       b(i,j) = s
                    end do
                    ! accumulate givens rotations into workspace array.
                    ppw = (nblst + 1)*(nblst - 2) - j + jcol + 1
                    ...
                    do jj = n,j + 1,-1
                       ...
                       if (jj < ihi) then
                          temp = b(jj + 1,jj + 1)
                          call la_ylartg(temp,b(jj + 1,jj),c,s,b(jj + 1,jj + 1))
                          b(jj + 1,jj) = czero
                          call la_yrot(jj - top,b(top + 1,jj + 1),1,b(top + 1,jj),1,c,s)

                          a(jj + 1,j) = cmplx(c,KIND=xdp)
                          b(jj + 1,j) = -conjg(s)
                       end if
                    end do
                    ! update a by transformations from right.
                    ...
                    if (jj > 0) then
                       do i = jj,1,-1
                          c = real(a(j + 1 + i,j),KIND=xdp)
                          call la_yrot(ihi - top,a(top + 1,j + i + 1),1,a(top + 1,j + i),1,c,- &
                                    conjg(b(j + 1 + i,j)))
                       end do
                    end if
                    ! update (j+1)th column of a by transformations from left.
                    if (j < jcol + nnb - 1) then
                       ...
                       ! triangular.
                       jrow = ihi - nblst + 1
                       call la_ygemv('CONJUGATE',nblst,len,cone,work,nblst,a(jrow,j + 1 &
                                 ),1,czero,work(pw),1)
                       ppw = pw + len
                       do i = jrow,jrow + nblst - len - 1
                          work(ppw) = a(i,j + 1)
                       ...
                       call la_ytrmv('LOWER','CONJUGATE','NON-UNIT',nblst - len,work( &
                                 len*nblst + 1),nblst,work(pw + len),1)
                       call la_ygemv('CONJUGATE',len,nblst - len,cone,work((len + 1)*nblst - &
                       len + 1),nblst,a(jrow + nblst - len,j + 1),1,cone,work(pw + len),1)
                       ...
                       do jrow = j0,jcol + 1,-nnb
                          ...
                          call la_ytrmv('UPPER','CONJUGATE','NON-UNIT',len,work(ppwo + &
                                    nnb),2*nnb,work(pw),1)
                          call la_ytrmv('LOWER','CONJUGATE','NON-UNIT',nnb,work(ppwo + &
                                    2*len*nnb),2*nnb,work(pw + len),1)
                          call la_ygemv('CONJUGATE',nnb,len,cone,work(ppwo),2*nnb,a( &
                                    jrow,j + 1),1,cone,work(pw),1)
                          call la_ygemv('CONJUGATE',len,nnb,cone,work(ppwo + 2*len*nnb + &
                                    nnb),2*nnb,a(jrow + nnb,j + 1),1,cone,work(pw + len),1)
                          ppw = pw
                          do i = jrow,jrow + len + nnb - 1
                             a(i,j + 1) = work(ppw)
                 ...
                 cola = n - jcol - nnb + 1
                 j = ihi - nblst + 1
                 call la_ygemm('CONJUGATE','NO TRANSPOSE',nblst,cola,nblst,cone,work, &
                           nblst,a(j,jcol + nnb),lda,czero,work(pw),nblst)
                 call la_ylacpy('ALL',nblst,cola,work(pw),nblst,a(j,jcol + nnb),lda)
                 ...
                 do j = j0,jcol + 1,-nnb
                    if (blk22) then
                       ...
                       ! where all blocks are nnb-by-nnb, u21 is upper
                       ! triangular and u12 is lower triangular.
                       call la_yunm22('LEFT','CONJUGATE',2*nnb,cola,nnb,nnb,work(ppwo) &
                                 ,2*nnb,a(j,jcol + nnb),lda,work(pw),lwork - pw + 1,ierr)
                    else
                       ! ignore the structure of u.
                       call la_ygemm('CONJUGATE','NO TRANSPOSE',2*nnb,cola,2*nnb,cone, &
                       work(ppwo),2*nnb,a(j,jcol + nnb),lda,czero,work(pw),2*nnb)

                       call la_ylacpy('ALL',2*nnb,cola,work(pw),2*nnb,a(j,jcol + nnb), &
                                  lda)
                    end if
                    ppwo = ppwo + 4*nnb*nnb
                 end do
                 ...
                 if (wantq) then
                    ...
                    call la_ygemm('NO TRANSPOSE','NO TRANSPOSE',nh,nblst,nblst,cone,q( &
                              topq,j),ldq,work,nblst,czero,work(pw),nh)
                    call la_ylacpy('ALL',nh,nblst,work(pw),nh,q(topq,j),ldq)
                    ...
                    do j = j0,jcol + 1,-nnb
                       ...
                       if (blk22) then
                          ! exploit the structure of u.
                          call la_yunm22('RIGHT','NO TRANSPOSE',nh,2*nnb,nnb,nnb,work( &
                                    ppwo),2*nnb,q(topq,j),ldq,work(pw),lwork - pw + 1,ierr)
                       else
                          ! ignore the structure of u.
                          call la_ygemm('NO TRANSPOSE','NO TRANSPOSE',nh,2*nnb,2*nnb, &
                          cone,q(topq,j),ldq,work(ppwo),2*nnb,czero,work(pw),nh)

                          call la_ylacpy('ALL',nh,2*nnb,work(pw),nh,q(topq,j),ldq)
                 ...
                 if (wantz .or. top > 0) then
                    ...
                    call la_ylaset('LOWER',ihi - jcol - 1,nnb,czero,czero,a(jcol + 2, &
                              jcol),lda)
                    call la_ylaset('LOWER',ihi - jcol - 1,nnb,czero,czero,b(jcol + 2, &
                              jcol),ldb)
                 end if
                 ! apply accumulated unitary matrices to a and b.
                 if (top > 0) then
                    j = ihi - nblst + 1
                    call la_ygemm('NO TRANSPOSE','NO TRANSPOSE',top,nblst,nblst,cone,a( &
                              1,j),lda,work,nblst,czero,work(pw),top)
                    call la_ylacpy('ALL',top,nblst,work(pw),top,a(1,j),lda)
                    ...
                    do j = j0,jcol + 1,-nnb
                       if (blk22) then
                          ! exploit the structure of u.
                          call la_yunm22('RIGHT','NO TRANSPOSE',top,2*nnb,nnb,nnb,work( &
                                    ppwo),2*nnb,a(1,j),lda,work(pw),lwork - pw + 1,ierr)
                       else
                          ! ignore the structure of u.
                          call la_ygemm('NO TRANSPOSE','NO TRANSPOSE',top,2*nnb,2*nnb, &
                          cone,a(1,j),lda,work(ppwo),2*nnb,czero,work(pw),top)

                          call la_ylacpy('ALL',top,2*nnb,work(pw),top,a(1,j),lda)

                       end if
                       ppwo = ppwo + 4*nnb*nnb
                    end do
                    j = ihi - nblst + 1
                    call la_ygemm('NO TRANSPOSE','NO TRANSPOSE',top,nblst,nblst,cone,b( &
                              1,j),ldb,work,nblst,czero,work(pw),top)
                    call la_ylacpy('ALL',top,nblst,work(pw),top,b(1,j),ldb)
                    ...
                    do j = j0,jcol + 1,-nnb
                       if (blk22) then
                          ...
                          call la_ylacpy('ALL',top,2*nnb,work(pw),top,b(1,j),ldb)
                 ...
                 if (wantz) then
                    ...
                    call la_ygemm('NO TRANSPOSE','NO TRANSPOSE',nh,nblst,nblst,cone,z( &
                              topq,j),ldz,work,nblst,czero,work(pw),nh)
                    call la_ylacpy('ALL',nh,nblst,work(pw),nh,z(topq,j),ldz)
                    ...
                    do j = j0,jcol + 1,-nnb
                       ...
                       if (blk22) then
                          ...
                          call la_ylacpy('ALL',nh,2*nnb,work(pw),nh,z(topq,j),ldz)
           ...
           if (jcol < ihi) call la_ygghrd(compq2,compz2,n,jcol,ihi,a,lda,b,ldb,q,ldq, &
                      z,ldz,ierr)
           work(1) = cmplx(lwkopt,KIND=xdp)
           return
     end subroutine la_ygghd3
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
     !> YHGEQZ: computes the eigenvalues of a complex matrix pair (H,T),
     !> where H is an upper Hessenberg matrix and T is upper triangular,
     !> using the single-shift QZ method.
     !> Matrix pairs of this type are produced by the reduction to
     !> generalized upper Hessenberg form of a complex matrix pair (A,B):
     !> A = Q1*H*Z1**H,  B = Q1*T*Z1**H,
     !> as computed by YGGHRD.
     !> If JOB='S', then the Hessenberg-triangular pair (H,T) is
     !> also reduced to generalized Schur form,
     !> H = Q*S*Z**H,  T = Q*P*Z**H,
     !> where Q and Z are unitary matrices and S and P are upper triangular.
     ...
     !> factorization may be postmultiplied into an input matrix Q1, and the
     !> unitary matrix Z may be postmultiplied into an input matrix Z1.
     !> If Q1 and Z1 are the unitary matrices from YGGHRD that reduced
     !> the matrix pair (A,B) to generalized Hessenberg form, then the output
     !> matrices Q1*Q and Z1*Z are the unitary factors from the generalized
     !> Schur factorization of (A,B):
     !> A = (Q1*Q)*S*(Z1*Z)**H,  B = (Q1*Q)*P*(Z1*Z)**H.
     ...
     subroutine la_yhgeqz(job,compq,compz,n,ilo,ihi,h,ldh,t,ldt,alpha,beta,q,ldq, &
                z,ldz,work,lwork,rwork,info)
        use la_constants_xdp,only:zero,half,one,czero,cone
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ! Scalar Arguments
           ...
           complex(xdp),intent(inout) :: h(ldh,*),q(ldq,*),t(ldt,*),z(ldz,*)
        ! =====================================================================
           ...
           integer(ilp) :: icompq,icompz,ifirst,ifrstm,iiter,ilast,ilastm,in,ischur, &
                     istart,j,jc,jch,jiter,jr,maxit
           real(xdp) :: absb,anorm,ascale,atol,bnorm,bscale,btol,c,safmin,temp,temp2, &
                     tempr,ulp
           complex(xdp) :: abi22,ad11,ad12,ad21,ad22,ctemp,ctemp2,ctemp3,eshift,s,shift, &
                     signbc,u12,x,abi12,y
           ! Intrinsic Functions
           intrinsic :: abs,real,cmplx,conjg,aimag,max,min,sqrt
           ! Statement Functions
           ...
           if (info /= 0) then
              call la_xerbla('YHGEQZ',-info)
              return
           else if (lquery) then
              return
           end if
           ! quick return if possible
           ! work( 1 ) = cmplx( 1,KIND=xdp)
           if (n <= 0) then
              work(1) = cmplx(1,KIND=xdp)
              return
           end if
           ...
           safmin = la_xlamch('S')
           ulp = la_xlamch('E')*la_xlamch('B')
           anorm = la_ylanhs('F',in,h(ilo,ilo),ldh,rwork)
           bnorm = la_ylanhs('F',in,t(ilo,ilo),ldt,rwork)
           atol = max(safmin,ulp*anorm)
           btol = max(safmin,ulp*bnorm)
           ascale = one/max(safmin,anorm)
           bscale = one/max(safmin,bnorm)
           ...
           do j = ihi + 1,n
              ...
              if (absb > safmin) then
                 ...
                 t(j,j) = absb
                 if (ilschr) then
                    call la_yscal(j - 1,signbc,t(1,j),1)
                    call la_yscal(j,signbc,h(1,j),1)
                 else
                    call la_yscal(1,signbc,h(j,j),1)
                 end if
                 if (ilz) call la_yscal(n,signbc,z(1,j),1)
              else
                 t(j,j) = czero
              end if
              alpha(j) = h(j,j)
           ...
           loop_170: do jiter = 1,maxit
              ...
              loop_40: do j = ilast - 1,ilo,-1
                 ...
                 if (abs(t(j,j)) < max(safmin,ulp*temp)) then
                    ...
                    if (ilazro .or. ilazr2) then
                       do jch = j,ilast - 1
                          ctemp = h(jch,jch)
                          call la_ylartg(ctemp,h(jch + 1,jch),c,s,h(jch,jch))
                          h(jch + 1,jch) = czero
                          call la_yrot(ilastm - jch,h(jch,jch + 1),ldh,h(jch + 1,jch + 1), &
                                    ldh,c,s)
                          call la_yrot(ilastm - jch,t(jch,jch + 1),ldt,t(jch + 1,jch + 1), &
                                    ldt,c,s)
                          if (ilq) call la_yrot(n,q(1,jch),1,q(1,jch + 1),1,c,conjg( &
                                     s))
                          if (ilazr2) h(jch,jch - 1) = h(jch,jch - 1)*c
                          ilazr2 = .false.
                          if (abs1(t(jch + 1,jch + 1)) >= btol) then
                       ...
                       do jch = j,ilast - 1
                          ctemp = t(jch,jch + 1)
                          call la_ylartg(ctemp,t(jch + 1,jch + 1),c,s,t(jch,jch + 1))

                          t(jch + 1,jch + 1) = czero
                          if (jch < ilastm - 1) call la_yrot(ilastm - jch - 1,t(jch,jch + 2),ldt, &
                                    t(jch + 1,jch + 2),ldt,c,s)
                          call la_yrot(ilastm - jch + 2,h(jch,jch - 1),ldh,h(jch + 1,jch - 1), &
                                    ldh,c,s)
                          if (ilq) call la_yrot(n,q(1,jch),1,q(1,jch + 1),1,c,conjg( &
                                     s))
                          ctemp = h(jch + 1,jch)
                          call la_ylartg(ctemp,h(jch + 1,jch - 1),c,s,h(jch + 1,jch))

                          h(jch + 1,jch - 1) = czero
                          call la_yrot(jch + 1 - ifrstm,h(ifrstm,jch),1,h(ifrstm,jch - 1), &
                                    1,c,s)
                          call la_yrot(jch - ifrstm,t(ifrstm,jch),1,t(ifrstm,jch - 1),1, &
                                     c,s)
                          if (ilz) call la_yrot(n,z(1,jch),1,z(1,jch - 1),1,c,s)
              ...
              50 continue
              ctemp = h(ilast,ilast)
              call la_ylartg(ctemp,h(ilast,ilast - 1),c,s,h(ilast,ilast))
              h(ilast,ilast - 1) = czero
              call la_yrot(ilast - ifrstm,h(ifrstm,ilast),1,h(ifrstm,ilast - 1),1,c,s &
                        )
              call la_yrot(ilast - ifrstm,t(ifrstm,ilast),1,t(ifrstm,ilast - 1),1,c,s &
                        )
              if (ilz) call la_yrot(n,z(1,ilast),1,z(1,ilast - 1),1,c,s)
              ! h(ilast,ilast-1)=0 -- standardize b, set alpha and beta
              60 continue
              absb = abs(t(ilast,ilast))
              if (absb > safmin) then
                 ...
                 t(ilast,ilast) = absb
                 if (ilschr) then
                    call la_yscal(ilast - ifrstm,signbc,t(ifrstm,ilast),1)
                    call la_yscal(ilast + 1 - ifrstm,signbc,h(ifrstm,ilast),1)
                 else
                    call la_yscal(1,signbc,h(ilast,ilast),1)
                 end if
                 if (ilz) call la_yscal(n,signbc,z(1,ilast),1)
              else
                 t(ilast,ilast) = czero
              end if
              alpha(ilast) = h(ilast,ilast)
              ...
              ! magnitude)
              if ((iiter/10)*10 /= iiter) then
                 ! the wilkinson shift (aep p.512_xdp), i.e., the eigenvalue of
                 ! the bottom-right 2x2 block of a inv(b) which is nearest to
                 ! the bottom-right element.
                 ! we factor b as u*d, where u has unit diagonals, and
                 ! compute (a*inv(d))*inv(u).
                 ...
                 if (ctemp /= zero) then
                    ...
                    y = temp*sqrt((x/temp)**2 + (ctemp/temp)**2)
                    if (temp2 > zero) then
                       if (real(x/temp2,KIND=xdp)*real(y,KIND=xdp) + aimag(x/temp2)*aimag(y) &
                                 < zero) y = -y
                    end if
                    shift = shift - ctemp*la_yladiv(ctemp, (x + y))
                 end if
              else
                 ! exceptional shift.  chosen for no particularly good reason.
                 if ((iiter/20)*20 == iiter .and. bscale*abs1(t(ilast,ilast)) > safmin) &
              ...
              ! initial q
              ctemp2 = ascale*h(istart + 1,istart)
              call la_ylartg(ctemp,ctemp2,c,s,ctemp3)
              ! sweep
              loop_150: do j = istart,ilast - 1
                 if (j > istart) then
                    ctemp = h(j,j - 1)
                    call la_ylartg(ctemp,h(j + 1,j - 1),c,s,h(j,j - 1))
                    h(j + 1,j - 1) = czero
                 end if
                 do jc = j,ilastm
                    ctemp = c*h(j,jc) + s*h(j + 1,jc)
                 ...
                 ctemp = t(j + 1,j + 1)
                 call la_ylartg(ctemp,t(j + 1,j),c,s,t(j + 1,j + 1))
                 t(j + 1,j) = czero
                 do jr = ifrstm,min(j + 2,ilast)
                    ctemp = c*h(jr,j + 1) + s*h(jr,j)
                    h(jr,j) = -conjg(s)*h(jr,j + 1) + c*h(jr,j)
           ...
           ! exit (other than argument error) -- return optimal workspace size
           210 continue
           work(1) = cmplx(n,KIND=xdp)
           return
     end subroutine la_yhgeqz
#endif
#ifdef LA_WITH_QP
