...
def compare(old_body, new_body, old_name, new_name, upper_bases):
    ...
    NORM/tag   they also need the role of an upper-case routine tag, i.e. text inside a comment
               or a quoted literal, to be ignored: the stale 'DGETRF'-style tags the q and w
               copies inherited.  Code is never touched by this rung, only upper-case tags.
    """
    ...
    if K.ws_norm(UPPER_TAG.sub("@TAG@", a2)) == K.ws_norm(UPPER_TAG.sub("@TAG@", b2)):
        return "NORM/tag", renames, None
    return "FAIL", {}, diff(K.ws_norm(a2), K.ws_norm(b2), old_name, new_name)
...
def main():
    ...
    renames = {old: new for old, (new, _) in allow.items() if new != "REMOVED"}
