...
module la_lapack_solve_chol
...
#ifdef LA_WITH_XDP
     !> XPPSV: computes the solution to a real system of linear equations
     !> A * X = B,
     !> where A is an N-by-N symmetric positive definite matrix stored in
     !> packed format and X and B are N-by-NRHS matrices.
     !> The Cholesky decomposition is used to factor A as
     ...
     pure subroutine la_xppsv(uplo,n,nrhs,ap,b,ldb,info)
        ! -- lapack driver routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ! Scalar Arguments
        ...
        ! =====================================================================
           ...
           if (info /= 0) then
              call la_xerbla('XPPSV ',-info)
              return
           end if
           ! compute the cholesky factorization a = u**t*u or a = l*l**t.
           call la_xpptrf(uplo,n,ap,info)
           if (info == 0) then
              ! solve the system a*x = b, overwriting b with x.
              call la_xpptrs(uplo,n,nrhs,ap,b,ldb,info)
           end if
           return
     end subroutine la_xppsv
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
     !> XPPSVX: uses the Cholesky factorization A = U**T*U or A = L*L**T to
     !> compute the solution to a real system of linear equations
     !> A * X = B,
     !> where A is an N-by-N symmetric positive definite matrix stored in
     !> packed format and X and B are N-by-NRHS matrices.
     ...
     subroutine la_xppsvx(fact,uplo,n,nrhs,ap,afp,equed,s,b,ldb,x,ldx,rcond,ferr, &
                berr,work,iwork,info)
        ...
        ! =====================================================================
           ...
           logical(lk) :: equil,nofact,rcequ
           integer(ilp) :: i,infequ,j
           real(xdp) :: amax,anorm,bignum,scond,smax,smin,smlnum
           ! Intrinsic Functions
           intrinsic :: max,min
           ! Executable Statements
           info = 0
           ...
           if (info /= 0) then
              call la_xerbla('XPPSVX',-info)
              return
           end if
           if (equil) then
              ! compute row and column scalings to equilibrate the matrix a.
              call la_xppequ(uplo,n,ap,s,scond,amax,infequ)
              if (infequ == 0) then
                 ! equilibrate the matrix.
                 call la_xlaqsp(uplo,n,ap,s,scond,amax,equed)
                 rcequ = la_lsame(equed,'Y')
              end if
           end if
           ! scale the right-hand side.
           ...
           if (nofact .or. equil) then
              ...
              call la_xpptrf(uplo,n,afp,info)
              ! return if info is non-zero.
              if (info > 0) then
                 rcond = zero
                 return
           ...
           ! compute the reciprocal of the condition number of a.
           call la_xppcon(uplo,n,afp,anorm,rcond,work,iwork,info)
           ! compute the solution matrix x.
           call la_xlacpy('FULL',n,nrhs,b,ldb,x,ldx)
           call la_xpptrs(uplo,n,nrhs,afp,x,ldx,info)
           ! use iterative refinement to improve the computed solution and
           ! compute error bounds and backward error estimates for it.
           call la_xpprfs(uplo,n,nrhs,ap,afp,b,ldb,x,ldx,ferr,berr,work,iwork, &
                     info)
           ! transform the solution matrix x to a solution of the original
           ! system.
           if (rcequ) then
...
#ifdef LA_WITH_XDP
     !> XPTSV: computes the solution to a real system of linear equations
     !> A*X = B, where A is an N-by-N symmetric positive definite tridiagonal
     !> matrix, and X and B are N-by-NRHS matrices.
     !> A is factored as A = L*D*L**T, and the factored form of A is then
     !> used to solve the system of equations.

     pure subroutine la_xptsv(n,nrhs,d,e,b,ldb,info)
        ! -- lapack driver routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ! Scalar Arguments
        ...
        ! =====================================================================
           ...
           if (info == 0) then
              ! solve the system a*x = b, overwriting b with x.
              call la_xpttrs(n,nrhs,d,e,b,ldb,info)
           end if
           return
     end subroutine la_xptsv
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
     !> XPTSVX: uses the factorization A = L*D*L**T to compute the solution
     !> to a real system of linear equations A*X = B, where A is an N-by-N
     !> symmetric positive definite tridiagonal matrix and X and B are
     !> N-by-NRHS matrices.
     !> Error bounds on the solution and a condition estimate are also
     ...
     pure subroutine la_xptsvx(fact,n,nrhs,d,e,df,ef,b,ldb,x,ldx,rcond,ferr,berr, &
                work,info)
        ...
        ! =====================================================================
           ...
           if (info /= 0) then
              call la_xerbla('XPTSVX',-info)
              return
           end if
           if (nofact) then
              ! compute the l*d*l**t (or u**t*d*u) factorization of a.
              call la_xcopy(n,d,1,df,1)
              if (n > 1) call la_xcopy(n - 1,e,1,ef,1)
              call la_xpttrf(n,df,ef,info)
              ! return if info is non-zero.
              if (info > 0) then
                 rcond = zero
                 return
           ...
           ! compute the norm of the matrix a.
           anorm = la_xlanst('1',n,d,e)
           ! compute the reciprocal of the condition number of a.
           call la_xptcon(n,df,ef,anorm,rcond,work,info)
           ! compute the solution vectors x.
           call la_xlacpy('FULL',n,nrhs,b,ldb,x,ldx)
           call la_xpttrs(n,nrhs,df,ef,x,ldx,info)
           ! use iterative refinement to improve the computed solutions and
           ! compute error bounds and backward error estimates for them.
           call la_xptrfs(n,nrhs,d,e,df,ef,b,ldb,x,ldx,ferr,berr,work,info)

           ! set info = n+1 if the matrix is singular to working precision.
           if (rcond < la_xlamch('EPSILON')) info = n + 1
           return
     end subroutine la_xptsvx
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
     !> XDPOSV: computes the solution to a real system of linear equations
     !> A * X = B,
     !> where A is an N-by-N symmetric positive definite matrix and X and B
     !> are N-by-NRHS matrices.
     !> XDPOSV first attempts to factorize the matrix in SINGLE PRECISION
     !> and use this factorization within an iterative refinement procedure
     ...
     subroutine la_xdposv(uplo,n,nrhs,a,lda,b,ldb,x,ldx,work,swork,iter,info)
        ...
        ! =====================================================================
           ...
           ! compute some constants.
           anrm = la_xlansy('I',uplo,n,a,lda,work)
           ...
           ! result in sa.
           call la_xlat2d(uplo,n,a,lda,swork(ptsa),n,info)
           if (info /= 0) then
              iter = -2
              go to 40
           end if
           ...
           call la_xsymm('LEFT',uplo,n,nrhs,negone,a,lda,x,ldx,one,work,n)
           ! check whether the nrhs normwise backward errors satisfy the
           ! stopping criterion. if yes, set iter=0 and return.
           do i = 1,nrhs
           ...
           loop_30: do iiter = 1,itermax
              ...
              ! compute r = b - ax (r is work).
              call la_xlacpy('ALL',n,nrhs,b,ldb,work,n)
              call la_xsymm('L',uplo,n,nrhs,negone,a,lda,x,ldx,one,work,n)
              ! check whether the nrhs normwise backward errors satisfy the
              ! stopping criterion. if yes, set iter=iiter>0 and return.
              do i = 1,nrhs
                 xnrm = abs(x(la_ixamax(n,x(1,i),1),i))
           ...
           ! performed iter=itermax iterations and never satisfied the
           ! stopping criterion, set up the iter flag accordingly and follow
           ! up on extended precision routine.
           iter = -itermax - 1
           40 continue
           ! single-precision iterative refinement failed to converge to a
           ! satisfactory solution, so we resort to extended precision.
           call la_xpotrf(uplo,n,a,lda,info)
           if (info /= 0) return
           call la_xlacpy('ALL',n,nrhs,b,ldb,x,ldx)
           call la_xpotrs(uplo,n,nrhs,a,lda,x,ldx,info)
           return
     end subroutine la_xdposv
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
     !> XPBSV: computes the solution to a real system of linear equations
     !> A * X = B,
     !> where A is an N-by-N symmetric positive definite band matrix and X
     !> and B are N-by-NRHS matrices.
     !> The Cholesky decomposition is used to factor A as
     ...
     pure subroutine la_xpbsv(uplo,n,kd,nrhs,ab,ldab,b,ldb,info)
        ! -- lapack driver routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ! Scalar Arguments
        ...
        ! =====================================================================
           ...
           if (info /= 0) then
              call la_xerbla('XPBSV ',-info)
              return
           end if
           ! compute the cholesky factorization a = u**t*u or a = l*l**t.
           call la_xpbtrf(uplo,n,kd,ab,ldab,info)
           if (info == 0) then
              ! solve the system a*x = b, overwriting b with x.
              call la_xpbtrs(uplo,n,kd,nrhs,ab,ldab,b,ldb,info)
           end if
           return
     end subroutine la_xpbsv
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
     !> XPBSVX: uses the Cholesky factorization A = U**T*U or A = L*L**T to
     !> compute the solution to a real system of linear equations
     !> A * X = B,
     !> where A is an N-by-N symmetric positive definite band matrix and X
     !> and B are N-by-NRHS matrices.
     ...
     subroutine la_xpbsvx(fact,uplo,n,kd,nrhs,ab,ldab,afb,ldafb,equed,s,b,ldb,x, &
               ldx,rcond,ferr,berr,work,iwork,info)
        use la_constants_xdp,only:zero,one
        ! -- lapack driver routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ...
        ! =====================================================================
           ...
           if (info /= 0) then
              call la_xerbla('XPBSVX',-info)
              return
           end if
           if (equil) then
              ! compute row and column scalings to equilibrate the matrix a.
              call la_xpbequ(uplo,n,kd,ab,ldab,s,scond,amax,infequ)
              if (infequ == 0) then
                 ! equilibrate the matrix.
                 call la_xlaqsb(uplo,n,kd,ab,ldab,s,scond,amax,equed)
                 rcequ = la_lsame(equed,'Y')
              end if
           end if
           ! scale the right-hand side.
           ...
           if (nofact .or. equil) then
              ...
              if (upper) then
                 do j = 1,n
                    j1 = max(j - kd,1)
                    call la_xcopy(j - j1 + 1,ab(kd + 1 - j + j1,j),1,afb(kd + 1 - j + j1,j),1)

                 end do
              else
                 do j = 1,n
                    j2 = min(j + kd,n)
                    call la_xcopy(j2 - j + 1,ab(1,j),1,afb(1,j),1)
                 end do
              end if
              call la_xpbtrf(uplo,n,kd,afb,ldafb,info)
              ! return if info is non-zero.
              if (info > 0) then
                 rcond = zero
                 return
           ...
           ! compute the norm of the matrix a.
           anorm = la_xlansb('1',uplo,n,kd,ab,ldab,work)
           ! compute the reciprocal of the condition number of a.
           call la_xpbcon(uplo,n,kd,afb,ldafb,anorm,rcond,work,iwork,info)
           ! compute the solution matrix x.
           call la_xlacpy('FULL',n,nrhs,b,ldb,x,ldx)
           call la_xpbtrs(uplo,n,kd,nrhs,afb,ldafb,x,ldx,info)
           ! use iterative refinement to improve the computed solution and
           ! compute error bounds and backward error estimates for it.
           call la_xpbrfs(uplo,n,kd,nrhs,ab,ldab,afb,ldafb,b,ldb,x,ldx,ferr,berr, &
                      work,iwork,info)
           ! transform the solution matrix x to a solution of the original
           ! system.
           if (rcequ) then
...
#ifdef LA_WITH_XDP
     !> XPOSV: computes the solution to a real system of linear equations
     !> A * X = B,
     !> where A is an N-by-N symmetric positive definite matrix and X and B
     !> are N-by-NRHS matrices.
     !> The Cholesky decomposition is used to factor A as
     ...
     pure subroutine la_xposv(uplo,n,nrhs,a,lda,b,ldb,info)
        ! -- lapack driver routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ! Scalar Arguments
        ...
        ! =====================================================================
           ...
           if (info == 0) then
              ! solve the system a*x = b, overwriting b with x.
              call la_xpotrs(uplo,n,nrhs,a,lda,b,ldb,info)
           end if
           return
     end subroutine la_xposv
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
     !> XPOSVX: uses the Cholesky factorization A = U**T*U or A = L*L**T to
     !> compute the solution to a real system of linear equations
     !> A * X = B,
     !> where A is an N-by-N symmetric positive definite matrix and X and B
     !> are N-by-NRHS matrices.
     ...
     subroutine la_xposvx(fact,uplo,n,nrhs,a,lda,af,ldaf,equed,s,b,ldb,x,ldx, &
               rcond,ferr,berr,work,iwork,info)
        use la_constants_xdp,only:zero,one
        ! -- lapack driver routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ...
        ! =====================================================================
           ...
           if (info /= 0) then
              call la_xerbla('XPOSVX',-info)
              return
           end if
           if (equil) then
              ! compute row and column scalings to equilibrate the matrix a.
              call la_xpoequ(n,a,lda,s,scond,amax,infequ)
              if (infequ == 0) then
                 ! equilibrate the matrix.
                 call la_xlaqsy(uplo,n,a,lda,s,scond,amax,equed)
                 rcequ = la_lsame(equed,'Y')
              end if
           end if
           ! scale the right hand side.
           ...
           if (nofact .or. equil) then
              ...
              call la_xpotrf(uplo,n,af,ldaf,info)
              ! return if info is non-zero.
              if (info > 0) then
                 rcond = zero
                 return
           ...
           ! compute the norm of the matrix a.
           anorm = la_xlansy('1',uplo,n,a,lda,work)
           ! compute the reciprocal of the condition number of a.
           call la_xpocon(uplo,n,af,ldaf,anorm,rcond,work,iwork,info)
           ! compute the solution matrix x.
           call la_xlacpy('FULL',n,nrhs,b,ldb,x,ldx)
           call la_xpotrs(uplo,n,nrhs,af,ldaf,x,ldx,info)
           ! use iterative refinement to improve the computed solution and
           ! compute error bounds and backward error estimates for it.
           call la_xporfs(uplo,n,nrhs,a,lda,af,ldaf,b,ldb,x,ldx,ferr,berr,work, &
                     iwork,info)
           ! transform the solution matrix x to a solution of the original
           ! system.
           if (rcequ) then
...
#ifdef LA_WITH_XDP
     !> YPPSV: computes the solution to a complex system of linear equations
     !> A * X = B,
     !> where A is an N-by-N Hermitian positive definite matrix stored in
     !> packed format and X and B are N-by-NRHS matrices.
     !> The Cholesky decomposition is used to factor A as
     ...
     pure subroutine la_yppsv(uplo,n,nrhs,ap,b,ldb,info)
        ! -- lapack driver routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ! Scalar Arguments
        ...
        ! =====================================================================
           ...
           if (info /= 0) then
              call la_xerbla('YPPSV ',-info)
              return
           end if
           ! compute the cholesky factorization a = u**h *u or a = l*l**h.
           call la_ypptrf(uplo,n,ap,info)
           if (info == 0) then
              ! solve the system a*x = b, overwriting b with x.
              call la_ypptrs(uplo,n,nrhs,ap,b,ldb,info)
           end if
           return
     end subroutine la_yppsv
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
     !> YPPSVX: uses the Cholesky factorization A = U**H * U or A = L * L**H to
     !> compute the solution to a complex system of linear equations
     !> A * X = B,
     !> where A is an N-by-N Hermitian positive definite matrix stored in
     !> packed format and X and B are N-by-NRHS matrices.
     ...
     subroutine la_yppsvx(fact,uplo,n,nrhs,ap,afp,equed,s,b,ldb,x,ldx,rcond,ferr, &
                berr,work,rwork,info)
        use la_constants_xdp,only:zero,one
        ! -- lapack driver routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ...
        ! =====================================================================
           ...
           if (info /= 0) then
              call la_xerbla('YPPSVX',-info)
              return
           end if
           if (equil) then
              ! compute row and column scalings to equilibrate the matrix a.
              call la_yppequ(uplo,n,ap,s,scond,amax,infequ)
              if (infequ == 0) then
                 ! equilibrate the matrix.
                 call la_ylaqhp(uplo,n,ap,s,scond,amax,equed)
                 rcequ = la_lsame(equed,'Y')
              end if
           end if
           ! scale the right-hand side.
           ...
           if (nofact .or. equil) then
              ...
              call la_ypptrf(uplo,n,afp,info)
              ! return if info is non-zero.
              if (info > 0) then
                 rcond = zero
                 return
           ...
           ! compute the reciprocal of the condition number of a.
           call la_yppcon(uplo,n,afp,anorm,rcond,work,rwork,info)
           ! compute the solution matrix x.
           call la_ylacpy('FULL',n,nrhs,b,ldb,x,ldx)
           call la_ypptrs(uplo,n,nrhs,afp,x,ldx,info)
           ! use iterative refinement to improve the computed solution and
           ! compute error bounds and backward error estimates for it.
           call la_ypprfs(uplo,n,nrhs,ap,afp,b,ldb,x,ldx,ferr,berr,work,rwork, &
                     info)
           ! transform the solution matrix x to a solution of the original
           ! system.
           if (rcequ) then
...
#ifdef LA_WITH_XDP
     !> YZPOSV: computes the solution to a complex system of linear equations
     !> A * X = B,
     !> where A is an N-by-N Hermitian positive definite matrix and X and B
     !> are N-by-NRHS matrices.
     !> YZPOSV first attempts to factorize the matrix in COMPLEX and use this
     !> factorization within an iterative refinement procedure to produce a
     !> solution with COMPLEX*16 normwise backward error quality (see below).
     !> If the approach fails the method switches to a COMPLEX*16
     !> factorization and solve.
     ...
     subroutine la_yzposv(uplo,n,nrhs,a,lda,b,ldb,x,ldx,work,swork,rwork,iter, &
               info)
        ...
        ! =====================================================================
           ...
           ! compute some constants.
           anrm = la_ylanhe('I',uplo,n,a,lda,rwork)
           eps = la_xlamch('EPSILON')
           cte = anrm*eps*sqrt(real(n,KIND=xdp))*bwdmax
           ! set the indices ptsa, ptsx for referencing sa and sx in swork.
           ptsa = 1
           ...
           ! convert a from extended precision to double precision and store the
           ! result in sa.
           call la_ylat2z(uplo,n,a,lda,swork(ptsa),n,info)
           if (info /= 0) then
              iter = -2
              go to 40
           end if
           ...
           call la_yhemm('LEFT',uplo,n,nrhs,cnegone,a,lda,x,ldx,cone,work,n)
           ...
           loop_30: do iiter = 1,itermax
              ...
              ! compute r = b - ax (r is work).
              call la_ylacpy('ALL',n,nrhs,b,ldb,work,n)
              call la_yhemm('L',uplo,n,nrhs,cnegone,a,lda,x,ldx,cone,work,n)
           ...
           ! single-precision iterative refinement failed to converge to a
           ! satisfactory solution, so we resort to extended precision.
           call la_ypotrf(uplo,n,a,lda,info)
           if (info /= 0) return
           call la_ylacpy('ALL',n,nrhs,b,ldb,x,ldx)
           call la_ypotrs(uplo,n,nrhs,a,lda,x,ldx,info)
           return
     end subroutine la_yzposv
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
     !> YPBSV: computes the solution to a complex system of linear equations
     !> A * X = B,
     !> where A is an N-by-N Hermitian positive definite band matrix and X
     !> and B are N-by-NRHS matrices.
     !> The Cholesky decomposition is used to factor A as
     ...
     pure subroutine la_ypbsv(uplo,n,kd,nrhs,ab,ldab,b,ldb,info)
        ! -- lapack driver routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ! Scalar Arguments
        ...
        ! =====================================================================
           ...
           if (info /= 0) then
              call la_xerbla('YPBSV ',-info)
              return
           end if
           ! compute the cholesky factorization a = u**h *u or a = l*l**h.
           call la_ypbtrf(uplo,n,kd,ab,ldab,info)
           if (info == 0) then
              ! solve the system a*x = b, overwriting b with x.
              call la_ypbtrs(uplo,n,kd,nrhs,ab,ldab,b,ldb,info)
           end if
           return
     end subroutine la_ypbsv
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
     !> YPBSVX: uses the Cholesky factorization A = U**H*U or A = L*L**H to
     !> compute the solution to a complex system of linear equations
     !> A * X = B,
     !> where A is an N-by-N Hermitian positive definite band matrix and X
     !> and B are N-by-NRHS matrices.
     ...
     subroutine la_ypbsvx(fact,uplo,n,kd,nrhs,ab,ldab,afb,ldafb,equed,s,b,ldb,x, &
               ldx,rcond,ferr,berr,work,rwork,info)
        use la_constants_xdp,only:zero,one
        ! -- lapack driver routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ...
        ! =====================================================================
           ...
           if (info /= 0) then
              call la_xerbla('YPBSVX',-info)
              return
           end if
           if (equil) then
              ! compute row and column scalings to equilibrate the matrix a.
              call la_ypbequ(uplo,n,kd,ab,ldab,s,scond,amax,infequ)
              if (infequ == 0) then
                 ! equilibrate the matrix.
                 call la_ylaqhb(uplo,n,kd,ab,ldab,s,scond,amax,equed)
                 rcequ = la_lsame(equed,'Y')
              end if
           end if
           ! scale the right-hand side.
           ...
           if (nofact .or. equil) then
              ...
              if (upper) then
                 do j = 1,n
                    j1 = max(j - kd,1)
                    call la_ycopy(j - j1 + 1,ab(kd + 1 - j + j1,j),1,afb(kd + 1 - j + j1,j),1)

                 end do
              else
                 do j = 1,n
                    j2 = min(j + kd,n)
                    call la_ycopy(j2 - j + 1,ab(1,j),1,afb(1,j),1)
                 end do
              end if
              call la_ypbtrf(uplo,n,kd,afb,ldafb,info)
              ! return if info is non-zero.
              if (info > 0) then
                 rcond = zero
                 return
           ...
           ! compute the norm of the matrix a.
           anorm = la_ylanhb('1',uplo,n,kd,ab,ldab,rwork)
           ! compute the reciprocal of the condition number of a.
           call la_ypbcon(uplo,n,kd,afb,ldafb,anorm,rcond,work,rwork,info)
           ! compute the solution matrix x.
           call la_ylacpy('FULL',n,nrhs,b,ldb,x,ldx)
           call la_ypbtrs(uplo,n,kd,nrhs,afb,ldafb,x,ldx,info)
           ! use iterative refinement to improve the computed solution and
           ! compute error bounds and backward error estimates for it.
           call la_ypbrfs(uplo,n,kd,nrhs,ab,ldab,afb,ldafb,b,ldb,x,ldx,ferr,berr, &
                      work,rwork,info)
           ! transform the solution matrix x to a solution of the original
           ! system.
           if (rcequ) then
...
#ifdef LA_WITH_XDP
     !> YPOSV: computes the solution to a complex system of linear equations
     !> A * X = B,
     !> where A is an N-by-N Hermitian positive definite matrix and X and B
     !> are N-by-NRHS matrices.
     !> The Cholesky decomposition is used to factor A as
     ...
     pure subroutine la_yposv(uplo,n,nrhs,a,lda,b,ldb,info)
        ! -- lapack driver routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ! Scalar Arguments
        ...
        ! =====================================================================
           ...
           if (info == 0) then
              ! solve the system a*x = b, overwriting b with x.
              call la_ypotrs(uplo,n,nrhs,a,lda,b,ldb,info)
           end if
           return
     end subroutine la_yposv
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
     !> YPOSVX: uses the Cholesky factorization A = U**H*U or A = L*L**H to
     !> compute the solution to a complex system of linear equations
     !> A * X = B,
     !> where A is an N-by-N Hermitian positive definite matrix and X and B
     !> are N-by-NRHS matrices.
     ...
     subroutine la_yposvx(fact,uplo,n,nrhs,a,lda,af,ldaf,equed,s,b,ldb,x,ldx, &
               rcond,ferr,berr,work,rwork,info)
        use la_constants_xdp,only:zero,one
        ! -- lapack driver routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ...
        ! =====================================================================
           ...
           if (info /= 0) then
              call la_xerbla('YPOSVX',-info)
              return
           end if
           if (equil) then
              ! compute row and column scalings to equilibrate the matrix a.
              call la_ypoequ(n,a,lda,s,scond,amax,infequ)
              if (infequ == 0) then
                 ! equilibrate the matrix.
                 call la_ylaqhe(uplo,n,a,lda,s,scond,amax,equed)
                 rcequ = la_lsame(equed,'Y')
              end if
           end if
           ! scale the right hand side.
           ...
           if (nofact .or. equil) then
              ...
              call la_ypotrf(uplo,n,af,ldaf,info)
              ! return if info is non-zero.
              if (info > 0) then
                 rcond = zero
                 return
           ...
           ! compute the norm of the matrix a.
           anorm = la_ylanhe('1',uplo,n,a,lda,rwork)
           ! compute the reciprocal of the condition number of a.
           call la_ypocon(uplo,n,af,ldaf,anorm,rcond,work,rwork,info)
           ! compute the solution matrix x.
           call la_ylacpy('FULL',n,nrhs,b,ldb,x,ldx)
           call la_ypotrs(uplo,n,nrhs,af,ldaf,x,ldx,info)
           ! use iterative refinement to improve the computed solution and
           ! compute error bounds and backward error estimates for it.
           call la_yporfs(uplo,n,nrhs,a,lda,af,ldaf,b,ldb,x,ldx,ferr,berr,work, &
                     rwork,info)
           ! transform the solution matrix x to a solution of the original
           ! system.
           if (rcequ) then
...
#ifdef LA_WITH_XDP
     !> YPTSV: computes the solution to a complex system of linear equations
     !> A*X = B, where A is an N-by-N Hermitian positive definite tridiagonal
     !> matrix, and X and B are N-by-NRHS matrices.
     !> A is factored as A = L*D*L**H, and the factored form of A is then
     !> used to solve the system of equations.

     pure subroutine la_yptsv(n,nrhs,d,e,b,ldb,info)
        ! -- lapack driver routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ! Scalar Arguments
        ...
        ! =====================================================================
           ...
           if (info /= 0) then
              call la_xerbla('YPTSV ',-info)
              return
           end if
           ! compute the l*d*l**h (or u**h*d*u) factorization of a.
           call la_ypttrf(n,d,e,info)
           if (info == 0) then
              ! solve the system a*x = b, overwriting b with x.
              call la_ypttrs('LOWER',n,nrhs,d,e,b,ldb,info)
           end if
           return
     end subroutine la_yptsv
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
     !> YPTSVX: uses the factorization A = L*D*L**H to compute the solution
     !> to a complex system of linear equations A*X = B, where A is an
     !> N-by-N Hermitian positive definite tridiagonal matrix and X and B
     !> are N-by-NRHS matrices.
     !> Error bounds on the solution and a condition estimate are also
     ...
     pure subroutine la_yptsvx(fact,n,nrhs,d,e,df,ef,b,ldb,x,ldx,rcond,ferr,berr, &
                work,rwork,info)
        use la_constants_xdp
        ! -- lapack driver routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ...
        ! =====================================================================
           ...
           if (info /= 0) then
              call la_xerbla('YPTSVX',-info)
              return
           end if
           if (nofact) then
              ! compute the l*d*l**h (or u**h*d*u) factorization of a.
              call la_xcopy(n,d,1,df,1)
              if (n > 1) call la_ycopy(n - 1,e,1,ef,1)
              call la_ypttrf(n,df,ef,info)
              ! return if info is non-zero.
              if (info > 0) then
                 rcond = zero
                 return
           ...
           ! compute the norm of the matrix a.
           anorm = la_ylanht('1',n,d,e)
           ! compute the reciprocal of the condition number of a.
           call la_yptcon(n,df,ef,anorm,rcond,rwork,info)
           ! compute the solution vectors x.
           call la_ylacpy('FULL',n,nrhs,b,ldb,x,ldx)
           call la_ypttrs('LOWER',n,nrhs,df,ef,x,ldx,info)
           ! use iterative refinement to improve the computed solutions and
           ! compute error bounds and backward error estimates for them.
           call la_yptrfs('LOWER',n,nrhs,d,e,df,ef,b,ldb,x,ldx,ferr,berr,work, &
                     rwork,info)
           ! set info = n+1 if the matrix is singular to working precision.
           if (rcond < la_xlamch('EPSILON')) info = n + 1
           return
     end subroutine la_yptsvx
#endif
#ifdef LA_WITH_QP
