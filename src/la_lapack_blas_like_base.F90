...
module la_lapack_blas_like_base
     ...
     pure subroutine la_xlacpy(uplo,m,n,a,lda,b,ldb)
        ! -- lapack auxiliary routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ! Scalar Arguments
...
#ifdef LA_WITH_XDP
     !> XLAG2D: converts a EXTENDED PRECISION matrix, SA, to a SINGLE
     !> PRECISION matrix, A.
     !> RMAX is the overflow for the SINGLE PRECISION arithmetic
     !> XLAG2D checks that all the entries of A are between -RMAX and
     !> RMAX. If not the conversion is aborted and a flag is raised.
     !> This is an auxiliary routine so there is no argument checking.

     pure subroutine la_xlag2d(m,n,a,lda,sa,ldsa,info)
        ! -- lapack auxiliary routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ! Scalar Arguments
     ...
     !> distribution (n <= 128).
     !> This is an auxiliary routine called by XLARNV and YLARNV.

     pure subroutine la_xlaruv(iseed,n,x)
        ...
        ! =====================================================================
           ...
           20 continue
              ...
              it1 = it1 + i1*mm(i,4) + i2*mm(i,3) + i3*mm(i,2) + i4*mm(i,1)
              it1 = mod(it1,ipw2)
              ! convert 48-bit integer to a realnumber in the interval (0,1,KIND=xdp)
              x(i) = r*(real(it1,KIND=xdp) + r*(real(it2,KIND=xdp) + r*(real(it3,KIND=xdp) + &
                        r*real(it4,KIND=xdp))))
              if (x(i) == 1.0_xdp) then
                 ! if a real number has n bits of precision, and the first
                 ! n bits of the 48-bit integer above happen to be all 1 (which
                 ! will occur about once every 2**n calls), then x( i ) will
                 ! be rounded to exactly one.
                 ! since x( i ) is not supposed to return exactly 0.0_xdp or 1.0_xdp,
                 ! the statistically correct thing to do in this situation is
                 ! simply to iterate again.
                 ! n.b. the case x( i ) = 0.0_xdp should not be possible.
                 i1 = i1 + 2
                 i2 = i2 + 2
                 i3 = i3 + 2
                 i4 = i4 + 2
     ...
     pure subroutine la_xlaset(uplo,m,n,alpha,beta,a,lda)
        ! -- lapack auxiliary routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ! Scalar Arguments
...
#ifdef LA_WITH_XDP
     !> XLAT2D: converts a EXTENDED PRECISION triangular matrix, SA, to a SINGLE
     !> PRECISION triangular matrix, A.
     !> RMAX is the overflow for the SINGLE PRECISION arithmetic
     !> DLAS2S checks that all the entries of A are between -RMAX and
     !> RMAX. If not the conversion is aborted and a flag is raised.
     ...
     pure subroutine la_xlat2d(uplo,n,a,lda,sa,ldsa,info)
        ! -- lapack auxiliary routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ! Scalar Arguments
...
#ifdef LA_WITH_XDP
     !> XTFTTP: copies a triangular matrix A from rectangular full packed
     !> format (TF) to standard packed format (TP).

     pure subroutine la_xtfttp(transr,uplo,n,arf,ap,info)
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ! Scalar Arguments
           ...
           integer(ilp),intent(in) :: n
           ! Array Arguments
           real(xdp),intent(out) :: ap(0:*)
           real(xdp),intent(in) :: arf(0:*)
        ! =====================================================================
           ! Parameters
           ! Local Scalars
           logical(lk) :: lower,nisodd,normaltransr
...
#ifdef LA_WITH_XDP
     !> XTFTTR: copies a triangular matrix A from rectangular full packed
     !> format (TF) to standard full format (TR).

     pure subroutine la_xtfttr(transr,uplo,n,arf,a,lda,info)
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ! Scalar Arguments
           ...
           integer(ilp),intent(in) :: n,lda
           ! Array Arguments
           real(xdp),intent(out) :: a(0:lda - 1,0:*)
           real(xdp),intent(in) :: arf(0:*)
        ! =====================================================================
           ! Local Scalars
           logical(lk) :: lower,nisodd,normaltransr
...
#ifdef LA_WITH_XDP
     !> XTPTTF: copies a triangular matrix A from standard packed format (TP)
     !> to rectangular full packed format (TF).

     pure subroutine la_xtpttf(transr,uplo,n,ap,arf,info)
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ! Scalar Arguments
           ...
           integer(ilp),intent(in) :: n
           ! Array Arguments
           real(xdp),intent(in) :: ap(0:*)
           real(xdp),intent(out) :: arf(0:*)
        ! =====================================================================
           ! Parameters
           ! Local Scalars
           logical(lk) :: lower,nisodd,normaltransr
...
#ifdef LA_WITH_XDP
     !> XTPTTR: copies a triangular matrix A from standard packed format (TP)
     !> to standard full format (TR).

     pure subroutine la_xtpttr(uplo,n,ap,a,lda,info)
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ! Scalar Arguments
...
#ifdef LA_WITH_XDP
     !> XTRTTF: copies a triangular matrix A from standard full format (TR)
     !> to rectangular full packed format (TF) .

     pure subroutine la_xtrttf(transr,uplo,n,a,lda,arf,info)
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ! Scalar Arguments
           ...
           integer(ilp),intent(in) :: n,lda
           ! Array Arguments
           real(xdp),intent(in) :: a(0:lda - 1,0:*)
           real(xdp),intent(out) :: arf(0:*)
        ! =====================================================================
           ! Local Scalars
           logical(lk) :: lower,nisodd,normaltransr
...
#ifdef LA_WITH_XDP
     !> XTRTTP: copies a triangular matrix A from full format (TR) to standard
     !> packed format (TP).

     pure subroutine la_xtrttp(uplo,n,a,lda,ap,info)
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ! Scalar Arguments
...
#ifdef LA_WITH_XDP
     !> DLAG2X: converts a SINGLE PRECISION matrix, SA, to a DOUBLE
     !> PRECISION matrix, A.
     !> Note that while it is possible to overflow while converting
     !> from double to single, it is not possible to overflow when
     !> converting from single to double.
     ...
     pure subroutine la_dlag2x(m,n,sa,ldsa,a,lda,info)
        ! -- lapack auxiliary routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ! Scalar Arguments
     ...
     pure subroutine la_xlarnv(idist,iseed,n,x)
        use la_constants_xdp
        ! -- lapack auxiliary routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
        ...
        ! =====================================================================
           ! Parameters
           integer(ilp),parameter :: lv = 128
           real(xdp),parameter :: twopi = 6.28318530717958647692528676655900576839e+0_xdp
           ...
           do 40 iv = 1,n,lv/2
              ...
              ! call la_xlaruv to generate il2 numbers from a uniform (0,1)
              ! distribution (il2 <= lv)
              call la_xlaruv(iseed,il2,u)
              if (idist == 1) then
                 ! copy generated numbers
                 do i = 1,il
                    x(iv + i - 1) = u(i)
...
#ifdef LA_WITH_XDP
     !> ZLAG2Y: converts a COMPLEX matrix, SA, to a COMPLEX*16 matrix, A.
     !> Note that while it is possible to overflow while converting
     !> from double to single, it is not possible to overflow when
     !> converting from single to double.
     !> This is an auxiliary routine so there is no argument checking.

     pure subroutine la_zlag2y(m,n,sa,ldsa,a,lda,info)
        ! -- lapack auxiliary routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ! Scalar Arguments
...
#ifdef LA_WITH_XDP
     !> YLACP2: copies all or part of a real two-dimensional matrix A to a
     !> complex matrix B.

     pure subroutine la_ylacp2(uplo,m,n,a,lda,b,ldb)
        ! -- lapack auxiliary routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ! Scalar Arguments
...
#ifdef LA_WITH_XDP
     !> YLACPY: copies all or part of a two-dimensional matrix A to another
     !> matrix B.

     pure subroutine la_ylacpy(uplo,m,n,a,lda,b,ldb)
        ! -- lapack auxiliary routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ! Scalar Arguments
...
#ifdef LA_WITH_XDP
     !> YLAG2Z: converts a COMPLEX*16 matrix, SA, to a COMPLEX matrix, A.
     !> RMAX is the overflow for the SINGLE PRECISION arithmetic
     !> YLAG2Z checks that all the entries of A are between -RMAX and
     !> RMAX. If not the conversion is aborted and a flag is raised.
     !> This is an auxiliary routine so there is no argument checking.

     pure subroutine la_ylag2z(m,n,a,lda,sa,ldsa,info)
        ! -- lapack auxiliary routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ! Scalar Arguments
        ...
        ! =====================================================================
           ...
           do j = 1,n
              do i = 1,m
                 if ((real(a(i,j),KIND=xdp) < -rmax) .or. (real(a(i,j),KIND=xdp) > rmax) &
                           .or. (aimag(a(i,j)) < -rmax) .or. (aimag(a(i,j)) > rmax)) then
                    info = 1
                    go to 30
                 end if
...
#ifdef LA_WITH_XDP
     !> YLARNV: returns a vector of n random complex numbers from a uniform or
     !> normal distribution.

     pure subroutine la_ylarnv(idist,iseed,n,x)
        use la_constants_xdp
        ! -- lapack auxiliary routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
        ...
        ! =====================================================================
           ...
           do 60 iv = 1,n,lv/2
              il = min(lv/2,n - iv + 1)
              ! call la_xlaruv to generate 2*il realnumbers from a uniform (0,1,KIND=xdp)
              ! distribution (2*il <= lv)
              call la_xlaruv(iseed,2*il,u)
              if (idist == 1) then
                 ! copy generated numbers
                 do i = 1,il
                    x(iv + i - 1) = cmplx(u(2*i - 1),u(2*i),KIND=xdp)
                 end do
              else if (idist == 2) then
                 ! convert generated numbers to uniform (-1,1) distribution
                 do i = 1,il
                    x(iv + i - 1) = cmplx(two*u(2*i - 1) - one,two*u(2*i) - one,KIND=xdp)
                 end do
              else if (idist == 3) then
                 ! convert generated numbers to normal (0,1) distribution
                 do i = 1,il
                 ...
                 ! distributed on the unit disk
                 do i = 1,il
                    x(iv + i - 1) = sqrt(u(2*i - 1))*exp(cmplx(zero,twopi*u(2*i),KIND=xdp))
                 ...
                 ! distributed on the unit circle
                 do i = 1,il
                    x(iv + i - 1) = exp(cmplx(zero,twopi*u(2*i),KIND=xdp))
                 end do
              end if
              60 continue
           return
     end subroutine la_ylarnv
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
     !> YLASET: initializes a 2-D array A to BETA on the diagonal and
     !> ALPHA on the offdiagonals.

     pure subroutine la_ylaset(uplo,m,n,alpha,beta,a,lda)
        ! -- lapack auxiliary routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ! Scalar Arguments
...
#ifdef LA_WITH_XDP
     !> YLAT2Z: converts a COMPLEX*16 triangular matrix, SA, to a COMPLEX
     !> triangular matrix, A.
     !> RMAX is the overflow for the SINGLE PRECISION arithmetic
     !> YLAT2Z checks that all the entries of A are between -RMAX and
     !> RMAX. If not the conversion is aborted and a flag is raised.
     !> This is an auxiliary routine so there is no argument checking.

     pure subroutine la_ylat2z(uplo,n,a,lda,sa,ldsa,info)
        ! -- lapack auxiliary routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ! Scalar Arguments
...
#ifdef LA_WITH_XDP
     !> YTFTTP: copies a triangular matrix A from rectangular full packed
     !> format (TF) to standard packed format (TP).

     pure subroutine la_ytfttp(transr,uplo,n,arf,ap,info)
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ! Scalar Arguments
           ...
           integer(ilp),intent(in) :: n
           ! Array Arguments
           complex(xdp),intent(out) :: ap(0:*)
           complex(xdp),intent(in) :: arf(0:*)
        ! =====================================================================
           ! Parameters
           ! Local Scalars
           logical(lk) :: lower,nisodd,normaltransr
...
#ifdef LA_WITH_XDP
     !> YTFTTR: copies a triangular matrix A from rectangular full packed
     !> format (TF) to standard full format (TR).

     pure subroutine la_ytfttr(transr,uplo,n,arf,a,lda,info)
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ! Scalar Arguments
           ...
           integer(ilp),intent(in) :: n,lda
           ! Array Arguments
           complex(xdp),intent(out) :: a(0:lda - 1,0:*)
           complex(xdp),intent(in) :: arf(0:*)
        ! =====================================================================
           ! Parameters
           ! Local Scalars
...
#ifdef LA_WITH_XDP
     !> YTPTTF: copies a triangular matrix A from standard packed format (TP)
     !> to rectangular full packed format (TF).

     pure subroutine la_ytpttf(transr,uplo,n,ap,arf,info)
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ! Scalar Arguments
           ...
           integer(ilp),intent(in) :: n
           ! Array Arguments
           complex(xdp),intent(in) :: ap(0:*)
           complex(xdp),intent(out) :: arf(0:*)
        ! =====================================================================
           ! Parameters
           ! Local Scalars
           logical(lk) :: lower,nisodd,normaltransr
...
#ifdef LA_WITH_XDP
     !> YTPTTR: copies a triangular matrix A from standard packed format (TP)
     !> to standard full format (TR).

     pure subroutine la_ytpttr(uplo,n,ap,a,lda,info)
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ! Scalar Arguments
...
#ifdef LA_WITH_XDP
     !> YTRTTF: copies a triangular matrix A from standard full format (TR)
     !> to rectangular full packed format (TF) .

     pure subroutine la_ytrttf(transr,uplo,n,a,lda,arf,info)
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ! Scalar Arguments
           ...
           integer(ilp),intent(in) :: n,lda
           ! Array Arguments
           complex(xdp),intent(in) :: a(0:lda - 1,0:*)
           complex(xdp),intent(out) :: arf(0:*)
        ! =====================================================================
           ! Parameters
           ! Local Scalars
...
#ifdef LA_WITH_XDP
     !> YTRTTP: copies a triangular matrix A from full format (TR) to standard
     !> packed format (TP).

     pure subroutine la_ytrttp(uplo,n,a,lda,ap,info)
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ! Scalar Arguments
