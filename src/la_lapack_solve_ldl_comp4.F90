...
module la_lapack_solve_ldl_comp4
...
#ifdef LA_WITH_XDP
     public :: la_yhetf2_rk
     public :: la_yhetf2_rook
     public :: la_yhetri_rook
     public :: la_ylahef_rk
     public :: la_ylahef_rook
     public :: la_ylaqsy
     public :: la_yhetrf_rk
     public :: la_yhetrf_rook
     public :: la_yhetrs_aa
     public :: la_yhetrs_rook
     public :: la_yhecon_rook
     public :: la_yhprfs
     public :: la_yhetrf_aa
     public :: la_ylahef_aa
#endif
...
#ifdef LA_WITH_XDP
     !> XLAQSY: equilibrates a symmetric matrix A using the scaling factors
     !> in the vector S.

     pure subroutine la_xlaqsy(uplo,n,a,lda,s,scond,amax,equed)
...
#ifdef LA_WITH_XDP
     !> YHETF2_RK: computes the factorization of a complex Hermitian matrix A
     !> using the bounded Bunch-Kaufman (rook) diagonal pivoting method:
     !> A = P*U*D*(U**H)*(P**T) or A = P*L*D*(L**H)*(P**T),
     !> where U (or L) is unit upper (or lower) triangular matrix,
     !> U**H (or L**H) is the conjugate of U (or L), P is a permutation
     ...
     pure subroutine la_yhetf2_rk(uplo,n,a,lda,e,ipiv,info)
        use la_constants_xdp
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
        ...
        ! ======================================================================
           ...
           logical(lk) :: done,upper
           integer(ilp) :: i,ii,imax,itemp,j,jmax,k,kk,kp,kstep,p
           real(xdp) :: absakk,alpha,colmax,d,d11,d22,r1,xtemp,rowmax,tt,sfmin
           complex(xdp) :: d12,d21,t,wk,wkm1,wkp1,z
           ! Intrinsic Functions
           intrinsic :: abs,real,cmplx,conjg,aimag,max,sqrt
           ! Statement Functions
           real(xdp) :: cabs1
              ...
              if ((max(absakk,colmax) == zero)) then
                 ...
                 if (.not. (absakk < alpha*colmax)) then
                    ...
                    12 continue
                       ...
                       if (imax > 1) then
                          itemp = la_iyamax(imax - 1,a(1,imax),1)
                          xtemp = cabs1(a(itemp,imax))
                          if (xtemp > rowmax) then
                             rowmax = xtemp
                             jmax = itemp
                          end if
                       end if
                       ! case(2)
                       ! equivalent to testing for
                       ! abs( real( w( imax,kw-1 ),KIND=xdp) )>=alpha*rowmax
                       ! (used to handle nan and inf)
                       if (.not. (abs(real(a(imax,imax),KIND=xdp)) < alpha*rowmax)) &
                                 then
                          ! interchange rows and columns k and imax,
                          ! use 1-by-1 pivot block
                          kp = imax
                 ...
                 if ((kstep == 2) .and. (p /= k)) then
                    ! (1) swap columnar parts
                    if (p > 1) call la_yswap(p - 1,a(1,k),1,a(1,p),1)
                    ! (2) swap and conjugate middle parts
                    do j = p + 1,k - 1
                       t = conjg(a(j,k))
                       a(j,k) = conjg(a(p,j))
                    ...
                    a(p,k) = conjg(a(p,k))
                    ! (4) swap diagonal elements at row-col intersection
                    r1 = real(a(k,k),KIND=xdp)
                    a(k,k) = real(a(p,p),KIND=xdp)
                    a(p,p) = r1
                    ! convert upper triangle of a into u form by applying
                    ! the interchanges in columns k+1:n.
                    if (k < n) call la_yswap(n - k,a(k,k + 1),lda,a(p,k + 1),lda)
                 end if
                 ! for both 1x1 and 2x2 pivots, interchange rows and
                 ! columns kk and kp in the leading submatrix a(1:k,1:k)
                 if (kp /= kk) then
                    ! (1) swap columnar parts
                    if (kp > 1) call la_yswap(kp - 1,a(1,kk),1,a(1,kp),1)
                    ! (2) swap and conjugate middle parts
                    do j = kp + 1,kk - 1
                       t = conjg(a(j,kk))
                       a(j,kk) = conjg(a(kp,j))
                 ...
                 if (kstep == 1) then
                    ...
                    if (k > 1) then
                       ! perform a rank-1 update of a(1:k-1,1:k-1) and
                       ! store u(k) in column k
                       if (abs(real(a(k,k),KIND=xdp)) >= sfmin) then
                          ! perform a rank-1 update of a(1:k-1,1:k-1) as
                          ! a := a - u(k)*d(k)*u(k)**t
                             ! = a - w(k)*1/d(k)*w(k)**t
                          d11 = one/real(a(k,k),KIND=xdp)
                          call la_yher(uplo,k - 1,-d11,a(1,k),1,a,lda)
                          ! store u(k) in column k
                          call la_yxscal(k - 1,d11,a(1,k),1)
                       else
                          ! store l(k) in column k
                          d11 = real(a(k,k),KIND=xdp)
                          do ii = 1,k - 1
                             a(ii,k) = a(ii,k)/d11
                          end do
                          ! perform a rank-1 update of a(k+1:n,k+1:n) as
                    ...
                    if (k > 2) then
                       ...
                       d11 = real(a(k,k)/d,KIND=xdp)
                       d22 = real(a(k - 1,k - 1)/d,KIND=xdp)
                       d12 = a(k - 1,k)/d
                       tt = one/(d11*d22 - one)
                       do j = k - 2,1,-1
                          ! compute  d21 * ( w(k)w(k+1) ) * inv(d(k)) for row j
              ...
              if (max(absakk,colmax) == zero) then
                 ...
                 if (.not. (absakk < alpha*colmax)) then
                    ...
                    42 continue
                       ...
                       if (imax < n) then
                          itemp = imax + la_iyamax(n - imax,a(imax + 1,imax),1)
                          xtemp = cabs1(a(itemp,imax))
                          if (xtemp > rowmax) then
                             rowmax = xtemp
                             jmax = itemp
                 ...
                 if ((kstep == 2) .and. (p /= k)) then
                    ! (1) swap columnar parts
                    if (p < n) call la_yswap(n - p,a(p + 1,k),1,a(p + 1,p),1)
                    ! (2) swap and conjugate middle parts
                    do j = k + 1,p - 1
                       t = conjg(a(j,k))
                       a(j,k) = conjg(a(p,j))
                    ...
                    ! convert lower triangle of a into l form by applying
                    ! the interchanges in columns 1:k-1.
                    if (k > 1) call la_yswap(k - 1,a(k,1),lda,a(p,1),lda)
                 end if
                 ! for both 1x1 and 2x2 pivots, interchange rows and
                 ! columns kk and kp in the trailing submatrix a(k:n,k:n)
                 if (kp /= kk) then
                    ...
                    if (k < n) then
                       ...
                       if (abs(real(a(k,k),KIND=xdp)) >= sfmin) then
                          ...
                          ! a := a - l(k)*d(k)*l(k)**t
                             ! = a - w(k)*(1/d(k))*w(k)**t
                          d11 = one/real(a(k,k),KIND=xdp)
                          call la_yher(uplo,n - k,-d11,a(k + 1,k),1,a(k + 1,k + 1),lda)

                          ! store l(k) in column k
                          call la_yxscal(n - k,d11,a(k + 1,k),1)
                       else
                          ! store l(k) in column k
                          d11 = real(a(k,k),KIND=xdp)
                          do ii = k + 1,n
...
#ifdef LA_WITH_XDP
     !> YHETF2_ROOK: computes the factorization of a complex Hermitian matrix A
     !> using the bounded Bunch-Kaufman ("rook") diagonal pivoting method:
     !> A = U*D*U**H  or  A = L*D*L**H
     !> where U (or L) is a product of permutation and unit upper (lower)
     !> triangular matrices, U**H is the conjugate transpose of U, and D is
     ...
     pure subroutine la_yhetf2_rook(uplo,n,a,lda,ipiv,info)
        use la_constants_xdp
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
        ...
        ! ======================================================================
           ...
           if (info /= 0) then
              call la_xerbla('YHETF2_ROOK',-info)
              return
           end if
           ! initialize alpha for use in choosing pivot block size.
           alpha = (one + sqrt(sevten))/eight
...
#ifdef LA_WITH_XDP
     !> YHETRI_ROOK: computes the inverse of a complex Hermitian indefinite matrix
     !> A using the factorization A = U*D*U**H or A = L*D*L**H computed by
     !> YHETRF_ROOK.

     pure subroutine la_yhetri_rook(uplo,n,a,lda,ipiv,work,info)
        use la_constants_xdp
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
        ...
        ! =====================================================================
           ...
           if (info /= 0) then
              call la_xerbla('YHETRI_ROOK',-info)
              return
           end if
           ! quick return if possible
           if (n == 0) return
           ...
           if (upper) then
              ...
              if (kstep == 1) then
                 ...
                 kp = ipiv(k)
                 if (kp /= k) then
                    if (kp > 1) call la_yswap(kp - 1,a(1,k),1,a(1,kp),1)
                    do j = kp + 1,k - 1
                       temp = conjg(a(j,k))
                       a(j,k) = conjg(a(kp,j))
                       a(kp,j) = temp
...
#ifdef LA_WITH_XDP
     !> YLAHEF_RK: computes a partial factorization of a complex Hermitian
     !> matrix A using the bounded Bunch-Kaufman (rook) diagonal
     !> pivoting method. The partial factorization has the form:
     !> A  =  ( I  U12 ) ( A11  0  ) (  I       0    )  if UPLO = 'U', or:
     !> ( 0  U22 ) (  0   D  ) ( U12**H U22**H )
     ...
     !> where the order of D is at most NB. The actual order is returned in
     !> the argument KB, and is either NB or NB-1, or N if N <= NB.
     !> YLAHEF_RK is an auxiliary routine called by YHETRF_RK. It uses
     !> blocked code (calling Level 3 BLAS) to update the submatrix
     !> A11 (if UPLO = 'U') or A22 (if UPLO = 'L').

     pure subroutine la_ylahef_rk(uplo,n,nb,kb,a,lda,e,ipiv,w,ldw,info)
        use la_constants_xdp
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
        ...
        ! =====================================================================
           ...
           logical(lk) :: done
           integer(ilp) :: imax,itemp,ii,j,jb,jj,jmax,k,kk,kkw,kp,kstep,kw,p
           real(xdp) :: absakk,alpha,colmax,xtemp,r1,rowmax,t,sfmin
           ...
           if (la_lsame(uplo,'U')) then
              ...
              p = k
              ! copy column k of a to column kw of w and update it
              if (k > 1) call la_ycopy(k - 1,a(1,k),1,w(1,kw),1)
              ...
              if (max(absakk,colmax) == zero) then
                 ...
                 if (info == 0) info = k
                 kp = k
                 a(k,k) = real(w(k,kw),KIND=xdp)
                 if (k > 1) call la_ycopy(k - 1,w(1,kw),1,a(1,k),1)
                 ! set e( k ) to zero
                 if (k > 1) e(k) = czero
              else
                 ! ============================================================
                 ...
                 if (.not. (absakk < alpha*colmax)) then
                    ...
                    12 continue
                       ! begin pivot search loop body
                       ! copy column imax to column kw-1 of w and update it
                       if (imax > 1) call la_ycopy(imax - 1,a(1,imax),1,w(1,kw - 1),1)
                       ...
                       if (k < n) then
                          call la_ygemv('NO TRANSPOSE',k,n - k,-cone,a(1,k + 1),lda,w( &
                                    imax,kw + 1),ldw,cone,w(1,kw - 1),1)
                       ...
                       if (imax > 1) then
                          itemp = la_iyamax(imax - 1,w(1,kw - 1),1)
                          xtemp = cabs1(w(itemp,kw - 1))
                          if (xtemp > rowmax) then
                             rowmax = xtemp
                             jmax = itemp
                          end if
                       ...
                       ! abs( real( w( imax,kw-1 ),KIND=xdp) )>=alpha*rowmax
                       ! (used to handle nan and inf)
                       if (.not. (abs(real(w(imax,kw - 1),KIND=xdp)) < alpha*rowmax)) &
                                 then
                          ! interchange rows and columns k and imax,
                          ! use 1-by-1 pivot block
                          kp = imax
                 ...
                 if ((kstep == 2) .and. (p /= k)) then
                    ...
                    ! k and k-1 of a for 2-by-2 pivot, since these columns
                    ! will be later overwritten.
                    a(p,p) = real(a(k,k),KIND=xdp)
                    call la_ycopy(k - 1 - p,a(p + 1,k),1,a(p,p + 1),lda)
                    call la_ylacgv(k - 1 - p,a(p,p + 1),lda)
                    if (p > 1) call la_ycopy(p - 1,a(1,k),1,a(1,p),1)
                    ! interchange rows k and p in the last k+1 to n columns of a
                    ! (columns k and k-1 of a for 2-by-2 pivot will be
                    ! later overwritten). interchange rows k and p
                    ! in last kkw to nb columns of w.
                    if (k < n) call la_yswap(n - k,a(k,k + 1),lda,a(p,k + 1),lda)
                    call la_yswap(n - kk + 1,w(k,kkw),ldw,w(p,kkw),ldw)
                 end if
                 ! interchange rows and columns kp and kk.
                 ! updated column kp is already stored in column kkw of w.
                 if (kp /= kk) then
              ...
              do j = ((k - 1)/nb)*nb + 1,1,-nb
                 ...
                 ! update the rectangular superdiagonal block
                 if (j >= 2) call la_ygemm('NO TRANSPOSE','TRANSPOSE',j - 1,jb,n - k,-cone,a( &
                           1,k + 1),lda,w(j,kw + 1),ldw,cone,a(1,j),lda)
              end do
              ! set kb to the number of columns factorized
              kb = n - k
              ...
              if (k > 1) then
                 call la_ygemv('NO TRANSPOSE',n - k + 1,k - 1,-cone,a(k,1),lda,w(k,1), &
                           ldw,cone,w(k,k),1)
              ...
              if (max(absakk,colmax) == zero) then
                 ...
                 if (info == 0) info = k
                 kp = k
                 a(k,k) = real(w(k,k),KIND=xdp)
                 if (k < n) call la_ycopy(n - k,w(k + 1,k),1,a(k + 1,k),1)
                 ! set e( k ) to zero
                 if (k < n) e(k) = czero
              else
                 ! ============================================================
                 ...
                 if (.not. (absakk < alpha*colmax)) then
                    ...
                    72 continue
                       ...
                       if (k > 1) then
                          call la_ygemv('NO TRANSPOSE',n - k + 1,k - 1,-cone,a(k,1),lda,w( &
                                    imax,1),ldw,cone,w(k,k + 1),1)
                       ...
                       if (imax < n) then
                          itemp = imax + la_iyamax(n - imax,w(imax + 1,k + 1),1)
                          xtemp = cabs1(w(itemp,k + 1))
                          if (xtemp > rowmax) then
                             rowmax = xtemp
                             jmax = itemp
                          end if
                       ...
                       ! case(2)
                       ! equivalent to testing for
                       ! abs( real( w( imax,k+1 ),KIND=xdp) )>=alpha*rowmax
                       ! (used to handle nan and inf)
                       if (.not. (abs(real(w(imax,k + 1),KIND=xdp)) < alpha*rowmax)) &
                                 then
                          ! interchange rows and columns k and imax,
                          ! use 1-by-1 pivot block
                          kp = imax
                 ...
                 if ((kstep == 2) .and. (p /= k)) then
                    ...
                    ! will be later overwritten.
                    a(p,p) = real(a(k,k),KIND=xdp)
                    call la_ycopy(p - k - 1,a(k + 1,k),1,a(p,k + 1),lda)
                    call la_ylacgv(p - k - 1,a(p,k + 1),lda)
                    if (p < n) call la_ycopy(n - p,a(p + 1,k),1,a(p + 1,p),1)
                    ! interchange rows k and p in first k-1 columns of a
                    ! (columns k and k+1 of a for 2-by-2 pivot will be
                    ! later overwritten). interchange rows k and p
                    ! in first kk columns of w.
                    if (k > 1) call la_yswap(k - 1,a(k,1),lda,a(p,1),lda)
                    call la_yswap(kk,w(k,1),ldw,w(p,1),ldw)
                 end if
                 ! interchange rows and columns kp and kk.
                 ! updated column kp is already stored in column kk of w.
                 if (kp /= kk) then
...
#ifdef LA_WITH_XDP
     !> YLAHEF_ROOK: computes a partial factorization of a complex Hermitian
     !> matrix A using the bounded Bunch-Kaufman ("rook") diagonal pivoting
     !> method. The partial factorization has the form:
     !> A  =  ( I  U12 ) ( A11  0  ) (  I      0     )  if UPLO = 'U', or:
     !> ( 0  U22 ) (  0   D  ) ( U12**H U22**H )
     ...
     !> the argument KB, and is either NB or NB-1, or N if N <= NB.
     !> Note that U**H denotes the conjugate transpose of U.
     !> YLAHEF_ROOK is an auxiliary routine called by YHETRF_ROOK. It uses
     !> blocked code (calling Level 3 BLAS) to update the submatrix
     !> A11 (if UPLO = 'U') or A22 (if UPLO = 'L').

     pure subroutine la_ylahef_rook(uplo,n,nb,kb,a,lda,ipiv,w,ldw,info)
        use la_constants_xdp
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
        ...
        ! =====================================================================
           ...
           if (la_lsame(uplo,'U')) then
              ...
              60 continue
                 ...
                 ! of the rows to swap back doesn't include diagonal element)
                 j = j + 1
                 if (jp2 /= jj .and. j <= n) call la_yswap(n - j + 1,a(jp2,j),lda,a(jj,j), &
                           lda)
                 jj = jj + 1
                 if (kstep == 2 .and. jp1 /= jj .and. j <= n) call la_yswap(n - j + 1,a(jp1,j), &
                           lda,a(jj,j),lda)
              if (j < n) go to 60
              ! set kb to the number of columns factorized
              kb = n - k
              ...
              120 continue
                 ...
                 ! of the rows to swap back doesn't include diagonal element)
                 j = j - 1
                 if (jp2 /= jj .and. j >= 1) call la_yswap(j,a(jp2,1),lda,a(jj,1),lda)

                 jj = jj - 1
                 if (kstep == 2 .and. jp1 /= jj .and. j >= 1) call la_yswap(j,a(jp1,1),lda,a( &
                            jj,1),lda)
              if (j > 1) go to 120
              ! set kb to the number of columns factorized
              kb = k - 1
...
#ifdef LA_WITH_XDP
     !> YLAQSY: equilibrates a symmetric matrix A using the scaling factors
     !> in the vector S.

     pure subroutine la_ylaqsy(uplo,n,a,lda,s,scond,amax,equed)
        use la_constants_xdp
        ! -- lapack auxiliary routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
...
#ifdef LA_WITH_XDP
     !> YHETRF_RK: computes the factorization of a complex Hermitian matrix A
     !> using the bounded Bunch-Kaufman (rook) diagonal pivoting method:
     !> A = P*U*D*(U**H)*(P**T) or A = P*L*D*(L**H)*(P**T),
     !> where U (or L) is unit upper (or lower) triangular matrix,
     !> U**H (or L**H) is the conjugate of U (or L), P is a permutation
     ...
     pure subroutine la_yhetrf_rk(uplo,n,a,lda,e,ipiv,work,lwork,info)
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ! Scalar Arguments
        ...
        ! =====================================================================
           ...
           if (info == 0) then
              ! determine the block size
              nb = la_ilaenv(1,'YHETRF_RK',uplo,n,-1,-1,-1)
              lwkopt = n*nb
              work(1) = lwkopt
           end if
           if (info /= 0) then
              call la_xerbla('YHETRF_RK',-info)
              return
           else if (lquery) then
              return
           end if
           ...
           if (nb > 1 .and. nb < n) then
              ...
              if (lwork < iws) then
                 nb = max(lwork/ldwork,1)
                 nbmin = max(2,la_ilaenv(2,'YHETRF_RK',uplo,n,-1,-1,-1))
              end if
           else
              iws = 1
           end if
           ...
           if (upper) then
              ! factorize a as u*d*u**t using the upper triangle of a
              ! k is the main loop index, decreasing from n to 1 in steps of
              ! kb, where kb is the number of columns factorized by la_ylahef_rk;
              ! kb is either nb or nb-1, or k for the last block
              k = n
              10 continue
              ! if k < 1, exit from loop
              ...
              if (k > nb) then
                 ! factorize columns k-kb+1:k of a and use blocked code to
                 ! update columns 1:k-kb
                 call la_ylahef_rk(uplo,k,nb,kb,a,lda,e,ipiv,work,ldwork,iinfo)

              else
                 ! use unblocked code to factorize columns 1:k of a
                 call la_yhetf2_rk(uplo,k,a,lda,e,ipiv,iinfo)
                 kb = k
              end if
              ! set info on the first occurrence of a zero pivot
              if (info == 0 .and. iinfo > 0) info = iinfo
              ...
              if (k < n) then
                 do i = k, (k - kb + 1),-1
                    ip = abs(ipiv(i))
                    if (ip /= i) then
                       call la_yswap(n - k,a(i,k + 1),lda,a(ip,k + 1),lda)
                    end if
                 end do
              end if
              ! decrease k and return to the start of the main loop
              ...
              if (k <= n - nb) then
                 ! factorize columns k:k+kb-1 of a and use blocked code to
                 ! update columns k+kb:n
                 call la_ylahef_rk(uplo,n - k + 1,nb,kb,a(k,k),lda,e(k),ipiv(k), &
                           work,ldwork,iinfo)
              else
                 ! use unblocked code to factorize columns k:n of a
                 call la_yhetf2_rk(uplo,n - k + 1,a(k,k),lda,e(k),ipiv(k),iinfo)
              ...
              if (k > 1) then
                 do i = k, (k + kb - 1),1
                    ip = abs(ipiv(i))
                    if (ip /= i) then
                       call la_yswap(k - 1,a(i,1),lda,a(ip,1),lda)
                    end if
                 end do
              end if
              ! increase k and return to the start of the main loop
...
#ifdef LA_WITH_XDP
     !> YHETRF_ROOK: computes the factorization of a complex Hermitian matrix A
     !> using the bounded Bunch-Kaufman ("rook") diagonal pivoting method.
     !> The form of the factorization is
     !> A = U*D*U**T  or  A = L*D*L**T
     !> where U (or L) is a product of permutation and unit upper (lower)
     ...
     pure subroutine la_yhetrf_rook(uplo,n,a,lda,ipiv,work,lwork,info)
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ! Scalar Arguments
        ...
        ! =====================================================================
           ...
           if (info /= 0) then
              call la_xerbla('YHETRF_ROOK',-info)
              return
           else if (lquery) then
              return
           end if
           ...
           if (nb > 1 .and. nb < n) then
              ...
              if (lwork < iws) then
                 nb = max(lwork/ldwork,1)
                 nbmin = max(2,la_ilaenv(2,'YHETRF_ROOK',uplo,n,-1,-1,-1))
              end if
           else
              iws = 1
           end if
           ...
           if (upper) then
              ! factorize a as u*d*u**t using the upper triangle of a
              ! k is the main loop index, decreasing from n to 1 in steps of
              ! kb, where kb is the number of columns factorized by la_ylahef_rook;
              ! kb is either nb or nb-1, or k for the last block
              k = n
              10 continue
              ! if k < 1, exit from loop
              ...
              if (k > nb) then
                 ! factorize columns k-kb+1:k of a and use blocked code to
                 ! update columns 1:k-kb
                 call la_ylahef_rook(uplo,k,nb,kb,a,lda,ipiv,work,ldwork,iinfo)

              else
                 ! use unblocked code to factorize columns 1:k of a
                 call la_yhetf2_rook(uplo,k,a,lda,ipiv,iinfo)
                 kb = k
              end if
              ! set info on the first occurrence of a zero pivot
              if (info == 0 .and. iinfo > 0) info = iinfo
              ...
              if (k <= n - nb) then
                 ! factorize columns k:k+kb-1 of a and use blocked code to
                 ! update columns k+kb:n
                 call la_ylahef_rook(uplo,n - k + 1,nb,kb,a(k,k),lda,ipiv(k),work, &
                           ldwork,iinfo)
              else
                 ! use unblocked code to factorize columns k:n of a
                 call la_yhetf2_rook(uplo,n - k + 1,a(k,k),lda,ipiv(k),iinfo)
                 kb = n - k + 1
              end if
              ! set info on the first occurrence of a zero pivot
              if (info == 0 .and. iinfo > 0) info = iinfo + k - 1
...
#ifdef LA_WITH_XDP
     !> YHETRS_AA: solves a system of linear equations A*X = B with a complex
     !> hermitian matrix A using the factorization A = U**H*T*U or
     !> A = L*T*L**H computed by YHETRF_AA.

     pure subroutine la_yhetrs_aa(uplo,n,nrhs,a,lda,ipiv,b,ldb,work,lwork,info)
        use la_constants_xdp
        ...
        ! =====================================================================
           ...
           if (upper) then
              ...
              if (n > 1) then
                 ...
                 ! compute u**h \ b -> b    [ (u**h \p**t * b) ]
                 call la_ytrsm('L','U','C','U',n - 1,nrhs,cone,a(1,2),lda,b(2,1), &
                            ldb)
              end if
              ! 2) solve with triangular matrix t
              ! compute t \ b -> b   [ t \ (u**h \p**t * b) ]
              call la_ylacpy('F',1,n,a(1,1),lda + 1,work(n),1)
              if (n > 1) then
                  call la_ylacpy('F',1,n - 1,a(1,2),lda + 1,work(2*n),1)
                  call la_ylacpy('F',1,n - 1,a(1,2),lda + 1,work(1),1)
                  call la_ylacgv(n - 1,work(1),1)
              end if
              call la_ygtsv(n,nrhs,work(1),work(n),work(2*n),b,ldb,info)
              ! 3) backward substitution with u
              if (n > 1) then
                 ! compute u \ b -> b   [ u \ (t \ (u**h \p**t * b) ) ]
                 call la_ytrsm('L','U','N','U',n - 1,nrhs,cone,a(1,2),lda,b(2,1), &
                           ldb)
                 ! pivot, p * b  [ p * (u**h \ (t \ (u \p**t * b) )) ]
                 do k = n,1,-1
                    kp = ipiv(k)
              ...
              if (n > 1) then
                 ...
                 ! compute l \ b -> b    [ (l \p**t * b) ]
                 call la_ytrsm('L','L','N','U',n - 1,nrhs,cone,a(2,1),lda,b(2,1), &
                           ldb)
              end if
              ! 2) solve with triangular matrix t
              ! compute t \ b -> b   [ t \ (l \p**t * b) ]
              call la_ylacpy('F',1,n,a(1,1),lda + 1,work(n),1)
              if (n > 1) then
                  call la_ylacpy('F',1,n - 1,a(2,1),lda + 1,work(1),1)
                  call la_ylacpy('F',1,n - 1,a(2,1),lda + 1,work(2*n),1)
                  call la_ylacgv(n - 1,work(2*n),1)
              end if
              call la_ygtsv(n,nrhs,work(1),work(n),work(2*n),b,ldb,info)
              ! 3) backward substitution with l**h
              if (n > 1) then
                 ! compute l**h \ b -> b   [ l**h \ (t \ (l \p**t * b) ) ]
                 call la_ytrsm('L','L','C','U',n - 1,nrhs,cone,a(2,1),lda,b(2,1), &
                            ldb)
                 ! pivot, p * b  [ p * (l**h \ (t \ (l \p**t * b) )) ]
                 do k = n,1,-1
                    kp = ipiv(k)
...
#ifdef LA_WITH_XDP
     !> YHETRS_ROOK: solves a system of linear equations A*X = B with a complex
     !> Hermitian matrix A using the factorization A = U*D*U**H or
     !> A = L*D*L**H computed by YHETRF_ROOK.

     pure subroutine la_yhetrs_rook(uplo,n,nrhs,a,lda,ipiv,b,ldb,info)
        use la_constants_xdp
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
        ...
        ! =====================================================================
           ...
           if (info /= 0) then
              call la_xerbla('YHETRS_ROOK',-info)
              return
           end if
           ! quick return if possible
           if (n == 0 .or. nrhs == 0) return
...
#ifdef LA_WITH_XDP
     !> YHECON_ROOK: estimates the reciprocal of the condition number of a complex
     !> Hermitian matrix A using the factorization A = U*D*U**H or
     !> A = L*D*L**H computed by ZHETRF_ROOK.
     !> An estimate is obtained for norm(inv(A)), and the reciprocal of the
     !> condition number is computed as RCOND = 1 / (ANORM * norm(inv(A))).

     pure subroutine la_yhecon_rook(uplo,n,a,lda,ipiv,anorm,rcond,work,info)
        use la_constants_xdp
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
        ...
        ! =====================================================================
           ...
           if (info /= 0) then
              call la_xerbla('YHECON_ROOK',-info)
              return
           end if
           ! quick return if possible
           rcond = zero
           ...
           if (kase /= 0) then
              ! multiply by inv(l*d*l**h) or inv(u*d*u**h).
              call la_yhetrs_rook(uplo,n,1,a,lda,ipiv,work,n,info)
              go to 30
           end if
           ! compute the estimate of the reciprocal condition number.
           if (ainvnm /= zero) rcond = (one/ainvnm)/anorm
           return
     end subroutine la_yhecon_rook
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
     !> YHPRFS: improves the computed solution to a system of linear
     !> equations when the coefficient matrix is Hermitian indefinite
     !> and packed, and provides error bounds and backward error estimates
     !> for the solution.

     pure subroutine la_yhprfs(uplo,n,nrhs,ap,afp,ipiv,b,ldb,x,ldx,ferr,berr,work, &
                rwork,info)
        ...
        ! =====================================================================
           ...
           loop_140: do j = 1,nrhs
              ...
              call la_yhpmv(uplo,n,-cone,ap,x(1,j),1,cone,work,1)
              ! compute componentwise relative backward error from formula
              ! max(i) ( abs(r(i)) / ( abs(a)*abs(x) + abs(b) )(i) )
              ! where abs(z) is the componentwise absolute value of the matrix
              ! or vector z.  if the i-th component of the denominator is less
              ...
              if (upper) then
                 do k = 1,n
                    ...
                    rwork(k) = rwork(k) + abs(real(ap(kk + k - 1),KIND=xdp))*xk + s
                    kk = kk + k
                 end do
              else
                 do k = 1,n
                    s = zero
                    xk = cabs1(x(k,j))
                    rwork(k) = rwork(k) + abs(real(ap(kk),KIND=xdp))*xk
                    ik = kk + 1
                    do i = k + 1,n
                       rwork(i) = rwork(i) + cabs1(ap(ik))*xk
                       s = s + cabs1(ap(ik))*cabs1(x(i,j))
              ...
              if (berr(j) > eps .and. two*berr(j) <= lstres .and. count <= itmax) then
                 ! update solution and try again.
                 call la_yhptrs(uplo,n,1,afp,ipiv,work,n,info)
...
#ifdef LA_WITH_XDP
     !> YHETRF_AA: computes the factorization of a complex hermitian matrix A
     !> using the Aasen's algorithm.  The form of the factorization is
     !> A = U**H*T*U  or  A = L*T*L**H
     !> where U (or L) is a product of permutation and unit upper (lower)
     !> triangular matrices, and T is a hermitian tridiagonal matrix.
     ...
     pure subroutine la_yhetrf_aa(uplo,n,a,lda,ipiv,work,lwork,info)
        use la_constants_xdp
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
        ...
        ! =====================================================================
           ...
           ! Executable Statements
           ! determine the block size
           nb = la_ilaenv(1,'YHETRF_AA',uplo,n,-1,-1,-1)
           ! test the input parameters.
           info = 0
           upper = la_lsame(uplo,'U')
           lquery = (lwork == -1)
           ...
           if (upper) then
              ...
              ! .....................................................
              ! copy first row a(1, 1:n) into h(1:n) (stored in work(1:n))
              call la_ycopy(n,a(1,1),lda,work(1),1)
              ! j is the main loop index, increasing from 1 to n in steps of
              ! jb, where jb is the number of columns factorized by la_ylahef;
              ! jb is either nb, or n-j+1 for the last block
              j = 0
              10 continue
              if (j >= n) go to 20
              ...
              k1 = max(1,j) - j
              ! panel factorization
              call la_ylahef_aa(uplo,2 - k1,n - j,jb,a(max(1,j),j + 1),lda,ipiv(j + 1), &
                        work,n,work(n*nb + 1))
              ! adjust ipiv and apply it back (j-th step picks (j+1)-th pivot)
              do j2 = j + 2,min(n,j + jb + 1)
                 ipiv(j2) = ipiv(j2) + j
                 if ((j2 /= ipiv(j2)) .and. ((j1 - k1) > 2)) then
                    call la_yswap(j1 - k1 - 2,a(1,j2),1,a(1,ipiv(j2)),1)
                 end if
              end do
              j = j + jb
              ! trailing submatrix update, where
              ...
              if (j < n) then
                ! if the first panel and jb=1 (nb=1), then nothing to do
                 if (j1 > 1 .or. jb > 1) then
                    ...
                    alpha = conjg(a(j,j + 1))
                    a(j,j + 1) = cone
                    call la_ycopy(n - j,a(j - 1,j + 1),lda,work((j + 1 - j1 + 1) + jb*n),1)

                    call la_yscal(n - j,alpha,work((j + 1 - j1 + 1) + jb*n),1)
                    ! k1 identifies if the previous column of the panel has been
                     ! explicitly stored, e.g., k1=0 and k2=1 for the first panel,
                     ! and k1=1 and k2=0 for the rest
                    if (j1 > 1) then
                    ...
                    do j2 = j + 1,n,nb
                       nj = min(nb,n - j2 + 1)
                       ! update (j2, j2) diagonal block with la_ygemv
                       j3 = j2
                       do mj = nj - 1,1,-1
                          call la_ygemm('CONJUGATE TRANSPOSE','TRANSPOSE',1,mj,jb + 1,-cone, &
                           a(j1 - k2,j3),lda,work((j3 - j1 + 1) + k1*n),n,cone,a(j3,j3),lda)

                          j3 = j3 + 1
                       end do
                       ! update off-diagonal block of j2-th block row with la_ygemm
                       call la_ygemm('CONJUGATE TRANSPOSE','TRANSPOSE',nj,n - j3 + 1,jb + 1,- &
                       cone,a(j1 - k2,j2),lda,work((j3 - j1 + 1) + k1*n),n,cone,a(j2,j3),lda &
                                 )
                    end do
                    ! recover t( j, j+1 )
                 ...
                 ! work(j+1, 1) stores h(j+1, 1)
                 call la_ycopy(n - j,a(j + 1,j + 1),lda,work(1),1)
              end if
              go to 10
           else
              ! .....................................................
              ...
              ! copy first column a(1:n, 1) into h(1:n, 1)
               ! (stored in work(1:n))
              call la_ycopy(n,a(1,1),1,work(1),1)
              ! j is the main loop index, increasing from 1 to n in steps of
              ! jb, where jb is the number of columns factorized by la_ylahef;
              ! jb is either nb, or n-j+1 for the last block
              j = 0
              ...
              k1 = max(1,j) - j
              ! panel factorization
              call la_ylahef_aa(uplo,2 - k1,n - j,jb,a(j + 1,max(1,j)),lda,ipiv(j + 1), &
                        work,n,work(n*nb + 1))
              ! adjust ipiv and apply it back (j-th step picks (j+1)-th pivot)
              do j2 = j + 2,min(n,j + jb + 1)
                 ipiv(j2) = ipiv(j2) + j
                 if ((j2 /= ipiv(j2)) .and. ((j1 - k1) > 2)) then
                    call la_yswap(j1 - k1 - 2,a(j2,1),lda,a(ipiv(j2),1),lda)
                 end if
              end do
              j = j + jb
              ! trailing submatrix update, where
              ...
              if (j < n) then
                ! if the first panel and jb=1 (nb=1), then nothing to do
                 if (j1 > 1 .or. jb > 1) then
                    ...
                    alpha = conjg(a(j + 1,j))
                    a(j + 1,j) = cone
                    call la_ycopy(n - j,a(j + 1,j - 1),1,work((j + 1 - j1 + 1) + jb*n),1)
                    call la_yscal(n - j,alpha,work((j + 1 - j1 + 1) + jb*n),1)
                    ! k1 identifies if the previous column of the panel has been
                     ! explicitly stored, e.g., k1=0 and k2=1 for the first panel,
                     ! and k1=1 and k2=0 for the rest
                    ...
                    do j2 = j + 1,n,nb
                       ...
                       j3 = j2
                       do mj = nj - 1,1,-1
                          call la_ygemm('NO TRANSPOSE','CONJUGATE TRANSPOSE',mj,1,jb + 1,- &
                          cone,work((j3 - j1 + 1) + k1*n),n,a(j3,j1 - k2),lda,cone,a(j3,j3), &
                                    lda)
                          j3 = j3 + 1
                       end do
                       ! update off-diagonal block of j2-th block column with la_ygemm
                       call la_ygemm('NO TRANSPOSE','CONJUGATE TRANSPOSE',n - j3 + 1,nj,jb + 1,- &
                       cone,work((j3 - j1 + 1) + k1*n),n,a(j2,j1 - k2),lda,cone,a(j3,j2),lda &
                                 )
                    end do
                    ! recover t( j+1, j )
                 ...
                 ! work(j+1, 1) stores h(j+1, 1)
                 call la_ycopy(n - j,a(j + 1,j + 1),1,work(1),1)
              end if
              go to 11
           end if
           20 continue
     ...
     pure subroutine la_ylahef_aa(uplo,j1,m,nb,a,lda,ipiv,h,ldh,work)
        use la_constants_xdp
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
        ...
        ! =====================================================================
           ...
           if (la_lsame(uplo,'U')) then
              ...
              if (j > min(m,nb)) go to 20
              ! k is the column to be factorized
               ! when being called from la_yhetrf_aa,
               ! > for the first block column, j1 is 1, hence j1+j-1 is j,
               ! > for the rest of the columns, j1 is 2, and j1+j-1 is j+1,
              k = j1 + j - 1
              if (j == m) then
              ...
              ! k is the column to be factorized
               ...
               ! > for the rest of the columns, k is j+1, skipping only the
                 ! first column
                 call la_ylacgv(j - k1,a(1,j),1)
                 call la_ygemv('NO TRANSPOSE',mj,j - k1,-cone,h(j,k1),ldh,a(1,j),1, &
                           cone,h(j,j),1)
                 call la_ylacgv(j - k1,a(1,j),1)
              end if
              ! copy h(i:n, i) into work
              call la_ycopy(mj,h(j,j),1,work(1),1)
              if (j > k1) then
                 ! compute work := work - l(j-1, j:n) * t(j-1,j),
                  ! where a(j-1, j) stores t(j-1, j) and a(j-2, j:n) stores u(j-1, j:n)
                 alpha = -conjg(a(k - 1,j))
                 call la_yaxpy(mj,alpha,a(k - 2,j),lda,work(1),1)
              end if
              ! set a(j, j) = t(j, j)
              a(k,j) = real(work(1),KIND=xdp)
              if (j < m) then
                 ! compute work(2:n) = t(j, j) l(j, (j+1):n)
                  ! where a(j, j) stores t(j, j) and a(j-1, (j+1):n) stores u(j, (j+1):n)
                 if (k > 1) then
                    alpha = -a(k,j)
                    call la_yaxpy(m - j,alpha,a(k - 1,j + 1),lda,work(2),1)
                 end if
                 ! find max(|work(2:n)|)
                 i2 = la_iyamax(m - j,work(2),1) + 1
                 piv = work(i2)
                 ! apply hermitian pivot
                 if ((i2 /= 2) .and. (piv /= 0)) then
                    ! swap work(i1) and work(i2)
                    ...
                    i1 = i1 + j - 1
                    i2 = i2 + j - 1
                    call la_yswap(i2 - i1 - 1,a(j1 + i1 - 1,i1 + 1),lda,a(j1 + i1,i2),1)

                    call la_ylacgv(i2 - i1,a(j1 + i1 - 1,i1 + 1),lda)
                    call la_ylacgv(i2 - i1 - 1,a(j1 + i1,i2),1)
                    ! swap a(i1, i2+1:n) with a(i2, i2+1:n)
                    if (i2 < m) call la_yswap(m - i2,a(j1 + i1 - 1,i2 + 1),lda,a(j1 + i2 - 1,i2 + 1), &
                               lda)
                    ! swap a(i1, i1) with a(i2,i2)
                    piv = a(i1 + j1 - 1,i1)
                    a(j1 + i1 - 1,i1) = a(j1 + i2 - 1,i2)
                    a(j1 + i2 - 1,i2) = piv
                    ! swap h(i1, 1:j1) with h(i2, 1:j1)
                    call la_yswap(i1 - 1,h(i1,1),ldh,h(i2,1),ldh)
                    ipiv(i1) = i2
                    if (i1 > (k1 - 1)) then
                       ! swap l(1:i1-1, i1) with l(1:i1-1, i2),
                        ! skipping the first column
                       call la_yswap(i1 - k1 + 1,a(1,i1),1,a(1,i2),1)
                    end if
                 else
                    ipiv(j + 1) = j + 1
                 end if
                 ...
                 if (j < nb) then
                    ! copy a(j+1:n, j+1) into h(j:n, j),
                    call la_ycopy(m - j,a(k + 1,j + 1),lda,h(j + 1,j + 1),1)
                 end if
                 ! compute l(j+2, j+1) = work( 3:n ) / t(j, j+1),
                  ! where a(j, j+1) = t(j, j+1) and a(j+2:n, j) = l(j+2:n, j+1)
                 if (j < (m - 1)) then
                    if (a(k,j + 1) /= czero) then
                       alpha = cone/a(k,j + 1)
                       call la_ycopy(m - j - 1,work(3),1,a(k,j + 2),lda)
                       call la_yscal(m - j - 1,alpha,a(k,j + 2),lda)
                    else
                       call la_ylaset('FULL',1,m - j - 1,czero,czero,a(k,j + 2),lda)
              ...
              ! k is the column to be factorized
               ...
               ! > for the rest of the columns, k is j+1, skipping only the
                 ! first column
                 call la_ylacgv(j - k1,a(j,1),lda)
                 call la_ygemv('NO TRANSPOSE',mj,j - k1,-cone,h(j,k1),ldh,a(j,1), &
                           lda,cone,h(j,j),1)
                 call la_ylacgv(j - k1,a(j,1),lda)
              end if
              ! copy h(j:n, j) into work
              ...
              if (j > k1) then
                 ! compute work := work - l(j:n, j-1) * t(j-1,j),
                  ! where a(j-1, j) = t(j-1, j) and a(j, j-2) = l(j, j-1)
                 alpha = -conjg(a(j,k - 1))
                 call la_yaxpy(mj,alpha,a(j,k - 2),1,work(1),1)
              end if
              ! set a(j, j) = t(j, j)
              a(j,k) = real(work(1),KIND=xdp)
              if (j < m) then
                 ! compute work(2:n) = t(j, j) l((j+1):n, j)
                  ! where a(j, j) = t(j, j) and a((j+1):n, j-1) = l((j+1):n, j)
                 if (k > 1) then
                    alpha = -a(j,k)
                    call la_yaxpy(m - j,alpha,a(j + 1,k - 1),1,work(2),1)
                 end if
                 ! find max(|work(2:n)|)
                 i2 = la_iyamax(m - j,work(2),1) + 1
                 piv = work(i2)
                 ...
                 if ((i2 /= 2) .and. (piv /= 0)) then
                    ...
                    i1 = i1 + j - 1
                    i2 = i2 + j - 1
                    call la_yswap(i2 - i1 - 1,a(i1 + 1,j1 + i1 - 1),1,a(i2,j1 + i1),lda)

                    call la_ylacgv(i2 - i1,a(i1 + 1,j1 + i1 - 1),1)
                    call la_ylacgv(i2 - i1 - 1,a(i2,j1 + i1),lda)
                    ! swap a(i2+1:n, i1) with a(i2+1:n, i2)
                    if (i2 < m) call la_yswap(m - i2,a(i2 + 1,j1 + i1 - 1),1,a(i2 + 1,j1 + i2 - 1), &
                              1)
                    ! swap a(i1, i1) with a(i2, i2)
                    piv = a(i1,j1 + i1 - 1)
                    a(i1,j1 + i1 - 1) = a(i2,j1 + i2 - 1)
                    ...
                    if (i1 > (k1 - 1)) then
                       ! swap l(1:i1-1, i1) with l(1:i1-1, i2),
                        ! skipping the first column
                       call la_yswap(i1 - k1 + 1,a(i1,1),lda,a(i2,1),lda)
                    end if
                 else
                    ipiv(j + 1) = j + 1
                 end if
                 ...
                 if (j < nb) then
                    ! copy a(j+1:n, j+1) into h(j+1:n, j),
                    call la_ycopy(m - j,a(j + 1,k + 1),1,h(j + 1,j + 1),1)
                 end if
                 ! compute l(j+2, j+1) = work( 3:n ) / t(j, j+1),
                  ! where a(j, j+1) = t(j, j+1) and a(j+2:n, j) = l(j+2:n, j+1)
                 if (j < (m - 1)) then
                    if (a(j + 1,k) /= czero) then
                       alpha = cone/a(j + 1,k)
                       call la_ycopy(m - j - 1,work(3),1,a(j + 2,k),1)
                       call la_yscal(m - j - 1,alpha,a(j + 2,k),1)
                    else
                       call la_ylaset('FULL',m - j - 1,1,czero,czero,a(j + 2,k),lda)
