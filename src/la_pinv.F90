...
module la_pseudoinverse
...
#ifdef LA_WITH_XDP
        module procedure la_pseudoinverse_x
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
        module procedure la_pseudoinverse_y
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
        module procedure la_pseudoinvert_x
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
        module procedure la_pseudoinvert_y
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
        module procedure la_pinv_x_operator
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
        module procedure la_pinv_y_operator
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
     ! Compute the in-place pseudo-inverse of matrix a
     subroutine la_pseudoinvert_x(a,pinva,rtol,err)
         !> Input matrix a[m,n]
         real(xdp),intent(inout) :: a(:,:)
         !> Output pseudo-inverse matrix
         real(xdp),intent(inout) :: pinva(:,:)
         !> [optional] ....
         real(xdp),optional,intent(in) :: rtol
         !> [optional] state return flag. On error if not requested, the code will stop
         type(la_state),optional,intent(out) :: err

         ! Local variables
         real(xdp) :: tolerance,cutoff
         real(xdp),allocatable :: s(:)
         real(xdp),allocatable :: u(:,:),vt(:,:)
         type(la_state) :: err0
         integer(ilp) :: m,n,k,i,j
         ...
         ! Singular value threshold
         tolerance = max(m,n)*epsilon(0.0_xdp)
         
         ! User threshold: fallback to default if <=0
         if (present(rtol)) then
            if (rtol > 0.0_xdp) tolerance = rtol
         end if
     ...
     end subroutine la_pseudoinvert_x

     ! Function interface
     function la_pseudoinverse_x(a,rtol,err) result(pinva)
         !> Input matrix a[m,n]
         ...
         type(la_state),optional,intent(out) :: err
         !> Matrix pseudo-inverse
         real(xdp) :: pinva(size(a,2,kind=ilp),size(a,1,kind=ilp))
         
         ! Use pointer to circumvent svd intent(inout) restriction
         real(xdp),pointer :: ap(:,:)
         ap => a
         
         call la_pseudoinvert_x(ap,pinva,rtol,err)

     end function la_pseudoinverse_x

     ! Inverse matrix operator
     function la_pinv_x_operator(a) result(pinva)
         !> Input matrix a[m,n]
         real(xdp),intent(in),target :: a(:,:)
         !> Result matrix
         real(xdp) :: pinva(size(a,2,kind=ilp),size(a,1,kind=ilp))
         ...
         call la_pseudoinvert_x(ap,pinva)

     end function la_pinv_x_operator
#endif
...
#ifdef LA_WITH_XDP
     ! Compute the in-place pseudo-inverse of matrix a
     subroutine la_pseudoinvert_y(a,pinva,rtol,err)
         !> Input matrix a[m,n]
         complex(xdp),intent(inout) :: a(:,:)
         !> Output pseudo-inverse matrix
         complex(xdp),intent(inout) :: pinva(:,:)
         !> [optional] ....
         real(xdp),optional,intent(in) :: rtol
         !> [optional] state return flag. On error if not requested, the code will stop
         type(la_state),optional,intent(out) :: err
     ...
     end subroutine la_pseudoinvert_y

     ! Function interface
     function la_pseudoinverse_y(a,rtol,err) result(pinva)
         !> Input matrix a[m,n]
         ...
         type(la_state),optional,intent(out) :: err
         !> Matrix pseudo-inverse
         complex(xdp) :: pinva(size(a,2,kind=ilp),size(a,1,kind=ilp))
         
         ! Use pointer to circumvent svd intent(inout) restriction
         complex(xdp),pointer :: ap(:,:)
         ap => a
         
         call la_pseudoinvert_y(ap,pinva,rtol,err)

     end function la_pseudoinverse_y

     ! Inverse matrix operator
     function la_pinv_y_operator(a) result(pinva)
         !> Input matrix a[m,n]
         complex(xdp),intent(in),target :: a(:,:)
         !> Result matrix
         complex(xdp) :: pinva(size(a,2,kind=ilp),size(a,1,kind=ilp))
         ...
         call la_pseudoinvert_y(ap,pinva)

     end function la_pinv_y_operator
#endif
