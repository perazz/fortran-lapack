...
module la_blas_level2_ban
     ...
     pure subroutine la_xgbmv(trans,m,n,kl,ku,alpha,a,lda,x,incx,beta,y,incy)
...
#ifdef LA_WITH_XDP
     !> YGBMV:  performs one of the matrix-vector operations
     !> y := alpha*A*x + beta*y,   or   y := alpha*A**T*x + beta*y,   or
     !> y := alpha*A**H*x + beta*y,
     !> where alpha and beta are scalars, x and y are vectors and A is an
     !> m by n band matrix, with kl sub-diagonals and ku super-diagonals.

     pure subroutine la_ygbmv(trans,m,n,kl,ku,alpha,a,lda,x,incx,beta,y,incy)
        use la_constants_xdp
        ! -- reference blas level2 routine --
        ! -- reference blas is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
...
#ifdef LA_WITH_XDP
     !> YHBMV:  performs the matrix-vector  operation
     !> y := alpha*A*x + beta*y,
     !> where alpha and beta are scalars, x and y are n element vectors and
     !> A is an n by n hermitian band matrix, with k super-diagonals.

     pure subroutine la_yhbmv(uplo,n,k,alpha,a,lda,x,incx,beta,y,incy)
        use la_constants_xdp
        ! -- reference blas level2 routine --
        ! -- reference blas is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
              ...
              ! form  y  when upper triangle of a is stored.
               ...
               if ((incx == 1) .and. (incy == 1)) then
                   do j = 1,n
                       ...
                       y(j) = y(j) + temp1*real(a(kplus1,j),KIND=xdp) + alpha*temp2
                   end do
               else
                   jx = kx
                   jy = ky
                   do j = 1,n
                       ...
                       y(jy) = y(jy) + temp1*real(a(kplus1,j),KIND=xdp) + alpha*temp2
                       jx = jx + incx
                       jy = jy + incy
                       if (j > k) then
                           kx = kx + incx
              ...
              ! form  y  when lower triangle of a is stored.
               if ((incx == 1) .and. (incy == 1)) then
                   do j = 1,n
                       temp1 = alpha*x(j)
                       temp2 = czero
                       y(j) = y(j) + temp1*real(a(1,j),KIND=xdp)
                       l = 1 - j
                       do i = j + 1,min(n,j + k)
                           y(i) = y(i) + temp1*a(l + i,j)
                           temp2 = temp2 + conjg(a(l + i,j))*x(i)
                   ...
                   do j = 1,n
                       temp1 = alpha*x(jx)
                       temp2 = czero
                       y(jy) = y(jy) + temp1*real(a(1,j),KIND=xdp)
                       l = 1 - j
                       ix = jx
                       iy = jy
                       do i = j + 1,min(n,j + k)
