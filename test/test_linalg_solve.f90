...
module test_linalg_solve
    ...
    subroutine test_q_solve(error)
        ...
        call check(error,all(abs(x - res) < abs(res*epsilon(0.0_qp))),'results match expected')
        if (allocated(error)) return

    end subroutine test_q_solve

    !> Simple linear system with multiple right hand sides
    subroutine test_q_solve_multiple(error)
        type(error_type),allocatable,intent(out) :: error
    ...
    subroutine test_w_solve(error)
        ...
        call check(error,all(abs(x - res) < abs(res)*1.0e-3_qp),'results match expected')
        if (allocated(error)) return

    end subroutine test_w_solve

    !> 2x2 Complex linear system
    subroutine test_2x2_w_solve(error)
        type(error_type),allocatable,intent(out) :: error

        type(la_state) :: state
        
        complex(qp),parameter :: i = (0.0_qp,1.0_qp)
        ...
        ! Exact result
        res = [(-0.28_qp,-0.04_qp), (0.36_qp,0.48_qp)]
        ...
        call check(error,all(abs(x - res) < abs(res)*sqrt(epsilon(0.0_qp))),'results match expected')
        if (allocated(error)) return
