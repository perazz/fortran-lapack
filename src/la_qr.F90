...
module la_qr
...
#ifdef LA_WITH_XDP
        module procedure get_qr_x_workspace
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
        module procedure get_qr_y_workspace
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
     ! Get workspace size for QR operations
     pure subroutine get_qr_x_workspace(a,lwork,err)
         !> Input matrix a[m,n]
         ...
         integer(ilp) :: m,n,k,info,lwork_qr,lwork_ord
         real(xdp) :: work_dummy(1),tau_dummy(1)
         type(la_state) :: err0
         ...
         lwork_qr = ceiling(real(work_dummy(1),kind=xdp),kind=ilp)
         ...
         lwork_ord = ceiling(real(work_dummy(1),kind=xdp),kind=ilp)
         
         ! Pick the largest size, so two operations can be performed with the same allocation
         lwork = max(lwork_qr,lwork_ord)
                  
     end subroutine get_qr_x_workspace
     
     ! Compute the solution to a real system of linear equations A * X = B
     pure subroutine la_x_qr(a,q,r,overwrite_a,storage,err)
         !> Input matrix a[m,n]
         real(xdp),intent(inout),target :: a(:,:)
         !> Orthogonal matrix Q ([m,m], or [m,k] if reduced)
         real(xdp),intent(out),contiguous,target :: q(:,:)
         !> Upper triangular matrix R ([m,n], or [k,n] if reduced)
         real(xdp),intent(out),contiguous,target :: r(:,:)
         !> [optional] Can A data be overwritten and destroyed?
         logical(lk),optional,intent(in) :: overwrite_a
         !> [optional] Provide pre-allocated workspace, size to be checked with qr_space
         real(xdp),intent(inout),optional,target :: storage(:)
         !> [optional] state return flag. On error if not requested, the code will stop
         type(la_state),optional,intent(out) :: err
         ...
         real(xdp),pointer :: amat(:,:),tau(:),work(:)
         ...
         ! Retrieve workspace size
         call get_qr_x_workspace(a,lwork,err0)
...
#ifdef LA_WITH_XDP
     ! Get workspace size for QR operations
     pure subroutine get_qr_y_workspace(a,lwork,err)
         !> Input matrix a[m,n]
         ...
         integer(ilp) :: m,n,k,info,lwork_qr,lwork_ord
         complex(xdp) :: work_dummy(1),tau_dummy(1)
         type(la_state) :: err0
     ...
     end subroutine get_qr_y_workspace
     
     ! Compute the solution to a real system of linear equations A * X = B
     pure subroutine la_y_qr(a,q,r,overwrite_a,storage,err)
         !> Input matrix a[m,n]
         complex(xdp),intent(inout),target :: a(:,:)
         !> Orthogonal matrix Q ([m,m], or [m,k] if reduced)
         complex(xdp),intent(out),contiguous,target :: q(:,:)
         !> Upper triangular matrix R ([m,n], or [k,n] if reduced)
         complex(xdp),intent(out),contiguous,target :: r(:,:)
         !> [optional] Can A data be overwritten and destroyed?
         logical(lk),optional,intent(in) :: overwrite_a
         !> [optional] Provide pre-allocated workspace, size to be checked with qr_space
         complex(xdp),intent(inout),optional,target :: storage(:)
         !> [optional] state return flag. On error if not requested, the code will stop
         type(la_state),optional,intent(out) :: err
         ...
         complex(xdp),pointer :: amat(:,:),tau(:),work(:)
         ...
         ! Retrieve workspace size
         call get_qr_y_workspace(a,lwork,err0)
