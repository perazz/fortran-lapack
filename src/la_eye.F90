...
#ifdef LA_WITH_XDP
        module procedure la_eye_x_errhandle
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
        module procedure la_eye_y_errhandle
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
        module procedure la_diag_x_from_scalar
        module procedure la_diag_x_from_array
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
        module procedure la_diag_y_from_scalar
        module procedure la_diag_y_from_array
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
        module procedure la_diag_x_errhandle_from_scalar
        module procedure la_diag_x_errhandle_from_array
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
        module procedure la_diag_y_errhandle_from_scalar
        module procedure la_diag_y_errhandle_from_array
#endif
#ifdef LA_WITH_QP
...
1        call err0%handle()
     ...
     !! This function returns a diagonal identity matrix, with the diagonal elements
     !! equal to 1 and all other elements set to 0.
     pure function la_eye_x(m,n,mold) result(eye)
         !> Number of rows of the identity matrix.
         integer(ilp),intent(in) :: m
         !> Number of columns of the identity matrix (optional).
         integer(ilp),optional,intent(in) :: n
         !> Data type, used to define the return type. Defaults to `real(real64)`.
         real(xdp),intent(in) :: mold
         !> Return matrix
         real(xdp),allocatable :: eye(:,:)
         ...
         !> Fill data
         do concurrent(i=1:m,j=1:cols)
            eye(i,j) = merge(1.0_xdp,0.0_xdp,i == j)
         end do

         ! Process output and return
1        call err0%handle()

     end function la_eye_x
#endif
...
1        call err0%handle()
     ...
     !! This function returns a diagonal identity matrix, with the diagonal elements
     !! equal to 1 and all other elements set to 0.
     pure function la_eye_y(m,n,mold) result(eye)
         !> Number of rows of the identity matrix.
         integer(ilp),intent(in) :: m
         !> Number of columns of the identity matrix (optional).
         integer(ilp),optional,intent(in) :: n
         !> Data type, used to define the return type. Defaults to `real(real64)`.
         complex(xdp),intent(in) :: mold
         !> Return matrix
         complex(xdp),allocatable :: eye(:,:)
...
#ifdef LA_WITH_XDP
     ! Return square diagonal matrix with diagonal values equal to the input scalar
     pure function la_diag_x_from_scalar(n,source) result(diag)
         !> Matrix size
         integer(ilp),intent(in) :: n
         !> Scalar diagonal value. Used to define the return type.
         real(xdp),intent(in) :: source
         !> Return matrix
         real(xdp),allocatable :: diag(:,:)
...
1        call err0%handle()
     ...
     end function la_diag_x_from_scalar

     ! Construct square diagonal matrix from an array of diagonal values
     pure function la_diag_x_from_array(source) result(diag)
         !> Array of diagonal values. Used to define the return type and the matrix size.
         real(xdp),intent(in) :: source(:)
         !> Return matrix
         real(xdp),allocatable :: diag(:,:)
     ...
     end function la_diag_x_from_array
#endif
...
#ifdef LA_WITH_XDP
     ! Return square diagonal matrix with diagonal values equal to the input scalar
     pure function la_diag_y_from_scalar(n,source) result(diag)
         !> Matrix size
         integer(ilp),intent(in) :: n
         !> Scalar diagonal value. Used to define the return type.
         complex(xdp),intent(in) :: source
         !> Return matrix
         complex(xdp),allocatable :: diag(:,:)
...
1        call err0%handle()
     ...
     end function la_diag_y_from_scalar

     ! Construct square diagonal matrix from an array of diagonal values
     pure function la_diag_y_from_array(source) result(diag)
         !> Array of diagonal values. Used to define the return type and the matrix size.
         complex(xdp),intent(in) :: source(:)
         !> Return matrix
         complex(xdp),allocatable :: diag(:,:)
     ...
     end function la_diag_y_from_array
#endif
...
1        call err0%handle(err)
     ...
     !! This function returns a diagonal identity matrix, with the diagonal elements
     !! equal to 1 and all other elements set to 0.
     function la_eye_x_errhandle(m,n,mold,err) result(eye)
         !> Number of rows of the identity matrix.
         integer(ilp),intent(in) :: m
         !> Number of columns of the identity matrix (optional).
         integer(ilp),optional,intent(in) :: n
...
1        call err0%handle(err)
     ...
     end function la_eye_x_errhandle
#endif
...
1        call err0%handle(err)
     ...
     !! This function returns a diagonal identity matrix, with the diagonal elements
     !! equal to 1 and all other elements set to 0.
     function la_eye_y_errhandle(m,n,mold,err) result(eye)
         !> Number of rows of the identity matrix.
         integer(ilp),intent(in) :: m
         !> Number of columns of the identity matrix (optional).
         integer(ilp),optional,intent(in) :: n
...
1        call err0%handle(err)
     ...
     end function la_eye_y_errhandle
#endif
...
#ifdef LA_WITH_XDP
     ! Return square diagonal matrix with diagonal values equal to the input scalar
     function la_diag_x_errhandle_from_scalar(n,source,err) result(diag)
         !> Matrix size
         integer(ilp),intent(in) :: n
         !> Scalar diagonal value. Used to define the return type.
         real(xdp),intent(in) :: source
...
1        call err0%handle(err)
     ...
     end function la_diag_x_errhandle_from_scalar

     ! Construct square diagonal matrix from an array of diagonal values
     function la_diag_x_errhandle_from_array(source,err) result(diag)
         !> Array of diagonal values. Used to define the return type and the matrix size.
         real(xdp),intent(in) :: source(:)
         !> [optional] state return flag. On error if not requested, the code will stop
         type(la_state),intent(out) :: err
     ...
     end function la_diag_x_errhandle_from_array
#endif
...
#ifdef LA_WITH_XDP
     ! Return square diagonal matrix with diagonal values equal to the input scalar
     function la_diag_y_errhandle_from_scalar(n,source,err) result(diag)
         !> Matrix size
         integer(ilp),intent(in) :: n
         !> Scalar diagonal value. Used to define the return type.
         complex(xdp),intent(in) :: source
...
1        call err0%handle(err)
     ...
     end function la_diag_y_errhandle_from_scalar

     ! Construct square diagonal matrix from an array of diagonal values
     function la_diag_y_errhandle_from_array(source,err) result(diag)
         !> Array of diagonal values. Used to define the return type and the matrix size.
         complex(xdp),intent(in) :: source(:)
         !> [optional] state return flag. On error if not requested, the code will stop
         type(la_state),intent(out) :: err
     ...
     end function la_diag_y_errhandle_from_array
#endif
