...
module la_blas_level2_gen
     ...
     pure subroutine la_xgemv(trans,m,n,alpha,a,lda,x,incx,beta,y,incy)
     ...
     pure subroutine la_xger(m,n,alpha,x,incx,y,incy,a,lda)
        use la_constants_xdp
        ! -- reference blas level2 routine --
        ! -- reference blas is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
...
#ifdef LA_WITH_XDP
     !> YGEMV:  performs one of the matrix-vector operations
     !> y := alpha*A*x + beta*y,   or   y := alpha*A**T*x + beta*y,   or
     !> y := alpha*A**H*x + beta*y,
     !> where alpha and beta are scalars, x and y are vectors and A is an
     !> m by n matrix.

     pure subroutine la_ygemv(trans,m,n,alpha,a,lda,x,incx,beta,y,incy)
        use la_constants_xdp
        ! -- reference blas level2 routine --
        ! -- reference blas is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
...
#ifdef LA_WITH_XDP
     !> YGERC:  performs the rank 1 operation
     !> A := alpha*x*y**H + A,
     !> where alpha is a scalar, x is an m element vector, y is an n element
     !> vector and A is an m by n matrix.

     pure subroutine la_ygerc(m,n,alpha,x,incx,y,incy,a,lda)
        use la_constants_xdp
        ! -- reference blas level2 routine --
        ! -- reference blas is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
...
#ifdef LA_WITH_XDP
     !> YGERU:  performs the rank 1 operation
     !> A := alpha*x*y**T + A,
     !> where alpha is a scalar, x is an m element vector, y is an n element
     !> vector and A is an m by n matrix.

     pure subroutine la_ygeru(m,n,alpha,x,incx,y,incy,a,lda)
        use la_constants_xdp
        ! -- reference blas level2 routine --
        ! -- reference blas is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
...
#ifdef LA_WITH_XDP
     !> YHEMV:  performs the matrix-vector  operation
     !> y := alpha*A*x + beta*y,
     !> where alpha and beta are scalars, x and y are n element vectors and
     !> A is an n by n hermitian matrix.

     pure subroutine la_yhemv(uplo,n,alpha,a,lda,x,incx,beta,y,incy)
        use la_constants_xdp
        ! -- reference blas level2 routine --
        ! -- reference blas is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
              ...
              ! form  y  when a is stored in upper triangle.
               if ((incx == 1) .and. (incy == 1)) then
                   do j = 1,n
                       ...
                       y(j) = y(j) + temp1*real(a(j,j),KIND=xdp) + alpha*temp2
                   end do
               else
                   jx = kx
                   jy = ky
                   do j = 1,n
                       ...
                       y(jy) = y(jy) + temp1*real(a(j,j),KIND=xdp) + alpha*temp2
                       jx = jx + incx
                       jy = jy + incy
                   end do
               end if
              ...
              ! form  y  when a is stored in lower triangle.
               if ((incx == 1) .and. (incy == 1)) then
                   do j = 1,n
                       temp1 = alpha*x(j)
                       temp2 = czero
                       y(j) = y(j) + temp1*real(a(j,j),KIND=xdp)
                       do i = j + 1,n
                           y(i) = y(i) + temp1*a(i,j)
                           temp2 = temp2 + conjg(a(i,j))*x(i)
                       end do
                   ...
                   do j = 1,n
                       temp1 = alpha*x(jx)
                       temp2 = czero
                       y(jy) = y(jy) + temp1*real(a(j,j),KIND=xdp)
                       ix = jx
                       iy = jy
                       do i = j + 1,n
                           ix = ix + incx
...
#ifdef LA_WITH_XDP
     !> YHER:   performs the hermitian rank 1 operation
     !> A := alpha*x*x**H + A,
     !> where alpha is a real scalar, x is an n element vector and A is an
     !> n by n hermitian matrix.

     pure subroutine la_yher(uplo,n,alpha,x,incx,a,lda)
        use la_constants_xdp
        ! -- reference blas level2 routine --
        ! -- reference blas is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
               ...
               if (incx == 1) then
                   do j = 1,n
                       if (x(j) /= czero) then
                           ...
                           a(j,j) = real(a(j,j),KIND=xdp) + real(x(j)*temp,KIND=xdp)
                       else
                           a(j,j) = real(a(j,j),KIND=xdp)
                       end if
                   end do
               else
                   jx = kx
                   do j = 1,n
                       if (x(jx) /= czero) then
                           ...
                           a(j,j) = real(a(j,j),KIND=xdp) + real(x(jx)*temp,KIND=xdp)
                       else
                           a(j,j) = real(a(j,j),KIND=xdp)
                       end if
                       jx = jx + incx
              ...
              ! form  a  when a is stored in lower triangle.
               if (incx == 1) then
                   do j = 1,n
                       if (x(j) /= czero) then
                           temp = alpha*conjg(x(j))
                           a(j,j) = real(a(j,j),KIND=xdp) + real(temp*x(j),KIND=xdp)
                           do i = j + 1,n
                               a(i,j) = a(i,j) + x(i)*temp
                           end do
                       else
                   ...
                   do j = 1,n
                       if (x(jx) /= czero) then
                           temp = alpha*conjg(x(jx))
                           a(j,j) = real(a(j,j),KIND=xdp) + real(temp*x(jx),KIND=xdp)
                           ix = jx
                           do i = j + 1,n
                               ix = ix + incx
                               a(i,j) = a(i,j) + x(ix)*temp
...
#ifdef LA_WITH_XDP
     !> YHER2:  performs the hermitian rank 2 operation
     !> A := alpha*x*y**H + conjg( alpha )*y*x**H + A,
     !> where alpha is a scalar, x and y are n element vectors and A is an n
     !> by n hermitian matrix.

     pure subroutine la_yher2(uplo,n,alpha,x,incx,y,incy,a,lda)
        use la_constants_xdp
        ! -- reference blas level2 routine --
        ! -- reference blas is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
               ...
               if ((incx == 1) .and. (incy == 1)) then
                   do j = 1,n
                       if ((x(j) /= czero) .or. (y(j) /= czero)) then
                           ...
                           a(j,j) = real(a(j,j),KIND=xdp) + real(x(j)*temp1 + y(j)*temp2,KIND=xdp)
                   ...
                   do j = 1,n
                       if ((x(jx) /= czero) .or. (y(jy) /= czero)) then
                           ...
                           a(j,j) = real(a(j,j),KIND=xdp) + real(x(jx)*temp1 + y(jy)*temp2,KIND=xdp)
