...
module la_eig
...
#ifdef LA_WITH_XDP
        module procedure la_eig_standard_x
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
        module procedure la_eig_standard_y
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
        module procedure la_real_eig_standard_x
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
        module procedure la_eig_generalized_x
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
        module procedure la_eig_generalized_y
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
        module procedure la_real_eig_generalized_x
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
        module procedure la_eigvals_standard_x
        module procedure la_eigvals_noerr_standard_x
        module procedure la_eigvals_generalized_x
        module procedure la_eigvals_noerr_generalized_x
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
        module procedure la_eigvals_standard_y
        module procedure la_eigvals_noerr_standard_y
        module procedure la_eigvals_generalized_y
        module procedure la_eigvals_noerr_generalized_y
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
        module procedure la_eigvalsh_x
        module procedure la_eigvalsh_noerr_x
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
        module procedure la_eigvalsh_y
        module procedure la_eigvalsh_noerr_y
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
        module procedure scale_general_eig_x
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
        module procedure scale_general_eig_y
#endif
#ifdef LA_WITH_QP
     ...
     function la_eigvals_standard_x(a,err) result(lambda)
     !! Return an array of eigenvalues of matrix A.
         !> Input matrix A[m,n]
         real(xdp),intent(in),dimension(:,:),target :: a
         !> [optional] state return flag. On error if not requested, the code will stop
         type(la_state),intent(out) :: err
         !> Array of eigenvalues
         complex(xdp),allocatable :: lambda(:)

         !> Create
         real(xdp),pointer,dimension(:,:) :: amat
         integer(ilp) :: m,n,k
         ...
         !> Compute eigenvalues only
         call la_eig_standard_x(amat,lambda,err=err)

     end function la_eigvals_standard_x

     function la_eigvals_noerr_standard_x(a) result(lambda)
     !! Return an array of eigenvalues of matrix A.
         !> Input matrix A[m,n]
         real(xdp),intent(in),dimension(:,:),target :: a
         !> Array of eigenvalues
         ...
         !> Compute eigenvalues only
         call la_eig_standard_x(amat,lambda,overwrite_a=.false.)

     end function la_eigvals_noerr_standard_x

     subroutine la_eig_standard_x(a,lambda,right,left, &
                                                       overwrite_a,err)
     !! Eigendecomposition of matrix A returning an array `lambda` of eigenvalues,
     !! and optionally right or left eigenvectors.
         !> Input matrix A[m,n]
         real(xdp),intent(inout),dimension(:,:),target :: a
         !> Array of eigenvalues
         complex(xdp),intent(out) :: lambda(:)
         !> [optional] RIGHT eigenvectors of A (as columns)
         complex(xdp),optional,intent(out),target :: right(:,:)
         !> [optional] LEFT eigenvectors of A (as columns)
         complex(xdp),optional,intent(out),target :: left(:,:)
         !> [optional] Can A data be overwritten and destroyed? (default: no)
         logical(lk),optional,intent(in) :: overwrite_a
         !> [optional] state return flag. On error if not requested, the code will stop
         type(la_state),optional,intent(out) :: err
         ...
         logical(lk) :: copy_a
         character :: task_u,task_v
         real(xdp),target :: work_dummy(1),u_dummy(1,1),v_dummy(1,1)
         real(xdp),allocatable :: work(:)
         real(xdp),dimension(:,:),pointer :: amat,umat,vmat
         real(xdp),pointer :: lreal(:),limag(:)
         ...
         get_geev: if (err0%ok()) then
             ...
             if (info == 0) then
                ...
                !> Prepare working storage
                lwork = nint(real(work_dummy(1),kind=xdp),kind=ilp)
                allocate (work(lwork))
             ...
             ! Finalize storage and process output flag
             lambda(:n) = cmplx(lreal(:n),limag(:n),kind=xdp)
             ...
             ! u(j+1) = VL(:,j) - i*VL(:,j+1).
             ! Convert these to complex numbers here.
             if (present(right)) call assign_real_eigenvectors_xdp(n,lambda,vmat,right)
             if (present(left)) call assign_real_eigenvectors_xdp(n,lambda,umat,left)
     ...
     end subroutine la_eig_standard_x
     
     function la_eigvals_generalized_x(a,b,err) result(lambda)
     !! Return an array of eigenvalues of matrix A.
         !> Input matrix A[m,n]
         real(xdp),intent(in),dimension(:,:),target :: a
         !> Generalized problem matrix B[n,n]
         real(xdp),intent(inout),dimension(:,:),target :: b
         !> [optional] state return flag. On error if not requested, the code will stop
         type(la_state),intent(out) :: err
         !> Array of eigenvalues
         complex(xdp),allocatable :: lambda(:)
         ...
         !> Create
         real(xdp),pointer,dimension(:,:) :: amat,bmat
         integer(ilp) :: m,n,k
         ...
         !> Compute eigenvalues only
         call la_eig_generalized_x(amat,bmat,lambda,err=err)

     end function la_eigvals_generalized_x

     function la_eigvals_noerr_generalized_x(a,b) result(lambda)
     !! Return an array of eigenvalues of matrix A.
         !> Input matrix A[m,n]
         real(xdp),intent(in),dimension(:,:),target :: a
         !> Generalized problem matrix B[n,n]
         ...
         !> Compute eigenvalues only
         call la_eig_generalized_x(amat,bmat,lambda,overwrite_a=.false.)

     end function la_eigvals_noerr_generalized_x

     subroutine la_eig_generalized_x(a,b,lambda,right,left, &
                                                       overwrite_a,overwrite_b,err)
     !! Eigendecomposition of matrix A returning an array `lambda` of eigenvalues,
     !! and optionally right or left eigenvectors.
         !> Input matrix A[m,n]
         ...
         real(xdp),target :: work_dummy(1),u_dummy(1,1),v_dummy(1,1)
         real(xdp),allocatable :: work(:)
         real(xdp),dimension(:,:),pointer :: amat,umat,vmat,bmat
         real(xdp),pointer :: lreal(:),limag(:)
         real(xdp),allocatable :: beta(:)
     ...
     end subroutine la_eig_generalized_x
     
     !> Return an array of eigenvalues of real symmetric / complex hermitian A
     function la_eigvalsh_x(a,upper_a,err) result(lambda)
         !> Input matrix A[m,n]
         real(xdp),intent(in),target :: a(:,:)
         !> [optional] Should the upper/lower half of A be used? Default: lower
         logical(lk),optional,intent(in) :: upper_a
         !> [optional] state return flag. On error if not requested, the code will stop
         type(la_state),intent(out) :: err
         !> Array of singular values
         real(xdp),allocatable :: lambda(:)
         
         real(xdp),pointer :: amat(:,:)
         integer(ilp) :: m,n,k
         ...
         !> Compute eigenvalues only
         call la_eigh_x(amat,lambda,upper_a=upper_a,overwrite_a=.false.,err=err)
         
     end function la_eigvalsh_x
     
     !> Return an array of eigenvalues of real symmetric / complex hermitian A
     function la_eigvalsh_noerr_x(a,upper_a) result(lambda)
         !> Input matrix A[m,n]
         real(xdp),intent(in),target :: a(:,:)
         !> [optional] Should the upper/lower half of A be used? Default: lower
         logical(lk),optional,intent(in) :: upper_a
         ...
         !> Compute eigenvalues only
         call la_eigh_x(amat,lambda,upper_a=upper_a,overwrite_a=.false.)

     end function la_eigvalsh_noerr_x

     !> Eigendecomposition of a real symmetric or complex Hermitian matrix A returning an array `lambda`
     !> of eigenvalues, and optionally right or left eigenvectors.
     subroutine la_eigh_x(a,lambda,vectors,upper_a,overwrite_a,err)
         !> Input matrix A[m,n]
         real(xdp),intent(inout),target :: a(:,:)
         !> Array of eigenvalues
         real(xdp),intent(out) :: lambda(:)
         !> The columns of vectors contain the orthonormal eigenvectors of A
         real(xdp),optional,intent(out),target :: vectors(:,:)
         !> [optional] Can A data be overwritten and destroyed?
         logical(lk),optional,intent(in) :: overwrite_a
         !> [optional] Should the upper/lower half of A be used? Default: lower
         logical(lk),optional,intent(in) :: upper_a
         ...
         logical(lk) :: copy_a
         character :: triangle,task
         real(xdp),target :: work_dummy(1)
         real(xdp),allocatable :: work(:)
         real(xdp),allocatable :: rwork(:)
         real(xdp),pointer :: amat(:,:)
     ...
     function la_eigvals_standard_y(a,err) result(lambda)
     !! Return an array of eigenvalues of matrix A.
         !> Input matrix A[m,n]
         complex(xdp),intent(in),dimension(:,:),target :: a
         !> [optional] state return flag. On error if not requested, the code will stop
         type(la_state),intent(out) :: err
         !> Array of eigenvalues
         complex(xdp),allocatable :: lambda(:)
         ...
         !> Create
         complex(xdp),pointer,dimension(:,:) :: amat
         integer(ilp) :: m,n,k
         ...
         !> Compute eigenvalues only
         call la_eig_standard_y(amat,lambda,err=err)

     end function la_eigvals_standard_y

     function la_eigvals_noerr_standard_y(a) result(lambda)
     !! Return an array of eigenvalues of matrix A.
         !> Input matrix A[m,n]
         complex(xdp),intent(in),dimension(:,:),target :: a
         !> Array of eigenvalues
         ...
         !> Compute eigenvalues only
         call la_eig_standard_y(amat,lambda,overwrite_a=.false.)

     end function la_eigvals_noerr_standard_y

     subroutine la_eig_standard_y(a,lambda,right,left, &
                                                       overwrite_a,err)
     !! Eigendecomposition of matrix A returning an array `lambda` of eigenvalues,
     !! and optionally right or left eigenvectors.
         !> Input matrix A[m,n]
         complex(xdp),intent(inout),dimension(:,:),target :: a
         !> Array of eigenvalues
         complex(xdp),intent(out) :: lambda(:)
         !> [optional] RIGHT eigenvectors of A (as columns)
         complex(xdp),optional,intent(out),target :: right(:,:)
         ...
         logical(lk) :: copy_a
         character :: task_u,task_v
         complex(xdp),target :: work_dummy(1),u_dummy(1,1),v_dummy(1,1)
         complex(xdp),allocatable :: work(:)
         complex(xdp),dimension(:,:),pointer :: amat,umat,vmat
         real(xdp),allocatable :: rwork(:)
     ...
     end subroutine la_eig_standard_y
     
     function la_eigvals_generalized_y(a,b,err) result(lambda)
     !! Return an array of eigenvalues of matrix A.
         !> Input matrix A[m,n]
         complex(xdp),intent(in),dimension(:,:),target :: a
         !> Generalized problem matrix B[n,n]
         complex(xdp),intent(inout),dimension(:,:),target :: b
         !> [optional] state return flag. On error if not requested, the code will stop
         type(la_state),intent(out) :: err
         !> Array of eigenvalues
         complex(xdp),allocatable :: lambda(:)
         ...
         !> Create
         complex(xdp),pointer,dimension(:,:) :: amat,bmat
         integer(ilp) :: m,n,k
         ...
         !> Compute eigenvalues only
         call la_eig_generalized_y(amat,bmat,lambda,err=err)

     end function la_eigvals_generalized_y

     function la_eigvals_noerr_generalized_y(a,b) result(lambda)
     !! Return an array of eigenvalues of matrix A.
         !> Input matrix A[m,n]
         complex(xdp),intent(in),dimension(:,:),target :: a
         !> Generalized problem matrix B[n,n]
         ...
         !> Compute eigenvalues only
         call la_eig_generalized_y(amat,bmat,lambda,overwrite_a=.false.)

     end function la_eigvals_noerr_generalized_y

     subroutine la_eig_generalized_y(a,b,lambda,right,left, &
                                                       overwrite_a,overwrite_b,err)
     !! Eigendecomposition of matrix A returning an array `lambda` of eigenvalues,
     !! and optionally right or left eigenvectors.
         !> Input matrix A[m,n]
         ...
         complex(xdp),target :: work_dummy(1),u_dummy(1,1),v_dummy(1,1)
         complex(xdp),allocatable :: work(:)
         complex(xdp),dimension(:,:),pointer :: amat,umat,vmat,bmat
         real(xdp),allocatable :: rwork(:)
         complex(xdp),allocatable :: beta(:)
     ...
     end subroutine la_eig_generalized_y
     
     !> Return an array of eigenvalues of real symmetric / complex hermitian A
     function la_eigvalsh_y(a,upper_a,err) result(lambda)
         !> Input matrix A[m,n]
         complex(xdp),intent(in),target :: a(:,:)
         !> [optional] Should the upper/lower half of A be used? Default: lower
         logical(lk),optional,intent(in) :: upper_a
         !> [optional] state return flag. On error if not requested, the code will stop
         type(la_state),intent(out) :: err
         ...
         !> Compute eigenvalues only
         call la_eigh_y(amat,lambda,upper_a=upper_a,overwrite_a=.false.,err=err)
         
     end function la_eigvalsh_y
     
     !> Return an array of eigenvalues of real symmetric / complex hermitian A
     function la_eigvalsh_noerr_y(a,upper_a) result(lambda)
         !> Input matrix A[m,n]
         complex(xdp),intent(in),target :: a(:,:)
         !> [optional] Should the upper/lower half of A be used? Default: lower
         logical(lk),optional,intent(in) :: upper_a
         ...
         !> Compute eigenvalues only
         call la_eigh_y(amat,lambda,upper_a=upper_a,overwrite_a=.false.)

     end function la_eigvalsh_noerr_y

     !> Eigendecomposition of a real symmetric or complex Hermitian matrix A returning an array `lambda`
     !> of eigenvalues, and optionally right or left eigenvectors.
     subroutine la_eigh_y(a,lambda,vectors,upper_a,overwrite_a,err)
         !> Input matrix A[m,n]
         complex(xdp),intent(inout),target :: a(:,:)
         !> Array of eigenvalues
         real(xdp),intent(out) :: lambda(:)
         !> The columns of vectors contain the orthonormal eigenvectors of A
         complex(xdp),optional,intent(out),target :: vectors(:,:)
         !> [optional] Can A data be overwritten and destroyed?
         logical(lk),optional,intent(in) :: overwrite_a
         !> [optional] Should the upper/lower half of A be used? Default: lower
         logical(lk),optional,intent(in) :: upper_a
         ...
         logical(lk) :: copy_a
         character :: triangle,task
         complex(xdp),target :: work_dummy(1)
         complex(xdp),allocatable :: work(:)
         real(xdp),allocatable :: rwork(:)
         complex(xdp),pointer :: amat(:,:)
     ...
     !> GEEV for real matrices returns complex eigenvalues in real arrays.
     !> Convert them to complex here, following the GEEV logic.
     pure subroutine assign_real_eigenvectors_xdp(n,lambda,lmat,out_mat)
        !> Problem size
        integer(ilp),intent(in) :: n
        !> Array of eigenvalues
        complex(xdp),intent(in) :: lambda(:)
        !> Real matrix as returned by geev
        real(xdp),intent(in) :: lmat(:,:)
        !> Complex matrix as returned by eig
        complex(xdp),intent(out) :: out_mat(:,:)
        ...
        do j = 1,n - 1
           if (lambda(j) == conjg(lambda(j + 1))) then
              out_mat(:,j) = cmplx(lmat(:,j),lmat(:,j + 1),kind=xdp)
              out_mat(:,j + 1) = cmplx(lmat(:,j),-lmat(:,j + 1),kind=xdp)
           end if
        end do
        
     end subroutine assign_real_eigenvectors_xdp
     
     subroutine la_real_eig_standard_x(a,lambda,right,left, &
                                                            overwrite_a,err)
      !! Eigendecomposition of matrix A returning an array `lambda` of real eigenvalues,
      !! and optionally right or left eigenvectors. Returns an error if the eigenvalues had
      !! non-trivial imaginary parts.
          ...
          type(la_state) :: err0
          integer(ilp) :: n
          complex(xdp),allocatable :: clambda(:)
          real(xdp),parameter :: rtol = epsilon(0.0_xdp)
          real(xdp),parameter :: atol = tiny(0.0_xdp)
          
          n = size(lambda,dim=1,kind=ilp)
          allocate (clambda(n))
          
          call la_eig_standard_x(a,clambda,right,left, &
                                                 overwrite_a,err0)
          ...
          ! Return real components only
          lambda(:n) = real(clambda,kind=xdp)
          
          call err0%handle(err)
          
     end subroutine la_real_eig_standard_x
     
     subroutine la_real_eig_generalized_x(a,b,lambda,right,left, &
                                                            overwrite_a,overwrite_b,err)
      !! Eigendecomposition of matrix A returning an array `lambda` of real eigenvalues,
      !! and optionally right or left eigenvectors. Returns an error if the eigenvalues had
      !! non-trivial imaginary parts.
          ...
          real(xdp),intent(inout),target :: a(:,:)
          !> Generalized problem matrix B[n,n]
          real(xdp),intent(inout),target :: b(:,:)
          !> Array of real eigenvalues
          real(xdp),intent(out) :: lambda(:)
          !> The columns of RIGHT contain the right eigenvectors of A
          complex(xdp),optional,intent(out),target :: right(:,:)
          ...
          call la_eig_generalized_x(a,b,clambda,right,left, &
                                                 overwrite_a,overwrite_b,err0)
     ...
     end subroutine la_real_eig_generalized_x
...
#ifdef LA_WITH_XDP
     !> Utility function: Scale generalized eigenvalue
     elemental complex(xdp) function scale_general_eig_x(alpha,beta) result(lambda)
         !! A generalized eigenvalue for a pair of matrices (A,B) is a scalar lambda or a ratio
         !! alpha/beta = lambda, such that A - lambda*B is singular. It is usually represented as the
         !! pair (alpha,beta), there is a reasonable interpretation for beta=0, and even for both
         !! being zero.
         ...
         if (beta == rzero) then
            if (alpha /= czero) then
                lambda = cmplx(ieee_value(1.0_xdp,ieee_positive_inf), &
                               ieee_value(1.0_xdp,ieee_positive_inf),kind=xdp)
            else
                lambda = ieee_value(1.0_xdp,ieee_quiet_nan)
            end if
         else
            lambda = alpha/beta
         end if
         
     end function scale_general_eig_x
#endif
...
#ifdef LA_WITH_XDP
     !> Utility function: Scale generalized eigenvalue
     elemental complex(xdp) function scale_general_eig_y(alpha,beta) result(lambda)
         !! A generalized eigenvalue for a pair of matrices (A,B) is a scalar lambda or a ratio
         !! alpha/beta = lambda, such that A - lambda*B is singular. It is usually represented as the
         !! pair (alpha,beta), there is a reasonable interpretation for beta=0, and even for both
         !! being zero.
         complex(xdp),intent(in) :: alpha
         complex(xdp),intent(in) :: beta
     ...
     end function scale_general_eig_y
#endif
