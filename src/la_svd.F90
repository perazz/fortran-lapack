...
#ifdef LA_WITH_XDP
     !> Singular values of matrix A
     function la_svdvals_x(a,err) result(s)
         !> Input matrix A[m,n]
         ...
         !> Compute singular values
         call la_svd_x(amat,s,overwrite_a=.false.,err=err)

     end function la_svdvals_x

     !> SVD of matrix A = U S V^T, returning S and optionally U and V^T
     subroutine la_svd_x(a,s,u,vt,overwrite_a,full_matrices,err)
         !> Input matrix A[m,n]
         real(xdp),intent(inout),target :: a(:,:)
         !> Array of singular values
         real(xdp),intent(out) :: s(:)
         !> The columns of U contain the eigenvectors of A A^T
         real(xdp),optional,intent(out),target :: u(:,:)
         !> The rows of V^T contain the eigenvectors of A^T A
         real(xdp),optional,intent(out),target :: vt(:,:)
         !> [optional] Can A data be overwritten and destroyed?
         logical(lk),optional,intent(in) :: overwrite_a
         !> [optional] full matrices have shape(u)==[m,m], shape(vh)==[n,n] (default); otherwise
         !> they are shape(u)==[m,k] and shape(vh)==[k,n] with k=min(m,n)
         ...
         logical(lk) :: copy_a,full_storage,compute_uv,alloc_u,alloc_vt,can_overwrite_a
         character :: task
         real(xdp),target :: work_dummy(1),u_dummy(1,1),vt_dummy(1,1)
         real(xdp),allocatable :: work(:)
         real(xdp),allocatable :: rwork(:)
         real(xdp),pointer :: amat(:,:),umat(:,:),vtmat(:,:)
...
#ifdef LA_WITH_XDP
     !> Singular values of matrix A
     function la_svdvals_y(a,err) result(s)
         !> Input matrix A[m,n]
         ...
         !> Compute singular values
         call la_svd_y(amat,s,overwrite_a=.false.,err=err)

     end function la_svdvals_y

     !> SVD of matrix A = U S V^T, returning S and optionally U and V^T
     subroutine la_svd_y(a,s,u,vt,overwrite_a,full_matrices,err)
         !> Input matrix A[m,n]
         complex(xdp),intent(inout),target :: a(:,:)
         !> Array of singular values
         real(xdp),intent(out) :: s(:)
         !> The columns of U contain the eigenvectors of A A^T
         complex(xdp),optional,intent(out),target :: u(:,:)
         !> The rows of V^T contain the eigenvectors of A^T A
         complex(xdp),optional,intent(out),target :: vt(:,:)
         !> [optional] Can A data be overwritten and destroyed?
         logical(lk),optional,intent(in) :: overwrite_a
         !> [optional] full matrices have shape(u)==[m,m], shape(vh)==[n,n] (default); otherwise
         !> they are shape(u)==[m,k] and shape(vh)==[k,n] with k=min(m,n)
         ...
         logical(lk) :: copy_a,full_storage,compute_uv,alloc_u,alloc_vt,can_overwrite_a
         character :: task
         complex(xdp),target :: work_dummy(1),u_dummy(1,1),vt_dummy(1,1)
         complex(xdp),allocatable :: work(:)
         real(xdp),allocatable :: rwork(:)
         complex(xdp),pointer :: amat(:,:),umat(:,:),vtmat(:,:)
