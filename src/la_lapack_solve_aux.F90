...
module la_lapack_solve_aux
...
#ifdef LA_WITH_XDP
     !> XLACN2: estimates the 1-norm of a square, real matrix A.
     !> Reverse communication is used for evaluating matrix-vector products.

     pure subroutine la_xlacn2(n,v,x,isgn,est,kase,isave)
        ...
        ! =====================================================================
           ...
           ! Local Scalars
           integer(ilp) :: i,jlast
           real(xdp) :: altsgn,estold,temp,xs
           ! Intrinsic Functions
           intrinsic :: abs,real,nint
           ! Executable Statements
           if (kase == 0) then
              do i = 1,n
                 x(i) = one/real(n,KIND=xdp)
              end do
              kase = 1
              isave(1) = 1
              return
           ...
           altsgn = one
           do i = 1,n
              x(i) = altsgn*(one + real(i - 1,KIND=xdp)/real(n - 1,KIND=xdp))
              altsgn = -altsgn
           end do
           kase = 1
           isave(1) = 5
           ...
           ! x has been overwritten by a*x.
           140 continue
           temp = two*(la_xasum(n,x,1)/real(3*n,KIND=xdp))
           if (temp > est) then
              call la_xcopy(n,x,1,v,1)
              est = temp
           end if
...
#ifdef LA_WITH_XDP
     !> XLACON: estimates the 1-norm of a square, real matrix A.
     !> Reverse communication is used for evaluating matrix-vector products.

     subroutine la_xlacon(n,v,x,isgn,est,kase)
        use la_constants_xdp
        ! -- lapack auxiliary routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
...
#ifdef LA_WITH_XDP
     !> XLA_LIN_BERR: computes component-wise relative backward error from
     !> the formula
     !> max(i) ( abs(R(i)) / ( abs(op(A_s))*abs(Y) + abs(B_s) )(i) )
     !> where abs(Z) is the component-wise absolute value of the matrix
     !> or vector Z.

     pure subroutine la_xla_lin_berr(n,nz,nrhs,res,ayb,berr)
        use la_constants_xdp
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
        ...
        ! =====================================================================
           ...
           ! if ayb is exactly 0.0_xdp (and if computed by sla_yyamv), then we know
           ! the true residual also must be exactly zero.
              end do
           end do
     end subroutine la_xla_lin_berr
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
     !> YLA_LIN_BERR: computes componentwise relative backward error from
     !> the formula
     !> max(i) ( abs(R(i)) / ( abs(op(A_s))*abs(Y) + abs(B_s) )(i) )
     !> where abs(Z) is the componentwise absolute value of the matrix
     !> or vector Z.

     pure subroutine la_yla_lin_berr(n,nz,nrhs,res,ayb,berr)
        use la_constants_xdp
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
        ...
        ! =====================================================================
           ...
           ! if ayb is exactly 0.0_xdp (and if computed by cla_yyamv), then we know
           ! the true residual also must be exactly zero.
              end do
           end do
     end subroutine la_yla_lin_berr
#endif
#ifdef LA_WITH_QP
     ...
     pure subroutine la_ylacn2(n,v,x,est,kase,isave)
        use la_constants_xdp,only:one,two,czero,cone
        ! -- lapack auxiliary routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ! Scalar Arguments
        ...
        ! =====================================================================
           ...
           ! Local Scalars
           integer(ilp) :: i,jlast
           real(xdp) :: absxi,altsgn,estold,safmin,temp
           ! Intrinsic Functions
           intrinsic :: abs,real,cmplx,aimag
           ! Executable Statements
           ...
           if (kase == 0) then
              do i = 1,n
                 x(i) = cmplx(one/real(n,KIND=xdp),KIND=xdp)
              end do
              kase = 1
              isave(1) = 1
              return
           ...
           do i = 1,n
              absxi = abs(x(i))
              if (absxi > safmin) then
                 x(i) = cmplx(real(x(i),KIND=xdp)/absxi,aimag(x(i))/absxi,KIND=xdp)
           ...
           altsgn = one
           do i = 1,n
              x(i) = cmplx(altsgn*(one + real(i - 1,KIND=xdp)/real(n - 1,KIND=xdp)),KIND=xdp)
           ...
           ! x has been overwritten by a*x.
           120 continue
           temp = two*(la_xysum1(n,x,1)/real(3*n,KIND=xdp))
           if (temp > est) then
              call la_ycopy(n,x,1,v,1)
              est = temp
           end if
...
#ifdef LA_WITH_XDP
     !> YLACON: estimates the 1-norm of a square, complex matrix A.
     !> Reverse communication is used for evaluating matrix-vector products.

     subroutine la_ylacon(n,v,x,est,kase)
        use la_constants_xdp,only:one,two,czero,cone
        ! -- lapack auxiliary routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
