module la_schur
...
#ifdef LA_WITH_XDP
      module procedure la_x_schur
      module procedure la_real_eig_x_schur
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
      module procedure la_y_schur
      module procedure la_real_eig_y_schur
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
      module procedure get_schur_x_workspace
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
      module procedure get_schur_y_workspace
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
    subroutine get_schur_x_workspace(a,lwork,err)
        !> Input matrix a[m,m]
        ...
        real(xdp) :: rwork_dummy(1)
        real(xdp) :: wr_dummy(1),wi_dummy(1),vs_dummy(1,1),work_dummy(1)
        type(la_state) :: err0
        ...
        call gees(jobvs,sort,do_not_select,n,amat,m,sdim,wr_dummy,wi_dummy, &
                  vs_dummy,m,work_dummy,lwork,bwork_dummy,info)
        if (info == 0) lwork = nint(real(work_dummy(1),kind=xdp),kind=ilp)
        call handle_gees_info(info,m,n,m,err0)
        call err0%handle(err)
    ...
    end subroutine get_schur_x_workspace
    
    ! Schur decomposition subroutine
    subroutine la_x_schur(a,t,z,eigvals,overwrite_a,storage,err)
        !> Input matrix a[m,m]
        real(xdp),intent(inout),target :: a(:,:)
        !> Schur form of A: upper-triangular or quasi-upper-triangular matrix T
        real(xdp),intent(out),contiguous,target :: t(:,:)
        !> Unitary/orthonormal transformation matrix Z
        real(xdp),optional,intent(out),contiguous,target :: z(:,:)
        !> [optional] Output eigenvalues that appear on the diagonal of T
        complex(xdp),optional,intent(out),contiguous,target :: eigvals(:)
        !> [optional] Provide pre-allocated workspace, size to be checked with schur_space
        real(xdp),optional,intent(inout),target :: storage(:)
        !> [optional] Can A data be overwritten and destroyed?
        logical(lk),optional,intent(in) :: overwrite_a
        !> [optional] State return flag. On error if not requested, the code will stop
        type(la_state),optional,intent(out) :: err
        ...
        real(xdp),target :: vs_dummy(1,1)
        real(xdp),pointer :: vs(:,:),work(:),eigs(:),eigi(:)
        character :: jobvs,sort
        type(la_state) :: err0
        ...
        if (present(storage)) then
            ...
            ! Query optimal workspace
            call get_schur_x_workspace(a,lwork,err0)
        ...
        eigenvalue_output: if (local_eigs) then
           ! Build complex eigenvalues
           if (present(eigvals)) eigvals = cmplx(eigs,eigi,kind=xdp)
           if (.not. overwrite_a_) deallocate (eigs,eigi)
        end if eigenvalue_output
        if (.not. present(storage)) deallocate (work)
        if (sort /= GEES_NOT) deallocate (bwork)
        ...
        contains
            ...
            pure logical(lk) function eig_select(alphar,alphai)
                real(xdp),intent(in) :: alphar,alphai
                complex(xdp) :: alpha
                alpha = cmplx(alphar,alphai,kind=xdp)
                eig_select = .false.
            end function eig_select

    end subroutine la_x_schur

    ! Schur decomposition subroutine: real eigenvalue interface
    subroutine la_real_eig_x_schur(a,t,z,eigvals,overwrite_a,storage,err)
        !> Input matrix a[m,m]
        real(xdp),intent(inout),target :: a(:,:)
        !> Schur form of A: upper-triangular or quasi-upper-triangular matrix T
        real(xdp),intent(out),contiguous,target :: t(:,:)
        ...
        real(xdp),optional,intent(out),contiguous,target :: z(:,:)
        !> Output eigenvalues that appear on the diagonal of T
        real(xdp),intent(out),contiguous,target :: eigvals(:)
        !> [optional] Provide pre-allocated workspace, size to be checked with schur_space
        real(xdp),optional,intent(inout),target :: storage(:)
        !> [optional] Can A data be overwritten and destroyed?
        logical(lk),optional,intent(in) :: overwrite_a
        ...
        type(la_state) :: err0
        integer(ilp) :: n
        complex(xdp),allocatable :: ceigvals(:)
        ...
        !> Compute Schur decomposition with complex eigenvalues
        call la_x_schur(a,t,z,ceigvals,overwrite_a,storage,err0)
        ...
        ! Return real components only
        eigvals(:n) = real(ceigvals,kind=xdp)
          
        call err0%handle(err)
        
    end subroutine la_real_eig_x_schur
#endif
...
#ifdef LA_WITH_XDP
    subroutine get_schur_y_workspace(a,lwork,err)
        !> Input matrix a[m,m]
        ...
        real(xdp) :: rwork_dummy(1)
        complex(xdp) :: wr_dummy(1),wi_dummy(1),vs_dummy(1,1),work_dummy(1)
        type(la_state) :: err0
    ...
    end subroutine get_schur_y_workspace
    
    ! Schur decomposition subroutine
    subroutine la_y_schur(a,t,z,eigvals,overwrite_a,storage,err)
        !> Input matrix a[m,m]
        complex(xdp),intent(inout),target :: a(:,:)
        !> Schur form of A: upper-triangular or quasi-upper-triangular matrix T
        complex(xdp),intent(out),contiguous,target :: t(:,:)
        !> Unitary/orthonormal transformation matrix Z
        complex(xdp),optional,intent(out),contiguous,target :: z(:,:)
        !> [optional] Output eigenvalues that appear on the diagonal of T
        complex(xdp),optional,intent(out),contiguous,target :: eigvals(:)
        !> [optional] Provide pre-allocated workspace, size to be checked with schur_space
        complex(xdp),optional,intent(inout),target :: storage(:)
        !> [optional] Can A data be overwritten and destroyed?
        logical(lk),optional,intent(in) :: overwrite_a
        !> [optional] State return flag. On error if not requested, the code will stop
        type(la_state),optional,intent(out) :: err
        ...
        logical(lk),pointer :: bwork(:)
        real(xdp),allocatable :: rwork(:)
        complex(xdp),target :: vs_dummy(1,1)
        complex(xdp),pointer :: vs(:,:),work(:),eigs(:)
        character :: jobvs,sort
        type(la_state) :: err0
        ...
        if (present(storage)) then
            ...
            ! Query optimal workspace
            call get_schur_y_workspace(a,lwork,err0)
    ...
    end subroutine la_y_schur

    ! Schur decomposition subroutine: real eigenvalue interface
    subroutine la_real_eig_y_schur(a,t,z,eigvals,overwrite_a,storage,err)
        !> Input matrix a[m,m]
        complex(xdp),intent(inout),target :: a(:,:)
        !> Schur form of A: upper-triangular or quasi-upper-triangular matrix T
        complex(xdp),intent(out),contiguous,target :: t(:,:)
        ...
        !> Compute Schur decomposition with complex eigenvalues
        call la_y_schur(a,t,z,ceigvals,overwrite_a,storage,err0)
    ...
    end subroutine la_real_eig_y_schur
#endif
