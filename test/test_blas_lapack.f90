...
contains
    ...
    subroutine test_gemvrqp(error)
        ...
        real(qp) :: A(3,3),x(3),y(3),ylap(3),yintr(3),alpha,beta
        real(qp),parameter :: tol = 1000*epsilon(1.0_qp)
        call random_number(alpha)
        call random_number(beta)
        call random_number(A)
        call random_number(x)
    ...
    end subroutine test_gemvrqp

    ! Find matrix inverse from LU decomposition
    subroutine test_getrirqp(error)
        !> Error handling
        type(error_type),allocatable,intent(out) :: error
