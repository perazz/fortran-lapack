...
module la_lapack_lsq_constrained
     ...
     pure subroutine la_xggglm(n,m,p,a,lda,b,ldb,d,x,y,work,lwork,info)
        ...
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ...
           integer(ilp),intent(in) :: lda,ldb,lwork,m,n,p
           ! Array Arguments
           real(xdp),intent(inout) :: a(lda,*),b(ldb,*),d(*)
           real(xdp),intent(out) :: work(*),x(*),y(*)
        ! ===================================================================
           ...
           if (info == 0) then
              if (n == 0) then
                 ...
                 nb1 = la_ilaenv(1,'XGEQRF',' ',n,m,-1,-1)
                 nb2 = la_ilaenv(1,'XGERQF',' ',n,m,-1,-1)
                 nb3 = la_ilaenv(1,'XORMQR',' ',n,m,p,-1)
                 nb4 = la_ilaenv(1,'XORMRQ',' ',n,m,p,-1)
                 nb = max(nb1,nb2,nb3,nb4)
                 lwkmin = m + n + p
                 lwkopt = m + np + max(n,p)*nb
              end if
           ...
           ! where r11 and t22 are upper triangular, and q and z are
           ! orthogonal.
           call la_xggqrf(n,m,p,a,lda,work,b,ldb,work(m + 1),work(m + np + 1),lwork - m - &
                     np,info)
           lopt = work(m + np + 1)
           ! update left-hand-side vector d = q**t*d = ( d1 ) m
                                                     ! ( d2 ) n-m
           call la_xormqr('LEFT','TRANSPOSE',n,1,m,a,lda,work,d,max(1,n),work(m + &
                     np + 1),lwork - m - np,info)
           lopt = max(lopt,int(work(m + np + 1),KIND=ilp))
           ! solve t22*y2 = d2 for y2
           if (n > m) then
              call la_xtrtrs('UPPER','NO TRANSPOSE','NON UNIT',n - m,1,b(m + 1,m + p - n + 1), &
                        ldb,d(m + 1),n - m,info)
              if (info > 0) then
                 info = 1
                 return
              end if
              call la_xcopy(n - m,d(m + 1),1,y(m + p - n + 1),1)
           end if
           ! set y1 = 0
           do i = 1,m + p - n
              y(i) = zero
           ...
           ! update d1 = d1 - t12*y2
           call la_xgemv('NO TRANSPOSE',m,n - m,-one,b(1,m + p - n + 1),ldb,y(m + p - n + 1),1, &
                     one,d,1)
           ! solve triangular system: r11*x = d1
           if (m > 0) then
              call la_xtrtrs('UPPER','NO TRANSPOSE','NON UNIT',m,1,a,lda,d,m,info)
              ...
              ! copy d to x
              call la_xcopy(m,d,1,x,1)
           end if
           ! backward transformation y = z**t *y
           call la_xormrq('LEFT','TRANSPOSE',p,1,np,b(max(1,n - p + 1),1),ldb,work( &
                     m + 1),y,max(1,p),work(m + np + 1),lwork - m - np,info)
           work(1) = m + np + max(lopt,int(work(m + np + 1),KIND=ilp))
           return
     end subroutine la_xggglm
#endif
#ifdef LA_WITH_QP
     ...
     pure subroutine la_xgglse(m,n,p,a,lda,b,ldb,c,d,x,work,lwork,info)
        use la_constants_xdp
        ! -- lapack driver routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ...
           integer(ilp),intent(in) :: lda,ldb,lwork,m,n,p
           ! Array Arguments
           real(xdp),intent(inout) :: a(lda,*),b(ldb,*),c(*),d(*)
           real(xdp),intent(out) :: work(*),x(*)
        ! =====================================================================
           ...
           if (info == 0) then
              if (n == 0) then
                 ...
                 nb3 = la_ilaenv(1,'XORMQR',' ',m,n,p,-1)
                 nb4 = la_ilaenv(1,'XORMRQ',' ',m,n,p,-1)
                 nb = max(nb1,nb2,nb3,nb4)
                 lwkmin = m + n + p
                 lwkopt = p + mn + max(m,n)*nb
              end if
           ...
           ! where t12 and r11 are upper triangular, and q and z are
           ! orthogonal.
           call la_xggrqf(p,m,n,b,ldb,work,a,lda,work(p + 1),work(p + mn + 1),lwork - p - &
                     mn,info)
           lopt = work(p + mn + 1)
           ! update c = z**t *c = ( c1 ) n-p
                                ! ( c2 ) m+p-n
           call la_xormqr('LEFT','TRANSPOSE',m,1,mn,a,lda,work(p + 1),c,max(1,m), &
                     work(p + mn + 1),lwork - p - mn,info)
           lopt = max(lopt,int(work(p + mn + 1),KIND=ilp))
           ! solve t12*x2 = d for x2
           if (p > 0) then
              call la_xtrtrs('UPPER','NO TRANSPOSE','NON-UNIT',p,1,b(1,n - p + 1),ldb,d, &
                         p,info)
              if (info > 0) then
                 info = 1
                 return
              ...
              ! put the solution in x
              call la_xcopy(p,d,1,x(n - p + 1),1)
              ! update c1
              call la_xgemv('NO TRANSPOSE',n - p,p,-one,a(1,n - p + 1),lda,d,1,one,c,1 &
                        )
           end if
           ! solve r11*x1 = c1 for x1
           if (n > p) then
              call la_xtrtrs('UPPER','NO TRANSPOSE','NON-UNIT',n - p,1,a,lda,c,n - p, &
                        info)
              if (info > 0) then
                 info = 2
                 return
              ...
              ! put the solutions in x
              call la_xcopy(n - p,c,1,x,1)
           end if
           ! compute the residual vector:
           if (m < n) then
              nr = m + p - n
              if (nr > 0) call la_xgemv('NO TRANSPOSE',nr,n - m,-one,a(n - p + 1,m + 1),lda,d( &
                        nr + 1),1,one,c(n - p + 1),1)
           else
              nr = p
           end if
           if (nr > 0) then
              call la_xtrmv('UPPER','NO TRANSPOSE','NON UNIT',nr,a(n - p + 1,n - p + 1),lda, &
                        d,1)
              call la_xaxpy(nr,-one,d,1,c(n - p + 1),1)
           end if
           ! backward transformation x = q**t*x
           call la_xormrq('LEFT','TRANSPOSE',n,1,p,b,ldb,work(1),x,n,work(p + mn + 1 &
                     ),lwork - p - mn,info)
           work(1) = p + mn + max(lopt,int(work(p + mn + 1),KIND=ilp))
           return
     end subroutine la_xgglse
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
     !> YGGGLM: solves a general Gauss-Markov linear model (GLM) problem:
     !> minimize || y ||_2   subject to   d = A*x + B*y
     !> x
     !> where A is an N-by-M matrix, B is an N-by-P matrix, and d is a
     !> given N-vector. It is assumed that M <= N <= M+P, and
     ...
     pure subroutine la_yggglm(n,m,p,a,lda,b,ldb,d,x,y,work,lwork,info)
        use la_constants_xdp
        ! -- lapack driver routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ...
           integer(ilp),intent(in) :: lda,ldb,lwork,m,n,p
           ! Array Arguments
           complex(xdp),intent(inout) :: a(lda,*),b(ldb,*),d(*)
           complex(xdp),intent(out) :: work(*),x(*),y(*)
        ! ===================================================================
           ...
           if (info == 0) then
              if (n == 0) then
                 ...
                 nb1 = la_ilaenv(1,'YGEQRF',' ',n,m,-1,-1)
                 nb2 = la_ilaenv(1,'YGERQF',' ',n,m,-1,-1)
                 nb3 = la_ilaenv(1,'YUNMQR',' ',n,m,p,-1)
                 nb4 = la_ilaenv(1,'YUNMRQ',' ',n,m,p,-1)
                 nb = max(nb1,nb2,nb3,nb4)
                 lwkmin = m + n + p
                 lwkopt = m + np + max(n,p)*nb
              end if
           ...
           ! where r11 and t22 are upper triangular, and q and z are
           ! unitary.
           call la_yggqrf(n,m,p,a,lda,work,b,ldb,work(m + 1),work(m + np + 1),lwork - m - &
                     np,info)
           lopt = real(work(m + np + 1),KIND=xdp)
           ! update left-hand-side vector d = q**h*d = ( d1 ) m
                                                     ! ( d2 ) n-m
           call la_yunmqr('LEFT','CONJUGATE TRANSPOSE',n,1,m,a,lda,work,d,max(1,n) &
                     ,work(m + np + 1),lwork - m - np,info)
           lopt = max(lopt,int(work(m + np + 1),KIND=ilp))
           ! solve t22*y2 = d2 for y2
           if (n > m) then
              call la_ytrtrs('UPPER','NO TRANSPOSE','NON UNIT',n - m,1,b(m + 1,m + p - n + 1), &
                        ldb,d(m + 1),n - m,info)
              if (info > 0) then
                 info = 1
                 return
              end if
              call la_ycopy(n - m,d(m + 1),1,y(m + p - n + 1),1)
           end if
           ! set y1 = 0
           do i = 1,m + p - n
              y(i) = czero
           ...
           ! update d1 = d1 - t12*y2
           call la_ygemv('NO TRANSPOSE',m,n - m,-cone,b(1,m + p - n + 1),ldb,y(m + p - n + 1),1, &
                      cone,d,1)
           ! solve triangular system: r11*x = d1
           if (m > 0) then
              call la_ytrtrs('UPPER','NO TRANSPOSE','NON UNIT',m,1,a,lda,d,m,info)
              ...
              ! copy d to x
              call la_ycopy(m,d,1,x,1)
           end if
           ! backward transformation y = z**h *y
           call la_yunmrq('LEFT','CONJUGATE TRANSPOSE',p,1,np,b(max(1,n - p + 1),1), &
                     ldb,work(m + 1),y,max(1,p),work(m + np + 1),lwork - m - np,info)
           work(1) = m + np + max(lopt,int(work(m + np + 1),KIND=ilp))
           return
     end subroutine la_yggglm
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
     !> YGGLSE: solves the linear equality-constrained least squares (LSE)
     !> problem:
     !> minimize || c - A*x ||_2   subject to   B*x = d
     !> where A is an M-by-N matrix, B is a P-by-N matrix, c is a given
     !> M-vector, and d is a given P-vector. It is assumed that
     ...
     pure subroutine la_ygglse(m,n,p,a,lda,b,ldb,c,d,x,work,lwork,info)
        use la_constants_xdp
        ! -- lapack driver routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ...
           integer(ilp),intent(in) :: lda,ldb,lwork,m,n,p
           ! Array Arguments
           complex(xdp),intent(inout) :: a(lda,*),b(ldb,*),c(*),d(*)
           complex(xdp),intent(out) :: work(*),x(*)
        ! =====================================================================
           ...
           if (info == 0) then
              if (n == 0) then
                 ...
                 nb3 = la_ilaenv(1,'YUNMQR',' ',m,n,p,-1)
                 nb4 = la_ilaenv(1,'YUNMRQ',' ',m,n,p,-1)
                 nb = max(nb1,nb2,nb3,nb4)
                 lwkmin = m + n + p
                 lwkopt = p + mn + max(m,n)*nb
              end if
           ...
           ! where t12 and r11 are upper triangular, and q and z are
           ! unitary.
           call la_yggrqf(p,m,n,b,ldb,work,a,lda,work(p + 1),work(p + mn + 1),lwork - p - &
                     mn,info)
           lopt = real(work(p + mn + 1),KIND=xdp)
           ! update c = z**h *c = ( c1 ) n-p
                             ! ( c2 ) m+p-n
           call la_yunmqr('LEFT','CONJUGATE TRANSPOSE',m,1,mn,a,lda,work(p + 1),c, &
                     max(1,m),work(p + mn + 1),lwork - p - mn,info)
           lopt = max(lopt,int(work(p + mn + 1),KIND=ilp))
           ! solve t12*x2 = d for x2
           if (p > 0) then
              call la_ytrtrs('UPPER','NO TRANSPOSE','NON-UNIT',p,1,b(1,n - p + 1),ldb,d, &
                         p,info)
              if (info > 0) then
                 info = 1
                 return
              ...
              ! put the solution in x
              call la_ycopy(p,d,1,x(n - p + 1),1)
              ! update c1
              call la_ygemv('NO TRANSPOSE',n - p,p,-cone,a(1,n - p + 1),lda,d,1,cone,c, &
                        1)
           end if
           ! solve r11*x1 = c1 for x1
           if (n > p) then
              call la_ytrtrs('UPPER','NO TRANSPOSE','NON-UNIT',n - p,1,a,lda,c,n - p, &
                        info)
              if (info > 0) then
                 info = 2
                 return
              ...
              ! put the solutions in x
              call la_ycopy(n - p,c,1,x,1)
           end if
           ! compute the residual vector:
           if (m < n) then
              nr = m + p - n
              if (nr > 0) call la_ygemv('NO TRANSPOSE',nr,n - m,-cone,a(n - p + 1,m + 1),lda,d( &
                         nr + 1),1,cone,c(n - p + 1),1)
           else
              nr = p
           end if
           if (nr > 0) then
              call la_ytrmv('UPPER','NO TRANSPOSE','NON UNIT',nr,a(n - p + 1,n - p + 1),lda, &
                        d,1)
              call la_yaxpy(nr,-cone,d,1,c(n - p + 1),1)
           end if
           ! backward transformation x = q**h*x
           call la_yunmrq('LEFT','CONJUGATE TRANSPOSE',n,1,p,b,ldb,work(1),x,n, &
                     work(p + mn + 1),lwork - p - mn,info)
           work(1) = p + mn + max(lopt,int(work(p + mn + 1),KIND=ilp))
           return
     end subroutine la_ygglse
#endif
#ifdef LA_WITH_QP
