...
#ifdef LA_WITH_XDP
    subroutine test_xlstsq_one(error)
        logical,intent(out) :: error
        ...
        error = state%error() .or. .not. all(abs(p - ab) < 1.0e-6_xdp) .or. rank /= 2
        if (error) return

        ! Repeat, letting the solver overwrite the coefficient matrix
        p = lstsq(M,y,overwrite_a=.true.,err=state)

        error = state%error() .or. .not. all(abs(p - ab) < 1.0e-6_xdp)

    end subroutine test_xlstsq_one
#endif
#ifdef LA_WITH_QP
