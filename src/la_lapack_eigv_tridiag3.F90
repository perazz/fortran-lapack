...
module la_lapack_eigv_tridiag3
     ...
     pure subroutine la_xstebz(range,order,n,vl,vu,il,iu,abstol,d,e,m,nsplit,w, &
               iblock,isplit,work,iwork,info)
        ...
        ! =====================================================================
           ! Parameters
           real(xdp),parameter :: fudge = 2.1_xdp
           real(xdp),parameter :: relfac = 2.0_xdp

           ! Local Scalars
           logical(lk) :: ncnvrg,toofew
           integer(ilp) :: ib,ibegin,idiscl,idiscu,ie,iend,iinfo,im,in,ioff,iorder, &
                     iout,irange,itmax,itmp1,iw,iwoff,j,jb,jdisc,je,nb,nwl,nwu
           real(xdp) :: atoli,bnorm,gl,gu,pivmin,rtoli,safemn,tmp1,tmp2,tnorm,ulp,wkill, &
                      wl,wlu,wu,wul
           ! Local Arrays
           integer(ilp) :: idumma(1)
           ! Intrinsic Functions
           ...
           if (irange == 3) then
              ...
              iwork(5) = il - 1
              iwork(6) = iu
              call la_xlaebz(3,itmax,n,2,2,nb,atoli,rtoli,pivmin,d,e,work,iwork( &
                        5),work(n + 1),work(n + 5),iout,iwork,w,iblock,iinfo)
              if (iwork(6) == iu) then
                 wl = work(n + 1)
                 wlu = work(n + 3)
           ...
           loop_70: do jb = 1,nsplit
              ...
              if (in == 1) then
                 ...
                 work(n + 1) = gl
                 work(n + in + 1) = gu
                 call la_xlaebz(1,0,in,in,1,nb,atoli,rtoli,pivmin,d(ibegin),e( &
                 ibegin),work(ibegin),idumma,work(n + 1),work(n + 2*in + 1),im,iwork,w(m + 1 &
                           ),iblock(m + 1),iinfo)
                 nwl = nwl + iwork(1)
                 nwu = nwu + iwork(in + 1)
                 ...
                 itmax = int((log(gu - gl + pivmin) - log(pivmin))/log(two),KIND=ilp) + &
                           2
                 call la_xlaebz(2,itmax,in,in,1,nb,atoli,rtoli,pivmin,d(ibegin),e( &
                  ibegin),work(ibegin),idumma,work(n + 1),work(n + 2*in + 1),iout,iwork,w( &
                            m + 1),iblock(m + 1),iinfo)
                 ! copy eigenvalues into w and iblock
                 ! use -jb for block number for unconverged eigenvalues.
...
#ifdef LA_WITH_XDP
     !> XSTEIN: computes the eigenvectors of a real symmetric tridiagonal
     !> matrix T corresponding to specified eigenvalues, using inverse
     !> iteration.
     !> The maximum number of iterations allowed for each eigenvector is
     !> specified by an internal parameter MAXITS (currently set to 5).

     pure subroutine la_xstein(n,d,e,m,w,iblock,isplit,z,ldz,work,iwork,ifail, &
               info)
        ...
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ...
           integer(ilp),intent(in) :: iblock(*),isplit(*)
           integer(ilp),intent(out) :: ifail(*),iwork(*)
           real(xdp),intent(in) :: d(*),e(*),w(*)
           real(xdp),intent(out) :: work(*),z(ldz,*)
        ! =====================================================================
           ! Parameters
           real(xdp),parameter :: odm3 = 1.0e-3_xdp
           real(xdp),parameter :: odm1 = 1.0e-1_xdp
           integer(ilp),parameter :: maxits = 5
           integer(ilp),parameter :: extra = 2
           ...
           integer(ilp) :: b1,blksiz,bn,gpind,i,iinfo,indrv1,indrv2,indrv3,indrv4, &
                     indrv5,its,j,j1,jblk,jmax,nblk,nrmchk
           real(xdp) :: xtpcrt,eps,eps1,nrm,onenrm,ortol,pertol,scl,sep,tol,xj,xjm, &
                     ytr
           ! Local Arrays
           integer(ilp) :: iseed(4)
           ! Intrinsic Functions
           ...
           ! initialize seed for random number generator la_xlarnv.
           do i = 1,4
              iseed(i) = 1
           end do
           ! initialize pointers.
           ...
           loop_160: do nblk = 1,iblock(m)
              ...
              loop_150: do j = j1,m
                 ...
                 nrmchk = 0
                 ! get random starting vector.
                 call la_xlarnv(2,iseed,blksiz,work(indrv1 + 1))
                 ! copy the matrix t so it won't be destroyed in factorization.
                 call la_xcopy(blksiz,d(b1),1,work(indrv4 + 1),1)
                 call la_xcopy(blksiz - 1,e(b1),1,work(indrv2 + 2),1)
                 call la_xcopy(blksiz - 1,e(b1),1,work(indrv3 + 1),1)
                 ! compute lu factors with partial pivoting  ( pt = lu )
                 tol = zero
                 call la_xlagtf(blksiz,work(indrv4 + 1),xj,work(indrv2 + 2),work(indrv3 + &
                           1),tol,work(indrv5 + 1),iwork,iinfo)
                 ! update iteration count.
                 70 continue
                 its = its + 1
                 if (its > maxits) go to 100
                 ! normalize and scale the righthand side vector pb.
                 jmax = la_ixamax(blksiz,work(indrv1 + 1),1)
                 scl = blksiz*onenrm*max(eps,abs(work(indrv4 + blksiz)))/abs(work(indrv1 + &
                           jmax))
                 call la_xscal(blksiz,scl,work(indrv1 + 1),1)
                 ! solve the system lu = pb.
                 call la_xlagts(-1,blksiz,work(indrv4 + 1),work(indrv2 + 2),work(indrv3 + &
                           1),work(indrv5 + 1),iwork,work(indrv1 + 1),tol,iinfo)
                 ! reorthogonalize by modified gram-schmidt if eigenvalues are
                 ! close enough.
                 if (jblk == 1) go to 90
                 ...
                 if (gpind /= j) then
                    do i = gpind,j - 1
                       ytr = -la_xdot(blksiz,work(indrv1 + 1),1,z(b1,i),1)
                       call la_xaxpy(blksiz,ytr,z(b1,i),1,work(indrv1 + 1),1)
                    end do
                 end if
                 ! check the infinity norm of the iterate.
                 90 continue
                 ...
                 ! accept iterate as jth eigenvector.
                 110 continue
                 scl = one/la_xnrm2(blksiz,work(indrv1 + 1),1)
                 jmax = la_ixamax(blksiz,work(indrv1 + 1),1)
                 if (work(indrv1 + jmax) < zero) scl = -scl
                 call la_xscal(blksiz,scl,work(indrv1 + 1),1)
                 120 continue
     ...
     pure subroutine la_xsterf(n,d,e,info)
        ...
        ! =====================================================================
           ...
           ! Local Scalars
           integer(ilp) :: i,iscale,jtot,l,l1,lend,lendsv,lsv,m,nmaxit
           real(xdp) :: alpha,anorm,bb,c,eps,eps2,gamma,oldc,oldgam,p,r,rt1,rt2,rte, &
                     s,safmax,safmin,sigma,ssfmax,ssfmin,rmax
           ! Intrinsic Functions
           intrinsic :: abs,sign,sqrt
           ! Executable Statements
           ...
           if (lend >= l) then
              ...
              p = d(l)
              if (m == l) go to 90
              ! if remaining matrix is 2 by 2, use la_xlae2 to compute its
              ! eigenvalues.
              if (m == l + 1) then
                 rte = sqrt(e(l))
                 call la_xlae2(d(l),rte,d(l + 1),rt1,rt2)
                 d(l) = rt1
                 d(l + 1) = rt2
                 e(l) = zero
                 l = l + 2
           ...
           ! undo scaling if necessary
           150 continue
           if (iscale == 1) call la_xlascl('G',0,0,ssfmax,anorm,lendsv - lsv + 1,1,d(lsv), &
                     n,info)
           if (iscale == 2) call la_xlascl('G',0,0,ssfmin,anorm,lendsv - lsv + 1,1,d(lsv), &
                     n,info)
           ! check for no convergence to an eigenvalue after a total
           ! of n*maxit iterations.
           if (jtot < nmaxit) go to 10
...
#ifdef LA_WITH_XDP
     !> XSTEV: computes all eigenvalues and, optionally, eigenvectors of a
     !> real symmetric tridiagonal matrix A.

     pure subroutine la_xstev(jobz,n,d,e,z,ldz,work,info)
        ...
        ! =====================================================================
           ...
           logical(lk) :: wantz
           integer(ilp) :: imax,iscale
           real(xdp) :: bignum,eps,rmax,rmin,safmin,sigma,smlnum,tnrm
           ! Intrinsic Functions
           intrinsic :: sqrt
           ! Executable Statements
           ! test the input parameters.
           ...
           if (iscale == 1) then
              call la_xscal(n,sigma,d,1)
              call la_xscal(n - 1,sigma,e(1),1)
           end if
           ! for eigenvalues only, call la_xsterf.  for eigenvalues and
           ! eigenvectors, call la_xsteqr.
           if (.not. wantz) then
              call la_xsterf(n,d,e,info)
           else
              call la_xsteqr('I',n,d,e,z,ldz,work,info)
           end if
           ! if matrix was scaled, then rescale eigenvalues appropriately.
           if (iscale == 1) then
              if (info == 0) then
              ...
              call la_xscal(imax,one/sigma,d,1)
           end if
           return
     end subroutine la_xstev
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
     !> XSTEVX: computes selected eigenvalues and, optionally, eigenvectors
     !> of a real symmetric tridiagonal matrix A.  Eigenvalues and
     !> eigenvectors can be selected by specifying either a range of values
     !> or a range of indices for the desired eigenvalues.

     pure subroutine la_xstevx(jobz,range,n,d,e,vl,vu,il,iu,abstol,m,w,z,ldz, &
               work,iwork,ifail,info)
        use la_constants_xdp,only:zero,one
        ! -- lapack driver routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ...
        ! =====================================================================
           ...
           integer(ilp) :: i,imax,indibl,indisp,indiwo,indwrk,iscale,itmp1,j,jj, &
                     nsplit
           real(xdp) :: bignum,eps,rmax,rmin,safmin,sigma,smlnum,tmp1,tnrm,vll, &
                     vuu
           ! Intrinsic Functions
           intrinsic :: max,min,sqrt
           ! Executable Statements
           ...
           ! if all eigenvalues are desired and abstol is less than zero, then
           ! call la_xsterf or la_dsteqr.  if this fails for some eigenvalue, then
           ! try la_xstebz.
           test = .false.
           if (indeig) then
              if (il == 1 .and. iu == n) then
           ...
           if ((alleig .or. test) .and. (abstol <= zero)) then
              call la_xcopy(n,d,1,w,1)
              call la_xcopy(n - 1,e(1),1,work(1),1)
              indwrk = n + 1
              if (.not. wantz) then
                 call la_xsterf(n,w,work,info)
              else
                 call la_xsteqr('I',n,w,work,z,ldz,work(indwrk),info)
                 if (info == 0) then
                    do i = 1,n
                       ifail(i) = 0
                    end do
           ...
           indisp = indibl + n
           indiwo = indisp + n
           call la_xstebz(range,order,n,vll,vuu,il,iu,abstol,d,e,m,nsplit,w, &
                     iwork(indibl),iwork(indisp),work(indwrk),iwork(indiwo),info)
           if (wantz) then
              call la_xstein(n,d,e,m,w,iwork(indibl),iwork(indisp),z,ldz,work( &
                        indwrk),iwork(indiwo),ifail,info)
           end if
           ! if matrix was scaled, then rescale eigenvalues appropriately.
           20 continue
     ...
     !> symmetric tridiagonal matrix using the divide and conquer method.
     !> The eigenvectors of a full or band real symmetric matrix can also be
     !> found if XSYTRD or XSPTRD or XSBTRD has been used to reduce this
     !> matrix to tridiagonal form.
     !> This code makes very mild assumptions about floating point
     !> arithmetic. It will work on machines with a guard digit in
     !> add/subtract, or on those binary machines without guard digits
     !> which subtract like the Cray X-MP, Cray Y-MP, Cray C-90, or Cray-2.
     !> It could conceivably fail on hexadecimal or decimal machines
     !> without guard digits, but we know of none.  See XLAED3 for details.

     pure subroutine la_xstedc(compz,n,d,e,z,ldz,work,lwork,iwork,liwork,info)
        ...
        ! =====================================================================
           ...
           if (info == 0) then
              ! compute the workspace requirements
              smlsiz = la_ilaenv(9,'XSTEDC',' ',0,0,0,0)
              if (n <= 1 .or. icompz == 0) then
                 liwmin = 1
                 lwmin = 1
              else if (n <= smlsiz) then
                 ...
                 lgn = int(log(real(n,KIND=xdp))/log(two),KIND=ilp)
                 if (2**lgn < n) lgn = lgn + 1
                 if (2**lgn < n) lgn = lgn + 1
                 if (icompz == 1) then
                    lwmin = 1 + 3*n + 2*n*lgn + 4*n**2
           ...
           ! eigenvalues, which requires (3n + 3n**2) real workspace and
           ! (2 + 5n + 2n lg(n)) integer workspace.
           ! since on many architectures la_xsterf is much faster than any other
           ! algorithm for finding eigenvalues only, it is used here
           ! as the default. if the conditional clause is removed, then
           ! information on the size of workspace needs to be changed.
           ! if compz = 'n', use la_xsterf to compute the eigenvalues.
           if (icompz == 0) then
              call la_xsterf(n,d,e,info)
              go to 50
           end if
           ...
           ! solve the problem with another solver.
           if (n <= smlsiz) then
              call la_xsteqr(compz,n,d,e,z,ldz,work,info)
           else
              ! if compz = 'v', the z matrix must be stored elsewhere for later
              ! use.
              if (icompz == 1) then
              ...
              if (icompz == 2) then
                 call la_xlaset('FULL',n,n,zero,one,z,ldz)
              end if
              ! scale.
              ...
              if (start <= n) then
                 ...
                 if (m > smlsiz) then
                    ! scale.
                    orgnrm = la_xlanst('M',m,d(start),e(start))
                    call la_xlascl('G',0,0,orgnrm,one,m,1,d(start),m,info)
                    call la_xlascl('G',0,0,orgnrm,one,m - 1,1,e(start),m - 1,info)
                    ...
                    call la_xlaed0(icompz,n,m,d(start),e(start),z(strtrw,start), &
                              ldz,work(1),n,work(storez),iwork,info)
                    if (info /= 0) then
                       info = (info/(m + 1) + start - 1)*(n + 1) + mod(info, (m + 1)) + start - &
                                 1
                    ...
                    ! scale back.
                    call la_xlascl('G',0,0,one,orgnrm,m,1,d(start),m,info)
                 else
                    if (icompz == 1) then
                       ! since qr won't update a z matrix which is larger than
                       ! the length of d, we must solve the sub-problem in a
                       ! workspace and then multiply back into z.
                       call la_xsteqr('I',m,d(start),e(start),work,m,work(m*m + 1), &
                                 info)
                       call la_xlacpy('A',n,m,z(1,start),ldz,work(storez),n)

                       call la_xgemm('N','N',n,m,m,one,work(storez),n,work,m,zero, &
                                 z(1,start),ldz)
                    else if (icompz == 2) then
                       call la_xsteqr('I',m,d(start),e(start),z(start,start),ldz, &
                                 work,info)
                    else
                       call la_xsterf(m,d(start),e(start),info)
                    end if
                    if (info /= 0) then
                       info = start*(n + 1) + finish
                       go to 50
...
#ifdef LA_WITH_XDP
     !> XSTEVD: computes all eigenvalues and, optionally, eigenvectors of a
     !> real symmetric tridiagonal matrix. If eigenvectors are desired, it
     !> uses a divide and conquer algorithm.
     !> The divide and conquer algorithm makes very mild assumptions about
     !> floating point arithmetic. It will work on machines with a guard
     ...
     pure subroutine la_xstevd(jobz,n,d,e,z,ldz,work,lwork,iwork,liwork,info)
        use la_constants_xdp,only:zero,one
        ...
        ! =====================================================================
           ...
           ! for eigenvalues only, call la_xsterf.  for eigenvalues and
           ! eigenvectors, call la_xstedc.
           if (.not. wantz) then
              call la_xsterf(n,d,e,info)
           else
              call la_xstedc('I',n,d,e,z,ldz,work,lwork,iwork,liwork,info)
           end if
           ! if matrix was scaled, then rescale eigenvalues appropriately.
           if (iscale == 1) call la_xscal(n,one/sigma,d,1)
           work(1) = lwmin
           iwork(1) = liwmin
           return
     end subroutine la_xstevd
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
     !> XPTEQR: computes all eigenvalues and, optionally, eigenvectors of a
     !> symmetric positive definite tridiagonal matrix by first factoring the
     !> matrix using XPTTRF, and then calling XBDSQR to compute the singular
     !> values of the bidiagonal factor.
     !> This routine computes the eigenvalues of the positive definite
     !> tridiagonal matrix to high relative accuracy.  This means that if the
     !> eigenvalues range over many orders of magnitude in size, then the
     ...
     !> more accurately than, for example, with the standard QR method.
     !> The eigenvectors of a full or band symmetric positive definite matrix
     !> can also be found if XSYTRD, XSPTRD, or XSBTRD has been used to
     !> reduce this matrix to tridiagonal form. (The reduction to tridiagonal
     !> form, however, may preclude the possibility of obtaining high
     !> relative accuracy in the small eigenvalues of the original matrix, if
     !> these eigenvalues range over many orders of magnitude.)

     pure subroutine la_xpteqr(compz,n,d,e,z,ldz,work,info)
        ...
        ! =====================================================================
           ...
           ! call la_xpttrf to factor the matrix.
           call la_xpttrf(n,d,e,info)
           if (info /= 0) return
           do i = 1,n
              d(i) = sqrt(d(i))
           end do
           ...
           ! call la_xbdsqr to compute the singular values/vectors of the
           ! bidiagonal factor.
           if (icompz > 0) then
              nru = n
           else
           ...
           call la_xbdsqr('LOWER',n,0,nru,0,d,e,vt,1,z,ldz,c,1,work,info)
     ...
     !> either an interval (VL,VU] or a range of indices IL:IU for the desired
     !> eigenvalues.
     !> XSTEGR is a compatibility wrapper around the improved XSTEMR routine.
     !> See XSTEMR for further details.
     !> One important change is that the ABSTOL parameter no longer provides any
     !> benefit and hence is no longer used.
     !> Note : XSTEGR and XSTEMR work only on machines which follow
     !> IEEE-754 floating-point standard in their handling of infinities and
     !> NaNs.  Normal execution may create these exceptiona values and hence
     !> may abort due to a floating point exception in environments which
     !> do not conform to the IEEE-754 standard.

     pure subroutine la_xstegr(jobz,range,n,d,e,vl,vu,il,iu,abstol,m,w,z,ldz, &
               isuppz,work,lwork,iwork,liwork,info)
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
        ...
        ! =====================================================================
           ...
           info = 0
           tryrac = .false.
           call la_xstemr(jobz,range,n,d,e,vl,vu,il,iu,m,w,z,ldz,n,isuppz, &
                     tryrac,work,lwork,iwork,liwork,info)
     end subroutine la_xstegr
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
     !> XSTEMR: computes selected eigenvalues and, optionally, eigenvectors
     !> of a real symmetric tridiagonal matrix T. Any such unreduced matrix has
     !> a well defined set of pairwise different real eigenvalues, the corresponding
     !> real eigenvectors are pairwise orthogonal.
     !> The spectrum may be computed either completely or partially by specifying
     ...
     !> UC Berkeley, May 1997.
     !> Further Details
     !> 1.XSTEMR works only on machines which follow IEEE-754
     !> floating-point standard in their handling of infinities and NaNs.
     !> This permits the use of efficient inner loops avoiding a check for
     !> zero divisors.

     pure subroutine la_xstemr(jobz,range,n,d,e,vl,vu,il,iu,m,w,z,ldz,nzc, &
               isuppz,tryrac,work,lwork,iwork,liwork,info)
        use la_constants_xdp,only:zero,one,four
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ! Scalar Arguments
        ...
        ! =====================================================================
           ! Parameters
           real(xdp),parameter :: minrgp = 1.0e-3_xdp
           ...
           iiu,ilast,in,indd,inde2,inderr,indgp,indgrs,indwrk,itmp,itmp2,j,jblk,jj, &
                     liwmin,lwmin,nsplit,nzcmin,offset,wbegin,wend
           real(xdp) :: bignum,cs,eps,pivmin,r1,r2,rmax,rmin,rtol1,rtol2,safmin,scale, &
                     smlnum,sn,thresh,tmp,tnrm,wl,wu
           ! Intrinsic Functions
           intrinsic :: max,min,sqrt
           ! Executable Statements
           ...
           lquery = ((lwork == -1) .or. (liwork == -1))
           zquery = (nzc == -1)
           ! la_xstemr needs work of size 6*n, iwork of size 3*n.
           ! in addition, la_xlarre needs work of size 6*n, iwork of size 5*n.
           ! furthermore, la_xlarrv needs work of size 12*n, iwork of size 7*n.
           if (wantz) then
              lwmin = 18*n
              liwmin = 10*n
           else
           ...
           if (valeig) then
              ! we do not reference vl, vu in the cases range = 'i','a'
              ! the interval (wl, wu] contains all the wanted eigenvalues.
              ! it is either given by the user or computed in la_xlarre.
              wl = vl
              wu = vu
           elseif (indeig) then
              ! we do not reference il, iu in the cases range = 'v','a'
           ...
           if (info == 0) then
              ...
              if (wantz .and. alleig) then
                 ...
                 call la_xlarrc('T',n,vl,vu,d,e,safmin,nzcmin,itmp,itmp2,info)
           ...
           if (n == 2) then
              if (.not. wantz) then
                 call la_xlae2(d(1),e(1),d(2),r1,r2)
              else if (wantz .and. (.not. zquery)) then
                 call la_xlaev2(d(1),e(1),d(2),r1,r2,cs,sn)
              end if
              if (alleig .or. (valeig .and. (r2 > wl) .and. (r2 <= wu)) .or. (indeig .and. (iil == 1))) &
                        then
                 m = m + 1
           ...
           ! continue with general n
              ...
              ! scale matrix to allowable range, if necessary.
              ! the allowable range is related to the pivmin parameter; see the
              ! comments in la_xlarrd.  the preference for scaling small values
              ! up is heuristic; we expect users' matrices not to be close to the
              ! rmax threshold.
              scale = one
              tnrm = la_xlanst('M',n,d,e)
              ...
              if (scale /= one) then
                 call la_xscal(n,scale,d,1)
                 call la_xscal(n - 1,scale,e,1)
                 tnrm = tnrm*scale
                 if (valeig) then
                    ! if eigenvalues in interval have to be found,
                    ! scale (wl, wu] accordingly
              ...
              ! into smaller subblocks if the corresponding off-diagonal elements
              ! are small
              ! thresh is the splitting parameter for la_xlarre
              ! a negative thresh forces the old splitting criterion based on the
              ! size of the off-diagonal. a positive thresh switches to splitting
              ! which preserves relative accuracy.
              if (tryrac) then
                 ! test whether the matrix warrants the more expensive relative approach.
                 call la_xlarrr(n,d,e,iinfo)
              else
                 ! the user does not care about relative accurately eigenvalues
                 iinfo = -1
              end if
              ...
              if (tryrac) then
                 ! copy original diagonal, needed to guarantee relative accuracy
                 call la_xcopy(n,d,1,work(indd),1)
              end if
              ! store the squares of the offdiagonal values of t
              do j = 1,n - 1
                 work(inde2 + j - 1) = e(j)**2
              ...
              ! set the tolerance parameters for bisection
              if (.not. wantz) then
                 ! la_xlarre computes the eigenvalues to full precision.
                 rtol1 = four*eps
                 rtol2 = four*eps
              else
                 ! la_xlarre computes the eigenvalues to less than full precision.
                 ! la_xlarrv will refine the eigenvalue approximations, and we can
                 ! need less accurate initial bisection in la_xlarre.
                 ! note: these settings do only affect the subset case and la_xlarre
                 rtol1 = sqrt(eps)
                 rtol2 = max(sqrt(eps)*5.0e-3_xdp,four*eps)
              end if
              call la_xlarre(range,n,wl,wu,iil,iiu,d,e,work(inde2),rtol1,rtol2, &
              thresh,nsplit,iwork(iinspl),m,w,work(inderr),work(indgp),iwork(iindbl), &
              iwork(iindw),work(indgrs),pivmin,work(indwrk),iwork(iindwk),iinfo)
              ...
              ! note that if range /= 'v', la_xlarre computes bounds on the desired
              ! part of the spectrum. all desired eigenvalues are contained in
              ! (wl,wu]
              if (wantz) then
                 ! compute the desired eigenvectors corresponding to the computed
                 ! eigenvalues
                 call la_xlarrv(n,wl,wu,d,e,pivmin,iwork(iinspl),m,1,m,minrgp, &
                 rtol1,rtol2,w,work(inderr),work(indgp),iwork(iindbl),iwork(iindw), &
                           work(indgrs),z,ldz,isuppz,work(indwrk),iwork(iindwk),iinfo)
                 if (iinfo /= 0) then
                    info = 20 + abs(iinfo)
                 ...
                 ! la_xlarre computes eigenvalues of the (shifted) root representation
                 ! la_xlarrv returns the eigenvalues of the unshifted matrix.
                 ! however, if the eigenvectors are not desired by the user, we need
                 ! to apply the corresponding shifts from la_xlarre to obtain the
                 ! eigenvalues of the original matrix.
                 do j = 1,m
                    itmp = iwork(iindbl + j - 1)
                    w(j) = w(j) + e(iwork(iinspl + itmp - 1))
              ...
              if (tryrac) then
                 ...
                 loop_39: do jblk = 1,iwork(iindbl + m - 1)
                    ...
                    ilast = iwork(iindw + wend - 1)
                    rtol2 = four*eps
                    call la_xlarrj(in,work(indd + ibegin - 1),work(inde2 + ibegin - 1),ifirst, &
                    ilast,rtol2,offset,w(wbegin),work(inderr + wbegin - 1),work(indwrk),iwork( &
                               iindwk),pivmin,tnrm,iinfo)
                    ibegin = iend + 1
                    wbegin = wend + 1
     ...
     !> eigenvectors can be selected by specifying either a range of values
     !> or a range of indices for the desired eigenvalues.
     !> Whenever possible, XSTEVR calls XSTEMR to compute the
     !> eigenspectrum using Relatively Robust Representations.  XSTEMR
     !> computes eigenvalues by the dqds algorithm, while orthogonal
     !> eigenvectors are computed from various "good" L D L^T representations
     !> (also known as Relatively Robust Representations). Gram-Schmidt
     !> orthogonalization is avoided as far as possible. More specifically,
     ...
     !> Computer Science Division Technical Report No. UCB//CSD-97-971,
     !> UC Berkeley, May 1997.
     !> Note 1 : XSTEVR calls XSTEMR when the full spectrum is requested
     !> on machines which conform to the ieee-754 floating point standard.
     !> XSTEVR calls XSTEBZ and XSTEIN on non-ieee machines and
     !> when partial spectrum requests are made.
     ...
     pure subroutine la_xstevr(jobz,range,n,d,e,vl,vu,il,iu,abstol,m,w,z,ldz, &
               isuppz,work,lwork,iwork,liwork,info)
        use la_constants_xdp,only:zero,one,two
        ! -- lapack driver routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ...
        ! =====================================================================
           ...
           ! Executable Statements
           ! test the input parameters.
           ieeeok = la_ilaenv(10,'XSTEVR','N',1,2,3,4)
           wantz = la_lsame(jobz,'V')
           alleig = la_lsame(range,'A')
           valeig = la_lsame(range,'V')
           indeig = la_lsame(range,'I')
           ...
           ! initialize indices into workspaces.  note: these indices are used only
           ! if la_xsterf or la_xstemr fail.
           ...
           indiwo = indisp + n
           ! if all eigenvalues are desired, then
           ! call la_xsterf or la_xstemr.  if this fails for some eigenvalue, then
           ! try la_xstebz.
           test = .false.
           if (indeig) then
              if (il == 1 .and. iu == n) then
           ...
           if ((alleig .or. test) .and. ieeeok == 1) then
              ...
              if (.not. wantz) then
                 ...
                 call la_xcopy(n,d,1,work(n + 1),1)
                 if (abstol <= two*n*eps) then
                    tryrac = .true.
                 else
                    tryrac = .false.
                 end if
                 call la_xstemr(jobz,'A',n,work(n + 1),work,vl,vu,il,iu,m,w,z,ldz, &
                            n,isuppz,tryrac,work(2*n + 1),lwork - 2*n,iwork,liwork,info)
              end if
              if (info == 0) then
                 m = n
           ...
           if (wantz) then
            ...
            ! causes problems with tests 19
            ! if (wantz .and. indeig ) z( 1,1) = z(1,1) / 1.002_xdp + .002
           work(1) = lwmin
           iwork(1) = liwmin
           return
     end subroutine la_xstevr
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
     !> YSTEIN: computes the eigenvectors of a real symmetric tridiagonal
     !> matrix T corresponding to specified eigenvalues, using inverse
     !> iteration.
     !> The maximum number of iterations allowed for each eigenvector is
     !> specified by an internal parameter MAXITS (currently set to 5).
     !> Although the eigenvectors are real, they are stored in a complex
     !> array, which may be passed to YUNMTR or YUPMTR for back
     !> transformation to the eigenvectors of a complex Hermitian matrix
     !> which was reduced to tridiagonal form.

     pure subroutine la_ystein(n,d,e,m,w,iblock,isplit,z,ldz,work,iwork,ifail, &
               info)
        use la_constants_xdp,only:zero,one,ten,czero,cone
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ! Scalar Arguments
              ...
              loop_170: do j = j1,m
                 ...
                 if (gpind /= j) then
                    do i = gpind,j - 1
                       ytr = zero
                       do jr = 1,blksiz
                          ytr = ytr + work(indrv1 + jr)*real(z(b1 - 1 + jr,i),KIND=xdp)
                       end do
                       do jr = 1,blksiz
                          work(indrv1 + jr) = work(indrv1 + jr) - ytr*real(z(b1 - 1 + jr,i), &
                                    KIND=xdp)
                       end do
                    end do
                 end if
                 ...
                 do i = 1,blksiz
                    z(b1 + i - 1,j) = cmplx(work(indrv1 + i),zero,KIND=xdp)
                 end do
                 ! save the shift to check eigenvalue spacing at next
                 ! iteration.
                 xjm = xj
...
#ifdef LA_WITH_XDP
     !> YPTEQR: computes all eigenvalues and, optionally, eigenvectors of a
     !> symmetric positive definite tridiagonal matrix by first factoring the
     !> matrix using XPTTRF and then calling YBDSQR to compute the singular
     !> values of the bidiagonal factor.
     !> This routine computes the eigenvalues of the positive definite
     !> tridiagonal matrix to high relative accuracy.  This means that if the
     !> eigenvalues range over many orders of magnitude in size, then the
     ...
     !> more accurately than, for example, with the standard QR method.
     !> The eigenvectors of a full or band positive definite Hermitian matrix
     !> can also be found if YHETRD, YHPTRD, or YHBTRD has been used to
     !> reduce this matrix to tridiagonal form.  (The reduction to
     !> tridiagonal form, however, may preclude the possibility of obtaining
     !> high relative accuracy in the small eigenvalues of the original
     !> matrix, if these eigenvalues range over many orders of magnitude.)

     pure subroutine la_ypteqr(compz,n,d,e,z,ldz,work,info)
        use la_constants_xdp
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
        ...
        ! ====================================================================
           ...
           ! call la_ybdsqr to compute the singular values/vectors of the
           ! bidiagonal factor.
           if (icompz > 0) then
              nru = n
           else
           ...
           call la_ybdsqr('LOWER',n,0,nru,0,d,e,vt,1,z,ldz,c,1,work,info)
...
#ifdef LA_WITH_XDP
     !> YSTEMR: computes selected eigenvalues and, optionally, eigenvectors
     !> of a real symmetric tridiagonal matrix T. Any such unreduced matrix has
     !> a well defined set of pairwise different real eigenvalues, the corresponding
     !> real eigenvectors are pairwise orthogonal.
     !> The spectrum may be computed either completely or partially by specifying
     ...
     !> UC Berkeley, May 1997.
     !> Further Details
     !> 1.YSTEMR works only on machines which follow IEEE-754
     !> floating-point standard in their handling of infinities and NaNs.
     !> This permits the use of efficient inner loops avoiding a check for
     !> zero divisors.
     !> 2. LAPACK routines can be used to reduce a complex Hermitean matrix to
     ...
     !> in general.
     !> Since LAPACK drivers overwrite the matrix data with the eigenvectors,
     !> YSTEMR accepts complex workspace to facilitate interoperability
     !> with YUNMTR or YUPMTR.

     pure subroutine la_ystemr(jobz,range,n,d,e,vl,vu,il,iu,m,w,z,ldz,nzc, &
               isuppz,tryrac,work,lwork,iwork,liwork,info)
        use la_constants_xdp,only:zero,one,four
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ...
        ! =====================================================================
           ...
           ! la_xstemr needs work of size 6*n, iwork of size 3*n.
           ! in addition, la_xlarre needs work of size 6*n, iwork of size 5*n.
           ! furthermore, la_ylarrv needs work of size 12*n, iwork of size 7*n.
           if (wantz) then
              lwmin = 18*n
              liwmin = 10*n
           else
           ...
           if (n == 2) then
              ...
              if (.not. wantz) then
                 ...
                 ! la_xlarre computes the eigenvalues to less than full precision.
                 ! la_ylarrv will refine the eigenvalue approximations, and we only
                 ! need less accurate initial bisection in la_xlarre.
                 ! note: these settings do only affect the subset case and la_xlarre
                 rtol1 = sqrt(eps)
                 rtol2 = max(sqrt(eps)*5.0e-3_xdp,four*eps)
              ...
              if (wantz) then
                 ! compute the desired eigenvectors corresponding to the computed
                 ! eigenvalues
                 call la_ylarrv(n,wl,wu,d,e,pivmin,iwork(iinspl),m,1,m,minrgp, &
                 rtol1,rtol2,w,work(inderr),work(indgp),iwork(iindbl),iwork(iindw), &
                           work(indgrs),z,ldz,isuppz,work(indwrk),iwork(iindwk),iinfo)
                 if (iinfo /= 0) then
                    info = 20 + abs(iinfo)
                 ...
                 ! la_xlarre computes eigenvalues of the (shifted) root representation
                 ! la_ylarrv returns the eigenvalues of the unshifted matrix.
                 ! however, if the eigenvectors are not desired by the user, we need
                 ! to apply the corresponding shifts from la_xlarre to obtain the
                 ! eigenvalues of the original matrix.
                 do j = 1,m
...
#ifdef LA_WITH_XDP
     !> YSTEDC: computes all eigenvalues and, optionally, eigenvectors of a
     !> symmetric tridiagonal matrix using the divide and conquer method.
     !> The eigenvectors of a full or band complex Hermitian matrix can also
     ...
     pure subroutine la_ystedc(compz,n,d,e,z,ldz,work,lwork,rwork,lrwork,iwork, &
               liwork,info)
        use la_constants_xdp,only:zero,one,two
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ...
        ! =====================================================================
           ...
           if (info == 0) then
              ! compute the workspace requirements
              smlsiz = la_ilaenv(9,'YSTEDC',' ',0,0,0,0)
              if (n <= 1 .or. icompz == 0) then
                 lwmin = 1
                 liwmin = 1
                 lrwmin = 1
           ...
           ! solve the problem with another solver.
           if (n <= smlsiz) then
              call la_ysteqr(compz,n,d,e,z,ldz,rwork,info)
           else
              ! if compz = 'i', we simply call la_xstedc instead.
              if (icompz == 2) then
                 call la_xlaset('FULL',n,n,zero,one,rwork,n)
                 ll = n*n + 1
                 call la_xstedc('I',n,d,e,rwork,n,rwork(ll),lrwork - ll + 1,iwork, &
                           liwork,info)
                 do j = 1,n
                    do i = 1,n
                       z(i,j) = rwork((j - 1)*n + i)
              ...
              if (start <= n) then
                 ...
                 if (m > smlsiz) then
                    ...
                    call la_ylaed0(n,m,d(start),e(start),z(1,start),ldz,work,n, &
                              rwork,iwork,info)
                    if (info > 0) then
                       info = (info/(m + 1) + start - 1)*(n + 1) + mod(info, (m + 1)) + start - &
                                 1
                    ...
                    call la_xsteqr('I',m,d(start),e(start),rwork,m,rwork(m*m + 1), &
                              info)
                    call la_ylacrm(n,m,z(1,start),ldz,rwork,m,work,n,rwork(m*m + 1) &
                               )
                    call la_ylacpy('A',n,m,work,n,z(1,start),ldz)
                    if (info > 0) then
                       info = start*(n + 1) + finish
                       go to 70
                    end if
...
#ifdef LA_WITH_XDP
     !> YSTEGR: computes selected eigenvalues and, optionally, eigenvectors
     !> of a real symmetric tridiagonal matrix T. Any such unreduced matrix has
     !> a well defined set of pairwise different real eigenvalues, the corresponding
     !> real eigenvectors are pairwise orthogonal.
     !> The spectrum may be computed either completely or partially by specifying
     !> either an interval (VL,VU] or a range of indices IL:IU for the desired
     !> eigenvalues.
     !> YSTEGR is a compatibility wrapper around the improved YSTEMR routine.
     !> See YSTEMR for further details.
     !> One important change is that the ABSTOL parameter no longer provides any
     !> benefit and hence is no longer used.
     !> Note : YSTEGR and YSTEMR work only on machines which follow
     !> IEEE-754 floating-point standard in their handling of infinities and
     !> NaNs.  Normal execution may create these exceptiona values and hence
     !> may abort due to a floating point exception in environments which
     !> do not conform to the IEEE-754 standard.

     pure subroutine la_ystegr(jobz,range,n,d,e,vl,vu,il,iu,abstol,m,w,z,ldz, &
               isuppz,work,lwork,iwork,liwork,info)
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
        ...
        ! =====================================================================
           ...
           info = 0
           tryrac = .false.
           call la_ystemr(jobz,range,n,d,e,vl,vu,il,iu,m,w,z,ldz,n,isuppz, &
                     tryrac,work,lwork,iwork,liwork,info)
     end subroutine la_ystegr
#endif
#ifdef LA_WITH_QP
