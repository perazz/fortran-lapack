...
module la_cholesky
...
#ifdef LA_WITH_XDP
        module procedure la_x_cholesky_inplace
        module procedure la_x_cholesky
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
        module procedure la_y_cholesky_inplace
        module procedure la_y_cholesky
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
        module procedure la_x_cholesky_fun
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
        module procedure la_y_cholesky_fun
#endif
#ifdef LA_WITH_QP
     ...
     ! Compute the Cholesky factorization of a symmetric / Hermitian matrix, A = L*L^T = U^T*U.
     ! The factorization is returned in-place, overwriting matrix A
     pure subroutine la_x_cholesky_inplace(a,lower,other_zeroed,err)
         !> Input matrix a[m,n]
     ...
     end subroutine la_x_cholesky_inplace

     ! Compute the Cholesky factorization of a symmetric / Hermitian matrix, A = L*L^T = U^T*U.
     ! The factorization is returned as a separate matrix
     pure subroutine la_x_cholesky(a,c,lower,other_zeroed,err)
         !> Input matrix a[n,n]
         ...
         if (lda < 1 .or. n < 1 .or. lda < n .or. ldc < n .or. nc < n) then
            ...
            ! Get cholesky factors
            call la_x_cholesky_inplace(c,lower,other_zeroed,err0)
     ...
     end subroutine la_x_cholesky

     ! Compute the Cholesky factorization of a symmetric / Hermitian matrix, A = L*L^T = U^T*U.
     ! Function interface
     pure function la_x_cholesky_fun(a,lower,other_zeroed) result(c)
         !> Input matrix a[n,n]
         real(xdp),intent(in) :: a(:,:)
         !> Output matrix with Cholesky factors c[n,n]
         real(xdp) :: c(size(a,1),size(a,2))
         !> [optional] is the lower or upper triangular factor required? Default = lower
         logical(lk),optional,intent(in) :: lower
         !> [optional] should the unused half of the return matrix be zeroed out? Default: yes
         logical(lk),optional,intent(in) :: other_zeroed
         ...
         call la_x_cholesky_inplace(c,lower,other_zeroed)

     end function la_x_cholesky_fun
#endif
     ...
     ! Compute the Cholesky factorization of a symmetric / Hermitian matrix, A = L*L^T = U^T*U.
     ! The factorization is returned in-place, overwriting matrix A
     pure subroutine la_y_cholesky_inplace(a,lower,other_zeroed,err)
         !> Input matrix a[m,n]
         ...
         logical(lk) :: lower_,other_zeroed_
         character :: triangle
         complex(xdp),parameter :: zero = 0.0_xdp
     ...
     end subroutine la_y_cholesky_inplace

     ! Compute the Cholesky factorization of a symmetric / Hermitian matrix, A = L*L^T = U^T*U.
     ! The factorization is returned as a separate matrix
     pure subroutine la_y_cholesky(a,c,lower,other_zeroed,err)
         !> Input matrix a[n,n]
         complex(xdp),intent(in) :: a(:,:)
         !> Output matrix with Cholesky factors c[n,n]
         complex(xdp),intent(out) :: c(:,:)
         !> [optional] is the lower or upper triangular factor required? Default = lower
         logical(lk),optional,intent(in) :: lower
         !> [optional] should the unused half of the return matrix be zeroed out? Default: yes
         logical(lk),optional,intent(in) :: other_zeroed
         ...
         if (lda < 1 .or. n < 1 .or. lda < n .or. ldc < n .or. nc < n) then
            ...
            ! Get cholesky factors
            call la_y_cholesky_inplace(c,lower,other_zeroed,err0)
     ...
     end subroutine la_y_cholesky

     ! Compute the Cholesky factorization of a symmetric / Hermitian matrix, A = L*L^T = U^T*U.
     ! Function interface
     pure function la_y_cholesky_fun(a,lower,other_zeroed) result(c)
         !> Input matrix a[n,n]
         complex(xdp),intent(in) :: a(:,:)
         !> Output matrix with Cholesky factors c[n,n]
         complex(xdp) :: c(size(a,1),size(a,2))
         !> [optional] is the lower or upper triangular factor required? Default = lower
         logical(lk),optional,intent(in) :: lower
         !> [optional] should the unused half of the return matrix be zeroed out? Default: yes
         logical(lk),optional,intent(in) :: other_zeroed
         ...
         call la_y_cholesky_inplace(c,lower,other_zeroed)

     end function la_y_cholesky_fun
#endif
