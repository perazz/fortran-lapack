...
module la_lapack_eigv_gen
...
#ifdef LA_WITH_XDP
     !> XGGES: computes for a pair of N-by-N real nonsymmetric matrices (A,B),
     !> the generalized eigenvalues, the generalized real Schur form (S,T),
     !> optionally, the left and/or right matrices of Schur vectors (VSL and
     !> VSR). This gives the generalized Schur factorization
     !> (A,B) = ( (VSL)*S*(VSR)**T, (VSL)*T*(VSR)**T )
     ...
     subroutine la_xgges(jobvsl,jobvsr,sort,selctg,n,a,lda,b,ldb,sdim,alphar, &
               alphai,beta,vsl,ldvsl,vsr,ldvsr,work,lwork,bwork,info)
        ...
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ...
           real(xdp),intent(out) :: alphai(*),alphar(*),beta(*),vsl(ldvsl,*),vsr(ldvsr,*), &
                     work(*)
           ! Function Arguments
           procedure(la_selctg_x) :: selctg
        ! =====================================================================
           ...
           integer(ilp) :: i,icols,ierr,ihi,ijobvl,ijobvr,ileft,ilo,ip,iright,irows, &
                     itau,iwrk,maxwrk,minwrk
           real(xdp) :: anrm,anrmto,bignum,bnrm,bnrmto,eps,pvsl,pvsr,safmax,safmin, &
                     smlnum
           ! Local Arrays
           integer(ilp) :: idum(1)
           real(xdp) :: dif(2)
           ...
           if (info == 0) then
              if (n > 0) then
                 minwrk = max(8*n,6*n + 16)
                 maxwrk = minwrk - n + n*la_ilaenv(1,'XGEQRF',' ',n,1,n,0)
                 maxwrk = max(maxwrk,minwrk - n + n*la_ilaenv(1,'XORMQR',' ',n,1,n,-1 &
                           ))
                 if (ilvsl) then
                    maxwrk = max(maxwrk,minwrk - n + n*la_ilaenv(1,'XORGQR',' ',n,1,n, &
                              -1))
                 end if
              else
                 minwrk = 1
           ...
           safmin = la_xlamch('S')
           safmax = one/safmin
           call la_xlabad(safmin,safmax)
           smlnum = sqrt(safmin)/eps
           bignum = one/smlnum
           ! scale a if max element outside range [smlnum,bignum]
           anrm = la_xlange('M',n,n,a,lda,work)
           ilascl = .false.
           if (anrm > zero .and. anrm < smlnum) then
              anrmto = smlnum
              ilascl = .true.
           ...
           if (ilascl) call la_xlascl('G',0,0,anrm,anrmto,n,n,a,lda,ierr)
           ! scale b if max element outside range [smlnum,bignum]
           bnrm = la_xlange('M',n,n,b,ldb,work)
           ilbscl = .false.
           if (bnrm > zero .and. bnrm < smlnum) then
              bnrmto = smlnum
              ilbscl = .true.
           ...
           if (ilbscl) call la_xlascl('G',0,0,bnrm,bnrmto,n,n,b,ldb,ierr)
           ! permute the matrix to make it more nearly triangular
           ! (workspace: need 6*n + 2*n space for storing balancing factors)
           ileft = 1
           iright = n + 1
           iwrk = iright + n
           call la_xggbal('P',n,a,lda,b,ldb,ilo,ihi,work(ileft),work(iright), &
                     work(iwrk),ierr)
           ! reduce b to triangular form (qr decomposition of b)
           ! (workspace: need n, prefer n*nb)
           irows = ihi + 1 - ilo
           ...
           itau = iwrk
           iwrk = itau + irows
           call la_xgeqrf(irows,icols,b(ilo,ilo),ldb,work(itau),work(iwrk),lwork + &
                     1 - iwrk,ierr)
           ! apply the orthogonal transformation to matrix a
           ! (workspace: need n, prefer n*nb)
           call la_xormqr('L','T',irows,icols,irows,b(ilo,ilo),ldb,work(itau),a( &
                     ilo,ilo),lda,work(iwrk),lwork + 1 - iwrk,ierr)
           ! initialize vsl
           ! (workspace: need n, prefer n*nb)
           if (ilvsl) then
              call la_xlaset('FULL',n,n,zero,one,vsl,ldvsl)
              if (irows > 1) then
                 call la_xlacpy('L',irows - 1,irows - 1,b(ilo + 1,ilo),ldb,vsl(ilo + 1,ilo) &
                           ,ldvsl)
              end if
              call la_xorgqr(irows,irows,irows,vsl(ilo,ilo),ldvsl,work(itau),work( &
                        iwrk),lwork + 1 - iwrk,ierr)
           end if
           ! initialize vsr
           if (ilvsr) call la_xlaset('FULL',n,n,zero,one,vsr,ldvsr)
           ! reduce to generalized hessenberg form
           ! (workspace: none needed)
           call la_xgghrd(jobvsl,jobvsr,n,ilo,ihi,a,lda,b,ldb,vsl,ldvsl,vsr,ldvsr, &
                      ierr)
           ! perform qz algorithm, computing schur vectors if desired
           ! (workspace: need n)
           iwrk = itau
           call la_xhgeqz('S',jobvsl,jobvsr,n,ilo,ihi,a,lda,b,ldb,alphar,alphai, &
                     beta,vsl,ldvsl,vsr,ldvsr,work(iwrk),lwork + 1 - iwrk,ierr)
           if (ierr /= 0) then
              if (ierr > 0 .and. ierr <= n) then
                 info = ierr
           ...
           if (wantst) then
              ! undo scaling on eigenvalues before selctging
              if (ilascl) then
                 call la_xlascl('G',0,0,anrmto,anrm,n,1,alphar,n,ierr)
                 call la_xlascl('G',0,0,anrmto,anrm,n,1,alphai,n,ierr)
              end if
              if (ilbscl) call la_xlascl('G',0,0,bnrmto,bnrm,n,1,beta,n,ierr)

              ! select eigenvalues
              do i = 1,n
                 bwork(i) = selctg(alphar(i),alphai(i),beta(i))
              end do
              call la_xtgsen(0,ilvsl,ilvsr,bwork,n,a,lda,b,ldb,alphar,alphai,beta, &
              vsl,ldvsl,vsr,ldvsr,sdim,pvsl,pvsr,dif,work(iwrk),lwork - iwrk + 1,idum,1, &
                        ierr)
              if (ierr == 1) info = n + 3
           end if
           ! apply back-permutation to vsl and vsr
           ! (workspace: none needed)
           if (ilvsl) call la_xggbak('P','L',n,ilo,ihi,work(ileft),work(iright),n, &
                     vsl,ldvsl,ierr)
           if (ilvsr) call la_xggbak('P','R',n,ilo,ihi,work(ileft),work(iright),n, &
                     vsr,ldvsr,ierr)
           ! check if unscaling would cause over/underflow, if so, rescale
           ! (alphar(i),alphai(i),beta(i)) so beta(i) is on the order of
           ! b(i,i) and alphar(i) and alphai(i) are on the order of a(i,i)
           ...
           ! undo scaling
           if (ilascl) then
              call la_xlascl('H',0,0,anrmto,anrm,n,n,a,lda,ierr)
              call la_xlascl('G',0,0,anrmto,anrm,n,1,alphar,n,ierr)
              call la_xlascl('G',0,0,anrmto,anrm,n,1,alphai,n,ierr)
           end if
           if (ilbscl) then
              call la_xlascl('U',0,0,bnrmto,bnrm,n,n,b,ldb,ierr)
              call la_xlascl('G',0,0,bnrmto,bnrm,n,1,beta,n,ierr)
           end if
           if (wantst) then
              ! check if reordering is correct
              lastsl = .true.
...
#ifdef LA_WITH_XDP
     !> XGGESX: computes for a pair of N-by-N real nonsymmetric matrices
     !> (A,B), the generalized eigenvalues, the real Schur form (S,T), and,
     !> optionally, the left and/or right matrices of Schur vectors (VSL and
     !> VSR).  This gives the generalized Schur factorization
     !> (A,B) = ( (VSL) S (VSR)**T, (VSL) T (VSR)**T )
     ...
     subroutine la_xggesx(jobvsl,jobvsr,sort,selctg,sense,n,a,lda,b,ldb,sdim, &
     alphar,alphai,beta,vsl,ldvsl,vsr,ldvsr,rconde,rcondv,work,lwork,iwork,liwork, &
               bwork,info)
        use la_constants_xdp,only:zero,one
        ! -- lapack driver routine --
        ...
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ...
           integer(ilp),intent(out) :: iwork(*)
           real(xdp),intent(inout) :: a(lda,*),b(ldb,*)
           real(xdp),intent(out) :: alphai(*),alphar(*),beta(*),rconde(2),rcondv(2),vsl( &
                     ldvsl,*),vsr(ldvsr,*),work(*)
           ! Function Arguments
           procedure(la_selctg_x) :: selctg
        ! =====================================================================
           ...
           integer(ilp) :: i,icols,ierr,ihi,ijob,ijobvl,ijobvr,ileft,ilo,ip,iright, &
                     irows,itau,iwrk,liwmin,lwrk,maxwrk,minwrk
           real(xdp) :: anrm,anrmto,bignum,bnrm,bnrmto,eps,pl,pr,safmax,safmin, &
                     smlnum
           ! Local Arrays
           real(xdp) :: dif(2)
           ! Intrinsic Functions
           ...
           if (wantst) then
              ...
              ! reorder eigenvalues, transform generalized schur vectors, and
              ! compute reciprocal condition numbers
              call la_xtgsen(ijob,ilvsl,ilvsr,bwork,n,a,lda,b,ldb,alphar,alphai, &
              beta,vsl,ldvsl,vsr,ldvsr,sdim,pl,pr,dif,work(iwrk),lwork - iwrk + 1,iwork, &
                        liwork,ierr)
              if (ijob >= 1) maxwrk = max(maxwrk,2*sdim*(n - sdim))
              if (ierr == -22) then
...
#ifdef LA_WITH_XDP
     !> XGGEV: computes for a pair of N-by-N real nonsymmetric matrices (A,B)
     !> the generalized eigenvalues, and optionally, the left and/or right
     !> generalized eigenvectors.
     !> A generalized eigenvalue for a pair of matrices (A,B) is a scalar
     !> lambda or a ratio alpha/beta = lambda, such that A - lambda*B is
     ...
     subroutine la_xggev(jobvl,jobvr,n,a,lda,b,ldb,alphar,alphai,beta,vl,ldvl,vr, &
               ldvr,work,lwork,info)
        use la_constants_xdp,only:zero,one
        ! -- lapack driver routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ...
           ! Array Arguments
           real(xdp),intent(inout) :: a(lda,*),b(ldb,*)
           real(xdp),intent(out) :: alphai(*),alphar(*),beta(*),vl(ldvl,*),vr(ldvr,*),work(*)
        ...
        ! =====================================================================
           ...
           integer(ilp) :: icols,ierr,ihi,ijobvl,ijobvr,ileft,ilo,in,iright,irows,itau, &
                     iwrk,jc,jr,maxwrk,minwrk
           real(xdp) :: anrm,anrmto,bignum,bnrm,bnrmto,eps,smlnum,temp
           ! Local Arrays
           logical(lk) :: ldumma(1)
           ! Intrinsic Functions
           intrinsic :: abs,max,sqrt
           ...
           if (info == 0) then
              minwrk = max(1,8*n)
              maxwrk = max(1,n*(7 + la_ilaenv(1,'XGEQRF',' ',n,1,n,0)))
              maxwrk = max(maxwrk,n*(7 + la_ilaenv(1,'XORMQR',' ',n,1,n,0)))

              if (ilvl) then
                 maxwrk = max(maxwrk,n*(7 + la_ilaenv(1,'XORGQR',' ',n,1,n,-1)))
           ...
           ! (workspace: need n, prefer n*nb)
           if (ilvl) then
              call la_xlaset('FULL',n,n,zero,one,vl,ldvl)
              if (irows > 1) then
                 call la_xlacpy('L',irows - 1,irows - 1,b(ilo + 1,ilo),ldb,vl(ilo + 1,ilo), &
                            ldvl)
              end if
              call la_xorgqr(irows,irows,irows,vl(ilo,ilo),ldvl,work(itau),work( &
                        iwrk),lwork + 1 - iwrk,ierr)
           end if
           ! initialize vr
           if (ilvr) call la_xlaset('FULL',n,n,zero,one,vr,ldvr)
           ! reduce to generalized hessenberg form
           ! (workspace: none needed)
           if (ilv) then
              ! eigenvectors requested -- work on whole matrix.
              call la_xgghrd(jobvl,jobvr,n,ilo,ihi,a,lda,b,ldb,vl,ldvl,vr,ldvr, &
                        ierr)
           else
              call la_xgghrd('N','N',irows,1,irows,a(ilo,ilo),lda,b(ilo,ilo), &
                        ldb,vl,ldvl,vr,ldvr,ierr)
           end if
           ! perform qz algorithm (compute eigenvalues, and optionally, the
           ! schur forms and schur vectors)
           ...
           call la_xhgeqz(chtemp,jobvl,jobvr,n,ilo,ihi,a,lda,b,ldb,alphar,alphai, &
                     beta,vl,ldvl,vr,ldvr,work(iwrk),lwork + 1 - iwrk,ierr)
           if (ierr /= 0) then
              if (ierr > 0 .and. ierr <= n) then
                 info = ierr
           ...
           if (ilv) then
              ...
              call la_xtgevc(chtemp,'B',ldumma,n,a,lda,b,ldb,vl,ldvl,vr,ldvr,n, &
                        in,work(iwrk),ierr)
              if (ierr /= 0) then
                 info = n + 2
                 go to 110
              ...
              ! (workspace: none needed)
              if (ilvl) then
                 call la_xggbak('P','L',n,ilo,ihi,work(ileft),work(iright),n,vl, &
                           ldvl,ierr)
                 loop_50: do jc = 1,n
                    if (alphai(jc) < zero) cycle loop_50
                    temp = zero
              ...
              if (ilvr) then
                 call la_xggbak('P','R',n,ilo,ihi,work(ileft),work(iright),n,vr, &
                           ldvr,ierr)
                 loop_100: do jc = 1,n
                    if (alphai(jc) < zero) cycle loop_100
                    temp = zero
...
#ifdef LA_WITH_XDP
     !> XGGEVX: computes for a pair of N-by-N real nonsymmetric matrices (A,B)
     !> the generalized eigenvalues, and optionally, the left and/or right
     !> generalized eigenvectors.
     !> Optionally also, it computes a balancing transformation to improve
     !> the conditioning of the eigenvalues and eigenvectors (ILO, IHI,
     ...
     subroutine la_xggevx(balanc,jobvl,jobvr,sense,n,a,lda,b,ldb,alphar,alphai, &
     beta,vl,ldvl,vr,ldvr,ilo,ihi,lscale,rscale,abnrm,bbnrm,rconde,rcondv,work,lwork, &
               iwork,bwork,info)
        use la_constants_xdp,only:zero,one
        ! -- lapack driver routine --
        ...
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ...
           integer(ilp),intent(out) :: ihi,ilo,info
           integer(ilp),intent(in) :: lda,ldb,ldvl,ldvr,lwork,n
           real(xdp),intent(out) :: abnrm,bbnrm
           ! Array Arguments
           logical(lk),intent(out) :: bwork(*)
           integer(ilp),intent(out) :: iwork(*)
           real(xdp),intent(inout) :: a(lda,*),b(ldb,*)
           real(xdp),intent(out) :: alphai(*),alphar(*),beta(*),lscale(*),rconde(*),rcondv(*) &
                     ,rscale(*),vl(ldvl,*),vr(ldvr,*),work(*)
        ! =====================================================================
           ...
           if (info == 0) then
              if (n == 0) then
                 ...
                 maxwrk = minwrk
                 maxwrk = max(maxwrk,n + n*la_ilaenv(1,'XGEQRF',' ',n,1,n,0))

                 maxwrk = max(maxwrk,n + n*la_ilaenv(1,'XORMQR',' ',n,1,n,0))

                 if (ilvl) then
                    maxwrk = max(maxwrk,n + n*la_ilaenv(1,'XORGQR',' ',n,1,n,0))
           ...
           ! permute and/or balance the matrix pair (a,b)
           ! (workspace: need 6*n if balanc = 's' or 'b', 1 otherwise)
           call la_xggbal(balanc,n,a,lda,b,ldb,ilo,ihi,lscale,rscale,work,ierr)

           ! compute abnrm and bbnrm
           abnrm = la_xlange('1',n,n,a,lda,work(1))
           if (ilascl) then
              work(1) = abnrm
              call la_xlascl('G',0,0,anrmto,anrm,1,1,work(1),1,ierr)
              abnrm = work(1)
           end if
           bbnrm = la_xlange('1',n,n,b,ldb,work(1))
           if (ilbscl) then
              work(1) = bbnrm
              call la_xlascl('G',0,0,bnrmto,bnrm,1,1,work(1),1,ierr)
              bbnrm = work(1)
           end if
           ! reduce b to triangular form (qr decomposition of b)
           ! (workspace: need n, prefer n*nb )
           ...
           ! compute eigenvectors and estimate condition numbers if desired
           ! (workspace: la_xtgevc: need 6*n
                       ! la_xtgsna: need 2*n*(n+2)+16 if sense = 'v' or 'b',
                               ! need n otherwise )
           if (ilv .or. .not. wantsn) then
              if (ilv) then
                 if (ilvl) then
              ...
              if (.not. wantsn) then
                 ! compute eigenvectors (la_xtgevc) and estimate condition
                 ! numbers (la_xtgsna). note that the definition of the condition
                 ! number is not invariant under transformation (u,v) to
                 ! (q*u, z*v), where (u,v) are eigenvectors of the generalized
                 ! schur form (s,t), q and z are orthogonal matrices. in order
                 ! to avoid using extra 2*n*n workspace, we have to recalculate
                 ...
                 loop_20: do i = 1,n
                    ...
                    ! (compute workspace: need up to 4*n + 6*n)
                    if (wantse .or. wantsb) then
                       call la_xtgevc('B','S',bwork,n,a,lda,b,ldb,work(1),n,work( &
                                 iwrk),n,mm,m,work(iwrk1),ierr)
                       if (ierr /= 0) then
                          info = n + 2
                          go to 130
                    ...
                    call la_xtgsna(sense,'S',bwork,n,a,lda,b,ldb,work(1),n,work( &
                    iwrk),n,rconde(i),rcondv(i),mm,m,work(iwrk1),lwork - iwrk1 + 1,iwork, &
                               ierr)
                 end do loop_20
              end if
           ...
           ! (workspace: none needed)
           if (ilvl) then
              call la_xggbak(balanc,'L',n,ilo,ihi,lscale,rscale,n,vl,ldvl,ierr)
           ...
           if (ilvr) then
              call la_xggbak(balanc,'R',n,ilo,ihi,lscale,rscale,n,vr,ldvr,ierr)
...
#ifdef LA_WITH_XDP
     !> XGEES: computes for an N-by-N real nonsymmetric matrix A, the
     !> eigenvalues, the real Schur form T, and, optionally, the matrix of
     !> Schur vectors Z.  This gives the Schur factorization A = Z*T*(Z**T).
     !> Optionally, it also orders the eigenvalues on the diagonal of the
     !> real Schur form so that selected eigenvalues are at the top left.
     ...
     subroutine la_xgees(jobvs,sort,select,n,a,lda,sdim,wr,wi,vs,ldvs,work,lwork, &
               bwork,info)
        use la_constants_xdp,only:zero,one
        ! -- lapack driver routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ...
           real(xdp),intent(out) :: vs(ldvs,*),wi(*),work(*),wr(*)
           ! Function Arguments
           procedure(la_select_x) :: select
        ! =====================================================================
           ...
           integer(ilp) :: hswork,i,i1,i2,ibal,icond,ierr,ieval,ihi,ilo,inxt,ip,itau, &
                     iwrk,maxwrk,minwrk
           real(xdp) :: anrm,bignum,cscale,eps,s,sep,smlnum
           ! Local Arrays
           integer(ilp) :: idum(1)
           real(xdp) :: dum(1)
           ! Intrinsic Functions
           ...
           ! compute workspace
            ! (note: comments in the code beginning "workspace:" describe the
             ...
             ! nb refers to the optimal block size for the immediately
             ! following subroutine, as returned by la_ilaenv.
             ! hswork refers to the workspace preferred by la_xhseqr, as
             ! calculated below. hswork is computed assuming ilo=1 and ihi=n,
             ! the worst case.)
           if (info == 0) then
              if (n == 0) then
                 ...
                 maxwrk = 2*n + n*la_ilaenv(1,'XGEHRD',' ',n,1,n,0)
                 minwrk = 3*n
                 call la_xhseqr('S',jobvs,n,1,n,a,lda,wr,wi,vs,ldvs,work,-1, &
                           ieval)
                 hswork = work(1)
                 if (.not. wantvs) then
                    maxwrk = max(maxwrk,n + hswork)
                 else
                    maxwrk = max(maxwrk,2*n + (n - 1)*la_ilaenv(1,'XORGHR',' ',n,1,n, &
                               -1))
                    maxwrk = max(maxwrk,n + hswork)
                 end if
              end if
           ...
           bignum = one/smlnum
           ! scale a if max element outside range [smlnum,bignum]
           anrm = la_xlange('M',n,n,a,lda,dum)
           scalea = .false.
           if (anrm > zero .and. anrm < smlnum) then
              scalea = .true.
              cscale = smlnum
           ...
           if (scalea) call la_xlascl('G',0,0,anrm,cscale,n,n,a,lda,ierr)
           ! permute the matrix to make it more nearly triangular
           ! (workspace: need n)
           ibal = 1
           call la_xgebal('P',n,a,lda,ilo,ihi,work(ibal),ierr)
           ! reduce to upper hessenberg form
           ! (workspace: need 3*n, prefer 2*n+n*nb)
           itau = n + ibal
           iwrk = n + itau
           call la_xgehrd(n,ilo,ihi,a,lda,work(itau),work(iwrk),lwork - iwrk + 1,ierr &
                     )
           if (wantvs) then
              ! copy householder vectors to vs
              call la_xlacpy('L',n,n,a,lda,vs,ldvs)
              ! generate orthogonal matrix in vs
              ! (workspace: need 3*n-1, prefer 2*n+(n-1)*nb)
              call la_xorghr(n,ilo,ihi,vs,ldvs,work(itau),work(iwrk),lwork - iwrk + 1, &
                        ierr)
           end if
           sdim = 0
           ! perform qr iteration, accumulating schur vectors in vs if desired
           ! (workspace: need n+1, prefer n+hswork (see comments) )
           iwrk = itau
           call la_xhseqr('S',jobvs,n,ilo,ihi,a,lda,wr,wi,vs,ldvs,work(iwrk), &
                     lwork - iwrk + 1,ieval)
           if (ieval > 0) info = ieval
           ! sort eigenvalues if desired
           if (wantst .and. info == 0) then
              if (scalea) then
                 call la_xlascl('G',0,0,cscale,anrm,n,1,wr,n,ierr)
                 call la_xlascl('G',0,0,cscale,anrm,n,1,wi,n,ierr)
              end if
              do i = 1,n
                 bwork(i) = select(wr(i),wi(i))
              end do
              ! reorder eigenvalues and transform schur vectors
              ! (workspace: none needed)
              call la_xtrsen('N',jobvs,bwork,n,a,lda,vs,ldvs,wr,wi,sdim,s,sep, &
                        work(iwrk),lwork - iwrk + 1,idum,1,icond)
              if (icond > 0) info = n + icond
           end if
           if (wantvs) then
              ! undo balancing
              ! (workspace: need n)
              call la_xgebak('P','R',n,ilo,ihi,work(ibal),n,vs,ldvs,ierr)
           end if
           if (scalea) then
              ! undo scaling for the schur form of a
              call la_xlascl('H',0,0,cscale,anrm,n,n,a,lda,ierr)
              call la_xcopy(n,a,lda + 1,wr,1)
              if (cscale == smlnum) then
                 ! if scaling back towards underflow, adjust wi if an
                 ! offdiagonal element of a 2-by-2 block in the schur form
                 ! underflows.
                 if (ieval > 0) then
                    i1 = ieval + 1
                    i2 = ihi - 1
                    call la_xlascl('G',0,0,cscale,anrm,ilo - 1,1,wi,max(ilo - 1,1), &
                              ierr)
                 else if (wantst) then
                    i1 = 1
                    i2 = n - 1
                 ...
                 loop_20: do i = i1,i2
                    ...
                    if (wi(i) == zero) then
                       ...
                       if (a(i + 1,i) == zero) then
                          ...
                          wi(i) = zero
                          wi(i + 1) = zero
                          if (i > 1) call la_xswap(i - 1,a(1,i),1,a(1,i + 1),1)
                          if (n > i + 1) call la_xswap(n - i - 1,a(i,i + 2),lda,a(i + 1,i + 2), &
                                    lda)
                          if (wantvs) then
                             call la_xswap(n,vs(1,i),1,vs(1,i + 1),1)
                          end if
                          a(i,i + 1) = a(i + 1,i)
                          a(i + 1,i) = zero
                       end if
              ...
              ! undo scaling for the imaginary part of the eigenvalues
              call la_xlascl('G',0,0,cscale,anrm,n - ieval,1,wi(ieval + 1),max(n - ieval, &
                         1),ierr)
           end if
           if (wantst .and. info == 0) then
              ! check if reordering successful
...
#ifdef LA_WITH_XDP
     !> XGEESX: computes for an N-by-N real nonsymmetric matrix A, the
     !> eigenvalues, the real Schur form T, and, optionally, the matrix of
     !> Schur vectors Z.  This gives the Schur factorization A = Z*T*(Z**T).
     !> Optionally, it also orders the eigenvalues on the diagonal of the
     !> real Schur form so that selected eigenvalues are at the top left;
     ...
     !> orthonormal basis for this invariant subspace.
     !> For further explanation of the reciprocal condition numbers RCONDE
     !> and RCONDV, see Section 4.10_xdp of the LAPACK Users' Guide (where
     !> these quantities are called s and sep respectively).
     !> A real matrix is in real Schur form if it is upper quasi-triangular
     !> with 1-by-1 and 2-by-2 blocks. 2-by-2 blocks will be standardized in
     !> the form
     ...
     subroutine la_xgeesx(jobvs,sort,select,sense,n,a,lda,sdim,wr,wi,vs,ldvs, &
               rconde,rcondv,work,lwork,iwork,liwork,bwork,info)
        use la_constants_xdp,only:zero,one
        ! -- lapack driver routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ...
           integer(ilp),intent(out) :: info,sdim
           integer(ilp),intent(in) :: lda,ldvs,liwork,lwork,n
           real(xdp),intent(out) :: rconde,rcondv
           ! Array Arguments
           logical(lk),intent(out) :: bwork(*)
           integer(ilp),intent(out) :: iwork(*)
           real(xdp),intent(inout) :: a(lda,*)
        ...
        ! =====================================================================
           ...
           integer(ilp) :: hswork,i,i1,i2,ibal,icond,ierr,ieval,ihi,ilo,inxt,ip,itau, &
                     iwrk,liwrk,lwrk,maxwrk,minwrk
           real(xdp) :: anrm,bignum,cscale,eps,smlnum
           ! Local Arrays
           real(xdp) :: dum(1)
           ! Intrinsic Functions
           intrinsic :: max,sqrt
           ...
           ! compute workspace
            ! (note: comments in the code beginning "rworkspace:" describe the
             ...
             ! the worst case.
             ! if sense = 'e', 'v' or 'b', then the amount of workspace needed
             ! depends on sdim, which is computed by the routine la_xtrsen later
             ! in the code.)
           if (info == 0) then
              liwrk = 1
              if (n == 0) then
           ...
           if (wantst .and. info == 0) then
              ...
              ! (iworkspace: if sense is 'v' or 'b', need sdim*(n-sdim)
                           ! otherwise, need 0 )
              call la_xtrsen(sense,jobvs,bwork,n,a,lda,vs,ldvs,wr,wi,sdim,rconde, &
                        rcondv,work(iwrk),lwork - iwrk + 1,iwork,liwork,icond)
              if (.not. wantsn) maxwrk = max(maxwrk,n + 2*sdim*(n - sdim))
              if (icond == -15) then
                 ! not enough real workspace
                 ...
                 ! la_xtrsen failed to reorder or to restore standard schur form
                 info = icond + n
              end if
           end if
           if (wantvs) then
           ...
           if (scalea) then
              ...
              if ((wantsv .or. wantsb) .and. info == 0) then
                 dum(1) = rcondv
                 call la_xlascl('G',0,0,cscale,anrm,1,1,dum,1,ierr)
                 rcondv = dum(1)
              end if
              if (cscale == smlnum) then
                 ! if scaling back towards underflow, adjust wi if an
                 ...
                 if (ieval > 0) then
                    i1 = ieval + 1
                    i2 = ihi - 1
                    call la_xlascl('G',0,0,cscale,anrm,ilo - 1,1,wi,n,ierr)
                 else if (wantst) then
                    i1 = 1
                    i2 = n - 1
                 else
...
#ifdef LA_WITH_XDP
     !> XGEEV: computes for an N-by-N real nonsymmetric matrix A, the
     !> eigenvalues and, optionally, the left and/or right eigenvectors.
     !> The right eigenvector v(j) of A satisfies
     !> A * v(j) = lambda(j) * v(j)
     !> where lambda(j) is its eigenvalue.
     ...
     subroutine la_xgeev(jobvl,jobvr,n,a,lda,wr,wi,vl,ldvl,vr,ldvr,work,lwork, &
               info)
        use la_constants_xdp,only:zero,one
        ! -- lapack driver routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ...
           ! Array Arguments
           real(xdp),intent(inout) :: a(lda,*)
           real(xdp),intent(out) :: vl(ldvl,*),vr(ldvr,*),wi(*),work(*),wr(*)
        ! =====================================================================
           ...
           integer(ilp) :: hswork,i,ibal,ierr,ihi,ilo,itau,iwrk,k,lwork_trevc,maxwrk, &
                     minwrk,nout
           real(xdp) :: anrm,bignum,cs,cscale,eps,r,scl,smlnum,sn
           ! Local Arrays
           logical(lk) :: select(1)
           real(xdp) :: dum(1)
           ! Intrinsic Functions
           ...
           if (info == 0) then
              if (n == 0) then
                 ...
                 if (wantvl) then
                    ...
                    maxwrk = max(maxwrk,2*n + (n - 1)*la_ilaenv(1,'XORGHR',' ',n,1,n, &
                               -1))
                    call la_xhseqr('S','V',n,1,n,a,lda,wr,wi,vl,ldvl,work,-1, &
                              info)
                    hswork = int(work(1),KIND=ilp)
                    maxwrk = max(maxwrk,n + 1,n + hswork)
                    call la_xtrevc3('L','B',select,n,a,lda,vl,ldvl,vr,ldvr,n,nout, &
                              work,-1,ierr)
                    lwork_trevc = int(work(1),KIND=ilp)
                    maxwrk = max(maxwrk,n + lwork_trevc)
                    maxwrk = max(maxwrk,4*n)
                    ...
                    maxwrk = max(maxwrk,2*n + (n - 1)*la_ilaenv(1,'XORGHR',' ',n,1,n, &
                               -1))
                    call la_xhseqr('S','V',n,1,n,a,lda,wr,wi,vr,ldvr,work,-1, &
                              info)
                    hswork = int(work(1),KIND=ilp)
                    maxwrk = max(maxwrk,n + 1,n + hswork)
                    call la_xtrevc3('R','B',select,n,a,lda,vl,ldvl,vr,ldvr,n,nout, &
                              work,-1,ierr)
                    lwork_trevc = int(work(1),KIND=ilp)
                    maxwrk = max(maxwrk,n + lwork_trevc)
                    maxwrk = max(maxwrk,4*n)
                    ...
                    minwrk = 3*n
                    call la_xhseqr('E','N',n,1,n,a,lda,wr,wi,vr,ldvr,work,-1, &
                              info)
                    hswork = int(work(1),KIND=ilp)
                    maxwrk = max(maxwrk,n + 1,n + hswork)
                 end if
           ...
           ! (workspace: need n)
           ibal = 1
           call la_xgebal('B',n,a,lda,ilo,ihi,work(ibal),ierr)
           ! reduce to upper hessenberg form
           ! (workspace: need 3*n, prefer 2*n+n*nb)
           itau = ibal + n
           iwrk = itau + n
           ...
           if (wantvl) then
              ...
              ! copy householder vectors to vl
              side = 'L'
              call la_xlacpy('L',n,n,a,lda,vl,ldvl)
              ! generate orthogonal matrix in vl
              ! (workspace: need 3*n-1, prefer 2*n+(n-1)*nb)
              call la_xorghr(n,ilo,ihi,vl,ldvl,work(itau),work(iwrk),lwork - iwrk + 1, &
                        ierr)
              ! perform qr iteration, accumulating schur vectors in vl
              ! (workspace: need n+1, prefer n+hswork (see comments) )
              iwrk = itau
              call la_xhseqr('S','V',n,ilo,ihi,a,lda,wr,wi,vl,ldvl,work(iwrk), &
                        lwork - iwrk + 1,info)
              if (wantvr) then
                 ! want left and right eigenvectors
                 ! copy schur vectors to vr
                 side = 'B'
                 call la_xlacpy('F',n,n,vl,ldvl,vr,ldvr)
              end if
           else if (wantvr) then
              ! want right eigenvectors
              ! copy householder vectors to vr
              side = 'R'
              call la_xlacpy('L',n,n,a,lda,vr,ldvr)
              ! generate orthogonal matrix in vr
              ! (workspace: need 3*n-1, prefer 2*n+(n-1)*nb)
              call la_xorghr(n,ilo,ihi,vr,ldvr,work(itau),work(iwrk),lwork - iwrk + 1, &
                        ierr)
              ! perform qr iteration, accumulating schur vectors in vr
              ! (workspace: need n+1, prefer n+hswork (see comments) )
              iwrk = itau
              call la_xhseqr('S','V',n,ilo,ihi,a,lda,wr,wi,vr,ldvr,work(iwrk), &
                        lwork - iwrk + 1,info)
           else
              ! compute eigenvalues only
              ! (workspace: need n+1, prefer n+hswork (see comments) )
              iwrk = itau
              call la_xhseqr('E','N',n,ilo,ihi,a,lda,wr,wi,vr,ldvr,work(iwrk), &
                        lwork - iwrk + 1,info)
           end if
           ! if info /= 0 from la_xhseqr, then quit
           if (info /= 0) go to 50
           if (wantvl .or. wantvr) then
              ! compute left and/or right eigenvectors
              ! (workspace: need 4*n, prefer n + n + 2*n*nb)
              call la_xtrevc3(side,'B',select,n,a,lda,vl,ldvl,vr,ldvr,n,nout,work( &
                         iwrk),lwork - iwrk + 1,ierr)
           end if
           if (wantvl) then
              ! undo balancing of left eigenvectors
              ! (workspace: need n)
              call la_xgebak('B','L',n,ilo,ihi,work(ibal),n,vl,ldvl,ierr)
              ! normalize left eigenvectors and make largest component real
              do i = 1,n
                 if (wi(i) == zero) then
                    scl = one/la_xnrm2(n,vl(1,i),1)
                    call la_xscal(n,scl,vl(1,i),1)
                 else if (wi(i) > zero) then
                    scl = one/la_xlapy2(la_xnrm2(n,vl(1,i),1),la_xnrm2(n, &
                              vl(1,i + 1),1))
                    call la_xscal(n,scl,vl(1,i),1)
                    call la_xscal(n,scl,vl(1,i + 1),1)
                    do k = 1,n
                       work(iwrk + k - 1) = vl(k,i)**2 + vl(k,i + 1)**2
                    end do
                    k = la_ixamax(n,work(iwrk),1)
                    call la_xlartg(vl(k,i),vl(k,i + 1),cs,sn,r)
                    call la_xrot(n,vl(1,i),1,vl(1,i + 1),1,cs,sn)
                    vl(k,i + 1) = zero
                 end if
              end do
           end if
           if (wantvr) then
              ! undo balancing of right eigenvectors
              ! (workspace: need n)
              call la_xgebak('B','R',n,ilo,ihi,work(ibal),n,vr,ldvr,ierr)
              ! normalize right eigenvectors and make largest component real
              do i = 1,n
                 if (wi(i) == zero) then
                    scl = one/la_xnrm2(n,vr(1,i),1)
                    call la_xscal(n,scl,vr(1,i),1)
                 else if (wi(i) > zero) then
                    scl = one/la_xlapy2(la_xnrm2(n,vr(1,i),1),la_xnrm2(n, &
                              vr(1,i + 1),1))
                    call la_xscal(n,scl,vr(1,i),1)
                    call la_xscal(n,scl,vr(1,i + 1),1)
                    do k = 1,n
                       work(iwrk + k - 1) = vr(k,i)**2 + vr(k,i + 1)**2
                    end do
                    k = la_ixamax(n,work(iwrk),1)
                    call la_xlartg(vr(k,i),vr(k,i + 1),cs,sn,r)
                    call la_xrot(n,vr(1,i),1,vr(1,i + 1),1,cs,sn)
                    vr(k,i + 1) = zero
                 end if
              end do
           end if
           ...
           50 continue
           if (scalea) then
              call la_xlascl('G',0,0,cscale,anrm,n - info,1,wr(info + 1),max(n - info,1 &
                        ),ierr)
              call la_xlascl('G',0,0,cscale,anrm,n - info,1,wi(info + 1),max(n - info,1 &
                        ),ierr)
              if (info > 0) then
                 call la_xlascl('G',0,0,cscale,anrm,ilo - 1,1,wr,n,ierr)
                 call la_xlascl('G',0,0,cscale,anrm,ilo - 1,1,wi,n,ierr)
              end if
           end if
           work(1) = maxwrk
           return
     end subroutine la_xgeev
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
     !> XGEEVX: computes for an N-by-N real nonsymmetric matrix A, the
     !> eigenvalues and, optionally, the left and/or right eigenvectors.
     !> Optionally also, it computes a balancing transformation to improve
     !> the conditioning of the eigenvalues and eigenvectors (ILO, IHI,
     !> SCALE, and ABNRM), reciprocal condition numbers for the eigenvalues
     ...
     !> Permuting rows and columns will not change the condition numbers
     !> (in exact arithmetic) but diagonal scaling will.  For further
     !> explanation of balancing, see section 4.10.2_xdp of the LAPACK
     !> Users' Guide.

     subroutine la_xgeevx(balanc,jobvl,jobvr,sense,n,a,lda,wr,wi,vl,ldvl,vr,ldvr, &
               ilo,ihi,scale,abnrm,rconde,rcondv,work,lwork,iwork,info)
        use la_constants_xdp,only:zero,one
        ! -- lapack driver routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ...
           integer(ilp),intent(out) :: ihi,ilo,info
           integer(ilp),intent(in) :: lda,ldvl,ldvr,lwork,n
           real(xdp),intent(out) :: abnrm
           ! Array Arguments
           integer(ilp),intent(out) :: iwork(*)
           real(xdp),intent(inout) :: a(lda,*)
           real(xdp),intent(out) :: rconde(*),rcondv(*),scale(*),vl(ldvl,*),vr(ldvr,*),wi(*), &
                      work(*),wr(*)
        ! =====================================================================
           ...
           if (info == 0) then
              if (n == 0) then
                 ...
                 maxwrk = n + n*la_ilaenv(1,'XGEHRD',' ',n,1,n,0)
                 if (wantvl) then
                    call la_xtrevc3('L','B',select,n,a,lda,vl,ldvl,vr,ldvr,n,nout, &
                              work,-1,ierr)
                    lwork_trevc = int(work(1),KIND=ilp)
                    ...
                    if (wntsnn) then
                       ...
                       call la_xhseqr('S','N',n,1,n,a,lda,wr,wi,vr,ldvr,work,-1, &
                                 info)
                    end if
                 end if
                 hswork = int(work(1),KIND=ilp)
                 if ((.not. wantvl) .and. (.not. wantvr)) then
                    ...
                    maxwrk = max(maxwrk,hswork)
                    maxwrk = max(maxwrk,n + (n - 1)*la_ilaenv(1,'XORGHR',' ',n,1,n,- &
                              1))
                    if ((.not. wntsnn) .and. (.not. wntsne)) maxwrk = max(maxwrk,n*n + 6*n)
           ...
           if (scalea) call la_xlascl('G',0,0,anrm,cscale,n,n,a,lda,ierr)
           ! balance the matrix and compute abnrm
           call la_xgebal(balanc,n,a,lda,ilo,ihi,scale,ierr)
           abnrm = la_xlange('1',n,n,a,lda,dum)
           if (scalea) then
              dum(1) = abnrm
              call la_xlascl('G',0,0,cscale,anrm,1,1,dum,1,ierr)
              abnrm = dum(1)
           ...
           if (wantvl) then
              ...
              ! (workspace: need 1, prefer hswork (see comments) )
              iwrk = itau
              call la_xhseqr(job,'N',n,ilo,ihi,a,lda,wr,wi,vr,ldvr,work(iwrk), &
                        lwork - iwrk + 1,info)
           end if
           ! if info /= 0 from la_xhseqr, then quit
           if (info /= 0) go to 50
           ...
           ! (workspace: need n*n+6*n unless sense = 'e')
           if (.not. wntsnn) then
              call la_xtrsna(sense,'A',select,n,a,lda,vl,ldvl,vr,ldvr,rconde, &
                        rcondv,n,nout,work(iwrk),n,iwork,icond)
           end if
           if (wantvl) then
              ! undo balancing of left eigenvectors
              call la_xgebak(balanc,'L',n,ilo,ihi,scale,n,vl,ldvl,ierr)
              ! normalize left eigenvectors and make largest component real
              do i = 1,n
                 if (wi(i) == zero) then
                    scl = one/la_xnrm2(n,vl(1,i),1)
           ...
           if (wantvr) then
              ! undo balancing of right eigenvectors
              call la_xgebak(balanc,'R',n,ilo,ihi,scale,n,vr,ldvr,ierr)
              ! normalize right eigenvectors and make largest component real
              do i = 1,n
                 if (wi(i) == zero) then
                    scl = one/la_xnrm2(n,vr(1,i),1)
           ...
           if (scalea) then
              ...
              if (info == 0) then
                 if ((wntsnv .or. wntsnb) .and. icond == 0) call la_xlascl('G',0,0,cscale, &
                            anrm,n,1,rcondv,n,ierr)
              else
                 call la_xlascl('G',0,0,cscale,anrm,ilo - 1,1,wr,n,ierr)
                 call la_xlascl('G',0,0,cscale,anrm,ilo - 1,1,wi,n,ierr)
...
#ifdef LA_WITH_XDP
     !> XGGES3: computes for a pair of N-by-N real nonsymmetric matrices (A,B),
     !> the generalized eigenvalues, the generalized real Schur form (S,T),
     !> optionally, the left and/or right matrices of Schur vectors (VSL and
     !> VSR). This gives the generalized Schur factorization
     !> (A,B) = ( (VSL)*S*(VSR)**T, (VSL)*T*(VSR)**T )
     ...
     subroutine la_xgges3(jobvsl,jobvsr,sort,selctg,n,a,lda,b,ldb,sdim,alphar, &
               alphai,beta,vsl,ldvsl,vsr,ldvsr,work,lwork,bwork,info)
        use la_constants_xdp,only:zero,one
        ! -- lapack driver routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ...
        ! =====================================================================
           ...
           ! compute workspace
           if (info == 0) then
              call la_xgeqrf(n,n,b,ldb,work,work,-1,ierr)
              lwkopt = max(6*n + 16,3*n + int(work(1),KIND=ilp))
              call la_xormqr('L','T',n,n,n,b,ldb,work,a,lda,work,-1,ierr)
              lwkopt = max(lwkopt,3*n + int(work(1),KIND=ilp))
              if (ilvsl) then
                 call la_xorgqr(n,n,n,vsl,ldvsl,work,work,-1,ierr)
                 lwkopt = max(lwkopt,3*n + int(work(1),KIND=ilp))
              end if
              call la_xgghd3(jobvsl,jobvsr,n,1,n,a,lda,b,ldb,vsl,ldvsl,vsr,ldvsr, &
                        work,-1,ierr)
              lwkopt = max(lwkopt,3*n + int(work(1),KIND=ilp))
              call la_xlaqz0('S',jobvsl,jobvsr,n,1,n,a,lda,b,ldb,alphar,alphai, &
                        beta,vsl,ldvsl,vsr,ldvsr,work,-1,0,ierr)
              lwkopt = max(lwkopt,2*n + int(work(1),KIND=ilp))
              if (wantst) then
                 call la_xtgsen(0,ilvsl,ilvsr,bwork,n,a,lda,b,ldb,alphar,alphai, &
                 beta,vsl,ldvsl,vsr,ldvsr,sdim,pvsl,pvsr,dif,work,-1,idum,1,ierr)
           ...
           if (ilvsr) call la_xlaset('FULL',n,n,zero,one,vsr,ldvsr)
           ! reduce to generalized hessenberg form
           call la_xgghd3(jobvsl,jobvsr,n,ilo,ihi,a,lda,b,ldb,vsl,ldvsl,vsr,ldvsr, &
                      work(iwrk),lwork + 1 - iwrk,ierr)
           ! perform qz algorithm, computing schur vectors if desired
           iwrk = itau
           call la_xlaqz0('S',jobvsl,jobvsr,n,ilo,ihi,a,lda,b,ldb,alphar,alphai, &
                     beta,vsl,ldvsl,vsr,ldvsr,work(iwrk),lwork + 1 - iwrk,0,ierr)
           if (ierr /= 0) then
              if (ierr > 0 .and. ierr <= n) then
                 info = ierr
...
#ifdef LA_WITH_XDP
     !> XGGEV3: computes for a pair of N-by-N real nonsymmetric matrices (A,B)
     !> the generalized eigenvalues, and optionally, the left and/or right
     !> generalized eigenvectors.
     !> A generalized eigenvalue for a pair of matrices (A,B) is a scalar
     !> lambda or a ratio alpha/beta = lambda, such that A - lambda*B is
     ...
     subroutine la_xggev3(jobvl,jobvr,n,a,lda,b,ldb,alphar,alphai,beta,vl,ldvl,vr, &
                ldvr,work,lwork,info)
        use la_constants_xdp,only:zero,one
        ! -- lapack driver routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ...
        ! =====================================================================
           ...
           if (info == 0) then
              ...
              lwkopt = max(lwkopt,3*n + int(work(1),KIND=ilp))
              if (ilvl) then
                 call la_xorgqr(n,n,n,vl,ldvl,work,work,-1,ierr)
                 lwkopt = max(lwkopt,3*n + int(work(1),KIND=ilp))
              end if
              if (ilv) then
                 call la_xgghd3(jobvl,jobvr,n,1,n,a,lda,b,ldb,vl,ldvl,vr,ldvr, &
                           work,-1,ierr)
                 lwkopt = max(lwkopt,3*n + int(work(1),KIND=ilp))
                 call la_xlaqz0('S',jobvl,jobvr,n,1,n,a,lda,b,ldb,alphar,alphai, &
                           beta,vl,ldvl,vr,ldvr,work,-1,0,ierr)
                 lwkopt = max(lwkopt,2*n + int(work(1),KIND=ilp))
              else
                 call la_xgghd3('N','N',n,1,n,a,lda,b,ldb,vl,ldvl,vr,ldvr,work,- &
                           1,ierr)
                 lwkopt = max(lwkopt,3*n + int(work(1),KIND=ilp))
                 call la_xlaqz0('E',jobvl,jobvr,n,1,n,a,lda,b,ldb,alphar,alphai, &
                           beta,vl,ldvl,vr,ldvr,work,-1,0,ierr)
                 lwkopt = max(lwkopt,2*n + int(work(1),KIND=ilp))
              end if
              work(1) = lwkopt
           ...
           if (ilv) then
              ! eigenvectors requested -- work on whole matrix.
              call la_xgghd3(jobvl,jobvr,n,ilo,ihi,a,lda,b,ldb,vl,ldvl,vr,ldvr, &
                        work(iwrk),lwork + 1 - iwrk,ierr)
           else
              call la_xgghd3('N','N',irows,1,irows,a(ilo,ilo),lda,b(ilo,ilo), &
                        ldb,vl,ldvl,vr,ldvr,work(iwrk),lwork + 1 - iwrk,ierr)
           end if
           ! perform qz algorithm (compute eigenvalues, and optionally, the
           ! schur forms and schur vectors)
           ...
           call la_xlaqz0(chtemp,jobvl,jobvr,n,ilo,ihi,a,lda,b,ldb,alphar,alphai, &
                     beta,vl,ldvl,vr,ldvr,work(iwrk),lwork + 1 - iwrk,0,ierr)
           if (ierr /= 0) then
              if (ierr > 0 .and. ierr <= n) then
                 info = ierr
...
#ifdef LA_WITH_XDP
     !> YGGES: computes for a pair of N-by-N complex nonsymmetric matrices
     !> (A,B), the generalized eigenvalues, the generalized complex Schur
     !> form (S, T), and optionally left and/or right Schur vectors (VSL
     !> and VSR). This gives the generalized Schur factorization
     !> (A,B) = ( (VSL)*S*(VSR)**H, (VSL)*T*(VSR)**H )
     ...
     subroutine la_ygges(jobvsl,jobvsr,sort,selctg,n,a,lda,b,ldb,sdim,alpha,beta, &
               vsl,ldvsl,vsr,ldvsr,work,lwork,rwork,bwork,info)
        ...
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ...
           complex(xdp),intent(out) :: alpha(*),beta(*),vsl(ldvsl,*),vsr(ldvsr,*),work(*)

           ! Function Arguments
           procedure(la_selctg_y) :: selctg
        ! =====================================================================
           ...
           integer(ilp) :: i,icols,ierr,ihi,ijobvl,ijobvr,ileft,ilo,iright,irows,irwrk, &
                     itau,iwrk,lwkmin,lwkopt
           real(xdp) :: anrm,anrmto,bignum,bnrm,bnrmto,eps,pvsl,pvsr,smlnum
           ! Local Arrays
           integer(ilp) :: idum(1)
           real(xdp) :: dif(2)
           ! Intrinsic Functions
           ...
           if (info == 0) then
              lwkmin = max(1,2*n)
              lwkopt = max(1,n + n*la_ilaenv(1,'YGEQRF',' ',n,1,n,0))
              lwkopt = max(lwkopt,n + n*la_ilaenv(1,'YUNMQR',' ',n,1,n,-1))
              if (ilvsl) then
                 lwkopt = max(lwkopt,n + n*la_ilaenv(1,'YUNGQR',' ',n,1,n,-1))
           ...
           bignum = one/smlnum
           ! scale a if max element outside range [smlnum,bignum]
           anrm = la_ylange('M',n,n,a,lda,rwork)
           ilascl = .false.
           if (anrm > zero .and. anrm < smlnum) then
              anrmto = smlnum
              ilascl = .true.
           ...
           if (ilascl) call la_ylascl('G',0,0,anrm,anrmto,n,n,a,lda,ierr)
           ! scale b if max element outside range [smlnum,bignum]
           bnrm = la_ylange('M',n,n,b,ldb,rwork)
           ilbscl = .false.
           if (bnrm > zero .and. bnrm < smlnum) then
              bnrmto = smlnum
              ilbscl = .true.
           ...
           if (ilbscl) call la_ylascl('G',0,0,bnrm,bnrmto,n,n,b,ldb,ierr)
           ! permute the matrix to make it more nearly triangular
           ! (real workspace: need 6*n)
           ileft = 1
           iright = n + 1
           irwrk = iright + n
           call la_yggbal('P',n,a,lda,b,ldb,ilo,ihi,rwork(ileft),rwork(iright), &
                     rwork(irwrk),ierr)
           ! reduce b to triangular form (qr decomposition of b)
           ! (complex workspace: need n, prefer n*nb)
           irows = ihi + 1 - ilo
           ...
           itau = 1
           iwrk = itau + irows
           call la_ygeqrf(irows,icols,b(ilo,ilo),ldb,work(itau),work(iwrk),lwork + &
                     1 - iwrk,ierr)
           ! apply the orthogonal transformation to matrix a
           ! (complex workspace: need n, prefer n*nb)
           call la_yunmqr('L','C',irows,icols,irows,b(ilo,ilo),ldb,work(itau),a( &
                     ilo,ilo),lda,work(iwrk),lwork + 1 - iwrk,ierr)
           ! initialize vsl
           ! (complex workspace: need n, prefer n*nb)
           if (ilvsl) then
              call la_ylaset('FULL',n,n,czero,cone,vsl,ldvsl)
              if (irows > 1) then
                 call la_ylacpy('L',irows - 1,irows - 1,b(ilo + 1,ilo),ldb,vsl(ilo + 1,ilo) &
                           ,ldvsl)
              end if
              call la_yungqr(irows,irows,irows,vsl(ilo,ilo),ldvsl,work(itau),work( &
                        iwrk),lwork + 1 - iwrk,ierr)
           end if
           ! initialize vsr
           if (ilvsr) call la_ylaset('FULL',n,n,czero,cone,vsr,ldvsr)
           ! reduce to generalized hessenberg form
           ! (workspace: none needed)
           call la_ygghrd(jobvsl,jobvsr,n,ilo,ihi,a,lda,b,ldb,vsl,ldvsl,vsr,ldvsr, &
                      ierr)
           sdim = 0
           ! perform qz algorithm, computing schur vectors if desired
           ! (complex workspace: need n)
           ! (real workspace: need n)
           iwrk = itau
           call la_yhgeqz('S',jobvsl,jobvsr,n,ilo,ihi,a,lda,b,ldb,alpha,beta,vsl, &
                     ldvsl,vsr,ldvsr,work(iwrk),lwork + 1 - iwrk,rwork(irwrk),ierr)
           if (ierr /= 0) then
              if (ierr > 0 .and. ierr <= n) then
                 info = ierr
           ...
           if (wantst) then
              ! undo scaling on eigenvalues before selecting
              if (ilascl) call la_ylascl('G',0,0,anrm,anrmto,n,1,alpha,n,ierr)

              if (ilbscl) call la_ylascl('G',0,0,bnrm,bnrmto,n,1,beta,n,ierr)

              ! select eigenvalues
              do i = 1,n
                 bwork(i) = selctg(alpha(i),beta(i))
              end do
              call la_ytgsen(0,ilvsl,ilvsr,bwork,n,a,lda,b,ldb,alpha,beta,vsl, &
              ldvsl,vsr,ldvsr,sdim,pvsl,pvsr,dif,work(iwrk),lwork - iwrk + 1,idum,1,ierr)
           ...
           ! apply back-permutation to vsl and vsr
           ! (workspace: none needed)
           if (ilvsl) call la_yggbak('P','L',n,ilo,ihi,rwork(ileft),rwork(iright), &
                     n,vsl,ldvsl,ierr)
           if (ilvsr) call la_yggbak('P','R',n,ilo,ihi,rwork(ileft),rwork(iright), &
                     n,vsr,ldvsr,ierr)
           ! undo scaling
           if (ilascl) then
              call la_ylascl('U',0,0,anrmto,anrm,n,n,a,lda,ierr)
              call la_ylascl('G',0,0,anrmto,anrm,n,1,alpha,n,ierr)
           end if
           if (ilbscl) then
              call la_ylascl('U',0,0,bnrmto,bnrm,n,n,b,ldb,ierr)
              call la_ylascl('G',0,0,bnrmto,bnrm,n,1,beta,n,ierr)
           end if
           if (wantst) then
              ! check if reordering is correct
              lastsl = .true.
...
#ifdef LA_WITH_XDP
     !> YGGESX: computes for a pair of N-by-N complex nonsymmetric matrices
     !> (A,B), the generalized eigenvalues, the complex Schur form (S,T),
     !> and, optionally, the left and/or right matrices of Schur vectors (VSL
     !> and VSR).  This gives the generalized Schur factorization
     !> (A,B) = ( (VSL) S (VSR)**H, (VSL) T (VSR)**H )
     ...
     subroutine la_yggesx(jobvsl,jobvsr,sort,selctg,sense,n,a,lda,b,ldb,sdim,alpha, &
      beta,vsl,ldvsl,vsr,ldvsr,rconde,rcondv,work,lwork,rwork,iwork,liwork,bwork,info)
        use la_constants_xdp,only:zero,one,czero,cone
        ...
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ...
           logical(lk),intent(out) :: bwork(*)
           integer(ilp),intent(out) :: iwork(*)
           real(xdp),intent(out) :: rconde(2),rcondv(2),rwork(*)
           complex(xdp),intent(inout) :: a(lda,*),b(ldb,*)
           complex(xdp),intent(out) :: alpha(*),beta(*),vsl(ldvsl,*),vsr(ldvsr,*),work(*)
        ...
        ! =====================================================================
           ...
           integer(ilp) :: i,icols,ierr,ihi,ijob,ijobvl,ijobvr,ileft,ilo,iright,irows, &
                     irwrk,itau,iwrk,liwmin,lwrk,maxwrk,minwrk
           real(xdp) :: anrm,anrmto,bignum,bnrm,bnrmto,eps,pl,pr,smlnum
           ! Local Arrays
           real(xdp) :: dif(2)
           ! Intrinsic Functions
           intrinsic :: max,sqrt
           ...
           if (info == 0) then
              if (n > 0) then
                 minwrk = 2*n
                 maxwrk = n*(1 + la_ilaenv(1,'YGEQRF',' ',n,1,n,0))
                 maxwrk = max(maxwrk,n*(1 + la_ilaenv(1,'YUNMQR',' ',n,1,n,-1)))

                 if (ilvsl) then
                    maxwrk = max(maxwrk,n*(1 + la_ilaenv(1,'YUNGQR',' ',n,1,n,-1)) &
                              )
                 end if
                 lwrk = maxwrk
                 if (ijob >= 1) lwrk = max(lwrk,n*n/2)
           ...
           if (wantst) then
              ! undo scaling on eigenvalues before selctging
              if (ilascl) call la_ylascl('G',0,0,anrmto,anrm,n,1,alpha,n,ierr)

              if (ilbscl) call la_ylascl('G',0,0,bnrmto,bnrm,n,1,beta,n,ierr)
              ...
              ! (complex workspace: if ijob >= 1, need max(1, 2*sdim*(n-sdim))
                                  ! otherwise, need 1 )
              call la_ytgsen(ijob,ilvsl,ilvsr,bwork,n,a,lda,b,ldb,alpha,beta,vsl, &
              ldvsl,vsr,ldvsr,sdim,pl,pr,dif,work(iwrk),lwork - iwrk + 1,iwork,liwork,ierr &
                        )
              if (ijob >= 1) maxwrk = max(maxwrk,2*sdim*(n - sdim))
              if (ierr == -21) then
...
#ifdef LA_WITH_XDP
     !> YGGEV: computes for a pair of N-by-N complex nonsymmetric matrices
     !> (A,B), the generalized eigenvalues, and optionally, the left and/or
     !> right generalized eigenvectors.
     !> A generalized eigenvalue for a pair of matrices (A,B) is a scalar
     !> lambda or a ratio alpha/beta = lambda, such that A - lambda*B is
     ...
     subroutine la_yggev(jobvl,jobvr,n,a,lda,b,ldb,alpha,beta,vl,ldvl,vr,ldvr, &
               work,lwork,rwork,info)
        use la_constants_xdp,only:zero,one,czero,cone
        ! -- lapack driver routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ...
           real(xdp),intent(out) :: rwork(*)
           complex(xdp),intent(inout) :: a(lda,*),b(ldb,*)
           complex(xdp),intent(out) :: alpha(*),beta(*),vl(ldvl,*),vr(ldvr,*),work(*)
        ! =====================================================================
           ...
           if (info == 0) then
              lwkmin = max(1,2*n)
              lwkopt = max(1,n + n*la_ilaenv(1,'YGEQRF',' ',n,1,n,0))
              lwkopt = max(lwkopt,n + n*la_ilaenv(1,'YUNMQR',' ',n,1,n,0))
              if (ilvl) then
                 lwkopt = max(lwkopt,n + n*la_ilaenv(1,'YUNGQR',' ',n,1,n,-1))
           ...
           ! (complex workspace: need n, prefer n*nb)
           if (ilvl) then
              call la_ylaset('FULL',n,n,czero,cone,vl,ldvl)
              if (irows > 1) then
                 call la_ylacpy('L',irows - 1,irows - 1,b(ilo + 1,ilo),ldb,vl(ilo + 1,ilo), &
                            ldvl)
              end if
              call la_yungqr(irows,irows,irows,vl(ilo,ilo),ldvl,work(itau),work( &
                        iwrk),lwork + 1 - iwrk,ierr)
           end if
           ! initialize vr
           if (ilvr) call la_ylaset('FULL',n,n,czero,cone,vr,ldvr)
           ! reduce to generalized hessenberg form
           if (ilv) then
              ! eigenvectors requested -- work on whole matrix.
              call la_ygghrd(jobvl,jobvr,n,ilo,ihi,a,lda,b,ldb,vl,ldvl,vr,ldvr, &
                        ierr)
           else
              call la_ygghrd('N','N',irows,1,irows,a(ilo,ilo),lda,b(ilo,ilo), &
                        ldb,vl,ldvl,vr,ldvr,ierr)
           end if
           ! perform qz algorithm (compute eigenvalues, and optionally, the
           ! schur form and schur vectors)
           ...
           call la_yhgeqz(chtemp,jobvl,jobvr,n,ilo,ihi,a,lda,b,ldb,alpha,beta,vl, &
                     ldvl,vr,ldvr,work(iwrk),lwork + 1 - iwrk,rwork(irwrk),ierr)
           if (ierr /= 0) then
              if (ierr > 0 .and. ierr <= n) then
                 info = ierr
           ...
           if (ilv) then
              ...
              call la_ytgevc(chtemp,'B',ldumma,n,a,lda,b,ldb,vl,ldvl,vr,ldvr,n, &
                        in,work(iwrk),rwork(irwrk),ierr)
              if (ierr /= 0) then
                 info = n + 2
                 go to 70
              ...
              ! (workspace: none needed)
              if (ilvl) then
                 call la_yggbak('P','L',n,ilo,ihi,rwork(ileft),rwork(iright),n,vl, &
                            ldvl,ierr)
                 loop_30: do jc = 1,n
                    temp = zero
                    do jr = 1,n
              ...
              if (ilvr) then
                 call la_yggbak('P','R',n,ilo,ihi,rwork(ileft),rwork(iright),n,vr, &
                            ldvr,ierr)
                 loop_60: do jc = 1,n
                    temp = zero
                    do jr = 1,n
...
#ifdef LA_WITH_XDP
     !> YGGEVX: computes for a pair of N-by-N complex nonsymmetric matrices
     !> (A,B) the generalized eigenvalues, and optionally, the left and/or
     !> right generalized eigenvectors.
     !> Optionally, it also computes a balancing transformation to improve
     !> the conditioning of the eigenvalues and eigenvectors (ILO, IHI,
     ...
     subroutine la_yggevx(balanc,jobvl,jobvr,sense,n,a,lda,b,ldb,alpha,beta,vl, &
     ldvl,vr,ldvr,ilo,ihi,lscale,rscale,abnrm,bbnrm,rconde,rcondv,work,lwork,rwork, &
               iwork,bwork,info)
        use la_constants_xdp,only:zero,one,czero,cone
        ! -- lapack driver routine --
        ...
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ...
           logical(lk),intent(out) :: bwork(*)
           integer(ilp),intent(out) :: iwork(*)
           real(xdp),intent(out) :: lscale(*),rconde(*),rcondv(*),rscale(*),rwork(*)
           complex(xdp),intent(inout) :: a(lda,*),b(ldb,*)
           complex(xdp),intent(out) :: alpha(*),beta(*),vl(ldvl,*),vr(ldvr,*),work(*)
        ! =====================================================================
           ...
           if (info == 0) then
              if (n == 0) then
                 ...
                 maxwrk = minwrk
                 maxwrk = max(maxwrk,n + n*la_ilaenv(1,'YGEQRF',' ',n,1,n,0))

                 maxwrk = max(maxwrk,n + n*la_ilaenv(1,'YUNMQR',' ',n,1,n,0))

                 if (ilvl) then
                    maxwrk = max(maxwrk,n + n*la_ilaenv(1,'YUNGQR',' ',n,1,n,0))
           ...
           ! permute and/or balance the matrix pair (a,b)
           ! (real workspace: need 6*n if balanc = 's' or 'b', 1 otherwise)
           call la_yggbal(balanc,n,a,lda,b,ldb,ilo,ihi,lscale,rscale,rwork,ierr)

           ! compute abnrm and bbnrm
           abnrm = la_ylange('1',n,n,a,lda,rwork(1))
           if (ilascl) then
              rwork(1) = abnrm
              call la_xlascl('G',0,0,anrmto,anrm,1,1,rwork(1),1,ierr)
              abnrm = rwork(1)
           end if
           bbnrm = la_ylange('1',n,n,b,ldb,rwork(1))
           if (ilbscl) then
              rwork(1) = bbnrm
              call la_xlascl('G',0,0,bnrmto,bnrm,1,1,rwork(1),1,ierr)
              bbnrm = rwork(1)
           end if
           ! reduce b to triangular form (qr decomposition of b)
           ! (complex workspace: need n, prefer n*nb )
           ...
           ! compute eigenvectors and estimate condition numbers if desired
           ! la_ytgevc: (complex workspace: need 2*n )
                   ! (real workspace:    need 2*n )
           ! la_ytgsna: (complex workspace: need 2*n*n if sense='v' or 'b')
                   ! (integer workspace: need n+2 )
           if (ilv .or. .not. wantsn) then
              if (ilv) then
                 if (ilvl) then
              ...
              if (.not. wantsn) then
                 ! compute eigenvectors (la_ytgevc) and estimate condition
                 ! numbers (la_ytgsna). note that the definition of the condition
                 ! number is not invariant under transformation (u,v) to
                 ! (q*u, z*v), where (u,v) are eigenvectors of the generalized
                 ! schur form (s,t), q and z are orthogonal matrices. in order
                 ! to avoid using extra 2*n*n workspace, we have to
                 ...
                 do i = 1,n
                    ...
                    iwrk1 = iwrk + n
                    if (wantse .or. wantsb) then
                       call la_ytgevc('B','S',bwork,n,a,lda,b,ldb,work(1),n,work( &
                                 iwrk),n,1,m,work(iwrk1),rwork,ierr)
                       if (ierr /= 0) then
                          info = n + 2
                          go to 90
                    ...
                    call la_ytgsna(sense,'S',bwork,n,a,lda,b,ldb,work(1),n,work( &
                    iwrk),n,rconde(i),rcondv(i),1,m,work(iwrk1),lwork - iwrk1 + 1,iwork, &
                              ierr)
                 end do
              end if
           ...
           ! (workspace: none needed)
           if (ilvl) then
              call la_yggbak(balanc,'L',n,ilo,ihi,lscale,rscale,n,vl,ldvl,ierr)
           ...
           if (ilvr) then
              call la_yggbak(balanc,'R',n,ilo,ihi,lscale,rscale,n,vr,ldvr,ierr)
...
#ifdef LA_WITH_XDP
     !> YGEES: computes for an N-by-N complex nonsymmetric matrix A, the
     !> eigenvalues, the Schur form T, and, optionally, the matrix of Schur
     !> vectors Z.  This gives the Schur factorization A = Z*T*(Z**H).
     !> Optionally, it also orders the eigenvalues on the diagonal of the
     !> Schur form so that selected eigenvalues are at the top left.
     ...
     subroutine la_ygees(jobvs,sort,select,n,a,lda,sdim,w,vs,ldvs,work,lwork, &
               rwork,bwork,info)
        use la_constants_xdp,only:zero,one
        ! -- lapack driver routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ...
           complex(xdp),intent(out) :: vs(ldvs,*),w(*),work(*)
           ! Function Arguments
           procedure(la_select_y) :: select
        ! =====================================================================
           ...
           ! compute workspace
            ! (note: comments in the code beginning "workspace:" describe the
             ...
             ! workspace. nb refers to the optimal block size for the
             ! immediately following subroutine, as returned by la_ilaenv.
             ! hswork refers to the workspace preferred by la_yhseqr, as
             ! calculated below. hswork is computed assuming ilo=1 and ihi=n,
             ! the worst case.)
           if (info == 0) then
              if (n == 0) then
                 ...
                 maxwrk = n + n*la_ilaenv(1,'YGEHRD',' ',n,1,n,0)
                 minwrk = 2*n
                 call la_yhseqr('S',jobvs,n,1,n,a,lda,w,vs,ldvs,work,-1,ieval)

                 hswork = real(work(1),KIND=xdp)
                 if (.not. wantvs) then
                    maxwrk = max(maxwrk,hswork)
                 else
                    maxwrk = max(maxwrk,n + (n - 1)*la_ilaenv(1,'YUNGHR',' ',n,1,n,- &
                              1))
                    maxwrk = max(maxwrk,hswork)
                 end if
              end if
           ...
           bignum = one/smlnum
           ! scale a if max element outside range [smlnum,bignum]
           anrm = la_ylange('M',n,n,a,lda,dum)
           scalea = .false.
           if (anrm > zero .and. anrm < smlnum) then
              scalea = .true.
              cscale = smlnum
           ...
           if (scalea) call la_ylascl('G',0,0,anrm,cscale,n,n,a,lda,ierr)
           ! permute the matrix to make it more nearly triangular
           ! (cworkspace: none)
           ! (rworkspace: need n)
           ibal = 1
           call la_ygebal('P',n,a,lda,ilo,ihi,rwork(ibal),ierr)
           ! reduce to upper hessenberg form
           ! (cworkspace: need 2*n, prefer n+n*nb)
           ! (rworkspace: none)
           itau = 1
           iwrk = n + itau
           call la_ygehrd(n,ilo,ihi,a,lda,work(itau),work(iwrk),lwork - iwrk + 1,ierr &
                     )
           if (wantvs) then
              ! copy householder vectors to vs
              call la_ylacpy('L',n,n,a,lda,vs,ldvs)
              ! generate unitary matrix in vs
              ! (cworkspace: need 2*n-1, prefer n+(n-1)*nb)
              ! (rworkspace: none)
              call la_yunghr(n,ilo,ihi,vs,ldvs,work(itau),work(iwrk),lwork - iwrk + 1, &
                        ierr)
           end if
           sdim = 0
           ! perform qr iteration, accumulating schur vectors in vs if desired
           ...
           ! (rworkspace: none)
           iwrk = itau
           call la_yhseqr('S',jobvs,n,ilo,ihi,a,lda,w,vs,ldvs,work(iwrk),lwork - &
                     iwrk + 1,ieval)
           if (ieval > 0) info = ieval
           ! sort eigenvalues if desired
           if (wantst .and. info == 0) then
              if (scalea) call la_ylascl('G',0,0,cscale,anrm,n,1,w,n,ierr)
              do i = 1,n
                 bwork(i) = select(w(i))
              end do
              ! reorder eigenvalues and transform schur vectors
              ! (cworkspace: none)
              ! (rworkspace: none)
              call la_ytrsen('N',jobvs,bwork,n,a,lda,vs,ldvs,w,sdim,s,sep,work( &
                        iwrk),lwork - iwrk + 1,icond)
           end if
           if (wantvs) then
              ! undo balancing
              ! (cworkspace: none)
              ! (rworkspace: need n)
              call la_ygebak('P','R',n,ilo,ihi,rwork(ibal),n,vs,ldvs,ierr)
           end if
           if (scalea) then
              ! undo scaling for the schur form of a
              call la_ylascl('U',0,0,cscale,anrm,n,n,a,lda,ierr)
              call la_ycopy(n,a,lda + 1,w,1)
           end if
           work(1) = maxwrk
           return
     end subroutine la_ygees
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
     !> YGEESX: computes for an N-by-N complex nonsymmetric matrix A, the
     !> eigenvalues, the Schur form T, and, optionally, the matrix of Schur
     !> vectors Z.  This gives the Schur factorization A = Z*T*(Z**H).
     !> Optionally, it also orders the eigenvalues on the diagonal of the
     !> Schur form so that selected eigenvalues are at the top left;
     ...
     subroutine la_ygeesx(jobvs,sort,select,sense,n,a,lda,sdim,w,vs,ldvs,rconde, &
               rcondv,work,lwork,rwork,bwork,info)
        use la_constants_xdp,only:zero,one
        ! -- lapack driver routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ...
        ! =====================================================================
           ...
           ! compute workspace
            ! (note: comments in the code beginning "workspace:" describe the
             ...
             ! the worst case.
             ! if sense = 'e', 'v' or 'b', then the amount of workspace needed
             ! depends on sdim, which is computed by the routine la_ytrsen later
             ! in the code.)
           if (info == 0) then
              if (n == 0) then
                 minwrk = 1
           ...
           if (wantst .and. info == 0) then
              ...
              ! (cworkspace: if sense is not 'n', need 2*sdim*(n-sdim)
                           ! otherwise, need none )
              ! (rworkspace: none)
              call la_ytrsen(sense,jobvs,bwork,n,a,lda,vs,ldvs,w,sdim,rconde, &
                        rcondv,work(iwrk),lwork - iwrk + 1,icond)
              if (.not. wantsn) maxwrk = max(maxwrk,2*sdim*(n - sdim))
              if (icond == -14) then
                 ! not enough complex workspace
...
#ifdef LA_WITH_XDP
     !> YGEEV: computes for an N-by-N complex nonsymmetric matrix A, the
     !> eigenvalues and, optionally, the left and/or right eigenvectors.
     !> The right eigenvector v(j) of A satisfies
     !> A * v(j) = lambda(j) * v(j)
     !> where lambda(j) is its eigenvalue.
     ...
     subroutine la_ygeev(jobvl,jobvr,n,a,lda,w,vl,ldvl,vr,ldvr,work,lwork,rwork, &
               info)
        use la_constants_xdp,only:zero,one
        ! -- lapack driver routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ...
           real(xdp),intent(out) :: rwork(*)
           complex(xdp),intent(inout) :: a(lda,*)
           complex(xdp),intent(out) :: vl(ldvl,*),vr(ldvr,*),w(*),work(*)
        ! =====================================================================
           ...
           integer(ilp) :: hswork,i,ibal,ierr,ihi,ilo,irwork,itau,iwrk,k,lwork_trevc, &
                     maxwrk,minwrk,nout
           real(xdp) :: anrm,bignum,cscale,eps,scl,smlnum
           complex(xdp) :: tmp
           ! Local Arrays
           logical(lk) :: select(1)
           real(xdp) :: dum(1)
           ...
           if (info == 0) then
              if (n == 0) then
                 ...
                 if (wantvl) then
                    maxwrk = max(maxwrk,n + (n - 1)*la_ilaenv(1,'YUNGHR',' ',n,1,n,- &
                              1))
                    call la_ytrevc3('L','B',select,n,a,lda,vl,ldvl,vr,ldvr,n,nout, &
                              work,-1,rwork,-1,ierr)
                    lwork_trevc = int(work(1),KIND=ilp)
                    maxwrk = max(maxwrk,n + lwork_trevc)
                    call la_yhseqr('S','V',n,1,n,a,lda,w,vl,ldvl,work,-1,info)

                 else if (wantvr) then
                    maxwrk = max(maxwrk,n + (n - 1)*la_ilaenv(1,'YUNGHR',' ',n,1,n,- &
                              1))
                    call la_ytrevc3('R','B',select,n,a,lda,vl,ldvl,vr,ldvr,n,nout, &
                              work,-1,rwork,-1,ierr)
                    lwork_trevc = int(work(1),KIND=ilp)
                    maxwrk = max(maxwrk,n + lwork_trevc)
                    call la_yhseqr('S','V',n,1,n,a,lda,w,vr,ldvr,work,-1,info)

                 else
                    call la_yhseqr('E','N',n,1,n,a,lda,w,vr,ldvr,work,-1,info)
           ...
           ! (rworkspace: need n)
           ibal = 1
           call la_ygebal('B',n,a,lda,ilo,ihi,rwork(ibal),ierr)
           ! reduce to upper hessenberg form
           ! (cworkspace: need 2*n, prefer n+n*nb)
           ! (rworkspace: none)
           itau = 1
           ...
           if (wantvl) then
              ...
              ! copy householder vectors to vl
              side = 'L'
              call la_ylacpy('L',n,n,a,lda,vl,ldvl)
              ! generate unitary matrix in vl
              ! (cworkspace: need 2*n-1, prefer n+(n-1)*nb)
              ! (rworkspace: none)
              call la_yunghr(n,ilo,ihi,vl,ldvl,work(itau),work(iwrk),lwork - iwrk + 1, &
                        ierr)
              ! perform qr iteration, accumulating schur vectors in vl
              ! (cworkspace: need 1, prefer hswork (see comments) )
              ! (rworkspace: none)
              iwrk = itau
              call la_yhseqr('S','V',n,ilo,ihi,a,lda,w,vl,ldvl,work(iwrk),lwork - &
                        iwrk + 1,info)
              if (wantvr) then
                 ! want left and right eigenvectors
                 ! copy schur vectors to vr
                 side = 'B'
                 call la_ylacpy('F',n,n,vl,ldvl,vr,ldvr)
              end if
           else if (wantvr) then
              ! want right eigenvectors
              ! copy householder vectors to vr
              side = 'R'
              call la_ylacpy('L',n,n,a,lda,vr,ldvr)
              ! generate unitary matrix in vr
              ! (cworkspace: need 2*n-1, prefer n+(n-1)*nb)
              ! (rworkspace: none)
              call la_yunghr(n,ilo,ihi,vr,ldvr,work(itau),work(iwrk),lwork - iwrk + 1, &
                        ierr)
              ! perform qr iteration, accumulating schur vectors in vr
              ! (cworkspace: need 1, prefer hswork (see comments) )
              ! (rworkspace: none)
              iwrk = itau
              call la_yhseqr('S','V',n,ilo,ihi,a,lda,w,vr,ldvr,work(iwrk),lwork - &
                        iwrk + 1,info)
           else
              ! compute eigenvalues only
              ! (cworkspace: need 1, prefer hswork (see comments) )
              ! (rworkspace: none)
              iwrk = itau
              call la_yhseqr('E','N',n,ilo,ihi,a,lda,w,vr,ldvr,work(iwrk),lwork - &
                        iwrk + 1,info)
           end if
           ! if info /= 0 from la_yhseqr, then quit
           if (info /= 0) go to 50
           if (wantvl .or. wantvr) then
              ! compute left and/or right eigenvectors
              ! (cworkspace: need 2*n, prefer n + 2*n*nb)
              ! (rworkspace: need 2*n)
              irwork = ibal + n
              call la_ytrevc3(side,'B',select,n,a,lda,vl,ldvl,vr,ldvr,n,nout,work( &
                         iwrk),lwork - iwrk + 1,rwork(irwork),n,ierr)
           end if
           if (wantvl) then
              ! undo balancing of left eigenvectors
              ! (cworkspace: none)
              ! (rworkspace: need n)
              call la_ygebak('B','L',n,ilo,ihi,rwork(ibal),n,vl,ldvl,ierr)
              ! normalize left eigenvectors and make largest component real
              do i = 1,n
                 scl = one/la_xynrm2(n,vl(1,i),1)
                 call la_yxscal(n,scl,vl(1,i),1)
                 do k = 1,n
                    rwork(irwork + k - 1) = real(vl(k,i),KIND=xdp)**2 + aimag(vl(k,i)) &
                              **2
                 end do
                 k = la_ixamax(n,rwork(irwork),1)
                 tmp = conjg(vl(k,i))/sqrt(rwork(irwork + k - 1))
                 call la_yscal(n,tmp,vl(1,i),1)
                 vl(k,i) = cmplx(real(vl(k,i),KIND=xdp),zero,KIND=xdp)
              end do
           end if
           if (wantvr) then
              ! undo balancing of right eigenvectors
              ! (cworkspace: none)
              ! (rworkspace: need n)
              call la_ygebak('B','R',n,ilo,ihi,rwork(ibal),n,vr,ldvr,ierr)
              ! normalize right eigenvectors and make largest component real
              do i = 1,n
                 scl = one/la_xynrm2(n,vr(1,i),1)
                 call la_yxscal(n,scl,vr(1,i),1)
                 do k = 1,n
                    rwork(irwork + k - 1) = real(vr(k,i),KIND=xdp)**2 + aimag(vr(k,i)) &
                              **2
                 end do
                 k = la_ixamax(n,rwork(irwork),1)
                 tmp = conjg(vr(k,i))/sqrt(rwork(irwork + k - 1))
                 call la_yscal(n,tmp,vr(1,i),1)
                 vr(k,i) = cmplx(real(vr(k,i),KIND=xdp),zero,KIND=xdp)
              end do
           end if
           ! undo scaling if necessary
           50 continue
           if (scalea) then
              call la_ylascl('G',0,0,cscale,anrm,n - info,1,w(info + 1),max(n - info,1) &
                        ,ierr)
              if (info > 0) then
                 call la_ylascl('G',0,0,cscale,anrm,ilo - 1,1,w,n,ierr)
              end if
           end if
           work(1) = maxwrk
           return
     end subroutine la_ygeev
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
     !> YGEEVX: computes for an N-by-N complex nonsymmetric matrix A, the
     !> eigenvalues and, optionally, the left and/or right eigenvectors.
     !> Optionally also, it computes a balancing transformation to improve
     !> the conditioning of the eigenvalues and eigenvectors (ILO, IHI,
     !> SCALE, and ABNRM), reciprocal condition numbers for the eigenvalues
     ...
     subroutine la_ygeevx(balanc,jobvl,jobvr,sense,n,a,lda,w,vl,ldvl,vr,ldvr,ilo, &
               ihi,scale,abnrm,rconde,rcondv,work,lwork,rwork,info)
        use la_constants_xdp,only:zero,one
        ! -- lapack driver routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ...
           real(xdp),intent(out) :: abnrm
           ! Array Arguments
           real(xdp),intent(out) :: rconde(*),rcondv(*),rwork(*),scale(*)
           complex(xdp),intent(inout) :: a(lda,*)
           complex(xdp),intent(out) :: vl(ldvl,*),vr(ldvr,*),w(*),work(*)
        ! =====================================================================
              ...
              if (n == 0) then
                 ...
                 if (wantvl) then
                    ...
                    if (wntsnn) then
                       ...
                       call la_yhseqr('S','N',n,1,n,a,lda,w,vr,ldvr,work,-1,info)
           ...
           if (scalea) call la_ylascl('G',0,0,anrm,cscale,n,n,a,lda,ierr)
           ! balance the matrix and compute abnrm
           call la_ygebal(balanc,n,a,lda,ilo,ihi,scale,ierr)
           abnrm = la_ylange('1',n,n,a,lda,dum)
           if (scalea) then
              dum(1) = abnrm
              call la_xlascl('G',0,0,cscale,anrm,1,1,dum,1,ierr)
              abnrm = dum(1)
           ...
           if (wantvl) then
              ...
              ! (rworkspace: none)
              iwrk = itau
              call la_yhseqr(job,'N',n,ilo,ihi,a,lda,w,vr,ldvr,work(iwrk),lwork - &
                        iwrk + 1,info)
           end if
           ! if info /= 0 from la_yhseqr, then quit
           if (info /= 0) go to 50
           ...
           ! (rworkspace: need 2*n unless sense = 'e')
           if (.not. wntsnn) then
              call la_ytrsna(sense,'A',select,n,a,lda,vl,ldvl,vr,ldvr,rconde, &
                        rcondv,n,nout,work(iwrk),n,rwork,icond)
           end if
           if (wantvl) then
              ! undo balancing of left eigenvectors
              call la_ygebak(balanc,'L',n,ilo,ihi,scale,n,vl,ldvl,ierr)
              ! normalize left eigenvectors and make largest component real
              do i = 1,n
                 scl = one/la_xynrm2(n,vl(1,i),1)
                 call la_yxscal(n,scl,vl(1,i),1)
                 do k = 1,n
                    rwork(k) = real(vl(k,i),KIND=xdp)**2 + aimag(vl(k,i))**2
                 end do
                 k = la_ixamax(n,rwork,1)
                 tmp = conjg(vl(k,i))/sqrt(rwork(k))
                 call la_yscal(n,tmp,vl(1,i),1)
                 vl(k,i) = cmplx(real(vl(k,i),KIND=xdp),zero,KIND=xdp)
              end do
           ...
           if (wantvr) then
              ! undo balancing of right eigenvectors
              call la_ygebak(balanc,'R',n,ilo,ihi,scale,n,vr,ldvr,ierr)
              ! normalize right eigenvectors and make largest component real
              do i = 1,n
                 scl = one/la_xynrm2(n,vr(1,i),1)
                 call la_yxscal(n,scl,vr(1,i),1)
                 do k = 1,n
                    rwork(k) = real(vr(k,i),KIND=xdp)**2 + aimag(vr(k,i))**2
                 end do
                 k = la_ixamax(n,rwork,1)
                 tmp = conjg(vr(k,i))/sqrt(rwork(k))
                 call la_yscal(n,tmp,vr(1,i),1)
...
#ifdef LA_WITH_XDP
     !> YGGES3: computes for a pair of N-by-N complex nonsymmetric matrices
     !> (A,B), the generalized eigenvalues, the generalized complex Schur
     !> form (S, T), and optionally left and/or right Schur vectors (VSL
     !> and VSR). This gives the generalized Schur factorization
     !> (A,B) = ( (VSL)*S*(VSR)**H, (VSL)*T*(VSR)**H )
     ...
     subroutine la_ygges3(jobvsl,jobvsr,sort,selctg,n,a,lda,b,ldb,sdim,alpha,beta, &
               vsl,ldvsl,vsr,ldvsr,work,lwork,rwork,bwork,info)
        use la_constants_xdp,only:zero,one,czero,cone
        ! -- lapack driver routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ...
        ! =====================================================================
           ...
           ! compute workspace
           if (info == 0) then
              call la_ygeqrf(n,n,b,ldb,work,work,-1,ierr)
              lwkopt = max(1,n + int(work(1),KIND=ilp))
              call la_yunmqr('L','C',n,n,n,b,ldb,work,a,lda,work,-1,ierr)
              lwkopt = max(lwkopt,n + int(work(1),KIND=ilp))
              if (ilvsl) then
                 call la_yungqr(n,n,n,vsl,ldvsl,work,work,-1,ierr)
                 lwkopt = max(lwkopt,n + int(work(1),KIND=ilp))
              end if
              call la_ygghd3(jobvsl,jobvsr,n,1,n,a,lda,b,ldb,vsl,ldvsl,vsr,ldvsr, &
                        work,-1,ierr)
              lwkopt = max(lwkopt,n + int(work(1),KIND=ilp))
              call la_ylaqz0('S',jobvsl,jobvsr,n,1,n,a,lda,b,ldb,alpha,beta,vsl, &
                        ldvsl,vsr,ldvsr,work,-1,rwork,0,ierr)
              lwkopt = max(lwkopt,int(work(1),KIND=ilp))
              if (wantst) then
                 call la_ytgsen(0,ilvsl,ilvsr,bwork,n,a,lda,b,ldb,alpha,beta,vsl, &
           ...
           if (ilvsr) call la_ylaset('FULL',n,n,czero,cone,vsr,ldvsr)
           ! reduce to generalized hessenberg form
           call la_ygghd3(jobvsl,jobvsr,n,ilo,ihi,a,lda,b,ldb,vsl,ldvsl,vsr,ldvsr, &
                      work(iwrk),lwork + 1 - iwrk,ierr)
           sdim = 0
           ! perform qz algorithm, computing schur vectors if desired
           iwrk = itau
           call la_ylaqz0('S',jobvsl,jobvsr,n,ilo,ihi,a,lda,b,ldb,alpha,beta,vsl, &
                     ldvsl,vsr,ldvsr,work(iwrk),lwork + 1 - iwrk,rwork(irwrk),0,ierr)
           if (ierr /= 0) then
              if (ierr > 0 .and. ierr <= n) then
                 info = ierr
...
#ifdef LA_WITH_XDP
     !> YGGEV3: computes for a pair of N-by-N complex nonsymmetric matrices
     !> (A,B), the generalized eigenvalues, and optionally, the left and/or
     !> right generalized eigenvectors.
     !> A generalized eigenvalue for a pair of matrices (A,B) is a scalar
     !> lambda or a ratio alpha/beta = lambda, such that A - lambda*B is
     ...
     subroutine la_yggev3(jobvl,jobvr,n,a,lda,b,ldb,alpha,beta,vl,ldvl,vr,ldvr, &
               work,lwork,rwork,info)
        use la_constants_xdp,only:zero,one,czero,cone
        ! -- lapack driver routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ...
        ! =====================================================================
           ...
           if (info == 0) then
              ...
              lwkopt = max(lwkopt,n + int(work(1),KIND=ilp))
              if (ilvl) then
                 call la_yungqr(n,n,n,vl,ldvl,work,work,-1,ierr)
                 lwkopt = max(lwkopt,n + int(work(1),KIND=ilp))
              end if
              if (ilv) then
                 call la_ygghd3(jobvl,jobvr,n,1,n,a,lda,b,ldb,vl,ldvl,vr,ldvr, &
                           work,-1,ierr)
                 lwkopt = max(lwkopt,n + int(work(1),KIND=ilp))
                 call la_ylaqz0('S',jobvl,jobvr,n,1,n,a,lda,b,ldb,alpha,beta,vl, &
                           ldvl,vr,ldvr,work,-1,rwork,0,ierr)
                 lwkopt = max(lwkopt,n + int(work(1),KIND=ilp))
              else
                 call la_ygghd3(jobvl,jobvr,n,1,n,a,lda,b,ldb,vl,ldvl,vr,ldvr, &
                           work,-1,ierr)
                 lwkopt = max(lwkopt,n + int(work(1),KIND=ilp))
                 call la_ylaqz0('E',jobvl,jobvr,n,1,n,a,lda,b,ldb,alpha,beta,vl, &
                           ldvl,vr,ldvr,work,-1,rwork,0,ierr)
                 lwkopt = max(lwkopt,n + int(work(1),KIND=ilp))
              end if
              work(1) = cmplx(lwkopt,KIND=xdp)
           ...
           if (ilv) then
              ! eigenvectors requested -- work on whole matrix.
              call la_ygghd3(jobvl,jobvr,n,ilo,ihi,a,lda,b,ldb,vl,ldvl,vr,ldvr, &
                        work(iwrk),lwork + 1 - iwrk,ierr)
           else
              call la_ygghd3('N','N',irows,1,irows,a(ilo,ilo),lda,b(ilo,ilo), &
                        ldb,vl,ldvl,vr,ldvr,work(iwrk),lwork + 1 - iwrk,ierr)
           end if
           ! perform qz algorithm (compute eigenvalues, and optionally, the
           ! schur form and schur vectors)
           ...
           call la_ylaqz0(chtemp,jobvl,jobvr,n,ilo,ihi,a,lda,b,ldb,alpha,beta,vl, &
                     ldvl,vr,ldvr,work(iwrk),lwork + 1 - iwrk,rwork(irwrk),0,ierr)
           if (ierr /= 0) then
              if (ierr > 0 .and. ierr <= n) then
                 info = ierr
