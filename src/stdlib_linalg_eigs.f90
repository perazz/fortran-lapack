module la_eig
     ...
     interface eig
        module procedure la_eig_s
        module procedure la_eig_d
        module procedure la_eig_q
        module procedure la_eig_c
        module procedure la_eig_z
        module procedure la_eig_w
     end interface eig

     interface eigvals
        module procedure la_eigvals_s
        module procedure la_eigvals_noerr_s
        module procedure la_eigvals_d
        module procedure la_eigvals_noerr_d
        module procedure la_eigvals_q
        module procedure la_eigvals_noerr_q
        module procedure la_eigvals_c
        module procedure la_eigvals_noerr_c
        module procedure la_eigvals_z
        module procedure la_eigvals_noerr_z
        module procedure la_eigvals_w
        module procedure la_eigvals_noerr_w
     ...
     function la_eigvals_s(a,err) result(lambda)
         !> Input matrix A[m,n]
         ...
         call la_eig_s(amat,lambda,overwrite_a=.false.,err=err)

     end function la_eigvals_s

     !> Return an array of eigenvalues of matrix A.
     function la_eigvals_noerr_s(a) result(lambda)
         !> Input matrix A[m,n]
         real(sp),intent(in),target :: a(:,:)
         !> Array of singular values
         complex(sp),allocatable :: lambda(:)
         ...
         !> Compute eigenvalues only
         call la_eig_s(amat,lambda,overwrite_a=.false.)

     end function la_eigvals_noerr_s

     !> Eigendecomposition of matrix A returning an array `lambda` of eigenvalues,
     !> and optionally right or left eigenvectors.
     subroutine la_eig_s(a,lambda,right,left,overwrite_a,err)
         !> Input matrix A[m,n]
...
2        if (copy_a) deallocate (amat)
     ...
     !> Return an array of eigenvalues of matrix A.
     function la_eigvals_d(a,err) result(lambda)
         !> Input matrix A[m,n]
         ...
         !> Compute eigenvalues only
         call la_eig_d(amat,lambda,overwrite_a=.false.,err=err)

     end function la_eigvals_d

     !> Return an array of eigenvalues of matrix A.
     function la_eigvals_noerr_d(a) result(lambda)
         !> Input matrix A[m,n]
         real(dp),intent(in),target :: a(:,:)
         !> Array of singular values
         complex(dp),allocatable :: lambda(:)
         ...
         !> Compute eigenvalues only
         call la_eig_d(amat,lambda,overwrite_a=.false.)

     end function la_eigvals_noerr_d

     !> Eigendecomposition of matrix A returning an array `lambda` of eigenvalues,
     !> and optionally right or left eigenvectors.
     subroutine la_eig_d(a,lambda,right,left,overwrite_a,err)
         !> Input matrix A[m,n]
...
2        if (copy_a) deallocate (amat)
     ...
     !> Return an array of eigenvalues of matrix A.
     function la_eigvals_q(a,err) result(lambda)
         !> Input matrix A[m,n]
         ...
         !> Compute eigenvalues only
         call la_eig_q(amat,lambda,overwrite_a=.false.,err=err)

     end function la_eigvals_q

     !> Return an array of eigenvalues of matrix A.
     function la_eigvals_noerr_q(a) result(lambda)
         !> Input matrix A[m,n]
         real(qp),intent(in),target :: a(:,:)
         !> Array of singular values
         complex(qp),allocatable :: lambda(:)
         ...
         !> Compute eigenvalues only
         call la_eig_q(amat,lambda,overwrite_a=.false.)

     end function la_eigvals_noerr_q

     !> Eigendecomposition of matrix A returning an array `lambda` of eigenvalues,
     !> and optionally right or left eigenvectors.
     subroutine la_eig_q(a,lambda,right,left,overwrite_a,err)
         !> Input matrix A[m,n]
...
2        if (copy_a) deallocate (amat)
     ...
     !> Return an array of eigenvalues of matrix A.
     function la_eigvals_c(a,err) result(lambda)
         !> Input matrix A[m,n]
         ...
         !> Compute eigenvalues only
         call la_eig_c(amat,lambda,overwrite_a=.false.,err=err)

     end function la_eigvals_c

     !> Return an array of eigenvalues of matrix A.
     function la_eigvals_noerr_c(a) result(lambda)
         !> Input matrix A[m,n]
         complex(sp),intent(in),target :: a(:,:)
         !> Array of singular values
         complex(sp),allocatable :: lambda(:)
         ...
         !> Compute eigenvalues only
         call la_eig_c(amat,lambda,overwrite_a=.false.)

     end function la_eigvals_noerr_c

     !> Eigendecomposition of matrix A returning an array `lambda` of eigenvalues,
     !> and optionally right or left eigenvectors.
     subroutine la_eig_c(a,lambda,right,left,overwrite_a,err)
         !> Input matrix A[m,n]
...
2        if (copy_a) deallocate (amat)
     ...
     !> Return an array of eigenvalues of matrix A.
     function la_eigvals_z(a,err) result(lambda)
         !> Input matrix A[m,n]
         ...
         !> Compute eigenvalues only
         call la_eig_z(amat,lambda,overwrite_a=.false.,err=err)

     end function la_eigvals_z

     !> Return an array of eigenvalues of matrix A.
     function la_eigvals_noerr_z(a) result(lambda)
         !> Input matrix A[m,n]
         complex(dp),intent(in),target :: a(:,:)
         !> Array of singular values
         complex(dp),allocatable :: lambda(:)
         ...
         !> Compute eigenvalues only
         call la_eig_z(amat,lambda,overwrite_a=.false.)

     end function la_eigvals_noerr_z

     !> Eigendecomposition of matrix A returning an array `lambda` of eigenvalues,
     !> and optionally right or left eigenvectors.
     subroutine la_eig_z(a,lambda,right,left,overwrite_a,err)
         !> Input matrix A[m,n]
...
2        if (copy_a) deallocate (amat)
     ...
     !> Return an array of eigenvalues of matrix A.
     function la_eigvals_w(a,err) result(lambda)
         !> Input matrix A[m,n]
         ...
         !> Compute eigenvalues only
         call la_eig_w(amat,lambda,overwrite_a=.false.,err=err)

     end function la_eigvals_w

     !> Return an array of eigenvalues of matrix A.
     function la_eigvals_noerr_w(a) result(lambda)
         !> Input matrix A[m,n]
         complex(qp),intent(in),target :: a(:,:)
         !> Array of singular values
         complex(qp),allocatable :: lambda(:)
         ...
         !> Compute eigenvalues only
         call la_eig_w(amat,lambda,overwrite_a=.false.)

     end function la_eigvals_noerr_w

     !> Eigendecomposition of matrix A returning an array `lambda` of eigenvalues,
     !> and optionally right or left eigenvectors.
     subroutine la_eig_w(a,lambda,right,left,overwrite_a,err)
         !> Input matrix A[m,n]
