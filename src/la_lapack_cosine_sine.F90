...
module la_lapack_cosine_sine
...
#ifdef LA_WITH_XDP
     !> XLAPMR: rearranges the rows of the M by N matrix X as specified
     !> by the permutation K(1),K(2),...,K(M) of the integers 1,...,M.
     !> If FORWRD = .TRUE.,  forward permutation:
     !> X(K(I),*) is moved X(I,*) for I = 1,2,...,M.
     !> If FORWRD = .FALSE., backward permutation:
     ...
     pure subroutine la_xlapmr(forwrd,m,n,x,ldx,k)
        ! -- lapack auxiliary routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ! Scalar Arguments
     ...
     pure subroutine la_xlapmt(forwrd,m,n,x,ldx,k)
        ! -- lapack auxiliary routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ! Scalar Arguments
...
#ifdef LA_WITH_XDP
     !> XORBDB6: orthogonalizes the column vector
     !> X = [ X1 ]
     !> [ X2 ]
     !> with respect to the columns of
     !> Q = [ Q1 ] .
     ...
     pure subroutine la_xorbdb6(m1,m2,n,x1,incx1,x2,incx2,q1,ldq1,q2,ldq2,work, &
               lwork,info)
        ...
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ...
           integer(ilp),intent(out) :: info
           ! Array Arguments
           real(xdp),intent(in) :: q1(ldq1,*),q2(ldq2,*)
           real(xdp),intent(out) :: work(*)
           real(xdp),intent(inout) :: x1(*),x2(*)
        ! =====================================================================
           ! Parameters
           real(xdp),parameter :: alphasq = 0.01_xdp
           real(xdp),parameter :: realone = 1.0_xdp
           real(xdp),parameter :: realzero = 0.0_xdp

           ! Local Scalars
           integer(ilp) :: i
           real(xdp) :: normsq1,normsq2,scl1,scl2,ssq1,ssq2
           ! Intrinsic Function
           intrinsic :: max
           ! Executable Statements
           ! test input arguments
           ...
           scl1 = realzero
           ssq1 = realone
           call la_xlassq(m1,x1,incx1,scl1,ssq1)
           scl2 = realzero
           ssq2 = realone
           call la_xlassq(m2,x2,incx2,scl2,ssq2)
           normsq1 = scl1**2*ssq1 + scl2**2*ssq2
           if (m1 == 0) then
              do i = 1,n
                 work(i) = zero
              ...
              call la_xgemv('C',m1,n,one,q1,ldq1,x1,incx1,zero,work,1)
           end if
           call la_xgemv('C',m2,n,one,q2,ldq2,x2,incx2,one,work,1)
           call la_xgemv('N',m1,n,negone,q1,ldq1,work,1,one,x1,incx1)
           call la_xgemv('N',m2,n,negone,q2,ldq2,work,1,one,x2,incx2)
           scl1 = realzero
           ssq1 = realone
           call la_xlassq(m1,x1,incx1,scl1,ssq1)
           scl2 = realzero
...
#ifdef LA_WITH_XDP
     !> XBBCSD: computes the CS decomposition of an orthogonal matrix in
     !> bidiagonal-block form,
     !> [ B11 | B12 0  0 ]
     !> [  0  |  0 -I  0 ]
     !> X = [----------------]
     ...
     !> than P, M-P, or M-Q. (If Q is not the smallest index, then X must be
     !> transposed and/or permuted. This can be done in constant time using
     !> the TRANS and SIGNS options. See XORCSD for details.)
     !> The bidiagonal matrices B11, B12, B21, and B22 are represented
     !> implicitly by angles THETA(1:Q) and PHI(1:Q-1).
     !> The orthogonal matrices U1, U2, V1T, and V2T are input/output.
     !> The input matrices are pre- or post-multiplied by the appropriate
     ...
     pure subroutine la_xbbcsd(jobu1,jobu2,jobv1t,jobv2t,trans,m,p,q,theta,phi,u1, &
     ldu1,u2,ldu2,v1t,ldv1t,v2t,ldv2t,b11d,b11e,b12d,b12e,b21d,b21e,b22d,b22e,work, &
               lwork,info)
        ...
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ...
           integer(ilp),intent(in) :: ldu1,ldu2,ldv1t,ldv2t,lwork,m,p,q
           ! Array Arguments
           real(xdp),intent(out) :: b11d(*),b11e(*),b12d(*),b12e(*),b21d(*),b21e(*),b22d(*), &
                      b22e(*),work(*)
           real(xdp),intent(inout) :: phi(*),theta(*)
           real(xdp),intent(inout) :: u1(ldu1,*),u2(ldu2,*),v1t(ldv1t,*),v2t(ldv2t,*)
        ! ===================================================================
           ! Parameters
           integer(ilp),parameter :: maxitr = 6
           real(xdp),parameter :: hundred = 100.0_xdp
           real(xdp),parameter :: meighth = -0.125_xdp
           real(xdp),parameter :: piover2 = 1.57079632679489661923132169163975144210_xdp
           ...
           integer(ilp) :: i,imin,imax,iter,iu1cs,iu1sn,iu2cs,iu2sn,iv1tcs,iv1tsn, &
                     iv2tcs,iv2tsn,j,lworkmin,lworkopt,maxit,mini
           real(xdp) :: b11bulge,b12bulge,b21bulge,b22bulge,dummy,eps,mu,nu,r,sigma11, &
                     sigma21,temp,thetamax,thetamin,thresh,tol,tolmul,unfl,x1,x2,y1,y2
           ! Intrinsic Functions
           intrinsic :: abs,atan2,cos,max,min,sin,sqrt
           ! Executable Statements
           ...
           do while (imax > 1)
              ...
              if (thetamax > piover2 - thresh) then
                 ...
                 ! compute shifts for b11 and b21 and use the lesser
                 call la_xlas2(b11d(imax - 1),b11e(imax - 1),b11d(imax),sigma11,dummy)

                 call la_xlas2(b21d(imax - 1),b21e(imax - 1),b21d(imax),sigma21,dummy)
              ...
              ! rotate to produce bulges in b11 and b21
              if (mu <= nu) then
                 call la_xlartgs(b11d(imin),b11e(imin),mu,work(iv1tcs + imin - 1),work(iv1tsn + &
                           imin - 1))
              else
                 call la_xlartgs(b21d(imin),b21e(imin),nu,work(iv1tcs + imin - 1),work(iv1tsn + &
                           imin - 1))
              end if
              temp = work(iv1tcs + imin - 1)*b11d(imin) + work(iv1tsn + imin - 1)*b11e(imin)
              b11e(imin) = work(iv1tcs + imin - 1)*b11e(imin) - work(iv1tsn + imin - 1)*b11d(imin)
              ...
              ! chase the bulges in b11(imin+1,imin) and b21(imin+1,imin)
              if (b11d(imin)**2 + b11bulge**2 > thresh**2) then
                 call la_xlartgp(b11bulge,b11d(imin),work(iu1sn + imin - 1),work(iu1cs + imin - 1), &
                            r)
              else if (mu <= nu) then
                 call la_xlartgs(b11e(imin),b11d(imin + 1),mu,work(iu1cs + imin - 1),work( &
                           iu1sn + imin - 1))
              else
                 call la_xlartgs(b12d(imin),b12e(imin),nu,work(iu1cs + imin - 1),work( &
                           iu1sn + imin - 1))
              end if
              if (b21d(imin)**2 + b21bulge**2 > thresh**2) then
                 call la_xlartgp(b21bulge,b21d(imin),work(iu2sn + imin - 1),work(iu2cs + imin - 1), &
                            r)
              else if (nu < mu) then
                 call la_xlartgs(b21e(imin),b21d(imin + 1),nu,work(iu2cs + imin - 1),work( &
                           iu2sn + imin - 1))
              else
                 call la_xlartgs(b22d(imin),b22e(imin),mu,work(iu2cs + imin - 1),work(iu2sn + &
                           imin - 1))
              end if
              work(iu2cs + imin - 1) = -work(iu2cs + imin - 1)
              work(iu2sn + imin - 1) = -work(iu2sn + imin - 1)
              ...
              do i = imin + 1,imax - 1
                 ...
                 ! chasing by applying the original shift again.
                 if (.not. restart11 .and. .not. restart21) then
                    call la_xlartgp(x2,x1,work(iv1tsn + i - 1),work(iv1tcs + i - 1),r)
                 else if (.not. restart11 .and. restart21) then
                    call la_xlartgp(b11bulge,b11e(i - 1),work(iv1tsn + i - 1),work(iv1tcs + i - 1), &
                              r)
                 else if (restart11 .and. .not. restart21) then
                    call la_xlartgp(b21bulge,b21e(i - 1),work(iv1tsn + i - 1),work(iv1tcs + i - 1), &
                              r)
                 else if (mu <= nu) then
                    call la_xlartgs(b11d(i),b11e(i),mu,work(iv1tcs + i - 1),work(iv1tsn + i - 1))

                 else
                    call la_xlartgs(b21d(i),b21e(i),nu,work(iv1tcs + i - 1),work(iv1tsn + i - 1))

                 end if
                 work(iv1tcs + i - 1) = -work(iv1tcs + i - 1)
                 work(iv1tsn + i - 1) = -work(iv1tsn + i - 1)
                 if (.not. restart12 .and. .not. restart22) then
                    call la_xlartgp(y2,y1,work(iv2tsn + i - 1 - 1),work(iv2tcs + i - 1 - 1),r)

                 else if (.not. restart12 .and. restart22) then
                    call la_xlartgp(b12bulge,b12d(i - 1),work(iv2tsn + i - 1 - 1),work(iv2tcs + i - 1 - &
                              1),r)
                 else if (restart12 .and. .not. restart22) then
                    call la_xlartgp(b22bulge,b22d(i - 1),work(iv2tsn + i - 1 - 1),work(iv2tcs + i - 1 - &
                              1),r)
                 else if (nu < mu) then
                    call la_xlartgs(b12e(i - 1),b12d(i),nu,work(iv2tcs + i - 1 - 1),work(iv2tsn + i - &
                              1 - 1))
                 else
                    call la_xlartgs(b22e(i - 1),b22d(i),mu,work(iv2tcs + i - 1 - 1),work(iv2tsn + i - &
                              1 - 1))
                 end if
                 temp = work(iv1tcs + i - 1)*b11d(i) + work(iv1tsn + i - 1)*b11e(i)
                 b11e(i) = work(iv1tcs + i - 1)*b11e(i) - work(iv1tsn + i - 1)*b11d(i)
                 ...
                 ! chasing by applying the original shift again.
                 if (.not. restart11 .and. .not. restart12) then
                    call la_xlartgp(x2,x1,work(iu1sn + i - 1),work(iu1cs + i - 1),r)
                 else if (.not. restart11 .and. restart12) then
                    call la_xlartgp(b11bulge,b11d(i),work(iu1sn + i - 1),work(iu1cs + i - 1),r)

                 else if (restart11 .and. .not. restart12) then
                    call la_xlartgp(b12bulge,b12e(i - 1),work(iu1sn + i - 1),work(iu1cs + i - 1),r)

                 else if (mu <= nu) then
                    call la_xlartgs(b11e(i),b11d(i + 1),mu,work(iu1cs + i - 1),work(iu1sn + i - 1))

                 else
                    call la_xlartgs(b12d(i),b12e(i),nu,work(iu1cs + i - 1),work(iu1sn + i - 1))

                 end if
                 if (.not. restart21 .and. .not. restart22) then
                    call la_xlartgp(y2,y1,work(iu2sn + i - 1),work(iu2cs + i - 1),r)
                 else if (.not. restart21 .and. restart22) then
                    call la_xlartgp(b21bulge,b21d(i),work(iu2sn + i - 1),work(iu2cs + i - 1),r)

                 else if (restart21 .and. .not. restart22) then
                    call la_xlartgp(b22bulge,b22e(i - 1),work(iu2sn + i - 1),work(iu2cs + i - 1),r)

                 else if (nu < mu) then
                    call la_xlartgs(b21e(i),b21e(i + 1),nu,work(iu2cs + i - 1),work(iu2sn + i - 1))

                 else
                    call la_xlartgs(b22d(i),b22e(i),mu,work(iu2cs + i - 1),work(iu2sn + i - 1))
              ...
              restart22 = b22d(imax - 1)**2 + b22bulge**2 <= thresh**2
              if (.not. restart12 .and. .not. restart22) then
                 call la_xlartgp(y2,y1,work(iv2tsn + imax - 1 - 1),work(iv2tcs + imax - 1 - 1),r)

              else if (.not. restart12 .and. restart22) then
                 call la_xlartgp(b12bulge,b12d(imax - 1),work(iv2tsn + imax - 1 - 1),work(iv2tcs + &
                           imax - 1 - 1),r)
              else if (restart12 .and. .not. restart22) then
                 call la_xlartgp(b22bulge,b22d(imax - 1),work(iv2tsn + imax - 1 - 1),work(iv2tcs + &
                           imax - 1 - 1),r)
              else if (nu < mu) then
                 call la_xlartgs(b12e(imax - 1),b12d(imax),nu,work(iv2tcs + imax - 1 - 1),work( &
                           iv2tsn + imax - 1 - 1))
              else
                 call la_xlartgs(b22e(imax - 1),b22d(imax),mu,work(iv2tcs + imax - 1 - 1),work( &
                           iv2tsn + imax - 1 - 1))
              end if
              temp = work(iv2tcs + imax - 1 - 1)*b12e(imax - 1) + work(iv2tsn + imax - 1 - 1)*b12d(imax)
              b12d(imax) = work(iv2tcs + imax - 1 - 1)*b12d(imax) - work(iv2tsn + imax - 1 - 1)*b12e(imax - 1)
              ...
              if (wantu1) then
                 if (colmajor) then
                    call la_xlasr('R','V','F',p,imax - imin + 1,work(iu1cs + imin - 1),work( &
                              iu1sn + imin - 1),u1(1,imin),ldu1)
                 else
                    call la_xlasr('L','V','F',imax - imin + 1,p,work(iu1cs + imin - 1),work( &
                              iu1sn + imin - 1),u1(imin,1),ldu1)
                 end if
              end if
              if (wantu2) then
                 if (colmajor) then
                    call la_xlasr('R','V','F',m - p,imax - imin + 1,work(iu2cs + imin - 1),work( &
                              iu2sn + imin - 1),u2(1,imin),ldu2)
                 else
                    call la_xlasr('L','V','F',imax - imin + 1,m - p,work(iu2cs + imin - 1),work( &
                              iu2sn + imin - 1),u2(imin,1),ldu2)
                 end if
              end if
              if (wantv1t) then
                 if (colmajor) then
                    call la_xlasr('L','V','F',imax - imin + 1,q,work(iv1tcs + imin - 1),work( &
                              iv1tsn + imin - 1),v1t(imin,1),ldv1t)
                 else
                    call la_xlasr('R','V','F',q,imax - imin + 1,work(iv1tcs + imin - 1),work( &
                              iv1tsn + imin - 1),v1t(1,imin),ldv1t)
                 end if
              end if
              if (wantv2t) then
                 if (colmajor) then
                    call la_xlasr('L','V','F',imax - imin + 1,m - q,work(iv2tcs + imin - 1),work( &
                              iv2tsn + imin - 1),v2t(imin,1),ldv2t)
                 else
                    call la_xlasr('R','V','F',m - q,imax - imin + 1,work(iv2tcs + imin - 1),work( &
                              iv2tsn + imin - 1),v2t(1,imin),ldv2t)
                 end if
              end if
              ! fix signs on b11(imax-1,imax) and b21(imax-1,imax)
              if (b11e(imax - 1) + b21e(imax - 1) > 0) then
                 ...
                 if (wantv1t) then
                    if (colmajor) then
                       call la_xscal(q,negone,v1t(imax,1),ldv1t)
                    else
                       call la_xscal(q,negone,v1t(1,imax),1)
                    end if
                 end if
              end if
              ! compute theta(imax)
              ...
              if (b11d(imax) + b12e(imax - 1) < 0) then
                 ...
                 if (wantu1) then
                    if (colmajor) then
                       call la_xscal(p,negone,u1(1,imax),1)
                    else
                       call la_xscal(p,negone,u1(imax,1),ldu1)
                    end if
                 end if
              end if
              if (b21d(imax) + b22e(imax - 1) > 0) then
                 ...
                 if (wantu2) then
                    if (colmajor) then
                       call la_xscal(m - p,negone,u2(1,imax),1)
                    else
                       call la_xscal(m - p,negone,u2(imax,1),ldu2)
                    end if
                 end if
              end if
              ! fix signs on b12(imax,imax) and b22(imax,imax)
              if (b12d(imax) + b22d(imax) < 0) then
                 if (wantv2t) then
                    if (colmajor) then
                       call la_xscal(m - q,negone,v2t(imax,1),ldv2t)
                    else
                       call la_xscal(m - q,negone,v2t(1,imax),1)
                    end if
                 end if
              end if
              ! test for negligible sines or cosines
           ...
           do i = 1,q
              ...
              if (mini /= i) then
                 ...
                 theta(i) = thetamin
                 if (colmajor) then
                    if (wantu1) call la_xswap(p,u1(1,i),1,u1(1,mini),1)
                    if (wantu2) call la_xswap(m - p,u2(1,i),1,u2(1,mini),1)
                    if (wantv1t) call la_xswap(q,v1t(i,1),ldv1t,v1t(mini,1),ldv1t)

                    if (wantv2t) call la_xswap(m - q,v2t(i,1),ldv2t,v2t(mini,1),ldv2t)

                 else
                    if (wantu1) call la_xswap(p,u1(i,1),ldu1,u1(mini,1),ldu1)
                    if (wantu2) call la_xswap(m - p,u2(i,1),ldu2,u2(mini,1),ldu2)
                    if (wantv1t) call la_xswap(q,v1t(1,i),1,v1t(1,mini),1)
                    if (wantv2t) call la_xswap(m - q,v2t(1,i),1,v2t(1,mini),1)
                 end if
              end if
           end do
           return
     end subroutine la_xbbcsd
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
     !> XORBDB: simultaneously bidiagonalizes the blocks of an M-by-M
     !> partitioned orthogonal matrix X:
     !> [ B11 | B12 0  0 ]
     !> [ X11 | X12 ]   [ P1 |    ] [  0  |  0 -I  0 ] [ Q1 |    ]**T
     !> X = [-----------] = [---------] [----------------] [---------]   .
     ...
     !> X11 is P-by-Q. Q must be no larger than P, M-P, or M-Q. (If this is
     !> not the case, then X must be transposed and/or permuted. This can be
     !> done in constant time using the TRANS and SIGNS options. See XORCSD
     !> for details.)
     !> The orthogonal matrices P1, P2, Q1, and Q2 are P-by-P, (M-P)-by-
     !> (M-P), Q-by-Q, and (M-Q)-by-(M-Q), respectively. They are
     !> represented implicitly by Householder vectors.
     ...
     subroutine la_xorbdb(trans,signs,m,p,q,x11,ldx11,x12,ldx12,x21,ldx21,x22, &
               ldx22,theta,phi,taup1,taup2,tauq1,tauq2,work,lwork,info)
        use la_constants_xdp
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ...
           integer(ilp),intent(in) :: ldx11,ldx12,ldx21,ldx22,lwork,m,p,q
           ! Array Arguments
           real(xdp),intent(out) :: phi(*),theta(*)
           real(xdp),intent(out) :: taup1(*),taup2(*),tauq1(*),tauq2(*),work(*)
           real(xdp),intent(inout) :: x11(ldx11,*),x12(ldx12,*),x21(ldx21,*),x22(ldx22,*)
        ...
        ! ====================================================================
           ...
           if (colmajor) then
              ...
              do i = 1,q
                 if (i == 1) then
                    call la_xscal(p - i + 1,z1,x11(i,i),1)
                 else
                    call la_xscal(p - i + 1,z1*cos(phi(i - 1)),x11(i,i),1)
                    call la_xaxpy(p - i + 1,-z1*z3*z4*sin(phi(i - 1)),x12(i,i - 1),1,x11(i,i),1)

                 end if
                 if (i == 1) then
                    call la_xscal(m - p - i + 1,z2,x21(i,i),1)
                 else
                    call la_xscal(m - p - i + 1,z2*cos(phi(i - 1)),x21(i,i),1)
                    call la_xaxpy(m - p - i + 1,-z2*z3*z4*sin(phi(i - 1)),x22(i,i - 1),1,x21(i,i), &
                              1)
                 end if
                 theta(i) = atan2(la_xnrm2(m - p - i + 1,x21(i,i),1),la_xnrm2(p - i + 1,x11( &
                           i,i),1))
                 if (p > i) then
                    call la_xlarfgp(p - i + 1,x11(i,i),x11(i + 1,i),1,taup1(i))
                 else if (p == i) then
                    call la_xlarfgp(p - i + 1,x11(i,i),x11(i,i),1,taup1(i))
                 end if
                 x11(i,i) = one
                 if (m - p > i) then
                    call la_xlarfgp(m - p - i + 1,x21(i,i),x21(i + 1,i),1,taup2(i))
                 else if (m - p == i) then
                    call la_xlarfgp(m - p - i + 1,x21(i,i),x21(i,i),1,taup2(i))
                 end if
                 x21(i,i) = one
                 if (q > i) then
                    call la_xlarf('L',p - i + 1,q - i,x11(i,i),1,taup1(i),x11(i,i + 1),ldx11, &
                              work)
                 end if
                 if (m - q + 1 > i) then
                    call la_xlarf('L',p - i + 1,m - q - i + 1,x11(i,i),1,taup1(i),x12(i,i),ldx12, &
                               work)
                 end if
                 if (q > i) then
                    call la_xlarf('L',m - p - i + 1,q - i,x21(i,i),1,taup2(i),x21(i,i + 1),ldx21, &
                               work)
                 end if
                 if (m - q + 1 > i) then
                    call la_xlarf('L',m - p - i + 1,m - q - i + 1,x21(i,i),1,taup2(i),x22(i,i), &
                              ldx22,work)
                 end if
                 if (i < q) then
                    call la_xscal(q - i,-z1*z3*sin(theta(i)),x11(i,i + 1),ldx11)
                    call la_xaxpy(q - i,z2*z3*cos(theta(i)),x21(i,i + 1),ldx21,x11(i,i + 1), &
                              ldx11)
                 end if
                 call la_xscal(m - q - i + 1,-z1*z4*sin(theta(i)),x12(i,i),ldx12)
                 call la_xaxpy(m - q - i + 1,z2*z4*cos(theta(i)),x22(i,i),ldx22,x12(i,i),ldx12 &
                           )
                 if (i < q) phi(i) = atan2(la_xnrm2(q - i,x11(i,i + 1),ldx11),la_xnrm2( &
                           m - q - i + 1,x12(i,i),ldx12))
                 if (i < q) then
                    if (q - i == 1) then
                       call la_xlarfgp(q - i,x11(i,i + 1),x11(i,i + 1),ldx11,tauq1(i))
                    else
                       call la_xlarfgp(q - i,x11(i,i + 1),x11(i,i + 2),ldx11,tauq1(i))
                    end if
                    x11(i,i + 1) = one
                 end if
                 if (q + i - 1 < m) then
                    if (m - q == i) then
                       call la_xlarfgp(m - q - i + 1,x12(i,i),x12(i,i),ldx12,tauq2(i))
                    else
                       call la_xlarfgp(m - q - i + 1,x12(i,i),x12(i,i + 1),ldx12,tauq2(i))

                    end if
                 end if
                 x12(i,i) = one
                 if (i < q) then
                    call la_xlarf('R',p - i,q - i,x11(i,i + 1),ldx11,tauq1(i),x11(i + 1,i + 1), &
                              ldx11,work)
                    call la_xlarf('R',m - p - i,q - i,x11(i,i + 1),ldx11,tauq1(i),x21(i + 1,i + 1), &
                              ldx21,work)
                 end if
                 if (p > i) then
                    call la_xlarf('R',p - i,m - q - i + 1,x12(i,i),ldx12,tauq2(i),x12(i + 1,i), &
                              ldx12,work)
                 end if
                 if (m - p > i) then
                    call la_xlarf('R',m - p - i,m - q - i + 1,x12(i,i),ldx12,tauq2(i),x22(i + 1,i), &
                              ldx22,work)
                 end if
              end do
              ! reduce columns q + 1, ..., p of x12, x22
              do i = q + 1,p
                 call la_xscal(m - q - i + 1,-z1*z4,x12(i,i),ldx12)
                 if (i >= m - q) then
                    call la_xlarfgp(m - q - i + 1,x12(i,i),x12(i,i),ldx12,tauq2(i))
                 else
                    call la_xlarfgp(m - q - i + 1,x12(i,i),x12(i,i + 1),ldx12,tauq2(i))
                 ...
                 if (m - p - q >= 1) call la_xlarf('R',m - p - q,m - q - i + 1,x12(i,i),ldx12,tauq2(i), &
                            x22(q + 1,i),ldx22,work)
              end do
              ! reduce columns p + 1, ..., m - q of x12, x22
              do i = 1,m - p - q
                 call la_xscal(m - p - q - i + 1,z2*z4,x22(q + i,p + i),ldx22)
                 if (i == m - p - q) then
                    call la_xlarfgp(m - p - q - i + 1,x22(q + i,p + i),x22(q + i,p + i),ldx22,tauq2(p + i))

                 else
                    call la_xlarfgp(m - p - q - i + 1,x22(q + i,p + i),x22(q + i,p + i + 1),ldx22,tauq2(p + i) &
                               )
                 end if
                 x22(q + i,p + i) = one
                 if (i < m - p - q) then
                    call la_xlarf('R',m - p - q - i,m - p - q - i + 1,x22(q + i,p + i),ldx22,tauq2(p + i), &
                              x22(q + i + 1,p + i),ldx22,work)
                 end if
              end do
           else
              ...
              do i = 1,q
                 if (i == 1) then
                    call la_xscal(p - i + 1,z1,x11(i,i),ldx11)
                 else
                    call la_xscal(p - i + 1,z1*cos(phi(i - 1)),x11(i,i),ldx11)
                    call la_xaxpy(p - i + 1,-z1*z3*z4*sin(phi(i - 1)),x12(i - 1,i),ldx12,x11(i,i), &
                               ldx11)
                 end if
                 if (i == 1) then
                    call la_xscal(m - p - i + 1,z2,x21(i,i),ldx21)
                 else
                    call la_xscal(m - p - i + 1,z2*cos(phi(i - 1)),x21(i,i),ldx21)
                    call la_xaxpy(m - p - i + 1,-z2*z3*z4*sin(phi(i - 1)),x22(i - 1,i),ldx22,x21(i, &
                              i),ldx21)
                 end if
                 theta(i) = atan2(la_xnrm2(m - p - i + 1,x21(i,i),ldx21),la_xnrm2(p - i + 1, &
                           x11(i,i),ldx11))
                 call la_xlarfgp(p - i + 1,x11(i,i),x11(i,i + 1),ldx11,taup1(i))
                 x11(i,i) = one
                 if (i == m - p) then
                    call la_xlarfgp(m - p - i + 1,x21(i,i),x21(i,i),ldx21,taup2(i))
                 else
                    call la_xlarfgp(m - p - i + 1,x21(i,i),x21(i,i + 1),ldx21,taup2(i))
                 end if
                 x21(i,i) = one
                 if (q > i) then
                    call la_xlarf('R',q - i,p - i + 1,x11(i,i),ldx11,taup1(i),x11(i + 1,i), &
                              ldx11,work)
                 end if
                 if (m - q + 1 > i) then
                    call la_xlarf('R',m - q - i + 1,p - i + 1,x11(i,i),ldx11,taup1(i),x12(i,i), &
                              ldx12,work)
                 end if
                 if (q > i) then
                    call la_xlarf('R',q - i,m - p - i + 1,x21(i,i),ldx21,taup2(i),x21(i + 1,i), &
                              ldx21,work)
                 end if
                 if (m - q + 1 > i) then
                    call la_xlarf('R',m - q - i + 1,m - p - i + 1,x21(i,i),ldx21,taup2(i),x22(i,i), &
                              ldx22,work)
                 end if
                 if (i < q) then
                    call la_xscal(q - i,-z1*z3*sin(theta(i)),x11(i + 1,i),1)
                    call la_xaxpy(q - i,z2*z3*cos(theta(i)),x21(i + 1,i),1,x11(i + 1,i),1)

                 end if
                 call la_xscal(m - q - i + 1,-z1*z4*sin(theta(i)),x12(i,i),1)
                 call la_xaxpy(m - q - i + 1,z2*z4*cos(theta(i)),x22(i,i),1,x12(i,i),1)

                 if (i < q) phi(i) = atan2(la_xnrm2(q - i,x11(i + 1,i),1),la_xnrm2(m - q - &
                           i + 1,x12(i,i),1))
                 if (i < q) then
                    if (q - i == 1) then
                       call la_xlarfgp(q - i,x11(i + 1,i),x11(i + 1,i),1,tauq1(i))
                    else
                       call la_xlarfgp(q - i,x11(i + 1,i),x11(i + 2,i),1,tauq1(i))
                    end if
                    x11(i + 1,i) = one
                 end if
                 if (m - q > i) then
                    call la_xlarfgp(m - q - i + 1,x12(i,i),x12(i + 1,i),1,tauq2(i))
                 else
                    call la_xlarfgp(m - q - i + 1,x12(i,i),x12(i,i),1,tauq2(i))
                 end if
                 x12(i,i) = one
                 if (i < q) then
                    call la_xlarf('L',q - i,p - i,x11(i + 1,i),1,tauq1(i),x11(i + 1,i + 1),ldx11, &
                               work)
                    call la_xlarf('L',q - i,m - p - i,x11(i + 1,i),1,tauq1(i),x21(i + 1,i + 1), &
                              ldx21,work)
                 end if
                 call la_xlarf('L',m - q - i + 1,p - i,x12(i,i),1,tauq2(i),x12(i,i + 1),ldx12, &
                           work)
                 if (m - p - i > 0) then
                    call la_xlarf('L',m - q - i + 1,m - p - i,x12(i,i),1,tauq2(i),x22(i,i + 1), &
                              ldx22,work)
                 end if
              end do
              ! reduce columns q + 1, ..., p of x12, x22
              do i = q + 1,p
                 call la_xscal(m - q - i + 1,-z1*z4,x12(i,i),1)
                 call la_xlarfgp(m - q - i + 1,x12(i,i),x12(i + 1,i),1,tauq2(i))
                 x12(i,i) = one
                 if (p > i) then
                    call la_xlarf('L',m - q - i + 1,p - i,x12(i,i),1,tauq2(i),x12(i,i + 1),ldx12, &
                 ...
                 if (m - p - q >= 1) call la_xlarf('L',m - q - i + 1,m - p - q,x12(i,i),1,tauq2(i), &
                           x22(i,q + 1),ldx22,work)
              end do
              ! reduce columns p + 1, ..., m - q of x12, x22
              do i = 1,m - p - q
                 call la_xscal(m - p - q - i + 1,z2*z4,x22(p + i,q + i),1)
                 if (m - p - q == i) then
                    call la_xlarfgp(m - p - q - i + 1,x22(p + i,q + i),x22(p + i,q + i),1,tauq2(p + i))

                 else
                    call la_xlarfgp(m - p - q - i + 1,x22(p + i,q + i),x22(p + i + 1,q + i),1,tauq2(p + i))

                    call la_xlarf('L',m - p - q - i + 1,m - p - q - i,x22(p + i,q + i),1,tauq2(p + i),x22(p + &
                              i,q + i + 1),ldx22,work)
                 end if
                 x22(p + i,q + i) = one
              end do
...
#ifdef LA_WITH_XDP
     !> XORBDB5: orthogonalizes the column vector
     !> X = [ X1 ]
     !> [ X2 ]
     !> with respect to the columns of
     !> Q = [ Q1 ] .
     ...
     pure subroutine la_xorbdb5(m1,m2,n,x1,incx1,x2,incx2,q1,ldq1,q2,ldq2,work, &
               lwork,info)
        use la_constants_xdp
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ...
        ! =====================================================================
           ...
           if (info /= 0) then
              call la_xerbla('XORBDB5',-info)
              return
           end if
           ! project x onto the orthogonal complement of q
           call la_xorbdb6(m1,m2,n,x1,incx1,x2,incx2,q1,ldq1,q2,ldq2,work,lwork, &
                     childinfo)
           ! if the projection is nonzero, then return
           if (la_xnrm2(m1,x1,incx1) /= zero .or. la_xnrm2(m2,x2,incx2) /= zero) &
                     then
              return
           end if
           ! project each standard basis vector e_1,...,e_m1 in turn, stopping
           ...
           do i = 1,m1
              ...
              call la_xorbdb6(m1,m2,n,x1,incx1,x2,incx2,q1,ldq1,q2,ldq2,work, &
                        lwork,childinfo)
              if (la_xnrm2(m1,x1,incx1) /= zero .or. la_xnrm2(m2,x2,incx2) /= zero) &
                        then
                 return
...
#ifdef LA_WITH_XDP
     !> XORCSD: computes the CS decomposition of an M-by-M partitioned
     !> orthogonal matrix X:
     !> [  I  0  0 |  0  0  0 ]
     !> [  0  C  0 |  0 -S  0 ]
     !> [ X11 | X12 ]   [ U1 |    ] [  0  0  0 |  0  0 -I ] [ V1 |    ]**T
     ...
     recursive subroutine la_xorcsd(jobu1,jobu2,jobv1t,jobv2t,trans,signs,m,p,q,x11, &
     ldx11,x12,ldx12,x21,ldx21,x22,ldx22,theta,u1,ldu1,u2,ldu2,v1t,ldv1t,v2t,ldv2t, &
               work,lwork,iwork,info)
        use la_constants_xdp
        ! -- lapack computational routine --
        ...
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ...
           ! Array Arguments
           integer(ilp),intent(out) :: iwork(*)
           real(xdp),intent(out) :: theta(*)
           real(xdp),intent(out) :: u1(ldu1,*),u2(ldu2,*),v1t(ldv1t,*),v2t(ldv2t,*),work(*)
        ...
        ! ===================================================================
           ...
           if (info == 0 .and. min(p,m - p) < min(q,m - q)) then
              ...
              call la_xorcsd(jobv1t,jobv2t,jobu1,jobu2,transt,signst,m,q,p,x11, &
              ldx11,x21,ldx21,x12,ldx12,x22,ldx22,theta,v1t,ldv1t,v2t,ldv2t,u1,ldu1, &
                        u2,ldu2,work,lwork,iwork,info)
              return
           end if
           ...
           if (info == 0 .and. m - q < q) then
              ...
              call la_xorcsd(jobu2,jobu1,jobv2t,jobv1t,trans,signst,m,m - p,m - q,x22, &
              ldx22,x21,ldx21,x12,ldx12,x11,ldx11,theta,u2,ldu2,u1,ldu1,v2t,ldv2t, &
                        v1t,ldv1t,work,lwork,iwork,info)
              return
           end if
           ...
           if (info == 0) then
              ...
              itauq2 = itauq1 + max(1,q)
              iorgqr = itauq2 + max(1,m - q)
              call la_xorgqr(m - q,m - q,m - q,u1,max(1,m - q),u1,work,-1,childinfo)
              lorgqrworkopt = int(work(1),KIND=ilp)
              lorgqrworkmin = max(1,m - q)
              iorglq = itauq2 + max(1,m - q)
              call la_xorglq(m - q,m - q,m - q,u1,max(1,m - q),u1,work,-1,childinfo)
              lorglqworkopt = int(work(1),KIND=ilp)
              lorglqworkmin = max(1,m - q)
              iorbdb = itauq2 + max(1,m - q)
              call la_xorbdb(trans,signs,m,p,q,x11,ldx11,x12,ldx12,x21,ldx21,x22, &
                        ldx22,theta,v1t,u1,u2,v1t,v2t,work,-1,childinfo)
              lorbdbworkopt = int(work(1),KIND=ilp)
              lorbdbworkmin = lorbdbworkopt
              ib11d = itauq2 + max(1,m - q)
              ...
              ib22e = ib22d + max(1,q)
              ibbcsd = ib22e + max(1,q - 1)
              call la_xbbcsd(jobu1,jobu2,jobv1t,jobv2t,trans,m,p,q,theta,theta,u1, &
              ldu1,u2,ldu2,v1t,ldv1t,v2t,ldv2t,u1,u1,u1,u1,u1,u1,u1,u1,work,-1, &
                        childinfo)
              lbbcsdworkopt = int(work(1),KIND=ilp)
              lbbcsdworkmin = lbbcsdworkopt
           ...
           if (colmajor) then
              if (wantu1 .and. p > 0) then
                 call la_xlacpy('L',p,q,x11,ldx11,u1,ldu1)
                 call la_xorgqr(p,p,q,u1,ldu1,work(itaup1),work(iorgqr),lorgqrwork, &
                           info)
              end if
              if (wantu2 .and. m - p > 0) then
                 call la_xlacpy('L',m - p,q,x21,ldx21,u2,ldu2)
                 call la_xorgqr(m - p,m - p,q,u2,ldu2,work(itaup2),work(iorgqr),lorgqrwork, &
                            info)
              end if
              if (wantv1t .and. q > 0) then
                 call la_xlacpy('U',q - 1,q - 1,x11(1,2),ldx11,v1t(2,2),ldv1t)
                 v1t(1,1) = one
                 do j = 2,q
                    v1t(1,j) = zero
                    v1t(j,1) = zero
                 end do
                 call la_xorglq(q - 1,q - 1,q - 1,v1t(2,2),ldv1t,work(itauq1),work(iorglq), &
                           lorglqwork,info)
              end if
              if (wantv2t .and. m - q > 0) then
                 call la_xlacpy('U',p,m - q,x12,ldx12,v2t,ldv2t)
                 if (m - p > q) then
                    call la_xlacpy('U',m - p - q,m - p - q,x22(q + 1,p + 1),ldx22,v2t(p + 1,p + 1), &
                              ldv2t)
                 end if
                 if (m > q) then
                    call la_xorglq(m - q,m - q,m - q,v2t,ldv2t,work(itauq2),work(iorglq), &
                              lorglqwork,info)
                 end if
              end if
           else
              if (wantu1 .and. p > 0) then
                 call la_xlacpy('U',q,p,x11,ldx11,u1,ldu1)
                 call la_xorglq(p,p,q,u1,ldu1,work(itaup1),work(iorglq),lorglqwork, &
                           info)
              end if
              if (wantu2 .and. m - p > 0) then
                 call la_xlacpy('U',q,m - p,x21,ldx21,u2,ldu2)
                 call la_xorglq(m - p,m - p,q,u2,ldu2,work(itaup2),work(iorglq),lorglqwork, &
                            info)
              end if
              if (wantv1t .and. q > 0) then
                 call la_xlacpy('L',q - 1,q - 1,x11(2,1),ldx11,v1t(2,2),ldv1t)
                 v1t(1,1) = one
                 do j = 2,q
                    v1t(1,j) = zero
                    v1t(j,1) = zero
                 end do
                 call la_xorgqr(q - 1,q - 1,q - 1,v1t(2,2),ldv1t,work(itauq1),work(iorgqr), &
                           lorgqrwork,info)
              end if
              if (wantv2t .and. m - q > 0) then
                 call la_xlacpy('L',m - q,p,x12,ldx12,v2t,ldv2t)
                 call la_xlacpy('L',m - p - q,m - p - q,x22(p + 1,q + 1),ldx22,v2t(p + 1,p + 1),ldv2t)

                 call la_xorgqr(m - q,m - q,m - q,v2t,ldv2t,work(itauq2),work(iorgqr), &
                           lorgqrwork,info)
              end if
           end if
           ! compute the csd of the matrix in bidiagonal-block form
           call la_xbbcsd(jobu1,jobu2,jobv1t,jobv2t,trans,m,p,q,theta,work(iphi),u1, &
            ldu1,u2,ldu2,v1t,ldv1t,v2t,ldv2t,work(ib11d),work(ib11e),work(ib12d),work( &
            ib12e),work(ib21d),work(ib21e),work(ib22d),work(ib22e),work(ibbcsd),lbbcsdwork, &
                      info)
           ! permute rows and columns to place identity submatrices in top-
           ...
           if (q > 0 .and. wantu2) then
              ...
              if (colmajor) then
                 call la_xlapmt(.false.,m - p,m - p,u2,ldu2,iwork)
              else
                 call la_xlapmr(.false.,m - p,m - p,u2,ldu2,iwork)
              end if
           end if
           if (m > 0 .and. wantv2t) then
              do i = 1,p
              ...
              if (.not. colmajor) then
                 call la_xlapmt(.false.,m - q,m - q,v2t,ldv2t,iwork)
              else
                 call la_xlapmr(.false.,m - q,m - q,v2t,ldv2t,iwork)
              end if
           end if
           return
           ! end la_xorcsd
     end subroutine la_xorcsd
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
     !> XORBDB1: simultaneously bidiagonalizes the blocks of a tall and skinny
     !> matrix X with orthonomal columns:
     !> [ B11 ]
     !> [ X11 ]   [ P1 |    ] [  0  ]
     !> [-----] = [---------] [-----] Q1**T .
     ...
     !> [  0  ]
     !> X11 is P-by-Q, and X21 is (M-P)-by-Q. Q must be no larger than P,
     !> M-P, or M-Q. Routines XORBDB2, XORBDB3, and XORBDB4 handle cases in
     !> which Q is not the minimum dimension.
     !> The orthogonal matrices P1, P2, and Q1 are P-by-P, (M-P)-by-(M-P),
     !> and (M-Q)-by-(M-Q), respectively. They are represented implicitly by
     !> Householder vectors.
     ...
     subroutine la_xorbdb1(m,p,q,x11,ldx11,x21,ldx21,theta,phi,taup1,taup2,tauq1, &
               work,lwork,info)
        use la_constants_xdp
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ...
           ! Array Arguments
           real(xdp),intent(out) :: phi(*),theta(*)
           real(xdp),intent(out) :: taup1(*),taup2(*),tauq1(*),work(*)
           real(xdp),intent(inout) :: x11(ldx11,*),x21(ldx21,*)
        ! ====================================================================
           ...
           do i = 1,q
              ...
              x11(i,i) = one
              x21(i,i) = one
              call la_xlarf('L',p - i + 1,q - i,x11(i,i),1,taup1(i),x11(i,i + 1),ldx11,work( &
                        ilarf))
              call la_xlarf('L',m - p - i + 1,q - i,x21(i,i),1,taup2(i),x21(i,i + 1),ldx21,work( &
                        ilarf))
              if (i < q) then
                 call la_xrot(q - i,x11(i,i + 1),ldx11,x21(i,i + 1),ldx21,c,s)
                 call la_xlarfgp(q - i,x21(i,i + 1),x21(i,i + 2),ldx21,tauq1(i))
                 s = x21(i,i + 1)
                 x21(i,i + 1) = one
                 call la_xlarf('R',p - i,q - i,x21(i,i + 1),ldx21,tauq1(i),x11(i + 1,i + 1), &
                           ldx11,work(ilarf))
                 call la_xlarf('R',m - p - i,q - i,x21(i,i + 1),ldx21,tauq1(i),x21(i + 1,i + 1), &
                           ldx21,work(ilarf))
                 c = sqrt(la_xnrm2(p - i,x11(i + 1,i + 1),1)**2 + la_xnrm2(m - p - i,x21(i + 1, &
                           i + 1),1)**2)
                 phi(i) = atan2(s,c)
                 call la_xorbdb5(p - i,m - p - i,q - i - 1,x11(i + 1,i + 1),1,x21(i + 1,i + 1),1,x11(i + 1, &
                           i + 2),ldx11,x21(i + 1,i + 2),ldx21,work(iorbdb5),lorbdb5,childinfo)
              end if
           end do
           return
     end subroutine la_xorbdb1
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
     !> XORBDB2: simultaneously bidiagonalizes the blocks of a tall and skinny
     !> matrix X with orthonomal columns:
     !> [ B11 ]
     !> [ X11 ]   [ P1 |    ] [  0  ]
     !> [-----] = [---------] [-----] Q1**T .
     ...
     !> [  0  ]
     !> X11 is P-by-Q, and X21 is (M-P)-by-Q. P must be no larger than M-P,
     !> Q, or M-Q. Routines XORBDB1, XORBDB3, and XORBDB4 handle cases in
     !> which P is not the minimum dimension.
     !> The orthogonal matrices P1, P2, and Q1 are P-by-P, (M-P)-by-(M-P),
     !> and (M-Q)-by-(M-Q), respectively. They are represented implicitly by
     !> Householder vectors.
     ...
     subroutine la_xorbdb2(m,p,q,x11,ldx11,x21,ldx21,theta,phi,taup1,taup2,tauq1, &
               work,lwork,info)
        use la_constants_xdp
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ...
        ! ====================================================================
           ...
           do i = 1,p
              if (i > 1) then
                 call la_xrot(q - i + 1,x11(i,i),ldx11,x21(i - 1,i),ldx21,c,s)
              end if
              call la_xlarfgp(q - i + 1,x11(i,i),x11(i,i + 1),ldx11,tauq1(i))
              c = x11(i,i)
              x11(i,i) = one
              call la_xlarf('R',p - i,q - i + 1,x11(i,i),ldx11,tauq1(i),x11(i + 1,i),ldx11, &
                        work(ilarf))
              call la_xlarf('R',m - p - i + 1,q - i + 1,x11(i,i),ldx11,tauq1(i),x21(i,i),ldx21, &
                        work(ilarf))
              s = sqrt(la_xnrm2(p - i,x11(i + 1,i),1)**2 + la_xnrm2(m - p - i + 1,x21(i,i),1 &
                        )**2)
              theta(i) = atan2(s,c)
              call la_xorbdb5(p - i,m - p - i + 1,q - i,x11(i + 1,i),1,x21(i,i),1,x11(i + 1,i + 1), &
                        ldx11,x21(i,i + 1),ldx21,work(iorbdb5),lorbdb5,childinfo)
              call la_xscal(p - i,negone,x11(i + 1,i),1)
              call la_xlarfgp(m - p - i + 1,x21(i,i),x21(i + 1,i),1,taup2(i))
              if (i < p) then
                 call la_xlarfgp(p - i,x11(i + 1,i),x11(i + 2,i),1,taup1(i))
                 phi(i) = atan2(x11(i + 1,i),x21(i,i))
                 c = cos(phi(i))
                 s = sin(phi(i))
                 x11(i + 1,i) = one
                 call la_xlarf('L',p - i,q - i,x11(i + 1,i),1,taup1(i),x11(i + 1,i + 1),ldx11, &
                           work(ilarf))
              end if
              x21(i,i) = one
              call la_xlarf('L',m - p - i + 1,q - i,x21(i,i),1,taup2(i),x21(i,i + 1),ldx21,work( &
...
#ifdef LA_WITH_XDP
     !> XORBDB3: simultaneously bidiagonalizes the blocks of a tall and skinny
     !> matrix X with orthonomal columns:
     !> [ B11 ]
     !> [ X11 ]   [ P1 |    ] [  0  ]
     !> [-----] = [---------] [-----] Q1**T .
     ...
     !> [  0  ]
     !> X11 is P-by-Q, and X21 is (M-P)-by-Q. M-P must be no larger than P,
     !> Q, or M-Q. Routines XORBDB1, XORBDB2, and XORBDB4 handle cases in
     !> which M-P is not the minimum dimension.
     !> The orthogonal matrices P1, P2, and Q1 are P-by-P, (M-P)-by-(M-P),
     !> and (M-Q)-by-(M-Q), respectively. They are represented implicitly by
     !> Householder vectors.
     ...
     subroutine la_xorbdb3(m,p,q,x11,ldx11,x21,ldx21,theta,phi,taup1,taup2,tauq1, &
               work,lwork,info)
        use la_constants_xdp
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ...
        ! ====================================================================
           ...
           do i = 1,m - p
              if (i > 1) then
                 call la_xrot(q - i + 1,x11(i - 1,i),ldx11,x21(i,i),ldx11,c,s)
              end if
              call la_xlarfgp(q - i + 1,x21(i,i),x21(i,i + 1),ldx21,tauq1(i))
              s = x21(i,i)
              x21(i,i) = one
              call la_xlarf('R',p - i + 1,q - i + 1,x21(i,i),ldx21,tauq1(i),x11(i,i),ldx11, &
                        work(ilarf))
              call la_xlarf('R',m - p - i,q - i + 1,x21(i,i),ldx21,tauq1(i),x21(i + 1,i),ldx21, &
                        work(ilarf))
              c = sqrt(la_xnrm2(p - i + 1,x11(i,i),1)**2 + la_xnrm2(m - p - i,x21(i + 1,i),1 &
                        )**2)
              theta(i) = atan2(s,c)
              call la_xorbdb5(p - i + 1,m - p - i,q - i,x11(i,i),1,x21(i + 1,i),1,x11(i,i + 1), &
                        ldx11,x21(i + 1,i + 1),ldx21,work(iorbdb5),lorbdb5,childinfo)
              call la_xlarfgp(p - i + 1,x11(i,i),x11(i + 1,i),1,taup1(i))
              if (i < m - p) then
                 call la_xlarfgp(m - p - i,x21(i + 1,i),x21(i + 2,i),1,taup2(i))
                 phi(i) = atan2(x21(i + 1,i),x11(i,i))
                 c = cos(phi(i))
                 s = sin(phi(i))
                 x21(i + 1,i) = one
                 call la_xlarf('L',m - p - i,q - i,x21(i + 1,i),1,taup2(i),x21(i + 1,i + 1),ldx21, &
                           work(ilarf))
              end if
              x11(i,i) = one
              call la_xlarf('L',p - i + 1,q - i,x11(i,i),1,taup1(i),x11(i,i + 1),ldx11,work( &
...
#ifdef LA_WITH_XDP
     !> XORBDB4: simultaneously bidiagonalizes the blocks of a tall and skinny
     !> matrix X with orthonomal columns:
     !> [ B11 ]
     !> [ X11 ]   [ P1 |    ] [  0  ]
     !> [-----] = [---------] [-----] Q1**T .
     ...
     !> [  0  ]
     !> X11 is P-by-Q, and X21 is (M-P)-by-Q. M-Q must be no larger than P,
     !> M-P, or Q. Routines XORBDB1, XORBDB2, and XORBDB3 handle cases in
     !> which M-Q is not the minimum dimension.
     !> The orthogonal matrices P1, P2, and Q1 are P-by-P, (M-P)-by-(M-P),
     !> and (M-Q)-by-(M-Q), respectively. They are represented implicitly by
     !> Householder vectors.
     ...
     subroutine la_xorbdb4(m,p,q,x11,ldx11,x21,ldx21,theta,phi,taup1,taup2,tauq1, &
               phantom,work,lwork,info)
        use la_constants_xdp
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ...
           ! Array Arguments
           real(xdp),intent(out) :: phi(*),theta(*)
           real(xdp),intent(out) :: phantom(*),taup1(*),taup2(*),tauq1(*),work(*)
           real(xdp),intent(inout) :: x11(ldx11,*),x21(ldx21,*)
        ! ====================================================================
           ...
           do i = 1,m - q
              if (i == 1) then
                 ...
                 call la_xorbdb5(p,m - p,q,phantom(1),1,phantom(p + 1),1,x11,ldx11,x21, &
                           ldx21,work(iorbdb5),lorbdb5,childinfo)
                 call la_xscal(p,negone,phantom(1),1)
                 call la_xlarfgp(p,phantom(1),phantom(2),1,taup1(1))
                 call la_xlarfgp(m - p,phantom(p + 1),phantom(p + 2),1,taup2(1))
                 theta(i) = atan2(phantom(1),phantom(p + 1))
                 c = cos(theta(i))
                 s = sin(theta(i))
                 phantom(1) = one
                 phantom(p + 1) = one
                 call la_xlarf('L',p,q,phantom(1),1,taup1(1),x11,ldx11,work(ilarf))

                 call la_xlarf('L',m - p,q,phantom(p + 1),1,taup2(1),x21,ldx21,work(ilarf) &
                            )
              else
                 call la_xorbdb5(p - i + 1,m - p - i + 1,q - i + 1,x11(i,i - 1),1,x21(i,i - 1),1,x11(i,i) &
                           ,ldx11,x21(i,i),ldx21,work(iorbdb5),lorbdb5,childinfo)
                 call la_xscal(p - i + 1,negone,x11(i,i - 1),1)
                 call la_xlarfgp(p - i + 1,x11(i,i - 1),x11(i + 1,i - 1),1,taup1(i))
                 call la_xlarfgp(m - p - i + 1,x21(i,i - 1),x21(i + 1,i - 1),1,taup2(i))
                 theta(i) = atan2(x11(i,i - 1),x21(i,i - 1))
                 c = cos(theta(i))
                 s = sin(theta(i))
                 x11(i,i - 1) = one
                 x21(i,i - 1) = one
                 call la_xlarf('L',p - i + 1,q - i + 1,x11(i,i - 1),1,taup1(i),x11(i,i),ldx11, &
                           work(ilarf))
                 call la_xlarf('L',m - p - i + 1,q - i + 1,x21(i,i - 1),1,taup2(i),x21(i,i),ldx21, &
                           work(ilarf))
              end if
              call la_xrot(q - i + 1,x11(i,i),ldx11,x21(i,i),ldx21,s,-c)
              call la_xlarfgp(q - i + 1,x21(i,i),x21(i,i + 1),ldx21,tauq1(i))
              c = x21(i,i)
              x21(i,i) = one
              call la_xlarf('R',p - i,q - i + 1,x21(i,i),ldx21,tauq1(i),x11(i + 1,i),ldx11, &
                        work(ilarf))
              call la_xlarf('R',m - p - i,q - i + 1,x21(i,i),ldx21,tauq1(i),x21(i + 1,i),ldx21, &
                        work(ilarf))
              if (i < m - q) then
                 s = sqrt(la_xnrm2(p - i,x11(i + 1,i),1)**2 + la_xnrm2(m - p - i,x21(i + 1,i), &
                            1)**2)
                 phi(i) = atan2(s,c)
              end if
           end do
           ...
           do i = m - q + 1,p
              ...
              call la_xlarf('R',p - i,q - i + 1,x11(i,i),ldx11,tauq1(i),x11(i + 1,i),ldx11, &
                        work(ilarf))
              call la_xlarf('R',q - p,q - i + 1,x11(i,i),ldx11,tauq1(i),x21(m - q + 1,i),ldx21, &
                        work(ilarf))
           end do
           ! reduce the bottom-right portion of x21 to [ 0 i ]
           do i = p + 1,q
              call la_xlarfgp(q - i + 1,x21(m - q + i - p,i),x21(m - q + i - p,i + 1),ldx21,tauq1(i))

              x21(m - q + i - p,i) = one
              call la_xlarf('R',q - i,q - i + 1,x21(m - q + i - p,i),ldx21,tauq1(i),x21(m - q + i - p + 1,i) &
                        ,ldx21,work(ilarf))
           end do
           return
     end subroutine la_xorbdb4
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
     !> XORCSD2BY1: computes the CS decomposition of an M-by-Q matrix X with
     !> orthonormal columns that has been partitioned into a 2-by-1 block
     !> structure:
     !> [  I1 0  0 ]
     !> [  0  C  0 ]
     ...
     subroutine la_xorcsd2by1(jobu1,jobu2,jobv1t,m,p,q,x11,ldx11,x21,ldx21,theta, &
               u1,ldu1,u2,ldu2,v1t,ldv1t,work,lwork,iwork,info)
        use la_constants_xdp
        ! -- lapack computational routine (3.5.0_xdp) --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ! Scalar Arguments
           character,intent(in) :: jobu1,jobu2,jobv1t
           ...
           ! Array Arguments
           real(xdp),intent(out) :: theta(*)
           real(xdp),intent(out) :: u1(ldu1,*),u2(ldu2,*),v1t(ldv1t,*),work(*)
           real(xdp),intent(inout) :: x11(ldx11,*),x21(ldx21,*)
           integer(ilp),intent(out) :: iwork(*)
        ! =====================================================================
           ...
           ! | tauq1 (max(1,q))                        | b12d (r)    |
           ! |-----------------------------------------| b12e (r-1)  |
           ! | la_xorbdb work | la_xorgqr work | la_xorglq work | b21d (r)    |
           ! |             |             |             | b21e (r-1)  |
           ! |             |             |             | b22d (r)    |
           ! |             |             |             | b22e (r-1)  |
           ! |             |             |             | la_xbbcsd work |
           ...
           if (info == 0) then
              ...
              lorglqopt = 1
              if (r == q) then
                 call la_xorbdb1(m,p,q,x11,ldx11,x21,ldx21,theta,dum1,dum1,dum1, &
                           dum1,work,-1,childinfo)
                 lorbdb = int(work(1),KIND=ilp)
                 if (wantu1 .and. p > 0) then
                    call la_xorgqr(p,p,q,u1,ldu1,dum1,work(1),-1,childinfo)
                    lorgqrmin = max(lorgqrmin,p)
                    lorgqropt = max(lorgqropt,int(work(1),KIND=ilp))
                 end if
                 if (wantu2 .and. m - p > 0) then
                    call la_xorgqr(m - p,m - p,q,u2,ldu2,dum1,work(1),-1,childinfo)

                    lorgqrmin = max(lorgqrmin,m - p)
                    lorgqropt = max(lorgqropt,int(work(1),KIND=ilp))
                 end if
                 if (wantv1t .and. q > 0) then
                    call la_xorglq(q - 1,q - 1,q - 1,v1t,ldv1t,dum1,work(1),-1,childinfo)

                    lorglqmin = max(lorglqmin,q - 1)
                    lorglqopt = max(lorglqopt,int(work(1),KIND=ilp))
                 end if
                 call la_xbbcsd(jobu1,jobu2,jobv1t,'N','N',m,p,q,theta,dum1,u1, &
                 ldu1,u2,ldu2,v1t,ldv1t,dum2,1,dum1,dum1,dum1,dum1,dum1,dum1,dum1,dum1, &
                            work(1),-1,childinfo)
                 lbbcsd = int(work(1),KIND=ilp)
              else if (r == p) then
                 call la_xorbdb2(m,p,q,x11,ldx11,x21,ldx21,theta,dum1,dum1,dum1, &
                           dum1,work(1),-1,childinfo)
                 lorbdb = int(work(1),KIND=ilp)
                 if (wantu1 .and. p > 0) then
                    call la_xorgqr(p - 1,p - 1,p - 1,u1(2,2),ldu1,dum1,work(1),-1,childinfo &
                              )
                    lorgqrmin = max(lorgqrmin,p - 1)
                    lorgqropt = max(lorgqropt,int(work(1),KIND=ilp))
                 end if
                 ...
                 if (wantv1t .and. q > 0) then
                    call la_xorglq(q,q,r,v1t,ldv1t,dum1,work(1),-1,childinfo)
                    lorglqmin = max(lorglqmin,q)
                    lorglqopt = max(lorglqopt,int(work(1),KIND=ilp))
                 end if
                 call la_xbbcsd(jobv1t,'N',jobu1,jobu2,'T',m,q,p,theta,dum1,v1t, &
                 ldv1t,dum2,1,u1,ldu1,u2,ldu2,dum1,dum1,dum1,dum1,dum1,dum1,dum1,dum1, &
                           work(1),-1,childinfo)
                 lbbcsd = int(work(1),KIND=ilp)
              else if (r == m - p) then
                 call la_xorbdb3(m,p,q,x11,ldx11,x21,ldx21,theta,dum1,dum1,dum1, &
                           dum1,work(1),-1,childinfo)
                 lorbdb = int(work(1),KIND=ilp)
                 if (wantu1 .and. p > 0) then
                    call la_xorgqr(p,p,q,u1,ldu1,dum1,work(1),-1,childinfo)
                 ...
                 if (wantu2 .and. m - p > 0) then
                    call la_xorgqr(m - p - 1,m - p - 1,m - p - 1,u2(2,2),ldu2,dum1,work(1),-1, &
                              childinfo)
                    lorgqrmin = max(lorgqrmin,m - p - 1)
                    lorgqropt = max(lorgqropt,int(work(1),KIND=ilp))
                 end if
                 ...
                 call la_xbbcsd('N',jobv1t,jobu2,jobu1,'T',m,m - q,m - p,theta,dum1, &
                 dum2,1,v1t,ldv1t,u2,ldu2,u1,ldu1,dum1,dum1,dum1,dum1,dum1,dum1,dum1, &
                           dum1,work(1),-1,childinfo)
                 lbbcsd = int(work(1),KIND=ilp)
              else
                 call la_xorbdb4(m,p,q,x11,ldx11,x21,ldx21,theta,dum1,dum1,dum1, &
                           dum1,dum1,work(1),-1,childinfo)
                 lorbdb = m + int(work(1),KIND=ilp)
                 if (wantu1 .and. p > 0) then
                    call la_xorgqr(p,p,m - q,u1,ldu1,dum1,work(1),-1,childinfo)
                    lorgqrmin = max(lorgqrmin,p)
                    lorgqropt = max(lorgqropt,int(work(1),KIND=ilp))
                 end if
                 if (wantu2 .and. m - p > 0) then
                    call la_xorgqr(m - p,m - p,m - q,u2,ldu2,dum1,work(1),-1,childinfo)

                    lorgqrmin = max(lorgqrmin,m - p)
                    lorgqropt = max(lorgqropt,int(work(1),KIND=ilp))
                 end if
                 if (wantv1t .and. q > 0) then
                    call la_xorglq(q,q,q,v1t,ldv1t,dum1,work(1),-1,childinfo)
                    lorglqmin = max(lorglqmin,q)
                    lorglqopt = max(lorglqopt,int(work(1),KIND=ilp))
                 end if
                 call la_xbbcsd(jobu2,jobu1,'N',jobv1t,'N',m,m - p,m - q,theta,dum1,u2, &
                 ldu2,u1,ldu1,dum2,1,v1t,ldv1t,dum1,dum1,dum1,dum1,dum1,dum1,dum1,dum1, &
                            work(1),-1,childinfo)
                 lbbcsd = int(work(1),KIND=ilp)
              end if
           ...
           if (info /= 0) then
              call la_xerbla('XORCSD2BY1',-info)
              return
           else if (lquery) then
              return
           end if
           ...
           if (r == q) then
              ! case 1: r = q
              ! simultaneously bidiagonalize x11 and x21
              call la_xorbdb1(m,p,q,x11,ldx11,x21,ldx21,theta,work(iphi),work(itaup1) &
                        ,work(itaup2),work(itauq1),work(iorbdb),lorbdb,childinfo)
              ! accumulate householder reflectors
              if (wantu1 .and. p > 0) then
                 call la_xlacpy('L',p,q,x11,ldx11,u1,ldu1)
                 call la_xorgqr(p,p,q,u1,ldu1,work(itaup1),work(iorgqr),lorgqr, &
                           childinfo)
              end if
              if (wantu2 .and. m - p > 0) then
                 call la_xlacpy('L',m - p,q,x21,ldx21,u2,ldu2)
                 call la_xorgqr(m - p,m - p,q,u2,ldu2,work(itaup2),work(iorgqr),lorgqr, &
                           childinfo)
              end if
              if (wantv1t .and. q > 0) then
                 v1t(1,1) = one
                 ...
                 call la_xlacpy('U',q - 1,q - 1,x21(1,2),ldx21,v1t(2,2),ldv1t)
                 call la_xorglq(q - 1,q - 1,q - 1,v1t(2,2),ldv1t,work(itauq1),work(iorglq), &
                           lorglq,childinfo)
              end if
              ! simultaneously diagonalize x11 and x21.
              call la_xbbcsd(jobu1,jobu2,jobv1t,'N','N',m,p,q,theta,work(iphi),u1, &
              ldu1,u2,ldu2,v1t,ldv1t,dum2,1,work(ib11d),work(ib11e),work(ib12d),work( &
              ib12e),work(ib21d),work(ib21e),work(ib22d),work(ib22e),work(ibbcsd),lbbcsd, &
                        childinfo)
              ! permute rows and columns to place zero submatrices in
              ...
              ! case 2: r = p
              ! simultaneously bidiagonalize x11 and x21
              call la_xorbdb2(m,p,q,x11,ldx11,x21,ldx21,theta,work(iphi),work(itaup1) &
                        ,work(itaup2),work(itauq1),work(iorbdb),lorbdb,childinfo)
              ! accumulate householder reflectors
              if (wantu1 .and. p > 0) then
                 u1(1,1) = one
                 ...
                 call la_xlacpy('L',p - 1,p - 1,x11(2,1),ldx11,u1(2,2),ldu1)
                 call la_xorgqr(p - 1,p - 1,p - 1,u1(2,2),ldu1,work(itaup1),work(iorgqr), &
                           lorgqr,childinfo)
              end if
              if (wantu2 .and. m - p > 0) then
                 call la_xlacpy('L',m - p,q,x21,ldx21,u2,ldu2)
              ...
              if (wantv1t .and. q > 0) then
                 call la_xlacpy('U',p,q,x11,ldx11,v1t,ldv1t)
                 call la_xorglq(q,q,r,v1t,ldv1t,work(itauq1),work(iorglq),lorglq, &
                           childinfo)
              end if
              ! simultaneously diagonalize x11 and x21.
              call la_xbbcsd(jobv1t,'N',jobu1,jobu2,'T',m,q,p,theta,work(iphi),v1t, &
              ldv1t,dum2,1,u1,ldu1,u2,ldu2,work(ib11d),work(ib11e),work(ib12d),work(ib12e) &
              ,work(ib21d),work(ib21e),work(ib22d),work(ib22e),work(ibbcsd),lbbcsd,childinfo &
                        )
              ! permute rows and columns to place identity submatrices in
              ...
              ! case 3: r = m-p
              ! simultaneously bidiagonalize x11 and x21
              call la_xorbdb3(m,p,q,x11,ldx11,x21,ldx21,theta,work(iphi),work(itaup1) &
                        ,work(itaup2),work(itauq1),work(iorbdb),lorbdb,childinfo)
              ! accumulate householder reflectors
              if (wantu1 .and. p > 0) then
                 call la_xlacpy('L',p,q,x11,ldx11,u1,ldu1)
              ...
              if (wantu2 .and. m - p > 0) then
                 ...
                 call la_xlacpy('L',m - p - 1,m - p - 1,x21(2,1),ldx21,u2(2,2),ldu2)
                 call la_xorgqr(m - p - 1,m - p - 1,m - p - 1,u2(2,2),ldu2,work(itaup2),work(iorgqr) &
                           ,lorgqr,childinfo)
              end if
              if (wantv1t .and. q > 0) then
                 call la_xlacpy('U',m - p,q,x21,ldx21,v1t,ldv1t)
                 call la_xorglq(q,q,r,v1t,ldv1t,work(itauq1),work(iorglq),lorglq, &
                           childinfo)
              end if
              ! simultaneously diagonalize x11 and x21.
              call la_xbbcsd('N',jobv1t,jobu2,jobu1,'T',m,m - q,m - p,theta,work(iphi), &
              dum2,1,v1t,ldv1t,u2,ldu2,u1,ldu1,work(ib11d),work(ib11e),work(ib12d),work( &
              ib12e),work(ib21d),work(ib21e),work(ib22d),work(ib22e),work(ibbcsd),lbbcsd, &
                        childinfo)
              ! permute rows and columns to place identity submatrices in
              ...
              if (q > r) then
                 ...
                 if (wantu1) then
                    call la_xlapmt(.false.,p,q,u1,ldu1,iwork)
                 end if
                 if (wantv1t) then
                    call la_xlapmr(.false.,q,q,v1t,ldv1t,iwork)
                 end if
              end if
           else
              ! case 4: r = m-q
              ! simultaneously bidiagonalize x11 and x21
              call la_xorbdb4(m,p,q,x11,ldx11,x21,ldx21,theta,work(iphi),work(itaup1) &
              ,work(itaup2),work(itauq1),work(iorbdb),work(iorbdb + m),lorbdb - m,childinfo)

              ! accumulate householder reflectors
              if (wantu2 .and. m - p > 0) then
                 call la_xcopy(m - p,work(iorbdb + p),1,u2,1)
              end if
              if (wantu1 .and. p > 0) then
                 call la_xcopy(p,work(iorbdb),1,u1,1)
                 do j = 2,p
                    u1(1,j) = zero
                 end do
                 call la_xlacpy('L',p - 1,m - q - 1,x11(2,1),ldx11,u1(2,2),ldu1)
                 call la_xorgqr(p,p,m - q,u1,ldu1,work(itaup1),work(iorgqr),lorgqr, &
                           childinfo)
              end if
              if (wantu2 .and. m - p > 0) then
                 do j = 2,m - p
                 ...
                 call la_xlacpy('L',m - p - 1,m - q - 1,x21(2,1),ldx21,u2(2,2),ldu2)
                 call la_xorgqr(m - p,m - p,m - q,u2,ldu2,work(itaup2),work(iorgqr),lorgqr, &
                           childinfo)
              end if
              if (wantv1t .and. q > 0) then
                 call la_xlacpy('U',m - q,q,x21,ldx21,v1t,ldv1t)
                 call la_xlacpy('U',p - (m - q),q - (m - q),x11(m - q + 1,m - q + 1),ldx11,v1t(m - q + 1,m - q + &
                           1),ldv1t)
                 call la_xlacpy('U',-p + q,q - p,x21(m - q + 1,p + 1),ldx21,v1t(p + 1,p + 1),ldv1t)

                 call la_xorglq(q,q,q,v1t,ldv1t,work(itauq1),work(iorglq),lorglq, &
                           childinfo)
              end if
              ! simultaneously diagonalize x11 and x21.
              call la_xbbcsd(jobu2,jobu1,'N',jobv1t,'N',m,m - p,m - q,theta,work(iphi), &
              u2,ldu2,u1,ldu1,dum2,1,v1t,ldv1t,work(ib11d),work(ib11e),work(ib12d),work( &
              ib12e),work(ib21d),work(ib21e),work(ib22d),work(ib22e),work(ibbcsd),lbbcsd, &
                        childinfo)
              ! permute rows and columns to place identity submatrices in
              ...
              if (p > r) then
                 ...
                 if (wantu1) then
                    call la_xlapmt(.false.,p,p,u1,ldu1,iwork)
                 end if
                 if (wantv1t) then
                    call la_xlapmr(.false.,p,q,v1t,ldv1t,iwork)
                 end if
              end if
           end if
           return
     end subroutine la_xorcsd2by1
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
     !> YLAPMR: rearranges the rows of the M by N matrix X as specified
     !> by the permutation K(1),K(2),...,K(M) of the integers 1,...,M.
     !> If FORWRD = .TRUE.,  forward permutation:
     !> X(K(I),*) is moved X(I,*) for I = 1,2,...,M.
     !> If FORWRD = .FALSE., backward permutation:
     ...
     pure subroutine la_ylapmr(forwrd,m,n,x,ldx,k)
        ! -- lapack auxiliary routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ! Scalar Arguments
...
#ifdef LA_WITH_XDP
     !> YLAPMT: rearranges the columns of the M by N matrix X as specified
     !> by the permutation K(1),K(2),...,K(N) of the integers 1,...,N.
     !> If FORWRD = .TRUE.,  forward permutation:
     !> X(*,K(J)) is moved X(*,J) for J = 1,2,...,N.
     !> If FORWRD = .FALSE., backward permutation:
     ...
     pure subroutine la_ylapmt(forwrd,m,n,x,ldx,k)
        ! -- lapack auxiliary routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ! Scalar Arguments
...
#ifdef LA_WITH_XDP
     !> YUNBDB: simultaneously bidiagonalizes the blocks of an M-by-M
     !> partitioned unitary matrix X:
     !> [ B11 | B12 0  0 ]
     !> [ X11 | X12 ]   [ P1 |    ] [  0  |  0 -I  0 ] [ Q1 |    ]**H
     !> X = [-----------] = [---------] [----------------] [---------]   .
     ...
     !> X11 is P-by-Q. Q must be no larger than P, M-P, or M-Q. (If this is
     !> not the case, then X must be transposed and/or permuted. This can be
     !> done in constant time using the TRANS and SIGNS options. See YUNCSD
     !> for details.)
     !> The unitary matrices P1, P2, Q1, and Q2 are P-by-P, (M-P)-by-
     !> (M-P), Q-by-Q, and (M-Q)-by-(M-Q), respectively. They are
     !> represented implicitly by Householder vectors.
     ...
     subroutine la_yunbdb(trans,signs,m,p,q,x11,ldx11,x12,ldx12,x21,ldx21,x22, &
               ldx22,theta,phi,taup1,taup2,tauq1,tauq2,work,lwork,info)
        use la_constants_xdp
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ...
           ! Array Arguments
           real(xdp),intent(out) :: phi(*),theta(*)
           complex(xdp),intent(out) :: taup1(*),taup2(*),tauq1(*),tauq2(*),work(*)
           complex(xdp),intent(inout) :: x11(ldx11,*),x12(ldx12,*),x21(ldx21,*),x22(ldx22,*)
        ...
        ! ====================================================================
           ...
           if (colmajor) then
              ...
              do i = 1,q
                 if (i == 1) then
                    call la_yscal(p - i + 1,cmplx(z1,0.0_xdp,KIND=xdp),x11(i,i),1)
                 else
                    call la_yscal(p - i + 1,cmplx(z1*cos(phi(i - 1)),0.0_xdp,KIND=xdp),x11(i,i), &
                              1)
                    call la_yaxpy(p - i + 1,cmplx(-z1*z3*z4*sin(phi(i - 1)),0.0_xdp,KIND=xdp),x12( &
                              i,i - 1),1,x11(i,i),1)
                 end if
                 if (i == 1) then
                    call la_yscal(m - p - i + 1,cmplx(z2,0.0_xdp,KIND=xdp),x21(i,i),1)
                 else
                    call la_yscal(m - p - i + 1,cmplx(z2*cos(phi(i - 1)),0.0_xdp,KIND=xdp),x21(i,i), &
                               1)
                    call la_yaxpy(m - p - i + 1,cmplx(-z2*z3*z4*sin(phi(i - 1)),0.0_xdp,KIND=xdp), &
                              x22(i,i - 1),1,x21(i,i),1)
                 end if
                 theta(i) = atan2(la_xynrm2(m - p - i + 1,x21(i,i),1),la_xynrm2(p - i + 1, &
                           x11(i,i),1))
                 if (p > i) then
                    call la_ylarfgp(p - i + 1,x11(i,i),x11(i + 1,i),1,taup1(i))
                 else if (p == i) then
                    call la_ylarfgp(p - i + 1,x11(i,i),x11(i,i),1,taup1(i))
                 end if
                 x11(i,i) = cone
                 if (m - p > i) then
                    call la_ylarfgp(m - p - i + 1,x21(i,i),x21(i + 1,i),1,taup2(i))
                 else if (m - p == i) then
                    call la_ylarfgp(m - p - i + 1,x21(i,i),x21(i,i),1,taup2(i))
                 end if
                 x21(i,i) = cone
                 if (q > i) then
                    call la_ylarf('L',p - i + 1,q - i,x11(i,i),1,conjg(taup1(i)),x11(i,i + 1), &
                              ldx11,work)
                    call la_ylarf('L',m - p - i + 1,q - i,x21(i,i),1,conjg(taup2(i)),x21(i,i + 1), &
                               ldx21,work)
                 end if
                 if (m - q + 1 > i) then
                    call la_ylarf('L',p - i + 1,m - q - i + 1,x11(i,i),1,conjg(taup1(i)),x12(i,i), &
                               ldx12,work)
                    call la_ylarf('L',m - p - i + 1,m - q - i + 1,x21(i,i),1,conjg(taup2(i)),x22(i, &
                              i),ldx22,work)
                 end if
                 if (i < q) then
                    call la_yscal(q - i,cmplx(-z1*z3*sin(theta(i)),0.0_xdp,KIND=xdp),x11(i,i + &
                              1),ldx11)
                    call la_yaxpy(q - i,cmplx(z2*z3*cos(theta(i)),0.0_xdp,KIND=xdp),x21(i,i + 1) &
                              ,ldx21,x11(i,i + 1),ldx11)
                 end if
                 call la_yscal(m - q - i + 1,cmplx(-z1*z4*sin(theta(i)),0.0_xdp,KIND=xdp),x12(i,i) &
                           ,ldx12)
                 call la_yaxpy(m - q - i + 1,cmplx(z2*z4*cos(theta(i)),0.0_xdp,KIND=xdp),x22(i,i), &
                            ldx22,x12(i,i),ldx12)
                 if (i < q) phi(i) = atan2(la_xynrm2(q - i,x11(i,i + 1),ldx11),la_xynrm2( &
                            m - q - i + 1,x12(i,i),ldx12))
                 if (i < q) then
                    call la_ylacgv(q - i,x11(i,i + 1),ldx11)
                    if (i == q - 1) then
                       call la_ylarfgp(q - i,x11(i,i + 1),x11(i,i + 1),ldx11,tauq1(i))
                    else
                       call la_ylarfgp(q - i,x11(i,i + 1),x11(i,i + 2),ldx11,tauq1(i))
                    end if
                    x11(i,i + 1) = cone
                 end if
                 if (m - q + 1 > i) then
                    call la_ylacgv(m - q - i + 1,x12(i,i),ldx12)
                    if (m - q == i) then
                       call la_ylarfgp(m - q - i + 1,x12(i,i),x12(i,i),ldx12,tauq2(i))
                    else
                       call la_ylarfgp(m - q - i + 1,x12(i,i),x12(i,i + 1),ldx12,tauq2(i))

                    end if
                 end if
                 x12(i,i) = cone
                 if (i < q) then
                    call la_ylarf('R',p - i,q - i,x11(i,i + 1),ldx11,tauq1(i),x11(i + 1,i + 1), &
                              ldx11,work)
                    call la_ylarf('R',m - p - i,q - i,x11(i,i + 1),ldx11,tauq1(i),x21(i + 1,i + 1), &
                              ldx21,work)
                 end if
                 if (p > i) then
                    call la_ylarf('R',p - i,m - q - i + 1,x12(i,i),ldx12,tauq2(i),x12(i + 1,i), &
                              ldx12,work)
                 end if
                 if (m - p > i) then
                    call la_ylarf('R',m - p - i,m - q - i + 1,x12(i,i),ldx12,tauq2(i),x22(i + 1,i), &
                              ldx22,work)
                 end if
                 if (i < q) call la_ylacgv(q - i,x11(i,i + 1),ldx11)
                 call la_ylacgv(m - q - i + 1,x12(i,i),ldx12)
              end do
              ! reduce columns q + 1, ..., p of x12, x22
              do i = q + 1,p
                 call la_yscal(m - q - i + 1,cmplx(-z1*z4,0.0_xdp,KIND=xdp),x12(i,i),ldx12)
                 ...
                 if (m - p - q >= 1) call la_ylarf('R',m - p - q,m - q - i + 1,x12(i,i),ldx12,tauq2(i), &
                            x22(q + 1,i),ldx22,work)
                 call la_ylacgv(m - q - i + 1,x12(i,i),ldx12)
              end do
              ! reduce columns p + 1, ..., m - q of x12, x22
              do i = 1,m - p - q
                 call la_yscal(m - p - q - i + 1,cmplx(z2*z4,0.0_xdp,KIND=xdp),x22(q + i,p + i),ldx22)

                 call la_ylacgv(m - p - q - i + 1,x22(q + i,p + i),ldx22)
                 call la_ylarfgp(m - p - q - i + 1,x22(q + i,p + i),x22(q + i,p + i + 1),ldx22,tauq2(p + i))

                 x22(q + i,p + i) = cone
                 call la_ylarf('R',m - p - q - i,m - p - q - i + 1,x22(q + i,p + i),ldx22,tauq2(p + i),x22( &
                           q + i + 1,p + i),ldx22,work)
                 call la_ylacgv(m - p - q - i + 1,x22(q + i,p + i),ldx22)
              end do
           else
              ...
              do i = 1,q
                 if (i == 1) then
                    call la_yscal(p - i + 1,cmplx(z1,0.0_xdp,KIND=xdp),x11(i,i),ldx11)
                 else
                    call la_yscal(p - i + 1,cmplx(z1*cos(phi(i - 1)),0.0_xdp,KIND=xdp),x11(i,i), &
                              ldx11)
                    call la_yaxpy(p - i + 1,cmplx(-z1*z3*z4*sin(phi(i - 1)),0.0_xdp,KIND=xdp),x12( &
                 ...
                 if (i == 1) then
                    call la_yscal(m - p - i + 1,cmplx(z2,0.0_xdp,KIND=xdp),x21(i,i),ldx21)
                 ...
                 theta(i) = atan2(la_xynrm2(m - p - i + 1,x21(i,i),ldx21),la_xynrm2(p - i + 1, &
                            x11(i,i),ldx11))
                 call la_ylacgv(p - i + 1,x11(i,i),ldx11)
                 call la_ylacgv(m - p - i + 1,x21(i,i),ldx21)
                 call la_ylarfgp(p - i + 1,x11(i,i),x11(i,i + 1),ldx11,taup1(i))
                 x11(i,i) = cone
                 if (i == m - p) then
                    call la_ylarfgp(m - p - i + 1,x21(i,i),x21(i,i),ldx21,taup2(i))
                 else
                    call la_ylarfgp(m - p - i + 1,x21(i,i),x21(i,i + 1),ldx21,taup2(i))
                 end if
                 x21(i,i) = cone
                 call la_ylarf('R',q - i,p - i + 1,x11(i,i),ldx11,taup1(i),x11(i + 1,i),ldx11, &
                           work)
                 call la_ylarf('R',m - q - i + 1,p - i + 1,x11(i,i),ldx11,taup1(i),x12(i,i), &
                           ldx12,work)
                 call la_ylarf('R',q - i,m - p - i + 1,x21(i,i),ldx21,taup2(i),x21(i + 1,i), &
                           ldx21,work)
                 call la_ylarf('R',m - q - i + 1,m - p - i + 1,x21(i,i),ldx21,taup2(i),x22(i,i), &
                           ldx22,work)
                 call la_ylacgv(p - i + 1,x11(i,i),ldx11)
                 call la_ylacgv(m - p - i + 1,x21(i,i),ldx21)
                 if (i < q) then
                    call la_yscal(q - i,cmplx(-z1*z3*sin(theta(i)),0.0_xdp,KIND=xdp),x11(i + 1, &
                              i),1)
                    call la_yaxpy(q - i,cmplx(z2*z3*cos(theta(i)),0.0_xdp,KIND=xdp),x21(i + 1,i) &
                              ,1,x11(i + 1,i),1)
                 end if
                 call la_yscal(m - q - i + 1,cmplx(-z1*z4*sin(theta(i)),0.0_xdp,KIND=xdp),x12(i,i) &
                           ,1)
                 call la_yaxpy(m - q - i + 1,cmplx(z2*z4*cos(theta(i)),0.0_xdp,KIND=xdp),x22(i,i), &
                            1,x12(i,i),1)
                 if (i < q) phi(i) = atan2(la_xynrm2(q - i,x11(i + 1,i),1),la_xynrm2(m - &
                           q - i + 1,x12(i,i),1))
                 if (i < q) then
                    call la_ylarfgp(q - i,x11(i + 1,i),x11(i + 2,i),1,tauq1(i))
                    x11(i + 1,i) = cone
                 end if
                 call la_ylarfgp(m - q - i + 1,x12(i,i),x12(i + 1,i),1,tauq2(i))
                 x12(i,i) = cone
                 if (i < q) then
                    call la_ylarf('L',q - i,p - i,x11(i + 1,i),1,conjg(tauq1(i)),x11(i + 1,i + 1), &
                               ldx11,work)
                    call la_ylarf('L',q - i,m - p - i,x11(i + 1,i),1,conjg(tauq1(i)),x21(i + 1,i + &
                              1),ldx21,work)
                 end if
                 call la_ylarf('L',m - q - i + 1,p - i,x12(i,i),1,conjg(tauq2(i)),x12(i,i + 1), &
                           ldx12,work)
                 if (m - p > i) then
                    call la_ylarf('L',m - q - i + 1,m - p - i,x12(i,i),1,conjg(tauq2(i)),x22(i,i + &
                              1),ldx22,work)
                 end if
              end do
              ! reduce columns q + 1, ..., p of x12, x22
              do i = q + 1,p
                 call la_yscal(m - q - i + 1,cmplx(-z1*z4,0.0_xdp,KIND=xdp),x12(i,i),1)
                 call la_ylarfgp(m - q - i + 1,x12(i,i),x12(i + 1,i),1,tauq2(i))
                 x12(i,i) = cone
                 if (p > i) then
                    call la_ylarf('L',m - q - i + 1,p - i,x12(i,i),1,conjg(tauq2(i)),x12(i,i + 1), &
                 ...
                 if (m - p - q >= 1) call la_ylarf('L',m - q - i + 1,m - p - q,x12(i,i),1,conjg(tauq2( &
                           i)),x22(i,q + 1),ldx22,work)
              end do
              ! reduce columns p + 1, ..., m - q of x12, x22
              do i = 1,m - p - q
                 call la_yscal(m - p - q - i + 1,cmplx(z2*z4,0.0_xdp,KIND=xdp),x22(p + i,q + i),1)

                 call la_ylarfgp(m - p - q - i + 1,x22(p + i,q + i),x22(p + i + 1,q + i),1,tauq2(p + i))

                 x22(p + i,q + i) = cone
                 if (m - p - q /= i) then
                    call la_ylarf('L',m - p - q - i + 1,m - p - q - i,x22(p + i,q + i),1,conjg(tauq2(p + i)), &
                               x22(p + i,q + i + 1),ldx22,work)
                 end if
              end do
           end if
           return
     end subroutine la_yunbdb
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
     !> YUNBDB6: orthogonalizes the column vector
     !> X = [ X1 ]
     !> [ X2 ]
     !> with respect to the columns of
     !> Q = [ Q1 ] .
     ...
     pure subroutine la_yunbdb6(m1,m2,n,x1,incx1,x2,incx2,q1,ldq1,q2,ldq2,work, &
               lwork,info)
        use la_constants_xdp
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ...
           integer(ilp),intent(out) :: info
           ! Array Arguments
           complex(xdp),intent(in) :: q1(ldq1,*),q2(ldq2,*)
           complex(xdp),intent(out) :: work(*)
           complex(xdp),intent(inout) :: x1(*),x2(*)
        ! =====================================================================
           ! Parameters
           real(xdp),parameter :: alphasq = 0.01_xdp
           real(xdp),parameter :: realone = 1.0_xdp
           ...
           scl1 = realzero
           ssq1 = realone
           call la_ylassq(m1,x1,incx1,scl1,ssq1)
           scl2 = realzero
           ssq2 = realone
           call la_ylassq(m2,x2,incx2,scl2,ssq2)
           normsq1 = scl1**2*ssq1 + scl2**2*ssq2
           if (m1 == 0) then
              do i = 1,n
                 work(i) = czero
              ...
              call la_ygemv('C',m1,n,cone,q1,ldq1,x1,incx1,czero,work,1)
           end if
           call la_ygemv('C',m2,n,cone,q2,ldq2,x2,incx2,cone,work,1)
           call la_ygemv('N',m1,n,cnegone,q1,ldq1,work,1,cone,x1,incx1)
           call la_ygemv('N',m2,n,cnegone,q2,ldq2,work,1,cone,x2,incx2)
           scl1 = realzero
           ssq1 = realone
           call la_ylassq(m1,x1,incx1,scl1,ssq1)
           scl2 = realzero
...
#ifdef LA_WITH_XDP
     !> YBBCSD: computes the CS decomposition of a unitary matrix in
     !> bidiagonal-block form,
     !> [ B11 | B12 0  0 ]
     !> [  0  |  0 -I  0 ]
     !> X = [----------------]
     ...
     !> than P, M-P, or M-Q. (If Q is not the smallest index, then X must be
     !> transposed and/or permuted. This can be done in constant time using
     !> the TRANS and SIGNS options. See YUNCSD for details.)
     !> The bidiagonal matrices B11, B12, B21, and B22 are represented
     !> implicitly by angles THETA(1:Q) and PHI(1:Q-1).
     !> The unitary matrices U1, U2, V1T, and V2T are input/output.
     !> The input matrices are pre- or post-multiplied by the appropriate
     ...
     pure subroutine la_ybbcsd(jobu1,jobu2,jobv1t,jobv2t,trans,m,p,q,theta,phi,u1, &
     ldu1,u2,ldu2,v1t,ldv1t,v2t,ldv2t,b11d,b11e,b12d,b12e,b21d,b21e,b22d,b22e,rwork, &
               lrwork,info)
        use la_constants_xdp,only:zero,one,ten,cnegone
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ! Scalar Arguments
           ...
           real(xdp),intent(out) :: b11d(*),b11e(*),b12d(*),b12e(*),b21d(*),b21e(*),b22d(*), &
                      b22e(*),rwork(*)
           real(xdp),intent(inout) :: phi(*),theta(*)
           complex(xdp),intent(inout) :: u1(ldu1,*),u2(ldu2,*),v1t(ldv1t,*),v2t(ldv2t,*)
        ...
        ! ===================================================================
           ...
           do while (imax > 1)
              ...
              ! rotate to produce bulges in b11 and b21
              if (mu <= nu) then
                 call la_xlartgs(b11d(imin),b11e(imin),mu,rwork(iv1tcs + imin - 1),rwork( &
                           iv1tsn + imin - 1))
              else
                 call la_xlartgs(b21d(imin),b21e(imin),nu,rwork(iv1tcs + imin - 1),rwork( &
                           iv1tsn + imin - 1))
              end if
              temp = rwork(iv1tcs + imin - 1)*b11d(imin) + rwork(iv1tsn + imin - 1)*b11e(imin)
              b11e(imin) = rwork(iv1tcs + imin - 1)*b11e(imin) - rwork(iv1tsn + imin - 1)*b11d(imin)
              ...
              ! chase the bulges in b11(imin+1,imin) and b21(imin+1,imin)
              if (b11d(imin)**2 + b11bulge**2 > thresh**2) then
                 call la_xlartgp(b11bulge,b11d(imin),rwork(iu1sn + imin - 1),rwork(iu1cs + imin - &
                           1),r)
              else if (mu <= nu) then
                 call la_xlartgs(b11e(imin),b11d(imin + 1),mu,rwork(iu1cs + imin - 1), &
                           rwork(iu1sn + imin - 1))
              else
                 call la_xlartgs(b12d(imin),b12e(imin),nu,rwork(iu1cs + imin - 1),rwork( &
                           iu1sn + imin - 1))
              end if
              if (b21d(imin)**2 + b21bulge**2 > thresh**2) then
                 call la_xlartgp(b21bulge,b21d(imin),rwork(iu2sn + imin - 1),rwork(iu2cs + imin - &
                           1),r)
              else if (nu < mu) then
                 call la_xlartgs(b21e(imin),b21d(imin + 1),nu,rwork(iu2cs + imin - 1), &
                           rwork(iu2sn + imin - 1))
              else
                 call la_xlartgs(b22d(imin),b22e(imin),mu,rwork(iu2cs + imin - 1),rwork(iu2sn + &
                           imin - 1))
              end if
              rwork(iu2cs + imin - 1) = -rwork(iu2cs + imin - 1)
              rwork(iu2sn + imin - 1) = -rwork(iu2sn + imin - 1)
              ...
              do i = imin + 1,imax - 1
                 ...
                 ! chasing by applying the original shift again.
                 if (.not. restart11 .and. .not. restart21) then
                    call la_xlartgp(x2,x1,rwork(iv1tsn + i - 1),rwork(iv1tcs + i - 1),r)
                 else if (.not. restart11 .and. restart21) then
                    call la_xlartgp(b11bulge,b11e(i - 1),rwork(iv1tsn + i - 1),rwork(iv1tcs + i - 1), &
                               r)
                 else if (restart11 .and. .not. restart21) then
                    call la_xlartgp(b21bulge,b21e(i - 1),rwork(iv1tsn + i - 1),rwork(iv1tcs + i - 1), &
                               r)
                 else if (mu <= nu) then
                    call la_xlartgs(b11d(i),b11e(i),mu,rwork(iv1tcs + i - 1),rwork(iv1tsn + i - 1) &
                               )
                 else
                    call la_xlartgs(b21d(i),b21e(i),nu,rwork(iv1tcs + i - 1),rwork(iv1tsn + i - 1) &
                               )
                 end if
                 rwork(iv1tcs + i - 1) = -rwork(iv1tcs + i - 1)
                 rwork(iv1tsn + i - 1) = -rwork(iv1tsn + i - 1)
                 if (.not. restart12 .and. .not. restart22) then
                    call la_xlartgp(y2,y1,rwork(iv2tsn + i - 1 - 1),rwork(iv2tcs + i - 1 - 1),r)

                 else if (.not. restart12 .and. restart22) then
                    call la_xlartgp(b12bulge,b12d(i - 1),rwork(iv2tsn + i - 1 - 1),rwork(iv2tcs + i - &
                              1 - 1),r)
                 else if (restart12 .and. .not. restart22) then
                    call la_xlartgp(b22bulge,b22d(i - 1),rwork(iv2tsn + i - 1 - 1),rwork(iv2tcs + i - &
                              1 - 1),r)
                 else if (nu < mu) then
                    call la_xlartgs(b12e(i - 1),b12d(i),nu,rwork(iv2tcs + i - 1 - 1),rwork(iv2tsn + &
                              i - 1 - 1))
                 else
                    call la_xlartgs(b22e(i - 1),b22d(i),mu,rwork(iv2tcs + i - 1 - 1),rwork(iv2tsn + &
                              i - 1 - 1))
                 end if
                 temp = rwork(iv1tcs + i - 1)*b11d(i) + rwork(iv1tsn + i - 1)*b11e(i)
                 b11e(i) = rwork(iv1tcs + i - 1)*b11e(i) - rwork(iv1tsn + i - 1)*b11d(i)
                 ...
                 ! chasing by applying the original shift again.
                 if (.not. restart11 .and. .not. restart12) then
                    call la_xlartgp(x2,x1,rwork(iu1sn + i - 1),rwork(iu1cs + i - 1),r)
                 else if (.not. restart11 .and. restart12) then
                    call la_xlartgp(b11bulge,b11d(i),rwork(iu1sn + i - 1),rwork(iu1cs + i - 1),r)

                 else if (restart11 .and. .not. restart12) then
                    call la_xlartgp(b12bulge,b12e(i - 1),rwork(iu1sn + i - 1),rwork(iu1cs + i - 1), &
                              r)
                 else if (mu <= nu) then
                    call la_xlartgs(b11e(i),b11d(i + 1),mu,rwork(iu1cs + i - 1),rwork(iu1sn + i - 1) &
                               )
                 else
                    call la_xlartgs(b12d(i),b12e(i),nu,rwork(iu1cs + i - 1),rwork(iu1sn + i - 1))

                 end if
                 if (.not. restart21 .and. .not. restart22) then
                    call la_xlartgp(y2,y1,rwork(iu2sn + i - 1),rwork(iu2cs + i - 1),r)
                 else if (.not. restart21 .and. restart22) then
                    call la_xlartgp(b21bulge,b21d(i),rwork(iu2sn + i - 1),rwork(iu2cs + i - 1),r)

                 else if (restart21 .and. .not. restart22) then
                    call la_xlartgp(b22bulge,b22e(i - 1),rwork(iu2sn + i - 1),rwork(iu2cs + i - 1), &
                              r)
                 else if (nu < mu) then
                    call la_xlartgs(b21e(i),b21e(i + 1),nu,rwork(iu2cs + i - 1),rwork(iu2sn + i - 1) &
                               )
                 else
                    call la_xlartgs(b22d(i),b22e(i),mu,rwork(iu2cs + i - 1),rwork(iu2sn + i - 1))
              ...
              restart22 = b22d(imax - 1)**2 + b22bulge**2 <= thresh**2
              if (.not. restart12 .and. .not. restart22) then
                 call la_xlartgp(y2,y1,rwork(iv2tsn + imax - 1 - 1),rwork(iv2tcs + imax - 1 - 1),r)

              else if (.not. restart12 .and. restart22) then
                 call la_xlartgp(b12bulge,b12d(imax - 1),rwork(iv2tsn + imax - 1 - 1),rwork(iv2tcs + &
                           imax - 1 - 1),r)
              else if (restart12 .and. .not. restart22) then
                 call la_xlartgp(b22bulge,b22d(imax - 1),rwork(iv2tsn + imax - 1 - 1),rwork(iv2tcs + &
                           imax - 1 - 1),r)
              else if (nu < mu) then
                 call la_xlartgs(b12e(imax - 1),b12d(imax),nu,rwork(iv2tcs + imax - 1 - 1),rwork( &
                           iv2tsn + imax - 1 - 1))
              else
                 call la_xlartgs(b22e(imax - 1),b22d(imax),mu,rwork(iv2tcs + imax - 1 - 1),rwork( &
                           iv2tsn + imax - 1 - 1))
              end if
              temp = rwork(iv2tcs + imax - 1 - 1)*b12e(imax - 1) + rwork(iv2tsn + imax - 1 - 1)*b12d(imax)
              ...
              if (wantu1) then
                 if (colmajor) then
                    call la_ylasr('R','V','F',p,imax - imin + 1,rwork(iu1cs + imin - 1),rwork( &
                              iu1sn + imin - 1),u1(1,imin),ldu1)
                 else
                    call la_ylasr('L','V','F',imax - imin + 1,p,rwork(iu1cs + imin - 1),rwork( &
                              iu1sn + imin - 1),u1(imin,1),ldu1)
                 end if
              end if
              if (wantu2) then
                 if (colmajor) then
                    call la_ylasr('R','V','F',m - p,imax - imin + 1,rwork(iu2cs + imin - 1),rwork( &
                              iu2sn + imin - 1),u2(1,imin),ldu2)
                 else
                    call la_ylasr('L','V','F',imax - imin + 1,m - p,rwork(iu2cs + imin - 1),rwork( &
                              iu2sn + imin - 1),u2(imin,1),ldu2)
                 end if
              end if
              if (wantv1t) then
                 if (colmajor) then
                    call la_ylasr('L','V','F',imax - imin + 1,q,rwork(iv1tcs + imin - 1),rwork( &
                              iv1tsn + imin - 1),v1t(imin,1),ldv1t)
                 else
                    call la_ylasr('R','V','F',q,imax - imin + 1,rwork(iv1tcs + imin - 1),rwork( &
                              iv1tsn + imin - 1),v1t(1,imin),ldv1t)
                 end if
              end if
              if (wantv2t) then
                 if (colmajor) then
                    call la_ylasr('L','V','F',imax - imin + 1,m - q,rwork(iv2tcs + imin - 1), &
                              rwork(iv2tsn + imin - 1),v2t(imin,1),ldv2t)
                 else
                    call la_ylasr('R','V','F',m - q,imax - imin + 1,rwork(iv2tcs + imin - 1), &
                              rwork(iv2tsn + imin - 1),v2t(1,imin),ldv2t)
                 end if
              end if
              ! fix signs on b11(imax-1,imax) and b21(imax-1,imax)
              if (b11e(imax - 1) + b21e(imax - 1) > 0) then
                 ...
                 if (wantv1t) then
                    if (colmajor) then
                       call la_yscal(q,cnegone,v1t(imax,1),ldv1t)
                    else
                       call la_yscal(q,cnegone,v1t(1,imax),1)
                    end if
                 end if
              end if
              ! compute theta(imax)
              ...
              if (b11d(imax) + b12e(imax - 1) < 0) then
                 ...
                 if (wantu1) then
                    if (colmajor) then
                       call la_yscal(p,cnegone,u1(1,imax),1)
                    else
                       call la_yscal(p,cnegone,u1(imax,1),ldu1)
                    end if
                 end if
              end if
              if (b21d(imax) + b22e(imax - 1) > 0) then
                 ...
                 if (wantu2) then
                    if (colmajor) then
                       call la_yscal(m - p,cnegone,u2(1,imax),1)
                    else
                       call la_yscal(m - p,cnegone,u2(imax,1),ldu2)
                    end if
                 end if
              end if
              ! fix signs on b12(imax,imax) and b22(imax,imax)
              if (b12d(imax) + b22d(imax) < 0) then
                 if (wantv2t) then
                    if (colmajor) then
                       call la_yscal(m - q,cnegone,v2t(imax,1),ldv2t)
                    else
                       call la_yscal(m - q,cnegone,v2t(1,imax),1)
                    end if
                 end if
              end if
              ! test for negligible sines or cosines
           ...
           do i = 1,q
              ...
              if (mini /= i) then
                 ...
                 theta(i) = thetamin
                 if (colmajor) then
                    if (wantu1) call la_yswap(p,u1(1,i),1,u1(1,mini),1)
                    if (wantu2) call la_yswap(m - p,u2(1,i),1,u2(1,mini),1)
                    if (wantv1t) call la_yswap(q,v1t(i,1),ldv1t,v1t(mini,1),ldv1t)

                    if (wantv2t) call la_yswap(m - q,v2t(i,1),ldv2t,v2t(mini,1),ldv2t)

                 else
                    if (wantu1) call la_yswap(p,u1(i,1),ldu1,u1(mini,1),ldu1)
                    if (wantu2) call la_yswap(m - p,u2(i,1),ldu2,u2(mini,1),ldu2)
                    if (wantv1t) call la_yswap(q,v1t(1,i),1,v1t(1,mini),1)
                    if (wantv2t) call la_yswap(m - q,v2t(1,i),1,v2t(1,mini),1)
                 end if
              end if
           end do
           return
     end subroutine la_ybbcsd
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
     !> YUNBDB5: orthogonalizes the column vector
     !> X = [ X1 ]
     !> [ X2 ]
     !> with respect to the columns of
     !> Q = [ Q1 ] .
     ...
     pure subroutine la_yunbdb5(m1,m2,n,x1,incx1,x2,incx2,q1,ldq1,q2,ldq2,work, &
               lwork,info)
        use la_constants_xdp
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ...
        ! =====================================================================
           ...
           if (info /= 0) then
              call la_xerbla('YUNBDB5',-info)
              return
           end if
           ! project x onto the orthogonal complement of q
           call la_yunbdb6(m1,m2,n,x1,incx1,x2,incx2,q1,ldq1,q2,ldq2,work,lwork, &
                     childinfo)
           ! if the projection is nonzero, then return
           if (la_xynrm2(m1,x1,incx1) /= czero .or. la_xynrm2(m2,x2,incx2) /= czero) &
                     then
              return
           end if
           ! project each standard basis vector e_1,...,e_m1 in turn, stopping
           ...
           do i = 1,m1
              ...
              call la_yunbdb6(m1,m2,n,x1,incx1,x2,incx2,q1,ldq1,q2,ldq2,work, &
                        lwork,childinfo)
              if (la_xynrm2(m1,x1,incx1) /= czero .or. la_xynrm2(m2,x2,incx2) /= czero) &
                        then
                 return
...
#ifdef LA_WITH_XDP
     !> YUNCSD: computes the CS decomposition of an M-by-M partitioned
     !> unitary matrix X:
     !> [  I  0  0 |  0  0  0 ]
     !> [  0  C  0 |  0 -S  0 ]
     !> [ X11 | X12 ]   [ U1 |    ] [  0  0  0 |  0  0 -I ] [ V1 |    ]**H
     ...
     recursive subroutine la_yuncsd(jobu1,jobu2,jobv1t,jobv2t,trans,signs,m,p,q,x11, &
     ldx11,x12,ldx12,x21,ldx21,x22,ldx22,theta,u1,ldu1,u2,ldu2,v1t,ldv1t,v2t,ldv2t, &
               work,lwork,rwork,lrwork,iwork,info)
        use la_constants_xdp
        ! -- lapack computational routine --
        ...
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ...
           complex(xdp),intent(out) :: u1(ldu1,*),u2(ldu2,*),v1t(ldv1t,*),v2t(ldv2t,*),work(*)
        ...
        ! ===================================================================
           ...
           if (info == 0 .and. min(p,m - p) < min(q,m - q)) then
              ...
              call la_yuncsd(jobv1t,jobv2t,jobu1,jobu2,transt,signst,m,q,p,x11, &
              ldx11,x21,ldx21,x12,ldx12,x22,ldx22,theta,v1t,ldv1t,v2t,ldv2t,u1,ldu1, &
                        u2,ldu2,work,lwork,rwork,lrwork,iwork,info)
              return
           end if
           ...
           if (info == 0 .and. m - q < q) then
              ...
              call la_yuncsd(jobu2,jobu1,jobv2t,jobv1t,trans,signst,m,m - p,m - q,x22, &
              ldx22,x21,ldx21,x12,ldx12,x11,ldx11,theta,u2,ldu2,u1,ldu1,v2t,ldv2t, &
                        v1t,ldv1t,work,lwork,rwork,lrwork,iwork,info)
              return
           end if
           ...
           if (info == 0) then
              ...
              ib22e = ib22d + max(1,q)
              ibbcsd = ib22e + max(1,q - 1)
              call la_ybbcsd(jobu1,jobu2,jobv1t,jobv2t,trans,m,p,q,theta,theta,u1, &
              ldu1,u2,ldu2,v1t,ldv1t,v2t,ldv2t,theta,theta,theta,theta,theta,theta, &
                        theta,theta,rwork,-1,childinfo)
              lbbcsdworkopt = int(rwork(1),KIND=ilp)
              lbbcsdworkmin = lbbcsdworkopt
              ...
              itauq2 = itauq1 + max(1,q)
              iorgqr = itauq2 + max(1,m - q)
              call la_yungqr(m - q,m - q,m - q,u1,max(1,m - q),u1,work,-1,childinfo)
              lorgqrworkopt = int(work(1),KIND=ilp)
              lorgqrworkmin = max(1,m - q)
              iorglq = itauq2 + max(1,m - q)
              call la_yunglq(m - q,m - q,m - q,u1,max(1,m - q),u1,work,-1,childinfo)
              lorglqworkopt = int(work(1),KIND=ilp)
              lorglqworkmin = max(1,m - q)
              iorbdb = itauq2 + max(1,m - q)
              call la_yunbdb(trans,signs,m,p,q,x11,ldx11,x12,ldx12,x21,ldx21,x22, &
                        ldx22,theta,theta,u1,u2,v1t,v2t,work,-1,childinfo)
              lorbdbworkopt = int(work(1),KIND=ilp)
              lorbdbworkmin = lorbdbworkopt
              lworkopt = max(iorgqr + lorgqrworkopt,iorglq + lorglqworkopt,iorbdb + &
           ...
           if (colmajor) then
              if (wantu1 .and. p > 0) then
                 call la_ylacpy('L',p,q,x11,ldx11,u1,ldu1)
                 call la_yungqr(p,p,q,u1,ldu1,work(itaup1),work(iorgqr),lorgqrwork, &
                           info)
              end if
              if (wantu2 .and. m - p > 0) then
                 call la_ylacpy('L',m - p,q,x21,ldx21,u2,ldu2)
                 call la_yungqr(m - p,m - p,q,u2,ldu2,work(itaup2),work(iorgqr),lorgqrwork, &
                            info)
              end if
              if (wantv1t .and. q > 0) then
                 call la_ylacpy('U',q - 1,q - 1,x11(1,2),ldx11,v1t(2,2),ldv1t)
                 v1t(1,1) = cone
                 do j = 2,q
                    v1t(1,j) = czero
                    v1t(j,1) = czero
                 end do
                 call la_yunglq(q - 1,q - 1,q - 1,v1t(2,2),ldv1t,work(itauq1),work(iorglq), &
                           lorglqwork,info)
              end if
              if (wantv2t .and. m - q > 0) then
                 call la_ylacpy('U',p,m - q,x12,ldx12,v2t,ldv2t)
                 if (m - p > q) then
                    call la_ylacpy('U',m - p - q,m - p - q,x22(q + 1,p + 1),ldx22,v2t(p + 1,p + 1), &
                              ldv2t)
                 end if
                 if (m > q) then
                    call la_yunglq(m - q,m - q,m - q,v2t,ldv2t,work(itauq2),work(iorglq), &
                              lorglqwork,info)
                 end if
              end if
           else
              if (wantu1 .and. p > 0) then
                 call la_ylacpy('U',q,p,x11,ldx11,u1,ldu1)
                 call la_yunglq(p,p,q,u1,ldu1,work(itaup1),work(iorglq),lorglqwork, &
                           info)
              end if
              if (wantu2 .and. m - p > 0) then
                 call la_ylacpy('U',q,m - p,x21,ldx21,u2,ldu2)
                 call la_yunglq(m - p,m - p,q,u2,ldu2,work(itaup2),work(iorglq),lorglqwork, &
                            info)
              end if
              if (wantv1t .and. q > 0) then
                 call la_ylacpy('L',q - 1,q - 1,x11(2,1),ldx11,v1t(2,2),ldv1t)
                 v1t(1,1) = cone
                 do j = 2,q
                    v1t(1,j) = czero
                    v1t(j,1) = czero
                 end do
                 call la_yungqr(q - 1,q - 1,q - 1,v1t(2,2),ldv1t,work(itauq1),work(iorgqr), &
                           lorgqrwork,info)
              end if
              if (wantv2t .and. m - q > 0) then
                 p1 = min(p + 1,m)
                 q1 = min(q + 1,m)
                 call la_ylacpy('L',m - q,p,x12,ldx12,v2t,ldv2t)
                 if (m > p + q) then
                    call la_ylacpy('L',m - p - q,m - p - q,x22(p1,q1),ldx22,v2t(p + 1,p + 1),ldv2t)

                 end if
                 call la_yungqr(m - q,m - q,m - q,v2t,ldv2t,work(itauq2),work(iorgqr), &
                           lorgqrwork,info)
              end if
           end if
           ! compute the csd of the matrix in bidiagonal-block form
           call la_ybbcsd(jobu1,jobu2,jobv1t,jobv2t,trans,m,p,q,theta,rwork(iphi), &
           u1,ldu1,u2,ldu2,v1t,ldv1t,v2t,ldv2t,rwork(ib11d),rwork(ib11e),rwork(ib12d), &
           rwork(ib12e),rwork(ib21d),rwork(ib21e),rwork(ib22d),rwork(ib22e),rwork(ibbcsd), &
                     lbbcsdwork,info)
           ! permute rows and columns to place identity submatrices in top-
           ...
           if (q > 0 .and. wantu2) then
              ...
              if (colmajor) then
                 call la_ylapmt(.false.,m - p,m - p,u2,ldu2,iwork)
              else
                 call la_ylapmr(.false.,m - p,m - p,u2,ldu2,iwork)
              end if
           end if
           if (m > 0 .and. wantv2t) then
              do i = 1,p
              ...
              if (.not. colmajor) then
                 call la_ylapmt(.false.,m - q,m - q,v2t,ldv2t,iwork)
              else
                 call la_ylapmr(.false.,m - q,m - q,v2t,ldv2t,iwork)
              end if
           end if
           return
           ! end la_yuncsd
     end subroutine la_yuncsd
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
     !> YUNBDB1: simultaneously bidiagonalizes the blocks of a tall and skinny
     !> matrix X with orthonomal columns:
     !> [ B11 ]
     !> [ X11 ]   [ P1 |    ] [  0  ]
     !> [-----] = [---------] [-----] Q1**T .
     ...
     !> [  0  ]
     !> X11 is P-by-Q, and X21 is (M-P)-by-Q. Q must be no larger than P,
     !> M-P, or M-Q. Routines YUNBDB2, YUNBDB3, and YUNBDB4 handle cases in
     !> which Q is not the minimum dimension.
     !> The unitary matrices P1, P2, and Q1 are P-by-P, (M-P)-by-(M-P),
     !> and (M-Q)-by-(M-Q), respectively. They are represented implicitly by
     !> Householder vectors.
     ...
     subroutine la_yunbdb1(m,p,q,x11,ldx11,x21,ldx21,theta,phi,taup1,taup2,tauq1, &
               work,lwork,info)
        use la_constants_xdp
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ...
           ! Array Arguments
           real(xdp),intent(out) :: phi(*),theta(*)
           complex(xdp),intent(out) :: taup1(*),taup2(*),tauq1(*),work(*)
           complex(xdp),intent(inout) :: x11(ldx11,*),x21(ldx21,*)
        ! ====================================================================
           ...
           do i = 1,q
              call la_ylarfgp(p - i + 1,x11(i,i),x11(i + 1,i),1,taup1(i))
              call la_ylarfgp(m - p - i + 1,x21(i,i),x21(i + 1,i),1,taup2(i))
              theta(i) = atan2(real(x21(i,i),KIND=xdp),real(x11(i,i),KIND=xdp))
              c = cos(theta(i))
              s = sin(theta(i))
              x11(i,i) = cone
              x21(i,i) = cone
              call la_ylarf('L',p - i + 1,q - i,x11(i,i),1,conjg(taup1(i)),x11(i,i + 1),ldx11, &
                        work(ilarf))
              call la_ylarf('L',m - p - i + 1,q - i,x21(i,i),1,conjg(taup2(i)),x21(i,i + 1), &
                        ldx21,work(ilarf))
              if (i < q) then
                 call la_yxrot(q - i,x11(i,i + 1),ldx11,x21(i,i + 1),ldx21,c,s)
                 call la_ylacgv(q - i,x21(i,i + 1),ldx21)
                 call la_ylarfgp(q - i,x21(i,i + 1),x21(i,i + 2),ldx21,tauq1(i))
                 s = real(x21(i,i + 1),KIND=xdp)
                 x21(i,i + 1) = cone
                 call la_ylarf('R',p - i,q - i,x21(i,i + 1),ldx21,tauq1(i),x11(i + 1,i + 1), &
                           ldx11,work(ilarf))
                 call la_ylarf('R',m - p - i,q - i,x21(i,i + 1),ldx21,tauq1(i),x21(i + 1,i + 1), &
                           ldx21,work(ilarf))
                 call la_ylacgv(q - i,x21(i,i + 1),ldx21)
                 c = sqrt(la_xynrm2(p - i,x11(i + 1,i + 1),1)**2 + la_xynrm2(m - p - i,x21(i + &
                           1,i + 1),1)**2)
                 phi(i) = atan2(s,c)
                 call la_yunbdb5(p - i,m - p - i,q - i - 1,x11(i + 1,i + 1),1,x21(i + 1,i + 1),1,x11(i + 1, &
                           i + 2),ldx11,x21(i + 1,i + 2),ldx21,work(iorbdb5),lorbdb5,childinfo)
              end if
           end do
           return
     end subroutine la_yunbdb1
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
     !> YUNBDB2: simultaneously bidiagonalizes the blocks of a tall and skinny
     !> matrix X with orthonomal columns:
     !> [ B11 ]
     !> [ X11 ]   [ P1 |    ] [  0  ]
     !> [-----] = [---------] [-----] Q1**T .
     ...
     !> [  0  ]
     !> X11 is P-by-Q, and X21 is (M-P)-by-Q. P must be no larger than M-P,
     !> Q, or M-Q. Routines YUNBDB1, YUNBDB3, and YUNBDB4 handle cases in
     !> which P is not the minimum dimension.
     !> The unitary matrices P1, P2, and Q1 are P-by-P, (M-P)-by-(M-P),
     !> and (M-Q)-by-(M-Q), respectively. They are represented implicitly by
     !> Householder vectors.
     ...
     subroutine la_yunbdb2(m,p,q,x11,ldx11,x21,ldx21,theta,phi,taup1,taup2,tauq1, &
               work,lwork,info)
        use la_constants_xdp
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ...
        ! ====================================================================
           ...
           do i = 1,p
              if (i > 1) then
                 call la_yxrot(q - i + 1,x11(i,i),ldx11,x21(i - 1,i),ldx21,c,s)
              end if
              call la_ylacgv(q - i + 1,x11(i,i),ldx11)
              call la_ylarfgp(q - i + 1,x11(i,i),x11(i,i + 1),ldx11,tauq1(i))
              c = real(x11(i,i),KIND=xdp)
              x11(i,i) = cone
              call la_ylarf('R',p - i,q - i + 1,x11(i,i),ldx11,tauq1(i),x11(i + 1,i),ldx11, &
                        work(ilarf))
              call la_ylarf('R',m - p - i + 1,q - i + 1,x11(i,i),ldx11,tauq1(i),x21(i,i),ldx21, &
                        work(ilarf))
              call la_ylacgv(q - i + 1,x11(i,i),ldx11)
              s = sqrt(la_xynrm2(p - i,x11(i + 1,i),1)**2 + la_xynrm2(m - p - i + 1,x21(i,i), &
                        1)**2)
              theta(i) = atan2(s,c)
              call la_yunbdb5(p - i,m - p - i + 1,q - i,x11(i + 1,i),1,x21(i,i),1,x11(i + 1,i + 1), &
                        ldx11,x21(i,i + 1),ldx21,work(iorbdb5),lorbdb5,childinfo)
              call la_yscal(p - i,cnegone,x11(i + 1,i),1)
              call la_ylarfgp(m - p - i + 1,x21(i,i),x21(i + 1,i),1,taup2(i))
              if (i < p) then
                 call la_ylarfgp(p - i,x11(i + 1,i),x11(i + 2,i),1,taup1(i))
                 phi(i) = atan2(real(x11(i + 1,i),KIND=xdp),real(x21(i,i),KIND=xdp))
                 c = cos(phi(i))
                 s = sin(phi(i))
                 x11(i + 1,i) = cone
                 call la_ylarf('L',p - i,q - i,x11(i + 1,i),1,conjg(taup1(i)),x11(i + 1,i + 1), &
                           ldx11,work(ilarf))
              end if
              x21(i,i) = cone
              call la_ylarf('L',m - p - i + 1,q - i,x21(i,i),1,conjg(taup2(i)),x21(i,i + 1), &
...
#ifdef LA_WITH_XDP
     !> YUNBDB3: simultaneously bidiagonalizes the blocks of a tall and skinny
     !> matrix X with orthonomal columns:
     !> [ B11 ]
     !> [ X11 ]   [ P1 |    ] [  0  ]
     !> [-----] = [---------] [-----] Q1**T .
     ...
     !> [  0  ]
     !> X11 is P-by-Q, and X21 is (M-P)-by-Q. M-P must be no larger than P,
     !> Q, or M-Q. Routines YUNBDB1, YUNBDB2, and YUNBDB4 handle cases in
     !> which M-P is not the minimum dimension.
     !> The unitary matrices P1, P2, and Q1 are P-by-P, (M-P)-by-(M-P),
     !> and (M-Q)-by-(M-Q), respectively. They are represented implicitly by
     !> Householder vectors.
     ...
     subroutine la_yunbdb3(m,p,q,x11,ldx11,x21,ldx21,theta,phi,taup1,taup2,tauq1, &
               work,lwork,info)
        use la_constants_xdp
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ...
        ! ====================================================================
           ...
           do i = 1,m - p
              if (i > 1) then
                 call la_yxrot(q - i + 1,x11(i - 1,i),ldx11,x21(i,i),ldx11,c,s)
              end if
              call la_ylacgv(q - i + 1,x21(i,i),ldx21)
              call la_ylarfgp(q - i + 1,x21(i,i),x21(i,i + 1),ldx21,tauq1(i))
              s = real(x21(i,i),KIND=xdp)
              x21(i,i) = cone
              call la_ylarf('R',p - i + 1,q - i + 1,x21(i,i),ldx21,tauq1(i),x11(i,i),ldx11, &
                        work(ilarf))
              call la_ylarf('R',m - p - i,q - i + 1,x21(i,i),ldx21,tauq1(i),x21(i + 1,i),ldx21, &
                        work(ilarf))
              call la_ylacgv(q - i + 1,x21(i,i),ldx21)
              c = sqrt(la_xynrm2(p - i + 1,x11(i,i),1)**2 + la_xynrm2(m - p - i,x21(i + 1,i), &
                        1)**2)
              theta(i) = atan2(s,c)
              call la_yunbdb5(p - i + 1,m - p - i,q - i,x11(i,i),1,x21(i + 1,i),1,x11(i,i + 1), &
                        ldx11,x21(i + 1,i + 1),ldx21,work(iorbdb5),lorbdb5,childinfo)
              call la_ylarfgp(p - i + 1,x11(i,i),x11(i + 1,i),1,taup1(i))
              if (i < m - p) then
                 call la_ylarfgp(m - p - i,x21(i + 1,i),x21(i + 2,i),1,taup2(i))
                 phi(i) = atan2(real(x21(i + 1,i),KIND=xdp),real(x11(i,i),KIND=xdp))
                 c = cos(phi(i))
                 s = sin(phi(i))
                 x21(i + 1,i) = cone
                 call la_ylarf('L',m - p - i,q - i,x21(i + 1,i),1,conjg(taup2(i)),x21(i + 1,i + 1), &
                           ldx21,work(ilarf))
              end if
              x11(i,i) = cone
              call la_ylarf('L',p - i + 1,q - i,x11(i,i),1,conjg(taup1(i)),x11(i,i + 1),ldx11, &
...
#ifdef LA_WITH_XDP
     !> YUNBDB4: simultaneously bidiagonalizes the blocks of a tall and skinny
     !> matrix X with orthonomal columns:
     !> [ B11 ]
     !> [ X11 ]   [ P1 |    ] [  0  ]
     !> [-----] = [---------] [-----] Q1**T .
     ...
     !> [  0  ]
     !> X11 is P-by-Q, and X21 is (M-P)-by-Q. M-Q must be no larger than P,
     !> M-P, or Q. Routines YUNBDB1, YUNBDB2, and YUNBDB3 handle cases in
     !> which M-Q is not the minimum dimension.
     !> The unitary matrices P1, P2, and Q1 are P-by-P, (M-P)-by-(M-P),
     !> and (M-Q)-by-(M-Q), respectively. They are represented implicitly by
     !> Householder vectors.
     ...
     subroutine la_yunbdb4(m,p,q,x11,ldx11,x21,ldx21,theta,phi,taup1,taup2,tauq1, &
               phantom,work,lwork,info)
        use la_constants_xdp
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ...
           ! Array Arguments
           real(xdp),intent(out) :: phi(*),theta(*)
           complex(xdp),intent(out) :: phantom(*),taup1(*),taup2(*),tauq1(*),work(*)
           complex(xdp),intent(inout) :: x11(ldx11,*),x21(ldx21,*)
        ! ====================================================================
           ...
           do i = 1,m - q
              if (i == 1) then
                 ...
                 call la_yunbdb5(p,m - p,q,phantom(1),1,phantom(p + 1),1,x11,ldx11,x21, &
                           ldx21,work(iorbdb5),lorbdb5,childinfo)
                 call la_yscal(p,cnegone,phantom(1),1)
                 call la_ylarfgp(p,phantom(1),phantom(2),1,taup1(1))
                 call la_ylarfgp(m - p,phantom(p + 1),phantom(p + 2),1,taup2(1))
                 theta(i) = atan2(real(phantom(1),KIND=xdp),real(phantom(p + 1),KIND=xdp))

                 c = cos(theta(i))
                 s = sin(theta(i))
                 phantom(1) = cone
                 phantom(p + 1) = cone
                 call la_ylarf('L',p,q,phantom(1),1,conjg(taup1(1)),x11,ldx11,work( &
                           ilarf))
                 call la_ylarf('L',m - p,q,phantom(p + 1),1,conjg(taup2(1)),x21,ldx21, &
                           work(ilarf))
              else
                 call la_yunbdb5(p - i + 1,m - p - i + 1,q - i + 1,x11(i,i - 1),1,x21(i,i - 1),1,x11(i,i) &
                           ,ldx11,x21(i,i),ldx21,work(iorbdb5),lorbdb5,childinfo)
                 call la_yscal(p - i + 1,cnegone,x11(i,i - 1),1)
                 call la_ylarfgp(p - i + 1,x11(i,i - 1),x11(i + 1,i - 1),1,taup1(i))
                 call la_ylarfgp(m - p - i + 1,x21(i,i - 1),x21(i + 1,i - 1),1,taup2(i))
                 theta(i) = atan2(real(x11(i,i - 1),KIND=xdp),real(x21(i,i - 1),KIND=xdp))
                 c = cos(theta(i))
                 s = sin(theta(i))
                 x11(i,i - 1) = cone
                 x21(i,i - 1) = cone
                 call la_ylarf('L',p - i + 1,q - i + 1,x11(i,i - 1),1,conjg(taup1(i)),x11(i,i), &
                           ldx11,work(ilarf))
                 call la_ylarf('L',m - p - i + 1,q - i + 1,x21(i,i - 1),1,conjg(taup2(i)),x21(i,i), &
                           ldx21,work(ilarf))
              end if
              call la_yxrot(q - i + 1,x11(i,i),ldx11,x21(i,i),ldx21,s,-c)
              call la_ylacgv(q - i + 1,x21(i,i),ldx21)
              call la_ylarfgp(q - i + 1,x21(i,i),x21(i,i + 1),ldx21,tauq1(i))
              c = real(x21(i,i),KIND=xdp)
              x21(i,i) = cone
              call la_ylarf('R',p - i,q - i + 1,x21(i,i),ldx21,tauq1(i),x11(i + 1,i),ldx11, &
                        work(ilarf))
              call la_ylarf('R',m - p - i,q - i + 1,x21(i,i),ldx21,tauq1(i),x21(i + 1,i),ldx21, &
                        work(ilarf))
              call la_ylacgv(q - i + 1,x21(i,i),ldx21)
              if (i < m - q) then
                 s = sqrt(la_xynrm2(p - i,x11(i + 1,i),1)**2 + la_xynrm2(m - p - i,x21(i + 1, &
                           i),1)**2)
                 phi(i) = atan2(s,c)
              end if
           end do
           ...
           do i = m - q + 1,p
              ...
              call la_ylarf('R',p - i,q - i + 1,x11(i,i),ldx11,tauq1(i),x11(i + 1,i),ldx11, &
                        work(ilarf))
              call la_ylarf('R',q - p,q - i + 1,x11(i,i),ldx11,tauq1(i),x21(m - q + 1,i),ldx21, &
                        work(ilarf))
              call la_ylacgv(q - i + 1,x11(i,i),ldx11)
           end do
           ! reduce the bottom-right portion of x21 to [ 0 i ]
           do i = p + 1,q
              call la_ylacgv(q - i + 1,x21(m - q + i - p,i),ldx21)
              call la_ylarfgp(q - i + 1,x21(m - q + i - p,i),x21(m - q + i - p,i + 1),ldx21,tauq1(i))

              x21(m - q + i - p,i) = cone
              call la_ylarf('R',q - i,q - i + 1,x21(m - q + i - p,i),ldx21,tauq1(i),x21(m - q + i - p + 1,i) &
                        ,ldx21,work(ilarf))
              call la_ylacgv(q - i + 1,x21(m - q + i - p,i),ldx21)
           end do
           return
     end subroutine la_yunbdb4
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
     !> YUNCSD2BY1: computes the CS decomposition of an M-by-Q matrix X with
     !> orthonormal columns that has been partitioned into a 2-by-1 block
     !> structure:
     !> [  I1 0  0 ]
     !> [  0  C  0 ]
     ...
     subroutine la_yuncsd2by1(jobu1,jobu2,jobv1t,m,p,q,x11,ldx11,x21,ldx21,theta, &
               u1,ldu1,u2,ldu2,v1t,ldv1t,work,lwork,rwork,lrwork,iwork,info)
        use la_constants_xdp
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ...
           real(xdp),intent(out) :: rwork(*)
           real(xdp),intent(out) :: theta(*)
           complex(xdp),intent(out) :: u1(ldu1,*),u2(ldu2,*),v1t(ldv1t,*),work(*)
           complex(xdp),intent(inout) :: x11(ldx11,*),x21(ldx21,*)
           integer(ilp),intent(out) :: iwork(*)
        ! =====================================================================
           ...
           ! | tauq1 (max(1,q))                        |
           ! |-----------------------------------------|
           ! | la_yunbdb work | la_yungqr work | la_yunglq work |
           ! |             |             |             |
           ! |             |             |             |
           ! |             |             |             |
           ! |             |             |             |
           ...
           if (info == 0) then
              ...
              lorglqopt = 1
              if (r == q) then
                 call la_yunbdb1(m,p,q,x11,ldx11,x21,ldx21,theta,dum,cdum,cdum, &
                           cdum,work,-1,childinfo)
                 lorbdb = int(work(1),KIND=ilp)
                 if (wantu1 .and. p > 0) then
                    call la_yungqr(p,p,q,u1,ldu1,cdum,work(1),-1,childinfo)
                    lorgqrmin = max(lorgqrmin,p)
                    lorgqropt = max(lorgqropt,int(work(1),KIND=ilp))
                 end if
                 if (wantu2 .and. m - p > 0) then
                    call la_yungqr(m - p,m - p,q,u2,ldu2,cdum,work(1),-1,childinfo)

                    lorgqrmin = max(lorgqrmin,m - p)
                    lorgqropt = max(lorgqropt,int(work(1),KIND=ilp))
                 end if
                 if (wantv1t .and. q > 0) then
                    call la_yunglq(q - 1,q - 1,q - 1,v1t,ldv1t,cdum,work(1),-1,childinfo)

                    lorglqmin = max(lorglqmin,q - 1)
                    lorglqopt = max(lorglqopt,int(work(1),KIND=ilp))
                 end if
                 call la_ybbcsd(jobu1,jobu2,jobv1t,'N','N',m,p,q,theta,dum,u1,ldu1, &
                  u2,ldu2,v1t,ldv1t,cdum,1,dum,dum,dum,dum,dum,dum,dum,dum,rwork(1),- &
                            1,childinfo)
                 lbbcsd = int(rwork(1),KIND=ilp)
              else if (r == p) then
                 call la_yunbdb2(m,p,q,x11,ldx11,x21,ldx21,theta,dum,cdum,cdum, &
                           cdum,work(1),-1,childinfo)
                 lorbdb = int(work(1),KIND=ilp)
                 if (wantu1 .and. p > 0) then
                    call la_yungqr(p - 1,p - 1,p - 1,u1(2,2),ldu1,cdum,work(1),-1,childinfo &
                              )
                    lorgqrmin = max(lorgqrmin,p - 1)
                    lorgqropt = max(lorgqropt,int(work(1),KIND=ilp))
                 end if
                 ...
                 if (wantv1t .and. q > 0) then
                    call la_yunglq(q,q,r,v1t,ldv1t,cdum,work(1),-1,childinfo)
                    lorglqmin = max(lorglqmin,q)
                    lorglqopt = max(lorglqopt,int(work(1),KIND=ilp))
                 end if
                 call la_ybbcsd(jobv1t,'N',jobu1,jobu2,'T',m,q,p,theta,dum,v1t, &
                 ldv1t,cdum,1,u1,ldu1,u2,ldu2,dum,dum,dum,dum,dum,dum,dum,dum,rwork( &
                           1),-1,childinfo)
                 lbbcsd = int(rwork(1),KIND=ilp)
              else if (r == m - p) then
                 call la_yunbdb3(m,p,q,x11,ldx11,x21,ldx21,theta,dum,cdum,cdum, &
                           cdum,work(1),-1,childinfo)
                 lorbdb = int(work(1),KIND=ilp)
                 if (wantu1 .and. p > 0) then
                    call la_yungqr(p,p,q,u1,ldu1,cdum,work(1),-1,childinfo)
                 ...
                 if (wantu2 .and. m - p > 0) then
                    call la_yungqr(m - p - 1,m - p - 1,m - p - 1,u2(2,2),ldu2,cdum,work(1),-1, &
                              childinfo)
                    lorgqrmin = max(lorgqrmin,m - p - 1)
                    lorgqropt = max(lorgqropt,int(work(1),KIND=ilp))
                 end if
                 ...
                 call la_ybbcsd('N',jobv1t,jobu2,jobu1,'T',m,m - q,m - p,theta,dum,cdum, &
                  1,v1t,ldv1t,u2,ldu2,u1,ldu1,dum,dum,dum,dum,dum,dum,dum,dum,rwork( &
                            1),-1,childinfo)
                 lbbcsd = int(rwork(1),KIND=ilp)
              else
                 call la_yunbdb4(m,p,q,x11,ldx11,x21,ldx21,theta,dum,cdum,cdum, &
                           cdum,cdum,work(1),-1,childinfo)
                 lorbdb = m + int(work(1),KIND=ilp)
                 if (wantu1 .and. p > 0) then
                    call la_yungqr(p,p,m - q,u1,ldu1,cdum,work(1),-1,childinfo)
                    lorgqrmin = max(lorgqrmin,p)
                    lorgqropt = max(lorgqropt,int(work(1),KIND=ilp))
                 end if
                 if (wantu2 .and. m - p > 0) then
                    call la_yungqr(m - p,m - p,m - q,u2,ldu2,cdum,work(1),-1,childinfo)

                    lorgqrmin = max(lorgqrmin,m - p)
                    lorgqropt = max(lorgqropt,int(work(1),KIND=ilp))
                 end if
                 if (wantv1t .and. q > 0) then
                    call la_yunglq(q,q,q,v1t,ldv1t,cdum,work(1),-1,childinfo)
                    lorglqmin = max(lorglqmin,q)
                    lorglqopt = max(lorglqopt,int(work(1),KIND=ilp))
                 end if
                 call la_ybbcsd(jobu2,jobu1,'N',jobv1t,'N',m,m - p,m - q,theta,dum,u2, &
                 ldu2,u1,ldu1,cdum,1,v1t,ldv1t,dum,dum,dum,dum,dum,dum,dum,dum,rwork( &
                           1),-1,childinfo)
                 lbbcsd = int(rwork(1),KIND=ilp)
              end if
           ...
           if (info /= 0) then
              call la_xerbla('YUNCSD2BY1',-info)
              return
           else if (lquery) then
              return
           end if
           ...
           if (r == q) then
              ! case 1: r = q
              ! simultaneously bidiagonalize x11 and x21
              call la_yunbdb1(m,p,q,x11,ldx11,x21,ldx21,theta,rwork(iphi),work( &
                        itaup1),work(itaup2),work(itauq1),work(iorbdb),lorbdb,childinfo)
              ! accumulate householder reflectors
              if (wantu1 .and. p > 0) then
                 call la_ylacpy('L',p,q,x11,ldx11,u1,ldu1)
                 call la_yungqr(p,p,q,u1,ldu1,work(itaup1),work(iorgqr),lorgqr, &
                           childinfo)
              end if
              if (wantu2 .and. m - p > 0) then
                 call la_ylacpy('L',m - p,q,x21,ldx21,u2,ldu2)
                 call la_yungqr(m - p,m - p,q,u2,ldu2,work(itaup2),work(iorgqr),lorgqr, &
                           childinfo)
              end if
              if (wantv1t .and. q > 0) then
                 v1t(1,1) = cone
                 ...
                 call la_ylacpy('U',q - 1,q - 1,x21(1,2),ldx21,v1t(2,2),ldv1t)
                 call la_yunglq(q - 1,q - 1,q - 1,v1t(2,2),ldv1t,work(itauq1),work(iorglq), &
                           lorglq,childinfo)
              end if
              ! simultaneously diagonalize x11 and x21.
              call la_ybbcsd(jobu1,jobu2,jobv1t,'N','N',m,p,q,theta,rwork(iphi),u1, &
              ldu1,u2,ldu2,v1t,ldv1t,cdum,1,rwork(ib11d),rwork(ib11e),rwork(ib12d),rwork( &
              ib12e),rwork(ib21d),rwork(ib21e),rwork(ib22d),rwork(ib22e),rwork(ibbcsd),lrwork - &
                        ibbcsd + 1,childinfo)
              ! permute rows and columns to place czero submatrices in
              ...
              ! case 2: r = p
              ! simultaneously bidiagonalize x11 and x21
              call la_yunbdb2(m,p,q,x11,ldx11,x21,ldx21,theta,rwork(iphi),work( &
                        itaup1),work(itaup2),work(itauq1),work(iorbdb),lorbdb,childinfo)
              ! accumulate householder reflectors
              if (wantu1 .and. p > 0) then
                 u1(1,1) = cone
                 ...
                 call la_ylacpy('L',p - 1,p - 1,x11(2,1),ldx11,u1(2,2),ldu1)
                 call la_yungqr(p - 1,p - 1,p - 1,u1(2,2),ldu1,work(itaup1),work(iorgqr), &
                           lorgqr,childinfo)
              end if
              if (wantu2 .and. m - p > 0) then
                 call la_ylacpy('L',m - p,q,x21,ldx21,u2,ldu2)
              ...
              if (wantv1t .and. q > 0) then
                 call la_ylacpy('U',p,q,x11,ldx11,v1t,ldv1t)
                 call la_yunglq(q,q,r,v1t,ldv1t,work(itauq1),work(iorglq),lorglq, &
                           childinfo)
              end if
              ! simultaneously diagonalize x11 and x21.
              call la_ybbcsd(jobv1t,'N',jobu1,jobu2,'T',m,q,p,theta,rwork(iphi),v1t, &
               ldv1t,cdum,1,u1,ldu1,u2,ldu2,rwork(ib11d),rwork(ib11e),rwork(ib12d),rwork( &
               ib12e),rwork(ib21d),rwork(ib21e),rwork(ib22d),rwork(ib22e),rwork(ibbcsd), &
                         lbbcsd,childinfo)
              ! permute rows and columns to place identity submatrices in
              ...
              ! case 3: r = m-p
              ! simultaneously bidiagonalize x11 and x21
              call la_yunbdb3(m,p,q,x11,ldx11,x21,ldx21,theta,rwork(iphi),work( &
                        itaup1),work(itaup2),work(itauq1),work(iorbdb),lorbdb,childinfo)
              ! accumulate householder reflectors
              if (wantu1 .and. p > 0) then
                 call la_ylacpy('L',p,q,x11,ldx11,u1,ldu1)
              ...
              if (wantu2 .and. m - p > 0) then
                 ...
                 call la_ylacpy('L',m - p - 1,m - p - 1,x21(2,1),ldx21,u2(2,2),ldu2)
                 call la_yungqr(m - p - 1,m - p - 1,m - p - 1,u2(2,2),ldu2,work(itaup2),work(iorgqr) &
                           ,lorgqr,childinfo)
              end if
              if (wantv1t .and. q > 0) then
                 call la_ylacpy('U',m - p,q,x21,ldx21,v1t,ldv1t)
                 call la_yunglq(q,q,r,v1t,ldv1t,work(itauq1),work(iorglq),lorglq, &
                           childinfo)
              end if
              ! simultaneously diagonalize x11 and x21.
              call la_ybbcsd('N',jobv1t,jobu2,jobu1,'T',m,m - q,m - p,theta,rwork(iphi), &
              cdum,1,v1t,ldv1t,u2,ldu2,u1,ldu1,rwork(ib11d),rwork(ib11e),rwork(ib12d), &
              rwork(ib12e),rwork(ib21d),rwork(ib21e),rwork(ib22d),rwork(ib22e),rwork(ibbcsd), &
                        lbbcsd,childinfo)
              ! permute rows and columns to place identity submatrices in
              ...
              if (q > r) then
                 ...
                 if (wantu1) then
                    call la_ylapmt(.false.,p,q,u1,ldu1,iwork)
                 end if
                 if (wantv1t) then
                    call la_ylapmr(.false.,q,q,v1t,ldv1t,iwork)
                 end if
              end if
           else
              ! case 4: r = m-q
              ! simultaneously bidiagonalize x11 and x21
              call la_yunbdb4(m,p,q,x11,ldx11,x21,ldx21,theta,rwork(iphi),work( &
              itaup1),work(itaup2),work(itauq1),work(iorbdb),work(iorbdb + m),lorbdb - m, &
                        childinfo)
              ! accumulate householder reflectors
              if (wantu2 .and. m - p > 0) then
                 call la_ycopy(m - p,work(iorbdb + p),1,u2,1)
              end if
              if (wantu1 .and. p > 0) then
                 call la_ycopy(p,work(iorbdb),1,u1,1)
                 do j = 2,p
                    u1(1,j) = czero
                 end do
                 call la_ylacpy('L',p - 1,m - q - 1,x11(2,1),ldx11,u1(2,2),ldu1)
                 call la_yungqr(p,p,m - q,u1,ldu1,work(itaup1),work(iorgqr),lorgqr, &
                           childinfo)
              end if
              if (wantu2 .and. m - p > 0) then
                 do j = 2,m - p
                 ...
                 call la_ylacpy('L',m - p - 1,m - q - 1,x21(2,1),ldx21,u2(2,2),ldu2)
                 call la_yungqr(m - p,m - p,m - q,u2,ldu2,work(itaup2),work(iorgqr),lorgqr, &
                           childinfo)
              end if
              if (wantv1t .and. q > 0) then
                 call la_ylacpy('U',m - q,q,x21,ldx21,v1t,ldv1t)
                 call la_ylacpy('U',p - (m - q),q - (m - q),x11(m - q + 1,m - q + 1),ldx11,v1t(m - q + 1,m - q + &
                           1),ldv1t)
                 call la_ylacpy('U',-p + q,q - p,x21(m - q + 1,p + 1),ldx21,v1t(p + 1,p + 1),ldv1t)

                 call la_yunglq(q,q,q,v1t,ldv1t,work(itauq1),work(iorglq),lorglq, &
                           childinfo)
              end if
              ! simultaneously diagonalize x11 and x21.
              call la_ybbcsd(jobu2,jobu1,'N',jobv1t,'N',m,m - p,m - q,theta,rwork(iphi), &
              u2,ldu2,u1,ldu1,cdum,1,v1t,ldv1t,rwork(ib11d),rwork(ib11e),rwork(ib12d), &
              rwork(ib12e),rwork(ib21d),rwork(ib21e),rwork(ib22d),rwork(ib22e),rwork(ibbcsd), &
                        lbbcsd,childinfo)
              ! permute rows and columns to place identity submatrices in
              ...
              if (p > r) then
                 ...
                 if (wantu1) then
                    call la_ylapmt(.false.,p,p,u1,ldu1,iwork)
                 end if
                 if (wantv1t) then
                    call la_ylapmr(.false.,p,q,v1t,ldv1t,iwork)
                 end if
              end if
           end if
           return
     end subroutine la_yuncsd2by1
#endif
#ifdef LA_WITH_QP
