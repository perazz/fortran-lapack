...
module la_lapack_eigv_tridiag2
     ...
     pure subroutine la_xlaebz(ijob,nitmax,n,mmax,minp,nbmin,abstol,reltol,pivmin,d, &
               e,e2,nval,ab,c,mout,nab,work,iwork,info)
        ...
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ...
           integer(ilp),intent(in) :: ijob,minp,mmax,n,nbmin,nitmax
           integer(ilp),intent(out) :: info,mout
           real(xdp),intent(in) :: abstol,pivmin,reltol
           ! Array Arguments
           integer(ilp),intent(out) :: iwork(*)
           integer(ilp),intent(inout) :: nab(mmax,*),nval(*)
           real(xdp),intent(inout) :: ab(mmax,*),c(*)
           real(xdp),intent(in) :: d(*),e(*),e2(*)
     ...
     pure subroutine la_xlaeda(n,tlvls,curlvl,curpbm,prmptr,perm,givptr,givcol,givnum, &
                q,qptr,z,ztemp,info)
        use la_constants_xdp
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ...
           ! Array Arguments
           integer(ilp),intent(in) :: givcol(2,*),givptr(*),perm(*),prmptr(*),qptr(*)
           real(xdp),intent(in) :: givnum(2,*),q(*)
           real(xdp),intent(out) :: z(*),ztemp(*)
        ! =====================================================================
           ...
           ! the sqrt in case the machine underestimates one of these square
           ! roots.
           bsiz1 = int(half + sqrt(real(qptr(curr + 1) - qptr(curr),KIND=xdp)),KIND=ilp)
           bsiz2 = int(half + sqrt(real(qptr(curr + 2) - qptr(curr + 1),KIND=xdp)),KIND=ilp)

           do k = 1,mid - bsiz1 - 1
              z(k) = zero
           end do
           call la_xcopy(bsiz1,q(qptr(curr) + bsiz1 - 1),bsiz1,z(mid - bsiz1),1)
           call la_xcopy(bsiz2,q(qptr(curr + 1)),bsiz2,z(mid),1)
           do k = mid + bsiz2,n
              z(k) = zero
           end do
           ! loop through remaining levels 1 -> curlvl applying the givens
           ...
           loop_70: do k = 1,curlvl - 1
             ...
             ! apply givens at curr and curr+1
              do i = givptr(curr),givptr(curr + 1) - 1
                 call la_xrot(1,z(zptr1 + givcol(1,i) - 1),1,z(zptr1 + givcol(2,i) - 1), &
                           1,givnum(1,i),givnum(2,i))
              end do
              do i = givptr(curr + 1),givptr(curr + 2) - 1
                 call la_xrot(1,z(mid - 1 + givcol(1,i)),1,z(mid - 1 + givcol(2,i)),1, &
                           givnum(1,i),givnum(2,i))
              end do
              psiz1 = prmptr(curr + 1) - prmptr(curr)
              psiz2 = prmptr(curr + 2) - prmptr(curr + 1)
              ...
              if (bsiz1 > 0) then
                 call la_xgemv('T',bsiz1,bsiz1,one,q(qptr(curr)),bsiz1,ztemp(1), &
                           1,zero,z(zptr1),1)
              end if
              call la_xcopy(psiz1 - bsiz1,ztemp(bsiz1 + 1),1,z(zptr1 + bsiz1),1)
              if (bsiz2 > 0) then
                 call la_xgemv('T',bsiz2,bsiz2,one,q(qptr(curr + 1)),bsiz2,ztemp( &
                           psiz1 + 1),1,zero,z(mid),1)
              end if
              call la_xcopy(psiz2 - bsiz2,ztemp(psiz1 + bsiz2 + 1),1,z(mid + bsiz2),1)

              ptr = ptr + 2**(tlvls - k)
           end do loop_70
           return
     end subroutine la_xlaeda
#endif
#ifdef LA_WITH_QP
     ...
     pure subroutine la_xlamrg(n1,n2,a,xtrd1,xtrd2,index)
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ! Scalar Arguments
           integer(ilp),intent(in) :: xtrd1,xtrd2,n1,n2
           ! Array Arguments
           integer(ilp),intent(out) :: index(*)
...
#ifdef LA_WITH_XDP
     !> Compute the splitting points with threshold SPLTOL.
     !> XLARRA: sets any "small" off-diagonal elements to zero.

     pure subroutine la_xlarra(n,d,e,e2,spltol,tnrm,nsplit,isplit,info)
        use la_constants_xdp
        ! -- lapack auxiliary routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ...
           integer(ilp),intent(out) :: info,nsplit
           integer(ilp),intent(in) :: n
           real(xdp),intent(in) :: spltol,tnrm
           ! Array Arguments
           integer(ilp),intent(out) :: isplit(*)
           real(xdp),intent(in) :: d(*)
           real(xdp),intent(inout) :: e(*),e2(*)
        ! =====================================================================
     ...
     pure subroutine la_xlarrc(jobt,n,vl,vu,d,e,pivmin,eigcnt,lcnt,rcnt,info)
        use la_constants_xdp
        ...
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ...
           integer(ilp),intent(out) :: eigcnt,info,lcnt,rcnt
           integer(ilp),intent(in) :: n
           real(xdp),intent(in) :: pivmin,vl,vu
           ! Array Arguments
        ...
        ! =====================================================================
           ...
           integer(ilp) :: i
           logical(lk) :: matt
           real(xdp) :: lpivot,rpivot,sl,su,tmp,tmp2
           ! Executable Statements
           info = 0
           ! quick return if possible
           if (n <= 0) then
...
#ifdef LA_WITH_XDP
     !> XLARRD: computes the eigenvalues of a symmetric tridiagonal
     !> matrix T to suitable accuracy. This is an auxiliary code to be
     !> called from XSTEMR.
     !> The user may ask for all eigenvalues, all eigenvalues
     !> in the half-open interval (VL, VU], or the IL-th through IU-th
     ...
     pure subroutine la_xlarrd(range,order,n,vl,vu,il,iu,gers,reltol,d,e,e2, &
               pivmin,nsplit,isplit,m,w,werr,wl,wu,iblock,indexw,work,iwork,info)
        use la_constants_xdp,only:zero,half,two
        ! -- lapack auxiliary routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ! Scalar Arguments
           ...
           integer(ilp),intent(in) :: il,iu,n,nsplit
           integer(ilp),intent(out) :: info,m
           real(xdp),intent(in) :: pivmin,reltol,vl,vu
           real(xdp),intent(out) :: wl,wu
           ! Array Arguments
           integer(ilp),intent(out) :: iblock(*),indexw(*),iwork(*)
           integer(ilp),intent(in) :: isplit(*)
           real(xdp),intent(in) :: d(*),e(*),e2(*),gers(*)
           real(xdp),intent(out) :: w(*),werr(*),work(*)
        ! =====================================================================
           ! Parameters
           real(xdp),parameter :: fudge = two
           integer(ilp),parameter :: allrng = 1
           integer(ilp),parameter :: valrng = 2
           integer(ilp),parameter :: indrng = 3
           ...
           integer(ilp) :: i,ib,ibegin,idiscl,idiscu,ie,iend,iinfo,im,in,ioff,iout, &
                     irange,itmax,itmp1,itmp2,iw,iwoff,j,jblk,jdisc,je,jee,nb,nwl,nwu
           real(xdp) :: atoli,eps,gl,gu,rtoli,tmp1,tmp2,tnorm,uflow,wkill,wlu, &
                     wul
           ! Local Arrays
           integer(ilp) :: idumma(1)
           ! Intrinsic Functions
           ...
           if (irange == indrng) then
              ! range='i': compute an interval containing eigenvalues
              ! il through iu. the initial interval [gl,gu] from the global
              ! gerschgorin bounds gl and gu is refined by la_xlaebz.
              itmax = int((log(tnorm + pivmin) - log(pivmin))/log(two),KIND=ilp) + 2
              work(n + 1) = gl
              work(n + 2) = gl
              work(n + 3) = gu
              ...
              iwork(5) = il - 1
              iwork(6) = iu
              call la_xlaebz(3,itmax,n,2,2,nb,atoli,rtoli,pivmin,d,e,e2,iwork(5) &
                        ,work(n + 1),work(n + 5),iout,iwork,w,iblock,iinfo)
              if (iinfo /= 0) then
                 info = iinfo
                 return
           ...
           loop_70: do jblk = 1,nsplit
              ...
              ! range = 'i', il = iu = 4
                ! original tridiagonal, d = [
                 ! -0.150102010615740e+00_xdp
                 ! -0.849897989384260e+00_xdp
                 ! -0.128208148052635e-15_xdp
                  ! 0.128257718286320e-15_xdp
                ! ];
                ! e = [
                 ! -0.357171383266986e+00_xdp
                 ! -0.180411241501588e-15_xdp
                 ! -0.175152352710251e-15_xdp
                ! ];
               ! else if( in==2 ) then
      ! *           2x2 block
                  ! disc = sqrt( (half*(d(ibegin)-d(iend)))**2 + e(ibegin)**2 )
      ...
      ! *              the uncertainty of eigenvalues of a 2x2 matrix is very small
                 ...
                 ! copy eigenvalues into w and iblock
                 ! use -jblk for block number for unconverged eigenvalues.
                 ! loop over the number of output intervals from la_xlaebz
                 do j = 1,iout
                    ! eigenvalue approximation is middle point of interval
                    tmp1 = half*(work(j + n) + work(j + in + n))
                    ! semi length of error interval
...
#ifdef LA_WITH_XDP
     !> Given the initial eigenvalue approximations of T, XLARRJ:
     !> does  bisection to refine the eigenvalues of T,
     !> W( IFIRST-OFFSET ) through W( ILAST-OFFSET ), to more accuracy. Initial
     !> guesses for these eigenvalues are input in W, the corresponding estimate
     !> of the error in these guesses in WERR. During bisection, intervals
     ...
     pure subroutine la_xlarrj(n,d,e2,ifirst,ilast,rtol,offset,w,werr,work,iwork, &
               pivmin,spdiam,info)
        use la_constants_xdp
        ! -- lapack auxiliary routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ...
           integer(ilp),intent(in) :: ifirst,ilast,n,offset
           integer(ilp),intent(out) :: info
           real(xdp),intent(in) :: pivmin,rtol,spdiam
           ! Array Arguments
           integer(ilp),intent(out) :: iwork(*)
           real(xdp),intent(in) :: d(*),e2(*)
           real(xdp),intent(inout) :: w(*),werr(*)
           real(xdp),intent(out) :: work(*)
        ! =====================================================================
           ...
           integer(ilp) :: cnt,i,i1,i2,ii,iter,j,k,next,nint,olnint,p,prev, &
                     savi1
           real(xdp) :: dplus,fac,left,mid,right,s,tmp,width
           ! Intrinsic Functions
           intrinsic :: abs,max
           ! Executable Statements
           info = 0
...
#ifdef LA_WITH_XDP
     !> XLARRK: computes one eigenvalue of a symmetric tridiagonal
     !> matrix T to suitable accuracy. This is an auxiliary code to be
     !> called from XSTEMR.
     !> To avoid overflow, the matrix must be scaled so that its
     !> largest element is no greater than overflow**(1/2) * underflow**(1/4) in absolute value, and for greatest
     ...
     pure subroutine la_xlarrk(n,iw,gl,gu,d,e2,pivmin,reltol,w,werr,info)
        use la_constants_xdp,only:zero,half,two
        ! -- lapack auxiliary routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ...
           integer(ilp),intent(out) :: info
           integer(ilp),intent(in) :: iw,n
           real(xdp),intent(in) :: pivmin,reltol,gl,gu
           real(xdp),intent(out) :: w,werr
           ! Array Arguments
           real(xdp),intent(in) :: d(*),e2(*)
        ! =====================================================================
           ! Parameters
           ...
           ! Local Scalars
           integer(ilp) :: i,it,itmax,negcnt
           real(xdp) :: atoli,eps,left,mid,right,rtoli,tmp1,tmp2,tnorm
           ! Intrinsic Functions
           intrinsic :: abs,int,log,max
           ! Executable Statements
           ! quick return if possible
     ...
     pure subroutine la_xlarrr(n,d,e,info)
        ...
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ...
           ! Array Arguments
           real(xdp),intent(in) :: d(*)
           real(xdp),intent(inout) :: e(*)
        ! =====================================================================
           ! Parameters
           real(xdp),parameter :: relcond = 0.999_xdp

           ! Local Scalars
           integer(ilp) :: i
           logical(lk) :: yesrel
           real(xdp) :: eps,safmin,smlnum,rmin,tmp,tmp2,offdig,offdig2
           ! Intrinsic Functions
           intrinsic :: abs
           ! Executable Statements
           ! quick return if possible
           ...
           ! condition number is reasonable, not 1/macheps.
           ! the threshold should be compatible with other thresholds used in the
           ! code. we set  offdig + offdig2 <= .999_xdp =: relcond, it corresponds
           ! to losing at most 3 decimal digits: 1 / (1 - (offdig + offdig2)) <= 1000
           ! instead of the current offdig + offdig2 < 1
           yesrel = .true.
           offdig = zero
...
#ifdef LA_WITH_XDP
     !> XLANEG: computes the Sturm count, the number of negative pivots
     !> encountered while factoring tridiagonal T - sigma I = L D L^T.
     !> This implementation works directly on the factors without forming
     !> the tridiagonal matrix T.  The Sturm count is also the number of
     !> eigenvalues of T less than sigma.
     !> This routine is called from XLARRB.
     !> The current routine does not use the PIVMIN parameter but rather
     !> requires IEEE-754 propagation of Infinities and NaNs.  This
     !> routine also has no input range restrictions but does require
     !> default exception handling such that x/0 produces Inf when x is
     ...
     pure integer(ilp) function la_xlaneg(n,d,lld,sigma,pivmin,r)
        use la_constants_xdp
        ! -- lapack auxiliary routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ! Scalar Arguments
           integer(ilp),intent(in) :: n,r
           real(xdp),intent(in) :: pivmin,sigma
           ! Array Arguments
           real(xdp),intent(in) :: d(*),lld(*)
        ! =====================================================================
           ! Parameters
           integer(ilp),parameter :: blklen = 128
           ...
           ! Local Scalars
           integer(ilp) :: bj,j,neg1,neg2,negcnt
           real(xdp) :: bsav,dminus,dplus,gamma,p,t,tmp
           logical(lk) :: sawnan
           ! Intrinsic Functions
           intrinsic :: min,max
           ! Executable Statements
     ...
     pure subroutine la_xlar1v(n,b1,bn,lambda,d,l,ld,lld,pivmin,gaptol,z,wantnc, &
               negcnt,ztz,mingma,r,isuppz,nrminv,resid,rqcorr,work)
        ...
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ...
           integer(ilp),intent(out) :: negcnt
           integer(ilp),intent(inout) :: r
           real(xdp),intent(in) :: gaptol,lambda,pivmin
           real(xdp),intent(out) :: mingma,nrminv,resid,rqcorr,ztz
           ! Array Arguments
           integer(ilp),intent(out) :: isuppz(*)
           real(xdp),intent(in) :: d(*),l(*),ld(*),lld(*)
           real(xdp),intent(out) :: work(*)
        ...
        ! =====================================================================
           ...
           logical(lk) :: sawnan1,sawnan2
           integer(ilp) :: i,indlpl,indp,inds,indumn,neg1,neg2,r1,r2
           real(xdp) :: dminus,dplus,eps,s,tmp
           ! Intrinsic Functions
           intrinsic :: abs
           ! Executable Statements
           eps = la_xlamch('PRECISION')
     ...
     pure subroutine la_xlarrb(n,d,lld,ifirst,ilast,rtol1,rtol2,offset,w,wgap,werr, &
               work,iwork,pivmin,spdiam,twist,info)
        use la_constants_xdp
        ! -- lapack auxiliary routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ...
           integer(ilp),intent(in) :: ifirst,ilast,n,offset,twist
           integer(ilp),intent(out) :: info
           real(xdp),intent(in) :: pivmin,rtol1,rtol2,spdiam
           ! Array Arguments
           integer(ilp),intent(out) :: iwork(*)
           real(xdp),intent(in) :: d(*),lld(*)
           real(xdp),intent(inout) :: w(*),werr(*),wgap(*)
           real(xdp),intent(out) :: work(*)
        ! =====================================================================
           ...
           ! Local Scalars
           integer(ilp) :: i,i1,ii,ip,iter,k,negcnt,next,nint,olnint,prev,r
           real(xdp) :: back,cvrgd,gap,left,lgap,mid,mnwdth,rgap,right,tmp,width
           ! Intrinsic Functions
           intrinsic :: abs,max,min
           ! Executable Statements
           info = 0
           ...
           loop_75: do i = i1,ilast
              ...
              back = werr(ii)
              20 continue
              negcnt = la_xlaneg(n,d,lld,left,pivmin,r)
              if (negcnt > i - 1) then
                 left = left - back
                 back = two*back
                 go to 20
              ...
              back = werr(ii)
              50 continue
              negcnt = la_xlaneg(n,d,lld,right,pivmin,r)
               if (negcnt < i) then
                  right = right + back
                  back = two*back
                  go to 50
           ...
           loop_100: do ip = 1,olnint
              ...
              prev = i
              ! perform one bisection step
              negcnt = la_xlaneg(n,d,lld,mid,pivmin,r)
              if (negcnt <= i - 1) then
                 work(k - 1) = mid
              else
                 work(k) = mid
     ...
     !> Given the initial representation L D L^T and its cluster of close
     !> eigenvalues (in a relative measure), W( CLSTRT ), W( CLSTRT+1 ), ...
     !> W( CLEND ), XLARRF: finds a new relatively robust representation
     !> L D L^T - SIGMA I = L(+) D(+) L(+)^T such that at least one of the
     !> eigenvalues of L(+) D(+) L(+)^T is relatively isolated.

     pure subroutine la_xlarrf(n,d,l,ld,clstrt,clend,w,wgap,werr,spdiam,clgapl, &
               clgapr,pivmin,sigma,dplus,lplus,work,info)
        use la_constants_xdp,only:one,two,four
        ! -- lapack auxiliary routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ! Scalar Arguments
           integer(ilp),intent(in) :: clstrt,clend,n
           integer(ilp),intent(out) :: info
           real(xdp),intent(in) :: clgapl,clgapr,pivmin,spdiam
           real(xdp),intent(out) :: sigma
           ! Array Arguments
           real(xdp),intent(in) :: d(*),l(*),ld(*),w(*),werr(*)
           real(xdp),intent(out) :: dplus(*),lplus(*),work(*)
           real(xdp),intent(inout) :: wgap(*)
        ! =====================================================================
           ! Parameters
           real(xdp),parameter :: quart = 0.25_xdp
           real(xdp),parameter :: maxgrowth1 = 8._xdp
           real(xdp),parameter :: maxgrowth2 = 8._xdp
           integer(ilp),parameter :: ktrymax = 1
           integer(ilp),parameter :: sleft = 1
           integer(ilp),parameter :: sright = 2
           ...
           logical(lk) :: dorrr1,forcer,nofail,sawnan1,sawnan2,tryrrr1
           integer(ilp) :: i,indx,ktry,shift
           real(xdp) :: avgap,bestshift,clwdth,eps,fact,fail,fail2,growthbound,ldelta, &
           ldmax,lsigma,max1,max2,mingap,oldp,prod,rdelta,rdmax,rrr1,rrr2,rsigma,s, &
                     smlgrowth,tmp,znm2
           ! Intrinsic Functions
           intrinsic :: abs
           ...
           ! compute the average gap length of the cluster
           clwdth = abs(w(clend) - w(clstrt)) + werr(clend) + werr(clstrt)
           avgap = clwdth/real(clend - clstrt,KIND=xdp)
           mingap = min(clgapl,clgapr)
           ! initial values for shifts to both ends of cluster
           lsigma = min(w(clstrt),w(clend)) - werr(clstrt)
           rsigma = max(w(clstrt),w(clend)) + werr(clend)
           ...
           s = la_xlamch('S')
           smlgrowth = one/s
           fail = real(n - 1,KIND=xdp)*mingap/(spdiam*eps)
           fail2 = real(n - 1,KIND=xdp)*mingap/(spdiam*sqrt(eps))
           bestshift = lsigma
           ! while (ktry <= ktrymax)
           ktry = 0
           growthbound = maxgrowth1*spdiam
           ...
           sawnan1 = sawnan1 .or. la_xisnan(max1)
           if (forcer .or. (max1 <= growthbound .and. .not. sawnan1)) then
              sigma = lsigma
              shift = sleft
              goto 100
           ...
           sawnan2 = sawnan2 .or. la_xisnan(max2)
           if (forcer .or. (max2 <= growthbound .and. .not. sawnan2)) then
              sigma = rsigma
              shift = sright
              goto 100
           ...
           ! refined test for rrr. this test supposes that no nan occurred.
           ! moreover, we use the refined rrr test only for isolated clusters.
           if ((clwdth < mingap/real(128,KIND=xdp)) .and. (min(max1,max2) < fail2) .and. (.not. sawnan1) &
                     .and. (.not. sawnan2)) then
              dorrr1 = .true.
           else
              dorrr1 = .false.
           ...
           elseif (shift == sright) then
              ! store new l and d back into dplus, lplus
              call la_xcopy(n,work,1,dplus,1)
              call la_xcopy(n - 1,work(n + 1),1,lplus,1)
           end if
           return
     end subroutine la_xlarrf
#endif
#ifdef LA_WITH_QP
     !> Given the initial representation L D L^T and its cluster of close
     !> eigenvalues (in a relative measure), W( CLSTRT ), W( CLSTRT+1 ), ...
     ...
     !> T = L D L**T given L, D and APPROXIMATIONS to the eigenvalues of L D L**T.
     !> The input eigenvalues should have been computed by XLARRE.

     pure subroutine la_xlarrv(n,vl,vu,d,l,pivmin,isplit,m,dol,dou,minrgp,rtol1, &
               rtol2,w,werr,wgap,iblock,indexw,gers,z,ldz,isuppz,work,iwork,info)
        use la_constants_xdp,only:zero,half,one,two,three,four
        ! -- lapack auxiliary routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ! Scalar Arguments
           integer(ilp),intent(in) :: dol,dou,ldz,m,n
           integer(ilp),intent(out) :: info
           real(xdp),intent(in) :: minrgp,pivmin,vl,vu
           real(xdp),intent(inout) :: rtol1,rtol2
           ! Array Arguments
           integer(ilp),intent(in) :: iblock(*),indexw(*),isplit(*)
           integer(ilp),intent(out) :: isuppz(*),iwork(*)
           real(xdp),intent(inout) :: d(*),l(*),w(*),werr(*),wgap(*)
           real(xdp),intent(in) :: gers(*)
           real(xdp),intent(out) :: work(*)
        ...
        ! =====================================================================
           ...
           integer(ilp) :: done,i,ibegin,idone,iend,ii,iindc1,iindc2,iindr,iindwk,iinfo, &
            ...
            offset,oldcls,oldfst,oldien,oldlst,oldncl,p,parity,q,wbegin,wend,windex, &
                      windmn,windpl,zfrom,zto,zusedl,zusedu,zusedw
           real(xdp) :: bstres,bstw,eps,fudge,gap,gaptol,gl,gu,lambda,left,lgap,mingma, &
           nrminv,resid,rgap,right,rqcorr,rqtol,savgap,sgndef,sigma,spdiam,ssigma,tau, &
                     tmp,tol,ztz
           ! Intrinsic Functions
           intrinsic :: abs,real,max,min
           ...
           ! the width of the part of z that is used
           zusedw = zusedu - zusedl + 1
           call la_xlaset('FULL',n,zusedw,zero,zero,z(1,zusedl),ldz)
           eps = la_xlamch('PRECISION')
           rqtol = two*eps
           ! set expert flags for standard code.
           tryrqc = .true.
           ...
           loop_170: do jblk = 1,iblock(m)
              ...
              ! high relative accuracy is required for the computation of the
              ! corresponding eigenvectors.
              call la_xcopy(im,w(wbegin),1,work(wbegin),1)
              ! we store in w the eigenvalue approximations w.r.t. the original
              ! matrix t.
              do i = 1,im
                 w(wbegin + i - 1) = w(wbegin + i - 1) + sigma
              ...
              if (idone < im) then
                 ...
                 loop_150: do i = 1,oldncl
                    ...
                    if (ndepth > 0) then
                       ...
                       call la_xcopy(in,z(ibegin,j),1,d(ibegin),1)
                       call la_xcopy(in - 1,z(ibegin,j + 1),1,l(ibegin),1)
                       sigma = z(iend,j + 1)
                       ! set the corresponding entries in z to zero
                       call la_xlaset('FULL',in,2,zero,zero,z(ibegin,j),ldz)
                    end if
                    ! compute dl and dll of current rrr
                    do j = ibegin,iend - 1
                       tmp = d(j)*l(j)
                    ...
                    if (ndepth > 0) then
                       ...
                       ! perform limited bisection (if necessary) to get approximate
                       ! eigenvalues to the precision needed.
                       call la_xlarrb(in,d(ibegin),work(indlld + ibegin - 1),p,q,rtol1, &
                       rtol2,offset,work(wbegin),wgap(wbegin),werr(wbegin),work(indwrk),iwork( &
                                  iindwk),pivmin,spdiam,in,iinfo)
                       if (iinfo /= 0) then
                          info = -1
                       ...
                       ! of wgap, the entries of work might stem from rrrs with
                       ! different shifts. the gaps from wbegin-1+oldfst to
                       ! wbegin-1+oldlst are correctly computed in la_xlarrb.
                       ! however, we only allow the gaps to become greater since
                       ! this is what should happen when we decrease werr
                       if (oldfst > 1) then
                          wgap(wbegin + oldfst - 2) = max(wgap(wbegin + oldfst - 2),w(wbegin + oldfst - 1) - &
                    ...
                    loop_140: do j = oldfst,oldlst
                       ...
                       if (newsiz > 1) then
                          ...
                          do k = 1,2
                             ...
                             offset = indexw(wbegin) - 1
                             call la_xlarrb(in,d(ibegin),work(indlld + ibegin - 1),p,p,rqtol, &
                             rqtol,offset,work(wbegin),wgap(wbegin),werr(wbegin),work(indwrk), &
                                       iwork(iindwk),pivmin,spdiam,in,iinfo)
                          end do
                          if ((wbegin + newlst - 1 < dol) .or. (wbegin + newfst - 1 > dou)) then
                          ...
                          ! compute rrr of child cluster.
                          ! note that the new rrr is stored in z
                          ! la_xlarrf needs lwork = 2*n
                          call la_xlarrf(in,d(ibegin),l(ibegin),work(indld + ibegin - 1), &
                          newfst,newlst,work(wbegin),wgap(wbegin),werr(wbegin),spdiam,lgap, &
                          rgap,pivmin,tau,z(ibegin,newftt),z(ibegin,newftt + 1),work(indwrk), &
                                    iinfo)
                          if (iinfo == 0) then
                             ! a new rrr for the cluster was found by la_xlarrf
                             ! update shift and store it
                             ssigma = sigma + tau
                             z(iend,newftt + 1) = ssigma
                             ! work() are the midpoints and werr() the semi-width
                          ...
                          ! compute eigenvector of singleton
                          iter = 0
                          tol = four*log(real(in,KIND=xdp))*eps
                          k = newfst
                          windex = wbegin + k - 1
                          windmn = max(windex - 1,1)
                          windpl = min(windex + 1,m)
                          ...
                          if (needbs) then
                             ...
                             itmp1 = iwork(iindr + windex)
                             offset = indexw(wbegin) - 1
                             call la_xlarrb(in,d(ibegin),work(indlld + ibegin - 1),indeig, &
                             indeig,zero,two*eps,offset,work(wbegin),wgap(wbegin),werr(wbegin), &
                                       work(indwrk),iwork(iindwk),pivmin,spdiam,itmp1,iinfo)
                             if (iinfo /= 0) then
                                info = -3
                          ...
                          ! given lambda, compute the eigenvector.
                          call la_xlar1v(in,1,in,lambda,d(ibegin),l(ibegin),work( &
                          indld + ibegin - 1),work(indlld + ibegin - 1),pivmin,gaptol,z(ibegin,windex &
                          ),.not. usedbs,negcnt,ztz,mingma,iwork(iindr + windex),isuppz( &
                                    2*windex - 1),nrminv,resid,rqcorr,work(indwrk))
                          if (iter == 0) then
             ...
             if (usedrq .and. usedbs .and. bstres <= resid) then
                             ...
                             if (stp2ii) then
                                ! improve error angle by second step
                                call la_xlar1v(in,1,in,lambda,d(ibegin),l(ibegin), &
                                work(indld + ibegin - 1),work(indlld + ibegin - 1),pivmin,gaptol,z( &
                                ibegin,windex),.not. usedbs,negcnt,ztz,mingma,iwork(iindr + &
                                windex),isuppz(2*windex - 1),nrminv,resid,rqcorr,work(indwrk &
                                          ))
                          ...
                          call la_xscal(zto - zfrom + 1,nrminv,z(zfrom,windex),1)
                          125 continue
                          ! update w
                          w(windex) = lambda + sigma
                          ! recompute the gaps on the left and right
...
#ifdef LA_WITH_XDP
     !> To find the desired eigenvalues of a given real symmetric
     !> tridiagonal matrix T, XLARRE: sets any "small" off-diagonal
     !> elements to zero, and for each unreduced block T_i, it finds
     !> (a) a suitable shift at one end of the block's spectrum,
     !> (b) the base representation, T_i - sigma_i I = L_i D_i L_i^T, and
     !> (c) eigenvalues of each L_i D_i L_i^T.
     !> The representations and eigenvalues found are then used by
     !> XSTEMR to compute the eigenvectors of T.
     !> The accuracy varies depending on whether bisection is used to
     !> find a few eigenvalues or the dqds algorithm (subroutine XLASQ2) to
     !> conpute all and then discard any unwanted one.
     !> As an added benefit, XLARRE also outputs the n
     !> Gerschgorin intervals for the matrices L_i D_i L_i^T.

     pure subroutine la_xlarre(range,n,vl,vu,il,iu,d,e,e2,rtol1,rtol2,spltol, &
               nsplit,isplit,m,w,werr,wgap,iblock,indexw,gers,pivmin,work,iwork,info)
        ...
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ...
           integer(ilp),intent(in) :: il,iu,n
           integer(ilp),intent(out) :: info,m,nsplit
           real(xdp),intent(out) :: pivmin
           real(xdp),intent(in) :: rtol1,rtol2,spltol
           real(xdp),intent(inout) :: vl,vu
           ! Array Arguments
           integer(ilp),intent(out) :: iblock(*),isplit(*),iwork(*),indexw(*)
           real(xdp),intent(inout) :: d(*),e(*),e2(*)
           real(xdp),intent(out) :: gers(*),w(*),werr(*),wgap(*),work(*)
        ! =====================================================================
           ! Parameters
           real(xdp),parameter :: hndrd = 100.0_xdp
           real(xdp),parameter :: pert = 8.0_xdp
           real(xdp),parameter :: fourth = one/four
           real(xdp),parameter :: fac = half
           real(xdp),parameter :: maxgrowth = 64.0_xdp
           real(xdp),parameter :: fudge = 2.0_xdp
           integer(ilp),parameter :: maxtry = 6
           integer(ilp),parameter :: allrng = 1
           integer(ilp),parameter :: indrng = 2
           integer(ilp),parameter :: valrng = 3
           ...
           integer(ilp) :: cnt,cnt1,cnt2,i,ibegin,idum,iend,iinfo,in,indl,indu,irange, &
                     j,jblk,mb,mm,wbegin,wend
           real(xdp) :: avgap,bsrtol,clwdth,dmax,dpivot,eabs,emax,eold,eps,gl,gu,isleft, &
                      isrght,rtl,rtol,s1,s2,safmin,sgndef,sigma,spdiam,tau,tmp,tmp1
           ! Local Arrays
           integer(ilp) :: iseed(4)
           ! Intrinsic Functions
           ...
           spdiam = gu - gl
           ! compute splitting points
           call la_xlarra(n,d,e,e2,spltol,spdiam,nsplit,isplit,iinfo)
           ! can force use of bisection instead of faster dqds.
           ! option left in the code for future multisection work.
           forceb = .false.
           ! initialize usedqd, dqds should be used for allrng unless someone
           ...
           if ((irange == allrng) .and. (.not. forceb)) then
              ...
              ! we call la_xlarrd to find crude approximations to the eigenvalues
              ! in the desired range. in case irange = indrng, we also obtain the
              ! interval (vl,vu] that contains all the wanted eigenvalues.
              ! an interval [left,right] has converged if
              ! right-left<rtol*max(abs(left),abs(right))
              ! la_xlarrd needs a work of size 4*n, iwork of size 3*n
              call la_xlarrd(range,'B',n,vl,vu,il,iu,gers,bsrtol,d,e,e2,pivmin, &
                        nsplit,isplit,mm,w,werr,vl,vu,iblock,indexw,work,iwork,iinfo)
              if (iinfo /= 0) then
                 info = -1
                 return
      ...
      ! **
           ...
           loop_170: do jblk = 1,nsplit
              ...
              if (((irange == allrng) .and. (.not. forceb)) .or. usedqd) then
                 ! case of dqds
                 ! find approximations to the extremal eigenvalues of the block
                 call la_xlarrk(in,1,gl,gu,d(ibegin),e2(ibegin),pivmin,rtl,tmp,tmp1, &
                           iinfo)
                 if (iinfo /= 0) then
                    info = -1
                    return
                 ...
                 isleft = max(gl,tmp - tmp1 - hndrd*eps*abs(tmp - tmp1))
                 call la_xlarrk(in,in,gl,gu,d(ibegin),e2(ibegin),pivmin,rtl,tmp,tmp1, &
                            iinfo)
                 if (iinfo /= 0) then
                    info = -1
                    return
              ...
              if ((irange == allrng) .and. (.not. forceb)) then
                 ...
                 ! la_xlarrd has computed iblock and indexw for each eigenvalue
                 ! approximation.
                 ! choose sigma
                 if (usedqd) then
                    s1 = isleft + fourth*spdiam
              ...
              ! compute the negcount at the 1/4 and 3/4 points
              if (mb > 1) then
                 call la_xlarrc('T',in,s1,s2,d(ibegin),e(ibegin),pivmin,cnt,cnt1, &
                           cnt2,iinfo)
              end if
              if (mb == 1) then
                 sigma = gl
              ...
              if (usedqd) then
                 ...
                 if (mb > 1) then
                    clwdth = w(wend) + werr(wend) - w(wbegin) - werr(wbegin)
                    avgap = abs(clwdth/real(wend - wbegin,KIND=xdp))
                    if (sgndef == one) then
                       tau = half*max(wgap(wbegin),avgap)
                       tau = max(tau,werr(wbegin))
                    else
              ...
              e(iend) = sigma
              ! store d and l.
              call la_xcopy(in,work,1,d(ibegin),1)
              call la_xcopy(in - 1,work(in + 1),1,e(ibegin),1)
              if (mb > 1) then
                 ! perturb each entry of the base representation by a small
                 ! (but random) relative amount to overcome difficulties with
                 ! glued matrices.
                 ...
                 call la_xlarnv(2,iseed,2*in - 1,work(1))
                 do i = 1,in - 1
                    d(ibegin + i - 1) = d(ibegin + i - 1)*(one + eps*pert*work(i))
                    e(ibegin + i - 1) = e(ibegin + i - 1)*(one + eps*pert*work(in + i))
                 end do
              ...
              ! don't update the gerschgorin intervals because keeping track
              ! of the updates would be too much work in la_xlarrv.
              ! we update w instead and use it to locate the proper gerschgorin
              ! intervals.
              ! compute the required eigenvalues of l d l' by bisection or dqds
              if (.not. usedqd) then
                 ! if la_xlarrd has been used, shift the eigenvalue approximations
                 ! according to their representation. this is necessary for
                 ! a uniform la_xlarrv since dqds computes eigenvalues of the
                 ! shifted representation. in la_xlarrv, w will always hold the
                 ! unshifted eigenvalue approximation.
                 do j = wbegin,wend
                    w(j) = w(j) - sigma
                    werr(j) = werr(j) + abs(w(j))*eps
                 end do
                 ! call la_xlarrb to reduce eigenvalue error of the approximations
                 ! from la_xlarrd
                 do i = ibegin,iend - 1
                    work(i) = d(i)*e(i)**2
                 end do
                 ! use bisection to find ev from indl to indu
                 call la_xlarrb(in,d(ibegin),work(ibegin),indl,indu,rtol1,rtol2,indl - 1, &
                 w(wbegin),wgap(wbegin),werr(wbegin),work(2*n + 1),iwork,pivmin,spdiam,in, &
                           iinfo)
                 if (iinfo /= 0) then
                    info = -4
                 ...
                 ! la_xlarrb computes all gaps correctly except for the last one
                 ! record distance to vu/gu
                 wgap(wend) = max(zero, (vu - sigma) - (w(wend) + werr(wend)))
                 do i = indl,indu
                    m = m + 1
                 ...
                 ! the eigenvalues of t. however, t is not guaranteed to define its
                 ! eigenvalues to high relative accuracy anyway.
                 ! set rtol to the order of the tolerance used in la_xlasq2
                 ! this is an estimated error, the worst case bound is 4*n*eps
                 ! which is usually too large and requires unnecessary work to be
                 ! done by bisection when computing the eigenvectors
                 rtol = log(real(in,KIND=xdp))*four*eps
                 j = ibegin
                 do i = 1,in - 1
                    work(2*i - 1) = abs(d(j))
                    work(2*i) = e(j)*e(j)*work(2*i - 1)
                 ...
                 do i = m - mb + 1,m
                    ! the value of rtol below should be the tolerance in la_xlasq2
                    werr(i) = rtol*abs(w(i))
                 end do
                 do i = m - mb + 1,m - 1
                    ! compute the right gap between the intervals
...
#ifdef LA_WITH_XDP
     !> YLAR1V: computes the (scaled) r-th column of the inverse of
     !> the sumbmatrix in rows B1 through BN of the tridiagonal matrix
     !> L D L**T - sigma I. When sigma is close to an eigenvalue, the
     !> computed vector is an accurate eigenvector. Usually, r corresponds
     !> to the index where the eigenvector is largest in magnitude.
     ...
     pure subroutine la_ylar1v(n,b1,bn,lambda,d,l,ld,lld,pivmin,gaptol,z,wantnc, &
               negcnt,ztz,mingma,r,isuppz,nrminv,resid,rqcorr,work)
        ...
        ! =====================================================================
           ...
           if (.not. sawnan1 .and. .not. sawnan2) then
              do i = r,bn - 1
                 ...
                 ztz = ztz + real(z(i + 1)*z(i + 1),KIND=xdp)
              end do
              260 continue
           else
              ! run slower loop if nan occurred.
...
#ifdef LA_WITH_XDP
     !> YLARRV: computes the eigenvectors of the tridiagonal matrix
     !> T = L D L**T given L, D and APPROXIMATIONS to the eigenvalues of L D L**T.
     !> The input eigenvalues should have been computed by XLARRE.

     pure subroutine la_ylarrv(n,vl,vu,d,l,pivmin,isplit,m,dol,dou,minrgp,rtol1, &
               rtol2,w,werr,wgap,iblock,indexw,gers,z,ldz,isuppz,work,iwork,info)
        use la_constants_xdp,only:zero,half,one,two,three,four,czero
        ! -- lapack auxiliary routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ! Scalar Arguments
        ...
        ! =====================================================================
           ...
           ! the width of the part of z that is used
           zusedw = zusedu - zusedl + 1
           call la_ylaset('FULL',n,zusedw,czero,czero,z(1,zusedl),ldz)
           eps = la_xlamch('PRECISION')
           rqtol = two*eps
           ! set expert flags for standard code.
           tryrqc = .true.
           ...
           loop_170: do jblk = 1,iblock(m)
              ...
              if (ibegin == iend) then
                 ...
                 z(ibegin,wbegin) = cmplx(one,zero,KIND=xdp)
                 isuppz(2*wbegin - 1) = ibegin
                 isuppz(2*wbegin) = ibegin
                 w(wbegin) = w(wbegin) + sigma
                 work(wbegin) = w(wbegin)
              ...
              if (idone < im) then
                 ...
                 loop_150: do i = 1,oldncl
                    ...
                    if (ndepth > 0) then
                       ...
                       do k = 1,in - 1
                          d(ibegin + k - 1) = real(z(ibegin + k - 1,j),KIND=xdp)
                          l(ibegin + k - 1) = real(z(ibegin + k - 1,j + 1),KIND=xdp)
                       end do
                       d(iend) = real(z(iend,j),KIND=xdp)
                       sigma = real(z(iend,j + 1),KIND=xdp)
                       ! set the corresponding entries in z to zero
                       call la_ylaset('FULL',in,2,czero,czero,z(ibegin,j),ldz)
                    ...
                    loop_140: do j = oldfst,oldlst
                       ...
                       if (newsiz > 1) then
                          ...
                          ! in the complex case, la_xlarrf cannot write
                          ! the new rrr directly into z and needs an intermediate
                          ! workspace
                          do k = 1,in - 1
                             z(ibegin + k - 1,newftt) = cmplx(work(indin1 + k - 1),zero,KIND=xdp)

                             z(ibegin + k - 1,newftt + 1) = cmplx(work(indin2 + k - 1),zero,KIND=xdp)

                          end do
                          z(iend,newftt) = cmplx(work(indin1 + in - 1),zero,KIND=xdp)
                          if (iinfo == 0) then
                             ! a new rrr for the cluster was found by la_xlarrf
                             ! update shift and store it
                             ssigma = sigma + tau
                             z(iend,newftt + 1) = cmplx(ssigma,zero,KIND=xdp)
                             ! work() are the midpoints and werr() the semi-width
                             ! note that the entries in w are unchanged.
                             do k = newfst,newlst
                                fudge = three*eps*abs(work(wbegin + k - 1))
                          ...
                          ! given lambda, compute the eigenvector.
                          call la_ylar1v(in,1,in,lambda,d(ibegin),l(ibegin),work( &
                          indld + ibegin - 1),work(indlld + ibegin - 1),pivmin,gaptol,z(ibegin,windex &
                          ),.not. usedbs,negcnt,ztz,mingma,iwork(iindr + windex),isuppz( &
                                    2*windex - 1),nrminv,resid,rqcorr,work(indwrk))
                          if (iter == 0) then
             ...
             if (usedrq .and. usedbs .and. bstres <= resid) then
                             ...
                             if (stp2ii) then
                                ! improve error angle by second step
                                call la_ylar1v(in,1,in,lambda,d(ibegin),l(ibegin), &
                                work(indld + ibegin - 1),work(indlld + ibegin - 1),pivmin,gaptol,z( &
                                ibegin,windex),.not. usedbs,negcnt,ztz,mingma,iwork(iindr + &
                                windex),isuppz(2*windex - 1),nrminv,resid,rqcorr,work(indwrk &
                                          ))
                          ...
                          call la_yxscal(zto - zfrom + 1,nrminv,z(zfrom,windex),1)
                          125 continue
                          ! update w
                          w(windex) = lambda + sigma
                          ! recompute the gaps on the left and right
