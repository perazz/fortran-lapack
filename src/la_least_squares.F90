...
#ifdef LA_WITH_XDP
        module procedure la_xlstsq_one
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
        module procedure la_ylstsq_one
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
        module procedure la_xlstsq_multiple
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
        module procedure la_ylstsq_multiple
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
     !> Workspace needed by real(xdp) gesv
     subroutine xgesv_space(m,n,nrhs,lrwork,liwork,lcwork)
         integer(ilp),intent(in) :: m,n,nrhs
         integer(ilp),intent(out) :: lrwork,liwork,lcwork
...
#ifdef LA_WITH_XDP
     !> Workspace needed by complex(xdp) gesv
     subroutine ygesv_space(m,n,nrhs,lrwork,liwork,lcwork)
         integer(ilp),intent(in) :: m,n,nrhs
         integer(ilp),intent(out) :: lrwork,liwork,lcwork
...
#ifdef LA_WITH_XDP
     !> Compute the least-squares solution to a real(xdp) system of linear equations Ax = B
     function la_xlstsq_one(a,b,cond,overwrite_a,rank,err) result(x)
         !> Input matrix a[n,n]
         real(xdp),intent(inout),target :: a(:,:)
         !> Right hand side vector or array, b[n] or b[n,nrhs]
         real(xdp),intent(in) :: b(:)
         !> [optional] cutoff for rank evaluation: singular values s(i)<=cond*maxval(s) are considered 0.
         real(xdp),optional,intent(in) :: cond
         !> [optional] Can A,b data be overwritten and destroyed?
         logical(lk),optional,intent(in) :: overwrite_a
         !> [optional] Return rank of A
         integer(ilp),optional,intent(out) :: rank
         ...
         integer(ilp),allocatable :: iwork(:)
         logical(lk) :: copy_a,large_enough_x
         real(xdp) :: acond,rcond
         real(xdp),allocatable :: singular(:),rwork(:)
         real(xdp),pointer :: xmat(:,:),amat(:,:)
         real(xdp),allocatable :: cwork(:)
         character(*),parameter :: this = 'lstsq'
         ...
         if (present(cond)) then
            ...
            rcond = epsilon(0.0_xdp)*mnmax
         end if
         if (rcond < 0) rcond = epsilon(0.0_xdp)*mnmax

         ! Allocate working space
         call xgesv_space(m,n,nrhs,lrwork,liwork,lcwork)
         allocate (rwork(lrwork),iwork(liwork))
...
#ifdef LA_WITH_XDP
     !> Compute the least-squares solution to a complex(xdp) system of linear equations Ax = B
     function la_ylstsq_one(a,b,cond,overwrite_a,rank,err) result(x)
         !> Input matrix a[n,n]
         ...
         type(la_state),optional,intent(out) :: err
         !> Result array/matrix x[n] or x[n,nrhs]
         complex(xdp),allocatable,target :: x(:)
         ...
         real(xdp) :: acond,rcond
         real(xdp),allocatable :: singular(:),rwork(:)
         complex(xdp),pointer :: xmat(:,:),amat(:,:)
         complex(xdp),allocatable :: cwork(:)
         character(*),parameter :: this = 'lstsq'
         ...
         ! Allocate working space
         call ygesv_space(m,n,nrhs,lrwork,liwork,lcwork)
         allocate (rwork(lrwork),cwork(lcwork),iwork(liwork))
...
#ifdef LA_WITH_XDP
     !> Compute the least-squares solution to a real(xdp) system of linear equations Ax = B
     function la_xlstsq_multiple(a,b,cond,overwrite_a,rank,err) result(x)
         !> Input matrix a[n,n]
         real(xdp),intent(inout),target :: a(:,:)
         !> Right hand side vector or array, b[n] or b[n,nrhs]
         real(xdp),intent(in) :: b(:,:)
...
1        call err0%handle(err)
     ...
     end function la_xlstsq_multiple
#endif
...
#ifdef LA_WITH_XDP
     !> Compute the least-squares solution to a complex(xdp) system of linear equations Ax = B
     function la_ylstsq_multiple(a,b,cond,overwrite_a,rank,err) result(x)
         !> Input matrix a[n,n]
         complex(xdp),intent(inout),target :: a(:,:)
         !> Right hand side vector or array, b[n] or b[n,nrhs]
         complex(xdp),intent(in) :: b(:,:)
...
1        call err0%handle(err)
     ...
     end function la_ylstsq_multiple
#endif
