...
module la_blas_aux
...
#ifdef LA_WITH_XDP
     !> XCABS1: computes |Re(.)| + |Im(.)| of a double complex number

     pure real(xdp) function la_xcabs1(z)
        ! -- reference blas level1 routine --
        ! -- reference blas is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ! Scalar Arguments
           complex(xdp),intent(in) :: z
        ! =====================================================================
           ! Intrinsic Functions
           intrinsic :: abs,real,aimag
           la_xcabs1 = abs(real(z,KIND=xdp)) + abs(aimag(z))
           return
     end function la_xcabs1
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
     !> IXAMAX: finds the index of the first element having maximum absolute value.

     pure integer(ilp) function la_ixamax(n,xx,incx)
        ! -- reference blas level1 routine --
        ! -- reference blas is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ! Scalar Arguments
     ...
     pure integer(ilp) function la_iyamax(n,yx,incx)
        ! -- reference blas level1 routine --
        ! -- reference blas is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ! Scalar Arguments
        ...
        ! =====================================================================
           ...
           if (incx == 1) then
              ...
              ix = ix + incx
              do i = 2,n
                 if (la_xcabs1(yx(ix)) > xmax) then
                    la_iyamax = i
                    xmax = la_xcabs1(yx(ix))
                 end if
                 ix = ix + incx
              end do
           end if
           return
     end function la_iyamax
#endif
#ifdef LA_WITH_QP
