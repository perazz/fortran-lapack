...
module la_lapack_others_sm
...
#ifdef LA_WITH_XDP
     public :: la_xla_gerpvgrw
     public :: la_xla_syamv
     public :: la_xla_syrcond
     public :: la_xla_syrpvgrw
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
     public :: la_yla_gerpvgrw
     public :: la_yla_syamv
     public :: la_yla_gbrcond_c
     public :: la_yla_gercond_c
     public :: la_yla_hercond_c
     public :: la_yla_porcond_c
     public :: la_yla_syrcond_c
     public :: la_yla_syrpvgrw
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
     !> XLA_GERPVGRW: computes the reciprocal pivot growth factor
     !> norm(A)/norm(U). The "max absolute element" norm is used. If this is
     !> much less than 1, the stability of the LU factorization of the
     !> (equilibrated) matrix A could be poor. This also means that the
     !> solution X, estimated condition numbers, and error bounds could be
     ...
     pure real(xdp) function la_xla_gerpvgrw(n,ncols,a,lda,af,ldaf)
        ...
        ! =====================================================================
           ...
           la_xla_gerpvgrw = rpvgrw
     end function la_xla_gerpvgrw
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
     !> XLA_SYAMV:  performs the matrix-vector operation
     !> y := alpha*abs(A)*abs(x) + beta*abs(y),
     !> where alpha and beta are scalars, x and y are vectors and A is an
     !> n by n symmetric matrix.
     !> This function is primarily used in calculating error bounds.
     ...
     subroutine la_xla_syamv(uplo,n,alpha,a,lda,x,incx,beta,y,incy)
        use la_constants_xdp
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
...
#ifdef LA_WITH_XDP
     !> XLA_SYRCOND: estimates the Skeel condition number of  op(A) * op2(C)
     !> where op2 is determined by CMODE as follows
     !> CMODE =  1    op2(C) = C
     !> CMODE =  0    op2(C) = I
     !> CMODE = -1    op2(C) = inv(C)
     ...
     real(xdp) function la_xla_syrcond(uplo,n,a,lda,af,ldaf,ipiv,cmode,c,info,work, &
               iwork)
        ...
        ! =====================================================================
           ...
           if (info /= 0) then
              call la_xerbla('XLA_SYRCOND',-info)
              return
           end if
           if (n == 0) then
              la_xla_syrcond = one
           ...
           if (kase /= 0) then
              if (kase == 2) then
                 ...
                 if (up) then
                    call la_xsytrs('U',n,1,af,ldaf,ipiv,work,n,info)
                 else
                    call la_xsytrs('L',n,1,af,ldaf,ipiv,work,n,info)
                 end if
                 ! multiply by inv(c).
                 if (cmode == 1) then
                    do i = 1,n
           ...
           ! compute the estimate of the reciprocal condition number.
           if (ainvnm /= zero) la_xla_syrcond = (one/ainvnm)
           return
     end function la_xla_syrcond
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
     !> XLA_SYRPVGRW: computes the reciprocal pivot growth factor
     !> norm(A)/norm(U). The "max absolute element" norm is used. If this is
     !> much less than 1, the stability of the LU factorization of the
     !> (equilibrated) matrix A could be poor. This also means that the
     !> solution X, estimated condition numbers, and error bounds could be
     ...
     real(xdp) function la_xla_syrpvgrw(uplo,n,info,a,lda,af,ldaf,ipiv,work)
        use la_constants_xdp
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
        ...
        ! =====================================================================
           ...
           ! permute the magnitudes of a above so they're in the same order as
           ! the factor.
           ! the iteration orders and permutations were copied from la_xsytrs.
           ...
           la_xla_syrpvgrw = rpvgrw
     end function la_xla_syrpvgrw
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
     !> YLA_GERPVGRW: computes the reciprocal pivot growth factor
     !> norm(A)/norm(U). The "max absolute element" norm is used. If this is
     !> much less than 1, the stability of the LU factorization of the
     !> (equilibrated) matrix A could be poor. This also means that the
     !> solution X, estimated condition numbers, and error bounds could be
     ...
     pure real(xdp) function la_yla_gerpvgrw(n,ncols,a,lda,af,ldaf)
        use la_constants_xdp
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
        ...
        ! =====================================================================
           ...
           la_yla_gerpvgrw = rpvgrw
     end function la_yla_gerpvgrw
#endif
#ifdef LA_WITH_QP
     ...
     subroutine la_yla_syamv(uplo,n,alpha,a,lda,x,incx,beta,y,incy)
        use la_constants_xdp
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
...
#ifdef LA_WITH_XDP
     !> YLA_GBRCOND_C: Computes the infinity norm condition number of
     !> op(A) * inv(diag(C)) where C is a EXTENDED PRECISION vector.

     real(xdp) function la_yla_gbrcond_c(trans,n,kl,ku,ab,ldab,afb,ldafb,ipiv,c, &
               capply,info,work,rwork)
        use la_constants_xdp
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ...
        ! =====================================================================
           ...
           if (info /= 0) then
              call la_xerbla('YLA_GBRCOND_C',-info)
              return
           end if
           ! compute norm of op(a)*op2(c).
           anorm = zero
           ...
           if (kase /= 0) then
              if (kase == 2) then
                 ...
                 if (notrans) then
                    call la_ygbtrs('NO TRANSPOSE',n,kl,ku,1,afb,ldafb,ipiv,work,n, &
                              info)
                 else
                    call la_ygbtrs('CONJUGATE TRANSPOSE',n,kl,ku,1,afb,ldafb,ipiv, &
                              work,n,info)
                 end if
                 ! multiply by inv(c).
                 if (capply) then
           ...
           ! compute the estimate of the reciprocal condition number.
           if (ainvnm /= zero) la_yla_gbrcond_c = one/ainvnm
           return
     end function la_yla_gbrcond_c
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
     !> YLA_GERCOND_C: computes the infinity norm condition number of
     !> op(A) * inv(diag(C)) where C is a EXTENDED PRECISION vector.

     real(xdp) function la_yla_gercond_c(trans,n,a,lda,af,ldaf,ipiv,c,capply,info, &
               work,rwork)
        use la_constants_xdp
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ...
        ! =====================================================================
           ...
           if (info /= 0) then
              call la_xerbla('YLA_GERCOND_C',-info)
              return
           end if
           ! compute norm of op(a)*op2(c).
           anorm = zero
           ...
           if (kase /= 0) then
              if (kase == 2) then
                 ...
                 if (notrans) then
                    call la_ygetrs('NO TRANSPOSE',n,1,af,ldaf,ipiv,work,n,info)

                 else
                    call la_ygetrs('CONJUGATE TRANSPOSE',n,1,af,ldaf,ipiv,work,n,info &
                              )
                 end if
                 ! multiply by inv(c).
                 if (capply) then
           ...
           ! compute the estimate of the reciprocal condition number.
           if (ainvnm /= zero) la_yla_gercond_c = one/ainvnm
           return
     end function la_yla_gercond_c
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
     !> YLA_HERCOND_C: computes the infinity norm condition number of
     !> op(A) * inv(diag(C)) where C is a EXTENDED PRECISION vector.

     real(xdp) function la_yla_hercond_c(uplo,n,a,lda,af,ldaf,ipiv,c,capply,info,work, &
                rwork)
        use la_constants_xdp
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ...
        ! =====================================================================
           ...
           if (info /= 0) then
              call la_xerbla('YLA_HERCOND_C',-info)
              return
           end if
           up = .false.
           if (la_lsame(uplo,'U')) up = .true.
           ...
           if (kase /= 0) then
              if (kase == 2) then
                 ...
                 if (up) then
                    call la_yhetrs('U',n,1,af,ldaf,ipiv,work,n,info)
                 else
                    call la_yhetrs('L',n,1,af,ldaf,ipiv,work,n,info)
                 end if
                 ! multiply by inv(c).
                 if (capply) then
                    do i = 1,n
           ...
           ! compute the estimate of the reciprocal condition number.
           if (ainvnm /= zero) la_yla_hercond_c = one/ainvnm
           return
     end function la_yla_hercond_c
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
     !> YLA_PORCOND_C: Computes the infinity norm condition number of
     !> op(A) * inv(diag(C)) where C is a EXTENDED PRECISION vector

     real(xdp) function la_yla_porcond_c(uplo,n,a,lda,af,ldaf,c,capply,info,work, &
               rwork)
        use la_constants_xdp
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ...
        ! =====================================================================
           ...
           if (info /= 0) then
              call la_xerbla('YLA_PORCOND_C',-info)
              return
           end if
           up = .false.
           if (la_lsame(uplo,'U')) up = .true.
           ...
           if (kase /= 0) then
              if (kase == 2) then
                 ...
                 if (up) then
                    call la_ypotrs('U',n,1,af,ldaf,work,n,info)
                 else
                    call la_ypotrs('L',n,1,af,ldaf,work,n,info)
                 end if
                 ! multiply by inv(c).
                 if (capply) then
                    do i = 1,n
           ...
           ! compute the estimate of the reciprocal condition number.
           if (ainvnm /= zero) la_yla_porcond_c = one/ainvnm
           return
     end function la_yla_porcond_c
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
     !> YLA_SYRCOND_C: Computes the infinity norm condition number of
     !> op(A) * inv(diag(C)) where C is a EXTENDED PRECISION vector.

     real(xdp) function la_yla_syrcond_c(uplo,n,a,lda,af,ldaf,ipiv,c,capply,info,work, &
                rwork)
        use la_constants_xdp
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ...
        ! =====================================================================
           ...
           if (info /= 0) then
              call la_xerbla('YLA_SYRCOND_C',-info)
              return
           end if
           up = .false.
           if (la_lsame(uplo,'U')) up = .true.
           ...
           if (kase /= 0) then
              if (kase == 2) then
                 ...
                 if (up) then
                    call la_ysytrs('U',n,1,af,ldaf,ipiv,work,n,info)
                 else
                    call la_ysytrs('L',n,1,af,ldaf,ipiv,work,n,info)
                 end if
                 ! multiply by inv(c).
                 if (capply) then
                    do i = 1,n
           ...
           ! compute the estimate of the reciprocal condition number.
           if (ainvnm /= zero) la_yla_syrcond_c = one/ainvnm
           return
     end function la_yla_syrcond_c
#endif
#ifdef LA_WITH_QP
...
#ifdef LA_WITH_XDP
     !> YLA_SYRPVGRW: computes the reciprocal pivot growth factor
     !> norm(A)/norm(U). The "max absolute element" norm is used. If this is
     !> much less than 1, the stability of the LU factorization of the
     !> (equilibrated) matrix A could be poor. This also means that the
     !> solution X, estimated condition numbers, and error bounds could be
     ...
     real(xdp) function la_yla_syrpvgrw(uplo,n,info,a,lda,af,ldaf,ipiv,work)
        use la_constants_xdp
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
        ...
        ! =====================================================================
           ...
           la_yla_syrpvgrw = rpvgrw
     end function la_yla_syrpvgrw
#endif
#ifdef LA_WITH_QP
