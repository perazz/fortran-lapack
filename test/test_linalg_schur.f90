...
module test_linalg_schur
    ...
    use la_constants
    use la_state_type,only:LINALG_VALUE_ERROR,la_state
    use linear_algebra,only:schur,schur_space
    ...
    subroutine test_schur_random_q(error)
        type(error_type),allocatable,intent(out) :: error
        ...
        integer(ilp) :: lwork
        real(qp),allocatable :: storage(:)
        real(qp),dimension(n,n) :: a,t,z,aorig
        type(la_state) :: state
        ...
        ! Check solution
        call check(error,all(schur_error(aorig,z,t) <= max(rtol*abs(aorig),eps)), &
                          'converged solution (real(qp) - overwrite A)')
        if (allocated(error)) return
        ...
        ! Check solution
        call check(error,all(schur_error(a,z,t) <= max(rtol*abs(a),eps)), &
                          'converged solution (real(qp) - external storage)')
        if (allocated(error)) return

    contains
    
        pure function schur_error(a,z,t) result(err)
            real(qp),intent(in),dimension(:,:) :: a,z,t
            real(qp),dimension(size(a,1),size(a,2)) :: err
            
            err = abs(matmul(matmul(z,t),transpose(z)) - a)
        end function schur_error
        
    end subroutine test_schur_random_q

    !> Test symmetric matrix (real eigenvalues)
    subroutine test_schur_symmetric_q(error)
        type(error_type),allocatable,intent(out) :: error
    ...
    subroutine test_schur_random_w(error)
        type(error_type),allocatable,intent(out) :: error
        ...
        integer(ilp) :: lwork
        complex(qp),allocatable :: storage(:)
        complex(qp),dimension(n,n) :: a,t,z,aorig
        real(qp),dimension(n,n) :: a_re,a_im
        type(la_state) :: state

        call random_number(a_re)
        call random_number(a_im)
        a = cmplx(a_re,a_im,kind=qp)
        aorig = a
        ...
        ! Check solution
        call check(error,all(schur_error(aorig,z,t) <= max(rtol*abs(aorig),eps)), &
                          'converged solution (complex(qp) - overwrite A)')
        if (allocated(error)) return
        ...
        ! Check solution
        call check(error,all(schur_error(a,z,t) <= max(rtol*abs(a),eps)), &
                          'converged solution (complex(qp) - external storage)')
        if (allocated(error)) return

    contains
    
        pure function schur_error(a,z,t) result(err)
            complex(qp),intent(in),dimension(:,:) :: a,z,t
            real(qp),dimension(size(a,1),size(a,2)) :: err
            
            err = abs(matmul(matmul(z,t),conjg(transpose(z))) - a)
        end function schur_error
        
    end subroutine test_schur_random_w

    !> Test symmetric matrix (real eigenvalues)
    subroutine test_schur_symmetric_w(error)
        type(error_type),allocatable,intent(out) :: error
