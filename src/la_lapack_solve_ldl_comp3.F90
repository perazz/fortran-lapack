...
module la_lapack_solve_ldl_comp3
     ...
     pure subroutine la_cheswapr(uplo,n,a,lda,i1,i2)
     ...
     end subroutine la_zheswapr
#ifdef LA_WITH_XDP
     !> YHESWAPR: applies an elementary permutation on the rows and the columns of
     !> a hermitian matrix.

     pure subroutine la_yheswapr(uplo,n,a,lda,i1,i2)
        ! -- lapack auxiliary routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ! Scalar Arguments
     ...
     pure subroutine la_chetf2(uplo,n,a,lda,ipiv,info)
        use la_constants_sp
           ...
           if (upper) then
              ...
              if ((max(absakk,colmax) == zero) .or. la_sisnan(absakk)) then
                 ...
                 if (kstep == 1) then
                    ...
                    call la_cher(uplo,k - 1,-r1,a(1,k),1,a,lda)
              ...
              if ((max(absakk,colmax) == zero) .or. la_sisnan(absakk)) then
                 ...
                 if (kstep == 1) then
                    ...
                    if (k < n) then
                       ...
                       r1 = one/real(a(k,k),KIND=sp)
                       call la_cher(uplo,n - k,-r1,a(k + 1,k),1,a(k + 1,k + 1),lda)
     ...
     pure subroutine la_zhetf2(uplo,n,a,lda,ipiv,info)
        use la_constants_dp
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
        ...
        ! =====================================================================
           ...
           if (upper) then
              ...
              if ((max(absakk,colmax) == zero) .or. la_disnan(absakk)) then
                 ...
                 if (kstep == 1) then
                    ...
                    call la_zher(uplo,k - 1,-r1,a(1,k),1,a,lda)
                    ! store u(k) in column k
              ...
              if ((max(absakk,colmax) == zero) .or. la_disnan(absakk)) then
                 ...
                 if (kstep == 1) then
                    ...
                    if (k < n) then
                       ...
                       ! a := a - l(k)*d(k)*l(k)**h = a - w(k)*(1/d(k))*w(k)**h
                       r1 = one/real(a(k,k),KIND=dp)
                       call la_zher(uplo,n - k,-r1,a(k + 1,k),1,a(k + 1,k + 1),lda)
           ...
           90 continue
           return
     end subroutine la_zhetf2
#ifdef LA_WITH_XDP
     !> YHETF2: computes the factorization of a complex Hermitian matrix A
     !> using the Bunch-Kaufman diagonal pivoting method:
     !> A = U*D*U**H  or  A = L*D*L**H
     !> where U (or L) is a product of permutation and unit upper (lower)
     !> triangular matrices, U**H is the conjugate transpose of U, and D is
     ...
     pure subroutine la_yhetf2(uplo,n,a,lda,ipiv,info)
        ...
        ! =====================================================================
           ...
           logical(lk) :: upper
           integer(ilp) :: i,imax,j,jmax,k,kk,kp,kstep
           real(xdp) :: absakk,alpha,colmax,d,d11,d22,r1,rowmax,tt
           complex(xdp) :: d12,d21,t,wk,wkm1,wkp1,zdum
           ! Intrinsic Functions
           intrinsic :: abs,real,cmplx,conjg,aimag,max,sqrt
           ! Statement Functions
           real(xdp) :: cabs1
           ...
           if (upper) then
              ...
              ! determine rows and columns to be interchanged and whether
              ! a 1-by-1 or 2-by-2 pivot block will be used
              absakk = abs(real(a(k,k),KIND=xdp))
              ! imax is the row-index of the largest off-diagonal element in
              ! column k, and colmax is its absolute value.
              ! determine both colmax and imax.
              if (k > 1) then
              ...
              if ((max(absakk,colmax) == zero) .or. la_xisnan(absakk)) then
                 ...
                 if (absakk >= alpha*colmax) then
                    ...
                    if (absakk >= alpha*colmax*(colmax/rowmax)) then
                       ! no interchange, use 1-by-1 pivot block
                       kp = k
                    else if (abs(real(a(imax,imax),KIND=xdp)) >= alpha*rowmax) then
                       ! interchange rows and columns k and imax, use 1-by-1
                       ! pivot block
                       kp = imax
                    else
                 ...
                 if (kp /= kk) then
                    ...
                    a(kp,kk) = conjg(a(kp,kk))
                    r1 = real(a(kk,kk),KIND=xdp)
                    a(kk,kk) = real(a(kp,kp),KIND=xdp)
                    a(kp,kp) = r1
                    if (kstep == 2) then
                       a(k,k) = real(a(k,k),KIND=xdp)
                       t = a(k - 1,k)
                    ...
                    a(k,k) = real(a(k,k),KIND=xdp)
                    if (kstep == 2) a(k - 1,k - 1) = real(a(k - 1,k - 1),KIND=xdp)
                 end if
                 ! update the leading submatrix
                 if (kstep == 1) then
                    ! 1-by-1 pivot block d(k): column k now holds
                    ...
                    ! perform a rank-1 update of a(1:k-1,1:k-1) as
                    ! a := a - u(k)*d(k)*u(k)**h = a - w(k)*1/d(k)*w(k)**h
                    r1 = one/real(a(k,k),KIND=xdp)
                    call la_yher(uplo,k - 1,-r1,a(1,k),1,a,lda)
                    ! store u(k) in column k
                    call la_yxscal(k - 1,r1,a(1,k),1)
                 else
                    ! 2-by-2 pivot block d(k): columns k and k-1 now hold
                    ! ( w(k-1) w(k) ) = ( u(k-1) u(k) )*d(k)
                    ! where u(k) and u(k-1) are the k-th and (k-1)-th columns
                    ...
                    ! a := a - ( u(k-1) u(k) )*d(k)*( u(k-1) u(k) )**h
                       ! = a - ( w(k-1) w(k) )*inv(d(k))*( w(k-1) w(k) )**h
                    if (k > 2) then
                       d = la_xlapy2(real(a(k - 1,k),KIND=xdp),aimag(a(k - 1,k)))

                       d22 = real(a(k - 1,k - 1),KIND=xdp)/d
                       d11 = real(a(k,k),KIND=xdp)/d
                       tt = one/(d11*d22 - one)
                       d12 = a(k - 1,k)/d
                       d = tt/d
                       do j = k - 2,1,-1
                          ...
                          a(j,k) = wk
                          a(j,k - 1) = wkm1
                          a(j,j) = cmplx(real(a(j,j),KIND=xdp),zero,KIND=xdp)
                       end do
                    end if
                 end if
              end if
              ...
              if ((max(absakk,colmax) == zero) .or. la_xisnan(absakk)) then
                 ...
                 if (kstep == 1) then
                    ...
                    if (k < n) then
                       ...
                       ! a := a - l(k)*d(k)*l(k)**h = a - w(k)*(1/d(k))*w(k)**h
                       r1 = one/real(a(k,k),KIND=xdp)
                       call la_yher(uplo,n - k,-r1,a(k + 1,k),1,a(k + 1,k + 1),lda)

                       ! store l(k) in column k
                       call la_yxscal(n - k,r1,a(k + 1,k),1)
                    end if
                 else
                    ! 2-by-2 pivot block d(k)
                    if (k < n - 1) then
                       ...
                       d11 = real(a(k + 1,k + 1),KIND=xdp)/d
                       d22 = real(a(k,k),KIND=xdp)/d
                       tt = one/(d11*d22 - one)
                       d21 = a(k + 1,k)/d
                       d = tt/d
                       do j = k + 2,n
           ...
           90 continue
           return
     end subroutine la_yhetf2
#endif
#ifdef LA_WITH_QP
     !> WHETF2: computes the factorization of a complex Hermitian matrix A
     !> using the Bunch-Kaufman diagonal pivoting method:
     !> A = U*D*U**H  or  A = L*D*L**H
     !> where U (or L) is a product of permutation and unit upper (lower)
     !> triangular matrices, U**H is the conjugate transpose of U, and D is
     ...
     pure subroutine la_whetf2(uplo,n,a,lda,ipiv,info)
        use la_constants_qp
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
              ...
              if ((max(absakk,colmax) == zero) .or. la_qisnan(absakk)) then
                 ...
                 if (absakk >= alpha*colmax) then
                    ...
                    if (absakk >= alpha*colmax*(colmax/rowmax)) then
                       ! no interchange, use 1-by-1 pivot block
                       kp = k
                    else if (abs(real(a(imax,imax),KIND=qp)) >= alpha*rowmax) then
                       ! interchange rows and columns k and imax, use 1-by-1
                       ! pivot block
                       kp = imax
                    else
                 ...
                 if (kstep == 1) then
                    ...
                    call la_wher(uplo,k - 1,-r1,a(1,k),1,a,lda)
                    ! store u(k) in column k
              ...
              if ((max(absakk,colmax) == zero) .or. la_qisnan(absakk)) then
                 ...
                 if (kstep == 1) then
                    ...
                    if (k < n) then
                       ...
                       ! a := a - l(k)*d(k)*l(k)**h = a - w(k)*(1/d(k))*w(k)**h
                       r1 = one/real(a(k,k),KIND=qp)
                       call la_wher(uplo,n - k,-r1,a(k + 1,k),1,a(k + 1,k + 1),lda)
     ...
     pure subroutine la_chetri(uplo,n,a,lda,ipiv,work,info)
        use la_constants_sp
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
        ...
        ! =====================================================================
           ...
           return
     end subroutine la_chetri
     !> ZHETRI: computes the inverse of a complex Hermitian indefinite matrix
     !> A using the factorization A = U*D*U**H or A = L*D*L**H computed by
     !> ZHETRF.

     pure subroutine la_zhetri(uplo,n,a,lda,ipiv,work,info)
        use la_constants_dp
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
        ...
        ! =====================================================================
           ...
           return
     end subroutine la_zhetri
#ifdef LA_WITH_XDP
     !> YHETRI: computes the inverse of a complex Hermitian indefinite matrix
     !> A using the factorization A = U*D*U**H or A = L*D*L**H computed by
     !> YHETRF.

     pure subroutine la_yhetri(uplo,n,a,lda,ipiv,work,info)
        use la_constants_xdp
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
        ...
        ! =====================================================================
           ...
           logical(lk) :: upper
           integer(ilp) :: j,k,kp,kstep
           real(xdp) :: ak,akp1,d,t
           complex(xdp) :: akkp1,temp
           ! Intrinsic Functions
           intrinsic :: abs,real,conjg,max
           ! Executable Statements
           ! test the input parameters.
           ...
           if (upper) then
              ...
              if (ipiv(k) > 0) then
                 ! 1 x 1 diagonal block
                 ! invert the diagonal block.
                 a(k,k) = one/real(a(k,k),KIND=xdp)
                 ! compute column k of the inverse.
                 if (k > 1) then
                    call la_ycopy(k - 1,a(1,k),1,work,1)
                    call la_yhemv(uplo,k - 1,-cone,a,lda,work,1,czero,a(1,k),1)

                    a(k,k) = a(k,k) - real(la_ydotc(k - 1,work,1,a(1,k),1), &
                              KIND=xdp)
                 end if
                 kstep = 1
              else
                 ...
                 ! invert the diagonal block.
                 t = abs(a(k,k + 1))
                 ak = real(a(k,k),KIND=xdp)/t
                 akp1 = real(a(k + 1,k + 1),KIND=xdp)/t
                 akkp1 = a(k,k + 1)/t
                 d = t*(ak*akp1 - one)
                 a(k,k) = akp1/d
                 a(k + 1,k + 1) = ak/d
                 ...
                 if (k > 1) then
                    ...
                    a(k,k) = a(k,k) - real(la_ydotc(k - 1,work,1,a(1,k),1), &
                              KIND=xdp)
                    a(k,k + 1) = a(k,k + 1) - la_ydotc(k - 1,a(1,k),1,a(1,k + 1),1)

                    call la_ycopy(k - 1,a(1,k + 1),1,work,1)
                    call la_yhemv(uplo,k - 1,-cone,a,lda,work,1,czero,a(1,k + 1),1)

                    a(k + 1,k + 1) = a(k + 1,k + 1) - real(la_ydotc(k - 1,work,1,a(1,k + 1), &
                              1),KIND=xdp)
                 end if
                 kstep = 2
              end if
              ...
              if (ipiv(k) > 0) then
                 ...
                 if (k < n) then
                    ...
                    call la_yhemv(uplo,n - k,-cone,a(k + 1,k + 1),lda,work,1,czero,a(k + &
                              1,k),1)
                    a(k,k) = a(k,k) - real(la_ydotc(n - k,work,1,a(k + 1,k),1), &
                              KIND=xdp)
                 end if
                 kstep = 1
              else
                 ...
                 ! invert the diagonal block.
                 t = abs(a(k,k - 1))
                 ak = real(a(k - 1,k - 1),KIND=xdp)/t
                 akp1 = real(a(k,k),KIND=xdp)/t
                 akkp1 = a(k,k - 1)/t
                 d = t*(ak*akp1 - one)
                 a(k - 1,k - 1) = akp1/d
                 a(k,k) = ak/d
                 ...
                 if (k < n) then
                    ...
                    a(k,k) = a(k,k) - real(la_ydotc(n - k,work,1,a(k + 1,k),1), &
                              KIND=xdp)
                    a(k,k - 1) = a(k,k - 1) - la_ydotc(n - k,a(k + 1,k),1,a(k + 1,k - 1),1 &
                              )
                    call la_ycopy(n - k,a(k + 1,k - 1),1,work,1)
                    call la_yhemv(uplo,n - k,-cone,a(k + 1,k + 1),lda,work,1,czero,a(k + &
                              1,k - 1),1)
                    a(k - 1,k - 1) = a(k - 1,k - 1) - real(la_ydotc(n - k,work,1,a(k + 1,k - 1) &
                              ,1),KIND=xdp)
                 end if
                 kstep = 2
              end if
     ...
     pure subroutine la_whetri(uplo,n,a,lda,ipiv,work,info)
        use la_constants_qp
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
     ...
     pure subroutine la_chetrs_3(uplo,n,nrhs,a,lda,e,ipiv,b,ldb,info)
        use la_constants_sp
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ...
           if (upper) then
              ...
              do while (i >= 1)
                 if (ipiv(i) > 0) then
                    ...
                    call la_csscal(nrhs,s,b(i,1),ldb)
                 else if (i > 1) then
                    akm1k = e(i)
              ...
              call la_ctrsm('L','U','C','U',n,nrhs,cone,a,lda,b,ldb)
              ! p * b  [ p * (u**h \ (d \ (u \p**t * b) )) ]
              ...
              call la_ctrsm('L','L','C','U',n,nrhs,cone,a,lda,b,ldb)
              ! p * b  [ p * (l**h \ (d \ (l \p**t * b) )) ]
              ! interchange rows k and ipiv(k) of matrix b in reverse order
     ...
     pure subroutine la_zhetrs_3(uplo,n,nrhs,a,lda,e,ipiv,b,ldb,info)
        use la_constants_dp
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
        ...
        ! =====================================================================
           ...
           if (upper) then
              ...
              do while (i >= 1)
                 if (ipiv(i) > 0) then
                    ...
                    call la_zdscal(nrhs,s,b(i,1),ldb)
                 else if (i > 1) then
                    akm1k = e(i)
                    akm1 = a(i - 1,i - 1)/akm1k
                    ak = a(i,i)/conjg(akm1k)
              ...
              ! compute (u**h \ b) -> b   [ u**h \ (d \ (u \p**t * b) ) ]
              call la_ztrsm('L','U','C','U',n,nrhs,cone,a,lda,b,ldb)
              ! p * b  [ p * (u**h \ (d \ (u \p**t * b) )) ]
              ! interchange rows k and ipiv(k) of matrix b in reverse order
              ! from the formation order of ipiv(i) vector for upper case.
              ! (we can do the simple loop over ipiv with increment 1,
              ...
              ! compute (l**h \ b) -> b   [ l**h \ (d \ (l \p**t * b) ) ]
              call la_ztrsm('L','L','C','U',n,nrhs,cone,a,lda,b,ldb)
              ! p * b  [ p * (l**h \ (d \ (l \p**t * b) )) ]
              ! interchange rows k and ipiv(k) of matrix b in reverse order
              ! from the formation order of ipiv(i) vector for lower case.
              ! (we can do the simple loop over ipiv with decrement -1,
           ...
           return
     end subroutine la_zhetrs_3
#ifdef LA_WITH_XDP
     !> YHETRS_3: solves a system of linear equations A * X = B with a complex
     !> Hermitian matrix A using the factorization computed
     !> by YHETRF_RK or ZHETRF_BK:
     !> A = P*U*D*(U**H)*(P**T) or A = P*L*D*(L**H)*(P**T),
     !> where U (or L) is unit upper (or lower) triangular matrix,
     !> U**H (or L**H) is the conjugate of U (or L), P is a permutation
     !> matrix, P**T is the transpose of P, and D is Hermitian and block
     ...
     pure subroutine la_yhetrs_3(uplo,n,nrhs,a,lda,e,ipiv,b,ldb,info)
        use la_constants_xdp
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
        ...
        ! =====================================================================
           ...
           if (upper) then
              ...
              do while (i >= 1)
                 if (ipiv(i) > 0) then
                    s = real(cone,KIND=xdp)/real(a(i,i),KIND=xdp)
                    call la_yxscal(nrhs,s,b(i,1),ldb)
                 else if (i > 1) then
                    akm1k = e(i)
                    akm1 = a(i - 1,i - 1)/akm1k
                    ak = a(i,i)/conjg(akm1k)
              ...
              ! compute (u**h \ b) -> b   [ u**h \ (d \ (u \p**t * b) ) ]
              call la_ytrsm('L','U','C','U',n,nrhs,cone,a,lda,b,ldb)
              ! p * b  [ p * (u**h \ (d \ (u \p**t * b) )) ]
              ! interchange rows k and ipiv(k) of matrix b in reverse order
              ! from the formation order of ipiv(i) vector for upper case.
              ! (we can do the simple loop over ipiv with increment 1,
              ...
              ! compute (l**h \ b) -> b   [ l**h \ (d \ (l \p**t * b) ) ]
              call la_ytrsm('L','L','C','U',n,nrhs,cone,a,lda,b,ldb)
              ! p * b  [ p * (l**h \ (d \ (l \p**t * b) )) ]
              ! interchange rows k and ipiv(k) of matrix b in reverse order
              ! from the formation order of ipiv(i) vector for lower case.
              ! (we can do the simple loop over ipiv with decrement -1,
     ...
     pure subroutine la_whetrs_3(uplo,n,nrhs,a,lda,e,ipiv,b,ldb,info)
        use la_constants_qp
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
        ...
        ! =====================================================================
           ...
           if (upper) then
              ...
              do while (i >= 1)
                 if (ipiv(i) > 0) then
                    s = real(cone,KIND=qp)/real(a(i,i),KIND=qp)
                    call la_wqscal(nrhs,s,b(i,1),ldb)
                 else if (i > 1) then
                    akm1k = e(i)
                    akm1 = a(i - 1,i - 1)/akm1k
                    ak = a(i,i)/conjg(akm1k)
              ...
              ! compute (u**h \ b) -> b   [ u**h \ (d \ (u \p**t * b) ) ]
              call la_wtrsm('L','U','C','U',n,nrhs,cone,a,lda,b,ldb)
              ! p * b  [ p * (u**h \ (d \ (u \p**t * b) )) ]
              ! interchange rows k and ipiv(k) of matrix b in reverse order
              ! from the formation order of ipiv(i) vector for upper case.
              ! (we can do the simple loop over ipiv with increment 1,
              ...
              ! compute (l**h \ b) -> b   [ l**h \ (d \ (l \p**t * b) ) ]
              call la_wtrsm('L','L','C','U',n,nrhs,cone,a,lda,b,ldb)
              ! p * b  [ p * (l**h \ (d \ (l \p**t * b) )) ]
              ! interchange rows k and ipiv(k) of matrix b in reverse order
              ! from the formation order of ipiv(i) vector for lower case.
              ! (we can do the simple loop over ipiv with decrement -1,
     ...
     pure subroutine la_chptrf(uplo,n,ap,ipiv,info)
        use la_constants_sp
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ...
           if (upper) then
              ...
              if (max(absakk,colmax) == zero) then
                 ...
                 if (kstep == 1) then
                    ...
                    call la_chpr(uplo,k - 1,-r1,ap(kc),1,ap)
                    ! store u(k) in column k
                    call la_csscal(k - 1,r1,ap(kc),1)
                 else
                    ! 2-by-2 pivot block d(k): columns k and k-1 now hold
                    ! ( w(k-1) w(k) ) = ( u(k-1) u(k) )*d(k)
                    ! where u(k) and u(k-1) are the k-th and (k-1)-th columns
                    ...
                    ! a := a - ( u(k-1) u(k) )*d(k)*( u(k-1) u(k) )**h
                       ! = a - ( w(k-1) w(k) )*inv(d(k))*( w(k-1) w(k) )**h
                    if (k > 2) then
                       d = la_slapy2(real(ap(k - 1 + (k - 1)*k/2),KIND=sp),aimag(ap(k - 1 + ( &
                                 k - 1)*k/2)))
              ...
              if (max(absakk,colmax) == zero) then
                 ...
                 if (kstep == 1) then
                    ...
                    if (k < n) then
                       ...
                       call la_chpr(uplo,n - k,-r1,ap(kc + 1),1,ap(kc + n - k + 1))
                       ! store l(k) in column k
                       call la_csscal(n - k,r1,ap(kc + 1),1)
                    end if
                 else
                    ! 2-by-2 pivot block d(k): columns k and k+1 now hold
                    ...
                    if (k < n - 1) then
                       ...
                       ! where l(k) and l(k+1) are the k-th and (k+1)-th
                       ! columns of l
                       d = la_slapy2(real(ap(k + 1 + (k - 1)*(2*n - k)/2),KIND=sp),aimag( &
                                 ap(k + 1 + (k - 1)*(2*n - k)/2)))
     ...
     pure subroutine la_zhptrf(uplo,n,ap,ipiv,info)
        use la_constants_dp
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
        ...
        ! =====================================================================
           ...
           if (upper) then
              ...
              if (max(absakk,colmax) == zero) then
                 ...
                 if (kstep == 1) then
                    ...
                    call la_zhpr(uplo,k - 1,-r1,ap(kc),1,ap)
                    ! store u(k) in column k
                    call la_zdscal(k - 1,r1,ap(kc),1)
                 else
                    ! 2-by-2 pivot block d(k): columns k and k-1 now hold
                    ! ( w(k-1) w(k) ) = ( u(k-1) u(k) )*d(k)
                    ! where u(k) and u(k-1) are the k-th and (k-1)-th columns
                    ...
                    ! a := a - ( u(k-1) u(k) )*d(k)*( u(k-1) u(k) )**h
                       ! = a - ( w(k-1) w(k) )*inv(d(k))*( w(k-1) w(k) )**h
                    if (k > 2) then
                       d = la_dlapy2(real(ap(k - 1 + (k - 1)*k/2),KIND=dp),aimag(ap(k - 1 + ( &
                                 k - 1)*k/2)))
              ...
              if (max(absakk,colmax) == zero) then
                 ...
                 if (kstep == 1) then
                    ...
                    if (k < n) then
                       ...
                       call la_zhpr(uplo,n - k,-r1,ap(kc + 1),1,ap(kc + n - k + 1))
                       ! store l(k) in column k
                       call la_zdscal(n - k,r1,ap(kc + 1),1)
                    end if
                 else
                    ! 2-by-2 pivot block d(k): columns k and k+1 now hold
                    ! ( w(k) w(k+1) ) = ( l(k) l(k+1) )*d(k)
                    ...
                    if (k < n - 1) then
                       ...
                       ! where l(k) and l(k+1) are the k-th and (k+1)-th
                       ! columns of l
                       d = la_dlapy2(real(ap(k + 1 + (k - 1)*(2*n - k)/2),KIND=dp),aimag( &
                                 ap(k + 1 + (k - 1)*(2*n - k)/2)))
           ...
           110 continue
           return
     end subroutine la_zhptrf
#ifdef LA_WITH_XDP
     !> YHPTRF: computes the factorization of a complex Hermitian packed
     !> matrix A using the Bunch-Kaufman diagonal pivoting method:
     !> A = U*D*U**H  or  A = L*D*L**H
     !> where U (or L) is a product of permutation and unit upper (lower)
     !> triangular matrices, and D is Hermitian and block diagonal with
     ...
     pure subroutine la_yhptrf(uplo,n,ap,ipiv,info)
        use la_constants_xdp
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
        ...
        ! =====================================================================
           ...
           if (upper) then
              ...
              ! determine rows and columns to be interchanged and whether
              ! a 1-by-1 or 2-by-2 pivot block will be used
              absakk = abs(real(ap(kc + k - 1),KIND=xdp))
              ! imax is the row-index of the largest off-diagonal element in
              ! column k, and colmax is its absolute value
              if (k > 1) then
              ...
              if (max(absakk,colmax) == zero) then
                 ...
                 if (info == 0) info = k
                 kp = k
                 ap(kc + k - 1) = real(ap(kc + k - 1),KIND=xdp)
              else
                 if (absakk >= alpha*colmax) then
                    ! no interchange, use 1-by-1 pivot block
                    kp = k
                    ...
                    if (absakk >= alpha*colmax*(colmax/rowmax)) then
                       ! no interchange, use 1-by-1 pivot block
                       kp = k
                    else if (abs(real(ap(kpc + imax - 1),KIND=xdp)) >= alpha*rowmax) then
                       ! interchange rows and columns k and imax, use 1-by-1
                       ! pivot block
                       kp = imax
                    else
                 ...
                 if (kp /= kk) then
                    ...
                    ap(kx + kk - 1) = conjg(ap(kx + kk - 1))
                    r1 = real(ap(knc + kk - 1),KIND=xdp)
                    ap(knc + kk - 1) = real(ap(kpc + kp - 1),KIND=xdp)
                    ap(kpc + kp - 1) = r1
                    if (kstep == 2) then
                       ap(kc + k - 1) = real(ap(kc + k - 1),KIND=xdp)
                       t = ap(kc + k - 2)
                    ...
                    ap(kc + k - 1) = real(ap(kc + k - 1),KIND=xdp)
                    if (kstep == 2) ap(kc - 1) = real(ap(kc - 1),KIND=xdp)
                 end if
                 ! update the leading submatrix
                 if (kstep == 1) then
                    ! 1-by-1 pivot block d(k): column k now holds
                    ...
                    ! perform a rank-1 update of a(1:k-1,1:k-1) as
                    ! a := a - u(k)*d(k)*u(k)**h = a - w(k)*1/d(k)*w(k)**h
                    r1 = one/real(ap(kc + k - 1),KIND=xdp)
                    call la_yhpr(uplo,k - 1,-r1,ap(kc),1,ap)
                    ! store u(k) in column k
                    call la_yxscal(k - 1,r1,ap(kc),1)
                 else
                    ! 2-by-2 pivot block d(k): columns k and k-1 now hold
                    ! ( w(k-1) w(k) ) = ( u(k-1) u(k) )*d(k)
                    ! where u(k) and u(k-1) are the k-th and (k-1)-th columns
                    ...
                    ! a := a - ( u(k-1) u(k) )*d(k)*( u(k-1) u(k) )**h
                       ! = a - ( w(k-1) w(k) )*inv(d(k))*( w(k-1) w(k) )**h
                    if (k > 2) then
                       d = la_xlapy2(real(ap(k - 1 + (k - 1)*k/2),KIND=xdp),aimag(ap(k - 1 + ( &
                                 k - 1)*k/2)))
                       d22 = real(ap(k - 1 + (k - 2)*(k - 1)/2),KIND=xdp)/d
                       d11 = real(ap(k + (k - 1)*k/2),KIND=xdp)/d
                       tt = one/(d11*d22 - one)
                       d12 = ap(k - 1 + (k - 1)*k/2)/d
                       d = tt/d
                       do j = k - 2,1,-1
                          ...
                          ap(j + (k - 1)*k/2) = wk
                          ap(j + (k - 2)*(k - 1)/2) = wkm1
                          ap(j + (j - 1)*j/2) = cmplx(real(ap(j + (j - 1)*j/2),KIND=xdp), &
                                    zero,KIND=xdp)
                       end do
                    end if
                 end if
              ...
              ! determine rows and columns to be interchanged and whether
              ! a 1-by-1 or 2-by-2 pivot block will be used
              absakk = abs(real(ap(kc),KIND=xdp))
              ! imax is the row-index of the largest off-diagonal element in
              ! column k, and colmax is its absolute value
              if (k < n) then
              ...
              if (max(absakk,colmax) == zero) then
                 ...
                 if (absakk >= alpha*colmax) then
                    ...
                    if (absakk >= alpha*colmax*(colmax/rowmax)) then
                       ! no interchange, use 1-by-1 pivot block
                       kp = k
                    else if (abs(real(ap(kpc),KIND=xdp)) >= alpha*rowmax) then
                       ! interchange rows and columns k and imax, use 1-by-1
                       ! pivot block
                       kp = imax
                    else
                 ...
                 if (kp /= kk) then
                    ...
                    ap(knc + kp - kk) = conjg(ap(knc + kp - kk))
                    r1 = real(ap(knc),KIND=xdp)
                    ap(knc) = real(ap(kpc),KIND=xdp)
                    ap(kpc) = r1
                    if (kstep == 2) then
                       ap(kc) = real(ap(kc),KIND=xdp)
                       t = ap(kc + 1)
                    ...
                    ap(kc) = real(ap(kc),KIND=xdp)
                    if (kstep == 2) ap(knc) = real(ap(knc),KIND=xdp)
                 end if
                 ! update the trailing submatrix
                 if (kstep == 1) then
                    ! 1-by-1 pivot block d(k): column k now holds
                    ...
                    if (k < n) then
                       ! perform a rank-1 update of a(k+1:n,k+1:n) as
                       ! a := a - l(k)*d(k)*l(k)**h = a - w(k)*(1/d(k))*w(k)**h
                       r1 = one/real(ap(kc),KIND=xdp)
                       call la_yhpr(uplo,n - k,-r1,ap(kc + 1),1,ap(kc + n - k + 1))
                       ! store l(k) in column k
                       call la_yxscal(n - k,r1,ap(kc + 1),1)
                    end if
                 else
                    ! 2-by-2 pivot block d(k): columns k and k+1 now hold
                    ! ( w(k) w(k+1) ) = ( l(k) l(k+1) )*d(k)
                    ...
                    if (k < n - 1) then
                       ...
                       ! where l(k) and l(k+1) are the k-th and (k+1)-th
                       ! columns of l
                       d = la_xlapy2(real(ap(k + 1 + (k - 1)*(2*n - k)/2),KIND=xdp),aimag( &
                                 ap(k + 1 + (k - 1)*(2*n - k)/2)))
                       d11 = real(ap(k + 1 + k*(2*n - k - 1)/2),KIND=xdp)/d
                       d22 = real(ap(k + (k - 1)*(2*n - k)/2),KIND=xdp)/d
                       tt = one/(d11*d22 - one)
                       d21 = ap(k + 1 + (k - 1)*(2*n - k)/2)/d
                       d = tt/d
                       do j = k + 2,n
     ...
     pure subroutine la_whptrf(uplo,n,ap,ipiv,info)
        use la_constants_qp
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
        ...
        ! =====================================================================
           ...
           if (upper) then
              ...
              ! determine rows and columns to be interchanged and whether
              ! a 1-by-1 or 2-by-2 pivot block will be used
              absakk = abs(real(ap(kc + k - 1),KIND=qp))
              ! imax is the row-index of the largest off-diagonal element in
              ! column k, and colmax is its absolute value
              if (k > 1) then
              ...
              if (max(absakk,colmax) == zero) then
                 ...
                 if (info == 0) info = k
                 kp = k
                 ap(kc + k - 1) = real(ap(kc + k - 1),KIND=qp)
              else
                 if (absakk >= alpha*colmax) then
                    ! no interchange, use 1-by-1 pivot block
                    kp = k
                    ...
                    if (absakk >= alpha*colmax*(colmax/rowmax)) then
                       ! no interchange, use 1-by-1 pivot block
                       kp = k
                    else if (abs(real(ap(kpc + imax - 1),KIND=qp)) >= alpha*rowmax) then
                       ! interchange rows and columns k and imax, use 1-by-1
                       ! pivot block
                       kp = imax
                    else
                 ...
                 if (kp /= kk) then
                    ...
                    ap(kx + kk - 1) = conjg(ap(kx + kk - 1))
                    r1 = real(ap(knc + kk - 1),KIND=qp)
                    ap(knc + kk - 1) = real(ap(kpc + kp - 1),KIND=qp)
                    ap(kpc + kp - 1) = r1
                    if (kstep == 2) then
                       ap(kc + k - 1) = real(ap(kc + k - 1),KIND=qp)
                       t = ap(kc + k - 2)
                    ...
                    ap(kc + k - 1) = real(ap(kc + k - 1),KIND=qp)
                    if (kstep == 2) ap(kc - 1) = real(ap(kc - 1),KIND=qp)
                 end if
                 ! update the leading submatrix
                 if (kstep == 1) then
                    ! 1-by-1 pivot block d(k): column k now holds
                    ...
                    ! perform a rank-1 update of a(1:k-1,1:k-1) as
                    ! a := a - u(k)*d(k)*u(k)**h = a - w(k)*1/d(k)*w(k)**h
                    r1 = one/real(ap(kc + k - 1),KIND=qp)
                    call la_whpr(uplo,k - 1,-r1,ap(kc),1,ap)
                    ! store u(k) in column k
                    call la_wqscal(k - 1,r1,ap(kc),1)
                 else
                    ! 2-by-2 pivot block d(k): columns k and k-1 now hold
                    ! ( w(k-1) w(k) ) = ( u(k-1) u(k) )*d(k)
                    ! where u(k) and u(k-1) are the k-th and (k-1)-th columns
                    ...
                    ! a := a - ( u(k-1) u(k) )*d(k)*( u(k-1) u(k) )**h
                       ! = a - ( w(k-1) w(k) )*inv(d(k))*( w(k-1) w(k) )**h
                    if (k > 2) then
                       d = la_qlapy2(real(ap(k - 1 + (k - 1)*k/2),KIND=qp),aimag(ap(k - 1 + ( &
                                 k - 1)*k/2)))
              ...
              if (max(absakk,colmax) == zero) then
                 ...
                 if (absakk >= alpha*colmax) then
                    ...
                    if (absakk >= alpha*colmax*(colmax/rowmax)) then
                       ! no interchange, use 1-by-1 pivot block
                       kp = k
                    else if (abs(real(ap(kpc),KIND=qp)) >= alpha*rowmax) then
                       ! interchange rows and columns k and imax, use 1-by-1
                       ! pivot block
                       kp = imax
                    else
                 ...
                 if (kp /= kk) then
                    ...
                    ap(knc + kp - kk) = conjg(ap(knc + kp - kk))
                    r1 = real(ap(knc),KIND=qp)
                    ap(knc) = real(ap(kpc),KIND=qp)
                    ap(kpc) = r1
                    if (kstep == 2) then
                       ap(kc) = real(ap(kc),KIND=qp)
                       t = ap(kc + 1)
                    ...
                    ap(kc) = real(ap(kc),KIND=qp)
                    if (kstep == 2) ap(knc) = real(ap(knc),KIND=qp)
                 end if
                 ! update the trailing submatrix
                 if (kstep == 1) then
                    ! 1-by-1 pivot block d(k): column k now holds
                    ...
                    if (k < n) then
                       ! perform a rank-1 update of a(k+1:n,k+1:n) as
                       ! a := a - l(k)*d(k)*l(k)**h = a - w(k)*(1/d(k))*w(k)**h
                       r1 = one/real(ap(kc),KIND=qp)
                       call la_whpr(uplo,n - k,-r1,ap(kc + 1),1,ap(kc + n - k + 1))
                       ! store l(k) in column k
                       call la_wqscal(n - k,r1,ap(kc + 1),1)
                    end if
                 else
                    ! 2-by-2 pivot block d(k): columns k and k+1 now hold
                    ! ( w(k) w(k+1) ) = ( l(k) l(k+1) )*d(k)
                    ...
                    if (k < n - 1) then
                       ...
                       ! where l(k) and l(k+1) are the k-th and (k+1)-th
                       ! columns of l
                       d = la_qlapy2(real(ap(k + 1 + (k - 1)*(2*n - k)/2),KIND=qp),aimag( &
                                 ap(k + 1 + (k - 1)*(2*n - k)/2)))
     ...
     pure subroutine la_chptri(uplo,n,ap,ipiv,work,info)
        use la_constants_sp
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
        ...
        ! =====================================================================
           ...
           if (upper) then
              ...
              if (ipiv(k) > 0) then
                 ...
                 ! compute column k of the inverse.
                 if (k > 1) then
                    call la_ccopy(k - 1,ap(kc),1,work,1)
                    call la_chpmv(uplo,k - 1,-cone,ap,work,1,czero,ap(kc),1)
                    ap(kc + k - 1) = ap(kc + k - 1) - real(la_cdotc(k - 1,work,1,ap(kc),1), &
                              KIND=sp)
                 end if
                 kstep = 1
              else
                 ...
                 if (k > 1) then
                    ...
                    ap(kc + k - 1) = ap(kc + k - 1) - real(la_cdotc(k - 1,work,1,ap(kc),1), &
                              KIND=sp)
                    ap(kcnext + k - 1) = ap(kcnext + k - 1) - la_cdotc(k - 1,ap(kc),1,ap( &
                              kcnext),1)
                    call la_ccopy(k - 1,ap(kcnext),1,work,1)
                    call la_chpmv(uplo,k - 1,-cone,ap,work,1,czero,ap(kcnext),1)

                    ap(kcnext + k) = ap(kcnext + k) - real(la_cdotc(k - 1,work,1,ap(kcnext &
                              ),1),KIND=sp)
                 end if
                 kstep = 2
                 kcnext = kcnext + k + 1
              ...
              if (kp /= k) then
                 ...
                 ! submatrix a(1:k+1,1:k+1)
                 kpc = (kp - 1)*kp/2 + 1
                 call la_cswap(kp - 1,ap(kc),1,ap(kpc),1)
                 kx = kpc + kp - 1
                 do j = kp + 1,k - 1
                    kx = kx + j - 1
              ...
              if (ipiv(k) > 0) then
                 ...
                 ! compute column k of the inverse.
                 if (k < n) then
                    call la_ccopy(n - k,ap(kc + 1),1,work,1)
                    call la_chpmv(uplo,n - k,-cone,ap(kc + n - k + 1),work,1,czero,ap(kc + 1) &
                              ,1)
                    ap(kc) = ap(kc) - real(la_cdotc(n - k,work,1,ap(kc + 1),1), &
                              KIND=sp)
                 end if
                 kstep = 1
              else
                 ...
                 if (k < n) then
                    call la_ccopy(n - k,ap(kc + 1),1,work,1)
                    call la_chpmv(uplo,n - k,-cone,ap(kc + (n - k + 1)),work,1,czero,ap( &
                              kc + 1),1)
                    ap(kc) = ap(kc) - real(la_cdotc(n - k,work,1,ap(kc + 1),1), &
                              KIND=sp)
                    ap(kcnext + 1) = ap(kcnext + 1) - la_cdotc(n - k,ap(kc + 1),1,ap(kcnext + &
                              2),1)
                    call la_ccopy(n - k,ap(kcnext + 2),1,work,1)
                    call la_chpmv(uplo,n - k,-cone,ap(kc + (n - k + 1)),work,1,czero,ap( &
                              kcnext + 2),1)
                    ap(kcnext) = ap(kcnext) - real(la_cdotc(n - k,work,1,ap(kcnext + 2), &
                              1),KIND=sp)
                 end if
                 kstep = 2
                 kcnext = kcnext - (n - k + 3)
              ...
              if (kp /= k) then
                 ...
                 if (kp < n) call la_cswap(n - kp,ap(kc + kp - k + 1),1,ap(kpc + 1),1)
                 kx = kc + kp - k
                 do j = k + 1,kp - 1
                    kx = kx + n - j + 1
           ...
           return
     end subroutine la_chptri
     !> ZHPTRI: computes the inverse of a complex Hermitian indefinite matrix
     !> A in packed storage using the factorization A = U*D*U**H or
     !> A = L*D*L**H computed by ZHPTRF.

     pure subroutine la_zhptri(uplo,n,ap,ipiv,work,info)
        use la_constants_dp
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
        ...
        ! =====================================================================
           ...
           if (upper) then
              ...
              if (ipiv(k) > 0) then
                 ...
                 ! compute column k of the inverse.
                 if (k > 1) then
                    call la_zcopy(k - 1,ap(kc),1,work,1)
                    call la_zhpmv(uplo,k - 1,-cone,ap,work,1,czero,ap(kc),1)
                    ap(kc + k - 1) = ap(kc + k - 1) - real(la_zdotc(k - 1,work,1,ap(kc),1), &
                              KIND=dp)
                 end if
                 kstep = 1
              else
                 ...
                 if (k > 1) then
                    ...
                    ap(kc + k - 1) = ap(kc + k - 1) - real(la_zdotc(k - 1,work,1,ap(kc),1), &
                              KIND=dp)
                    ap(kcnext + k - 1) = ap(kcnext + k - 1) - la_zdotc(k - 1,ap(kc),1,ap( &
                              kcnext),1)
                    call la_zcopy(k - 1,ap(kcnext),1,work,1)
                    call la_zhpmv(uplo,k - 1,-cone,ap,work,1,czero,ap(kcnext),1)

                    ap(kcnext + k) = ap(kcnext + k) - real(la_zdotc(k - 1,work,1,ap(kcnext &
                              ),1),KIND=dp)
                 end if
                 kstep = 2
                 kcnext = kcnext + k + 1
              ...
              if (kp /= k) then
                 ...
                 ! submatrix a(1:k+1,1:k+1)
                 kpc = (kp - 1)*kp/2 + 1
                 call la_zswap(kp - 1,ap(kc),1,ap(kpc),1)
                 kx = kpc + kp - 1
                 do j = kp + 1,k - 1
                    kx = kx + j - 1
                    temp = conjg(ap(kc + j - 1))
              ...
              if (ipiv(k) > 0) then
                 ...
                 ! compute column k of the inverse.
                 if (k < n) then
                    call la_zcopy(n - k,ap(kc + 1),1,work,1)
                    call la_zhpmv(uplo,n - k,-cone,ap(kc + n - k + 1),work,1,czero,ap(kc + 1) &
                              ,1)
                    ap(kc) = ap(kc) - real(la_zdotc(n - k,work,1,ap(kc + 1),1), &
                              KIND=dp)
                 end if
                 kstep = 1
              else
                 ...
                 if (k < n) then
                    call la_zcopy(n - k,ap(kc + 1),1,work,1)
                    call la_zhpmv(uplo,n - k,-cone,ap(kc + (n - k + 1)),work,1,czero,ap( &
                              kc + 1),1)
                    ap(kc) = ap(kc) - real(la_zdotc(n - k,work,1,ap(kc + 1),1), &
                              KIND=dp)
                    ap(kcnext + 1) = ap(kcnext + 1) - la_zdotc(n - k,ap(kc + 1),1,ap(kcnext + &
                              2),1)
                    call la_zcopy(n - k,ap(kcnext + 2),1,work,1)
                    call la_zhpmv(uplo,n - k,-cone,ap(kc + (n - k + 1)),work,1,czero,ap( &
                              kcnext + 2),1)
                    ap(kcnext) = ap(kcnext) - real(la_zdotc(n - k,work,1,ap(kcnext + 2), &
                              1),KIND=dp)
                 end if
                 kstep = 2
                 kcnext = kcnext - (n - k + 3)
              ...
              if (kp /= k) then
                 ...
                 ! submatrix a(k-1:n,k-1:n)
                 kpc = npp - (n - kp + 1)*(n - kp + 2)/2 + 1
                 if (kp < n) call la_zswap(n - kp,ap(kc + kp - k + 1),1,ap(kpc + 1),1)
                 kx = kc + kp - k
                 do j = k + 1,kp - 1
                    kx = kx + n - j + 1
                    temp = conjg(ap(kc + j - k))
           ...
           return
     end subroutine la_zhptri
#ifdef LA_WITH_XDP
     !> YHPTRI: computes the inverse of a complex Hermitian indefinite matrix
     !> A in packed storage using the factorization A = U*D*U**H or
     !> A = L*D*L**H computed by YHPTRF.

     pure subroutine la_yhptri(uplo,n,ap,ipiv,work,info)
        use la_constants_xdp
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
        ...
        ! =====================================================================
           ...
           if (upper) then
              ...
              if (ipiv(k) > 0) then
                 ! 1 x 1 diagonal block
                 ! invert the diagonal block.
                 ap(kc + k - 1) = one/real(ap(kc + k - 1),KIND=xdp)
                 ! compute column k of the inverse.
                 if (k > 1) then
                    call la_ycopy(k - 1,ap(kc),1,work,1)
                    call la_yhpmv(uplo,k - 1,-cone,ap,work,1,czero,ap(kc),1)
                    ap(kc + k - 1) = ap(kc + k - 1) - real(la_ydotc(k - 1,work,1,ap(kc),1), &
                              KIND=xdp)
                 end if
                 kstep = 1
              else
                 ...
                 ! invert the diagonal block.
                 t = abs(ap(kcnext + k - 1))
                 ak = real(ap(kc + k - 1),KIND=xdp)/t
                 akp1 = real(ap(kcnext + k),KIND=xdp)/t
                 akkp1 = ap(kcnext + k - 1)/t
                 d = t*(ak*akp1 - one)
                 ap(kc + k - 1) = akp1/d
                 ap(kcnext + k) = ak/d
                 ...
                 if (k > 1) then
                    ...
                    ap(kc + k - 1) = ap(kc + k - 1) - real(la_ydotc(k - 1,work,1,ap(kc),1), &
                              KIND=xdp)
                    ap(kcnext + k - 1) = ap(kcnext + k - 1) - la_ydotc(k - 1,ap(kc),1,ap( &
                              kcnext),1)
                    call la_ycopy(k - 1,ap(kcnext),1,work,1)
                    call la_yhpmv(uplo,k - 1,-cone,ap,work,1,czero,ap(kcnext),1)

                    ap(kcnext + k) = ap(kcnext + k) - real(la_ydotc(k - 1,work,1,ap(kcnext &
                              ),1),KIND=xdp)
                 end if
                 kstep = 2
                 kcnext = kcnext + k + 1
              ...
              if (kp /= k) then
                 ...
                 ! submatrix a(1:k+1,1:k+1)
                 kpc = (kp - 1)*kp/2 + 1
                 call la_yswap(kp - 1,ap(kc),1,ap(kpc),1)
                 kx = kpc + kp - 1
                 do j = kp + 1,k - 1
                    kx = kx + j - 1
                    temp = conjg(ap(kc + j - 1))
              ...
              if (ipiv(k) > 0) then
                 ! 1 x 1 diagonal block
                 ! invert the diagonal block.
                 ap(kc) = one/real(ap(kc),KIND=xdp)
                 ! compute column k of the inverse.
                 if (k < n) then
                    call la_ycopy(n - k,ap(kc + 1),1,work,1)
                    call la_yhpmv(uplo,n - k,-cone,ap(kc + n - k + 1),work,1,czero,ap(kc + 1) &
                              ,1)
                    ap(kc) = ap(kc) - real(la_ydotc(n - k,work,1,ap(kc + 1),1), &
                              KIND=xdp)
                 end if
                 kstep = 1
              else
                 ...
                 ! invert the diagonal block.
                 t = abs(ap(kcnext + 1))
                 ak = real(ap(kcnext),KIND=xdp)/t
                 akp1 = real(ap(kc),KIND=xdp)/t
                 akkp1 = ap(kcnext + 1)/t
                 d = t*(ak*akp1 - one)
                 ap(kcnext) = akp1/d
                 ap(kc) = ak/d
                 ...
                 if (k < n) then
                    call la_ycopy(n - k,ap(kc + 1),1,work,1)
                    call la_yhpmv(uplo,n - k,-cone,ap(kc + (n - k + 1)),work,1,czero,ap( &
                              kc + 1),1)
                    ap(kc) = ap(kc) - real(la_ydotc(n - k,work,1,ap(kc + 1),1), &
                              KIND=xdp)
                    ap(kcnext + 1) = ap(kcnext + 1) - la_ydotc(n - k,ap(kc + 1),1,ap(kcnext + &
                              2),1)
                    call la_ycopy(n - k,ap(kcnext + 2),1,work,1)
                    call la_yhpmv(uplo,n - k,-cone,ap(kc + (n - k + 1)),work,1,czero,ap( &
                              kcnext + 2),1)
                    ap(kcnext) = ap(kcnext) - real(la_ydotc(n - k,work,1,ap(kcnext + 2), &
                              1),KIND=xdp)
                 end if
                 kstep = 2
                 kcnext = kcnext - (n - k + 3)
              ...
              if (kp /= k) then
                 ...
                 ! submatrix a(k-1:n,k-1:n)
                 kpc = npp - (n - kp + 1)*(n - kp + 2)/2 + 1
                 if (kp < n) call la_yswap(n - kp,ap(kc + kp - k + 1),1,ap(kpc + 1),1)
                 kx = kc + kp - k
                 do j = k + 1,kp - 1
                    kx = kx + n - j + 1
                    temp = conjg(ap(kc + j - k))
           ...
           return
     end subroutine la_yhptri
#endif
#ifdef LA_WITH_QP
     !> WHPTRI: computes the inverse of a complex Hermitian indefinite matrix
     !> A in packed storage using the factorization A = U*D*U**H or
     !> A = L*D*L**H computed by WHPTRF.

     pure subroutine la_whptri(uplo,n,ap,ipiv,work,info)
        use la_constants_qp
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
        ...
        ! =====================================================================
           ...
           if (upper) then
              ...
              if (ipiv(k) > 0) then
                 ! 1 x 1 diagonal block
                 ! invert the diagonal block.
                 ap(kc + k - 1) = one/real(ap(kc + k - 1),KIND=qp)
                 ! compute column k of the inverse.
                 if (k > 1) then
                    call la_wcopy(k - 1,ap(kc),1,work,1)
                    call la_whpmv(uplo,k - 1,-cone,ap,work,1,czero,ap(kc),1)
                    ap(kc + k - 1) = ap(kc + k - 1) - real(la_wdotc(k - 1,work,1,ap(kc),1), &
                              KIND=qp)
                 end if
                 kstep = 1
              else
                 ...
                 ! invert the diagonal block.
                 t = abs(ap(kcnext + k - 1))
                 ak = real(ap(kc + k - 1),KIND=qp)/t
                 akp1 = real(ap(kcnext + k),KIND=qp)/t
                 akkp1 = ap(kcnext + k - 1)/t
                 d = t*(ak*akp1 - one)
                 ap(kc + k - 1) = akp1/d
                 ap(kcnext + k) = ak/d
                 ...
                 if (k > 1) then
                    ...
                    ap(kc + k - 1) = ap(kc + k - 1) - real(la_wdotc(k - 1,work,1,ap(kc),1), &
                              KIND=qp)
                    ap(kcnext + k - 1) = ap(kcnext + k - 1) - la_wdotc(k - 1,ap(kc),1,ap( &
                              kcnext),1)
                    call la_wcopy(k - 1,ap(kcnext),1,work,1)
                    call la_whpmv(uplo,k - 1,-cone,ap,work,1,czero,ap(kcnext),1)

                    ap(kcnext + k) = ap(kcnext + k) - real(la_wdotc(k - 1,work,1,ap(kcnext &
                              ),1),KIND=qp)
                 end if
                 kstep = 2
                 kcnext = kcnext + k + 1
              ...
              if (kp /= k) then
                 ...
                 ! submatrix a(1:k+1,1:k+1)
                 kpc = (kp - 1)*kp/2 + 1
                 call la_wswap(kp - 1,ap(kc),1,ap(kpc),1)
                 kx = kpc + kp - 1
                 do j = kp + 1,k - 1
                    kx = kx + j - 1
                    temp = conjg(ap(kc + j - 1))
              ...
              if (ipiv(k) > 0) then
                 ! 1 x 1 diagonal block
                 ! invert the diagonal block.
                 ap(kc) = one/real(ap(kc),KIND=qp)
                 ! compute column k of the inverse.
                 if (k < n) then
                    call la_wcopy(n - k,ap(kc + 1),1,work,1)
                    call la_whpmv(uplo,n - k,-cone,ap(kc + n - k + 1),work,1,czero,ap(kc + 1) &
                              ,1)
                    ap(kc) = ap(kc) - real(la_wdotc(n - k,work,1,ap(kc + 1),1), &
                              KIND=qp)
                 end if
                 kstep = 1
              else
                 ...
                 ! invert the diagonal block.
                 t = abs(ap(kcnext + 1))
                 ak = real(ap(kcnext),KIND=qp)/t
                 akp1 = real(ap(kc),KIND=qp)/t
                 akkp1 = ap(kcnext + 1)/t
                 d = t*(ak*akp1 - one)
                 ap(kcnext) = akp1/d
                 ap(kc) = ak/d
                 ...
                 if (k < n) then
                    call la_wcopy(n - k,ap(kc + 1),1,work,1)
                    call la_whpmv(uplo,n - k,-cone,ap(kc + (n - k + 1)),work,1,czero,ap( &
                              kc + 1),1)
                    ap(kc) = ap(kc) - real(la_wdotc(n - k,work,1,ap(kc + 1),1), &
                              KIND=qp)
                    ap(kcnext + 1) = ap(kcnext + 1) - la_wdotc(n - k,ap(kc + 1),1,ap(kcnext + &
                              2),1)
                    call la_wcopy(n - k,ap(kcnext + 2),1,work,1)
                    call la_whpmv(uplo,n - k,-cone,ap(kc + (n - k + 1)),work,1,czero,ap( &
                              kcnext + 2),1)
                    ap(kcnext) = ap(kcnext) - real(la_wdotc(n - k,work,1,ap(kcnext + 2), &
                              1),KIND=qp)
                 end if
                 kstep = 2
                 kcnext = kcnext - (n - k + 3)
              ...
              if (kp /= k) then
                 ...
                 ! submatrix a(k-1:n,k-1:n)
                 kpc = npp - (n - kp + 1)*(n - kp + 2)/2 + 1
                 if (kp < n) call la_wswap(n - kp,ap(kc + kp - k + 1),1,ap(kpc + 1),1)
                 kx = kc + kp - k
                 do j = k + 1,kp - 1
                    kx = kx + n - j + 1
                    temp = conjg(ap(kc + j - k))
     ...
     pure subroutine la_clahef(uplo,n,nb,kb,a,lda,ipiv,w,ldw,info)
        use la_constants_sp
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
        ...
        ! =====================================================================
           ...
           if (la_lsame(uplo,'U')) then
              ...
              call la_ccopy(k - 1,a(1,k),1,w(1,kw),1)
              ...
              if (max(absakk,colmax) == zero) then
                 ...
                 if (absakk >= alpha*colmax) then
                    ...
                    call la_ccopy(imax - 1,a(1,imax),1,w(1,kw - 1),1)
                    ...
                    if (k < n) then
                       call la_cgemv('NO TRANSPOSE',k,n - k,-cone,a(1,k + 1),lda,w(imax, &
     ...
     pure subroutine la_zlahef(uplo,n,nb,kb,a,lda,ipiv,w,ldw,info)
        use la_constants_dp
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
        ...
        ! =====================================================================
           ...
           if (la_lsame(uplo,'U')) then
              ...
              kstep = 1
              ! copy column k of a to column kw of w and update it
              call la_zcopy(k - 1,a(1,k),1,w(1,kw),1)
              ...
              if (max(absakk,colmax) == zero) then
                 ...
                 if (absakk >= alpha*colmax) then
                    ...
                    ! begin pivot search along imax row
                    ! copy column imax to column kw-1 of w and update it
                    call la_zcopy(imax - 1,a(1,imax),1,w(1,kw - 1),1)
                    ...
                    if (k < n) then
                       call la_zgemv('NO TRANSPOSE',k,n - k,-cone,a(1,k + 1),lda,w(imax, &
                                  kw + 1),ldw,cone,w(1,kw - 1),1)
           ...
           return
     end subroutine la_zlahef
#ifdef LA_WITH_XDP
     !> YLAHEF: computes a partial factorization of a complex Hermitian
     !> matrix A using the Bunch-Kaufman diagonal pivoting method. The
     !> partial factorization has the form:
     !> A  =  ( I  U12 ) ( A11  0  ) (  I      0     )  if UPLO = 'U', or:
     !> ( 0  U22 ) (  0   D  ) ( U12**H U22**H )
     ...
     !> the argument KB, and is either NB or NB-1, or N if N <= NB.
     !> Note that U**H denotes the conjugate transpose of U.
     !> YLAHEF is an auxiliary routine called by YHETRF. It uses blocked code
     !> (calling Level 3 BLAS) to update the submatrix A11 (if UPLO = 'U') or
     !> A22 (if UPLO = 'L').

     pure subroutine la_ylahef(uplo,n,nb,kb,a,lda,ipiv,w,ldw,info)
        use la_constants_xdp
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
        ...
        ! =====================================================================
           ...
           ! Local Scalars
           integer(ilp) :: imax,j,jb,jj,jmax,jp,k,kk,kkw,kp,kstep,kw
           real(xdp) :: absakk,alpha,colmax,r1,rowmax,t
           complex(xdp) :: d11,d21,d22,z
           ! Intrinsic Functions
           intrinsic :: abs,real,conjg,aimag,max,min,sqrt
           ! Statement Functions
           real(xdp) :: cabs1
           ...
           if (la_lsame(uplo,'U')) then
              ...
              kstep = 1
              ! copy column k of a to column kw of w and update it
              call la_ycopy(k - 1,a(1,k),1,w(1,kw),1)
              w(k,kw) = real(a(k,k),KIND=xdp)
              if (k < n) then
                 call la_ygemv('NO TRANSPOSE',k,n - k,-cone,a(1,k + 1),lda,w(k,kw + 1), &
                           ldw,cone,w(1,kw),1)
                 w(k,kw) = real(w(k,kw),KIND=xdp)
              end if
              ! determine rows and columns to be interchanged and whether
              ! a 1-by-1 or 2-by-2 pivot block will be used
              absakk = abs(real(w(k,kw),KIND=xdp))
              ! imax is the row-index of the largest off-diagonal element in
              ! column k, and colmax is its absolute value.
              ! determine both colmax and imax.
              if (k > 1) then
              ...
              if (max(absakk,colmax) == zero) then
                 ...
                 if (absakk >= alpha*colmax) then
                    ...
                    ! begin pivot search along imax row
                    ! copy column imax to column kw-1 of w and update it
                    call la_ycopy(imax - 1,a(1,imax),1,w(1,kw - 1),1)
                    w(imax,kw - 1) = real(a(imax,imax),KIND=xdp)
                    call la_ycopy(k - imax,a(imax,imax + 1),lda,w(imax + 1,kw - 1),1)

                    call la_ylacgv(k - imax,w(imax + 1,kw - 1),1)
                    if (k < n) then
                       call la_ygemv('NO TRANSPOSE',k,n - k,-cone,a(1,k + 1),lda,w(imax, &
                                  kw + 1),ldw,cone,w(1,kw - 1),1)
                       w(imax,kw - 1) = real(w(imax,kw - 1),KIND=xdp)
                    end if
                    ! jmax is the column-index of the largest off-diagonal
                    ! element in row imax, and rowmax is its absolute value.
                    ! determine only rowmax.
                    ...
                    if (absakk >= alpha*colmax*(colmax/rowmax)) then
                       ...
                       kp = k
                    ! case(3)
                    else if (abs(real(w(imax,kw - 1),KIND=xdp)) >= alpha*rowmax) then
                       ! interchange rows and columns k and imax, use 1-by-1
                       ! pivot block
                       kp = imax
                       ! copy column kw-1 of w to column kw of w
                 ...
                 if (kp /= kk) then
                    ...
                    ! (or k and k-1 for 2-by-2 pivot) of a, since these columns
                    ! will be later overwritten.
                    a(kp,kp) = real(a(kk,kk),KIND=xdp)
                    call la_ycopy(kk - 1 - kp,a(kp + 1,kk),1,a(kp,kp + 1),lda)
                    call la_ylacgv(kk - 1 - kp,a(kp,kp + 1),lda)
                 ...
                 if (kstep == 1) then
                    ...
                    ! and not stored)
                       ...
                       ! a(1:k-1,k) := u(1:k-1,k) = w(1:k-1,kw)/d(k,k)
                    ! (note: no need to use for hermitian matrix
                    ! a( k, k ) = real( w( k, k),KIND=xdp) to separately copy diagonal
                    ! element d(k,k) from w (potentially saves only one load))
                    ...
                    if (k > 2) then
                       ...
                       d11 = w(k,kw)/conjg(d21)
                       d22 = w(k - 1,kw - 1)/d21
                       t = one/(real(d11*d22,KIND=xdp) - one)
                       d21 = t/d21
                       ! update elements in columns a(k-1) and a(k) as
                       ! dot products of rows of ( w(kw-1) w(kw) ) and columns
                       ! of d**(-1)
              ...
              kstep = 1
              ! copy column k of a to column k of w and update it
              w(k,k) = real(a(k,k),KIND=xdp)
              if (k < n) call la_ycopy(n - k,a(k + 1,k),1,w(k + 1,k),1)
              call la_ygemv('NO TRANSPOSE',n - k + 1,k - 1,-cone,a(k,1),lda,w(k,1),ldw, &
                         cone,w(k,k),1)
              w(k,k) = real(w(k,k),KIND=xdp)
              ! determine rows and columns to be interchanged and whether
              ! a 1-by-1 or 2-by-2 pivot block will be used
              absakk = abs(real(w(k,k),KIND=xdp))
              ! imax is the row-index of the largest off-diagonal element in
              ! column k, and colmax is its absolute value.
              ! determine both colmax and imax.
              if (k < n) then
              ...
              if (max(absakk,colmax) == zero) then
                 ...
                 if (absakk >= alpha*colmax) then
                    ...
                    call la_ylacgv(imax - k,w(k,k + 1),1)
                    w(imax,k + 1) = real(a(imax,imax),KIND=xdp)
                    if (imax < n) call la_ycopy(n - imax,a(imax + 1,imax),1,w(imax + 1,k + 1), &
                              1)
                    call la_ygemv('NO TRANSPOSE',n - k + 1,k - 1,-cone,a(k,1),lda,w(imax, &
                              1),ldw,cone,w(k,k + 1),1)
                    w(imax,k + 1) = real(w(imax,k + 1),KIND=xdp)
                    ! jmax is the column-index of the largest off-diagonal
                    ! element in row imax, and rowmax is its absolute value.
                    ! determine only rowmax.
                    ...
                    if (absakk >= alpha*colmax*(colmax/rowmax)) then
                       ...
                       kp = k
                    ! case(3)
                    else if (abs(real(w(imax,k + 1),KIND=xdp)) >= alpha*rowmax) then
                       ! interchange rows and columns k and imax, use 1-by-1
                       ! pivot block
                       kp = imax
                       ! copy column k+1 of w to column k of w
                 ...
                 if (kp /= kk) then
                    ...
                    call la_ylacgv(kp - kk - 1,a(kp,kk + 1),lda)
                 ...
                 if (kstep == 1) then
                    ...
                    ! (2) conjugate columns w(k) and w(k+1)
                    call la_ylacgv(n - k,w(k + 1,k),1)
                    call la_ylacgv(n - k - 1,w(k + 2,k + 1),1)
                 end if
              end if
              ! store details of the interchanges in ipiv
              if (kstep == 1) then
     ...
     !> the argument KB, and is either NB or NB-1, or N if N <= NB.
     !> Note that U**H denotes the conjugate transpose of U.
     !> WLAHEF is an auxiliary routine called by WHETRF. It uses blocked code
     !> (calling Level 3 BLAS) to update the submatrix A11 (if UPLO = 'U') or
     !> A22 (if UPLO = 'L').

     pure subroutine la_wlahef(uplo,n,nb,kb,a,lda,ipiv,w,ldw,info)
        use la_constants_qp
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
        ...
        ! =====================================================================
           ...
           if (la_lsame(uplo,'U')) then
              ...
              kstep = 1
              ! copy column k of a to column kw of w and update it
              call la_wcopy(k - 1,a(1,k),1,w(1,kw),1)
              ...
              if (max(absakk,colmax) == zero) then
                 ...
                 if (absakk >= alpha*colmax) then
                    ...
                    ! begin pivot search along imax row
                    ! copy column imax to column kw-1 of w and update it
                    call la_wcopy(imax - 1,a(1,imax),1,w(1,kw - 1),1)
                    ...
                    if (k < n) then
                       call la_wgemv('NO TRANSPOSE',k,n - k,-cone,a(1,k + 1),lda,w(imax, &
                                  kw + 1),ldw,cone,w(1,kw - 1),1)
                    ...
                    if (absakk >= alpha*colmax*(colmax/rowmax)) then
                       ...
                       kp = k
                    ! case(3)
                    else if (abs(real(w(imax,kw - 1),KIND=qp)) >= alpha*rowmax) then
                       ! interchange rows and columns k and imax, use 1-by-1
                       ! pivot block
                       kp = imax
                       ! copy column kw-1 of w to column kw of w
              ...
              if (max(absakk,colmax) == zero) then
                 ...
                 if (absakk >= alpha*colmax) then
                    ...
                    if (absakk >= alpha*colmax*(colmax/rowmax)) then
                       ...
                       kp = k
                    ! case(3)
                    else if (abs(real(w(imax,k + 1),KIND=qp)) >= alpha*rowmax) then
                       ! interchange rows and columns k and imax, use 1-by-1
                       ! pivot block
                       kp = imax
                       ! copy column k+1 of w to column k of w
     ...
     pure subroutine la_cheequb(uplo,n,a,lda,s,scond,amax,work,info)
     ...
     pure subroutine la_zheequb(uplo,n,a,lda,s,scond,amax,work,info)
        ...
        ! =====================================================================
           ...
           scond = max(smin,smlnum)/min(smax,bignum)
     end subroutine la_zheequb
#ifdef LA_WITH_XDP
     !> YHEEQUB: computes row and column scalings intended to equilibrate a
     !> Hermitian matrix A (with respect to the Euclidean norm) and reduce
     !> its condition number. The scale factors S are computed by the BIN
     !> algorithm (see references) so that the scaled matrix B with elements
     !> B(i,j) = S(i)*A(i,j)*S(j) has a condition number within a factor N of
     ...
     pure subroutine la_yheequb(uplo,n,a,lda,s,scond,amax,work,info)
        ...
        ! =====================================================================
           ...
           do iter = 1,max_iter
              ...
              avg = zero
              do i = 1,n
                 avg = avg + real(s(i)*work(i),KIND=xdp)
              end do
              avg = avg/n
              std = zero
              do i = n + 1,2*n
     ...
     pure subroutine la_wheequb(uplo,n,a,lda,s,scond,amax,work,info)
        ...
        ! =====================================================================
           ...
           do iter = 1,max_iter
              ...
              avg = zero
              do i = 1,n
                 avg = avg + real(s(i)*work(i),KIND=qp)
              end do
              avg = avg/n
              std = zero
              do i = n + 1,2*n
     ...
     pure subroutine la_chetrf(uplo,n,a,lda,ipiv,work,lwork,info)
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ! Scalar Arguments
        ...
        ! =====================================================================
           ...
           if (nb > 1 .and. nb < n) then
              ...
              if (lwork < iws) then
                 ...
                 nbmin = max(2,la_ilaenv(2,'CHETRF',uplo,n,-1,-1,-1))
              end if
           else
              iws = 1
           end if
           ...
           if (upper) then
              ! factorize a as u*d*u**h using the upper triangle of a
              ! k is the main loop index, decreasing from n to 1 in steps of
              ! kb, where kb is the number of columns factorized by la_clahef;
              ...
              if (k > nb) then
                 ...
                 call la_clahef(uplo,k,nb,kb,a,lda,ipiv,work,n,iinfo)
              else
                 ! use unblocked code to factorize columns 1:k of a
                 call la_chetf2(uplo,k,a,lda,ipiv,iinfo)
                 kb = k
              end if
              ...
              if (k <= n - nb) then
                 ...
                 call la_clahef(uplo,n - k + 1,nb,kb,a(k,k),lda,ipiv(k),work,n, &
                           iinfo)
              else
                 ! use unblocked code to factorize columns k:n of a
                 call la_chetf2(uplo,n - k + 1,a(k,k),lda,ipiv(k),iinfo)
                 kb = n - k + 1
              end if
              ! set info on the first occurrence of a zero pivot
     ...
     pure subroutine la_zhetrf(uplo,n,a,lda,ipiv,work,lwork,info)
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ! Scalar Arguments
        ...
        ! =====================================================================
           ...
           if (nb > 1 .and. nb < n) then
              ...
              if (lwork < iws) then
                 nb = max(lwork/ldwork,1)
                 nbmin = max(2,la_ilaenv(2,'ZHETRF',uplo,n,-1,-1,-1))
              end if
           else
              iws = 1
           end if
           ...
           if (upper) then
              ! factorize a as u*d*u**h using the upper triangle of a
              ! k is the main loop index, decreasing from n to 1 in steps of
              ! kb, where kb is the number of columns factorized by la_zlahef;
              ! kb is either nb or nb-1, or k for the last block
              k = n
              10 continue
              ! if k < 1, exit from loop
              ...
              if (k > nb) then
                 ! factorize columns k-kb+1:k of a and use blocked code to
                 ! update columns 1:k-kb
                 call la_zlahef(uplo,k,nb,kb,a,lda,ipiv,work,n,iinfo)
              else
                 ! use unblocked code to factorize columns 1:k of a
                 call la_zhetf2(uplo,k,a,lda,ipiv,iinfo)
                 kb = k
              end if
              ! set info on the first occurrence of a zero pivot
              if (info == 0 .and. iinfo > 0) info = iinfo
              ...
              if (k <= n - nb) then
                 ! factorize columns k:k+kb-1 of a and use blocked code to
                 ! update columns k+kb:n
                 call la_zlahef(uplo,n - k + 1,nb,kb,a(k,k),lda,ipiv(k),work,n, &
                           iinfo)
              else
                 ! use unblocked code to factorize columns k:n of a
                 call la_zhetf2(uplo,n - k + 1,a(k,k),lda,ipiv(k),iinfo)
                 kb = n - k + 1
              end if
              ! set info on the first occurrence of a zero pivot
              if (info == 0 .and. iinfo > 0) info = iinfo + k - 1
           ...
           work(1) = lwkopt
           return
     end subroutine la_zhetrf
#ifdef LA_WITH_XDP
     !> YHETRF: computes the factorization of a complex Hermitian matrix A
     !> using the Bunch-Kaufman diagonal pivoting method.  The form of the
     !> factorization is
     !> A = U*D*U**H  or  A = L*D*L**H
     !> where U (or L) is a product of permutation and unit upper (lower)
     ...
     pure subroutine la_yhetrf(uplo,n,a,lda,ipiv,work,lwork,info)
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ! Scalar Arguments
        ...
        ! =====================================================================
           ...
           if (nb > 1 .and. nb < n) then
              ...
              if (lwork < iws) then
                 nb = max(lwork/ldwork,1)
                 nbmin = max(2,la_ilaenv(2,'YHETRF',uplo,n,-1,-1,-1))
              end if
           else
              iws = 1
           end if
           ...
           if (upper) then
              ! factorize a as u*d*u**h using the upper triangle of a
              ! k is the main loop index, decreasing from n to 1 in steps of
              ! kb, where kb is the number of columns factorized by la_ylahef;
              ! kb is either nb or nb-1, or k for the last block
              k = n
              10 continue
              ! if k < 1, exit from loop
              ...
              if (k > nb) then
                 ! factorize columns k-kb+1:k of a and use blocked code to
                 ! update columns 1:k-kb
                 call la_ylahef(uplo,k,nb,kb,a,lda,ipiv,work,n,iinfo)
              else
                 ! use unblocked code to factorize columns 1:k of a
                 call la_yhetf2(uplo,k,a,lda,ipiv,iinfo)
                 kb = k
              end if
              ! set info on the first occurrence of a zero pivot
              if (info == 0 .and. iinfo > 0) info = iinfo
              ...
              if (k <= n - nb) then
                 ! factorize columns k:k+kb-1 of a and use blocked code to
                 ! update columns k+kb:n
                 call la_ylahef(uplo,n - k + 1,nb,kb,a(k,k),lda,ipiv(k),work,n, &
                           iinfo)
              else
                 ! use unblocked code to factorize columns k:n of a
                 call la_yhetf2(uplo,n - k + 1,a(k,k),lda,ipiv(k),iinfo)
                 kb = n - k + 1
              end if
              ! set info on the first occurrence of a zero pivot
              if (info == 0 .and. iinfo > 0) info = iinfo + k - 1
     ...
     pure subroutine la_whetrf(uplo,n,a,lda,ipiv,work,lwork,info)
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ! Scalar Arguments
        ...
        ! =====================================================================
           ...
           if (nb > 1 .and. nb < n) then
              ...
              if (lwork < iws) then
                 nb = max(lwork/ldwork,1)
                 nbmin = max(2,la_ilaenv(2,'WHETRF',uplo,n,-1,-1,-1))
              end if
           else
              iws = 1
           end if
           ...
           if (upper) then
              ! factorize a as u*d*u**h using the upper triangle of a
              ! k is the main loop index, decreasing from n to 1 in steps of
              ! kb, where kb is the number of columns factorized by la_wlahef;
              ! kb is either nb or nb-1, or k for the last block
              k = n
              10 continue
              ! if k < 1, exit from loop
              ...
              if (k > nb) then
                 ! factorize columns k-kb+1:k of a and use blocked code to
                 ! update columns 1:k-kb
                 call la_wlahef(uplo,k,nb,kb,a,lda,ipiv,work,n,iinfo)
              else
                 ! use unblocked code to factorize columns 1:k of a
                 call la_whetf2(uplo,k,a,lda,ipiv,iinfo)
                 kb = k
              end if
              ! set info on the first occurrence of a zero pivot
              if (info == 0 .and. iinfo > 0) info = iinfo
              ...
              if (k <= n - nb) then
                 ! factorize columns k:k+kb-1 of a and use blocked code to
                 ! update columns k+kb:n
                 call la_wlahef(uplo,n - k + 1,nb,kb,a(k,k),lda,ipiv(k),work,n, &
                           iinfo)
              else
                 ! use unblocked code to factorize columns k:n of a
                 call la_whetf2(uplo,n - k + 1,a(k,k),lda,ipiv(k),iinfo)
                 kb = n - k + 1
              end if
              ! set info on the first occurrence of a zero pivot
              if (info == 0 .and. iinfo > 0) info = iinfo + k - 1
     ...
     pure subroutine la_chetrs(uplo,n,nrhs,a,lda,ipiv,b,ldb,info)
        use la_constants_sp
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
        ...
        ! =====================================================================
           ...
           return
     end subroutine la_chetrs
     !> ZHETRS: solves a system of linear equations A*X = B with a complex
     !> Hermitian matrix A using the factorization A = U*D*U**H or
     !> A = L*D*L**H computed by ZHETRF.

     pure subroutine la_zhetrs(uplo,n,nrhs,a,lda,ipiv,b,ldb,info)
        use la_constants_dp
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
        ...
        ! =====================================================================
           ...
           return
     end subroutine la_zhetrs
#ifdef LA_WITH_XDP
     !> YHETRS: solves a system of linear equations A*X = B with a complex
     !> Hermitian matrix A using the factorization A = U*D*U**H or
     !> A = L*D*L**H computed by YHETRF.

     pure subroutine la_yhetrs(uplo,n,nrhs,a,lda,ipiv,b,ldb,info)
        use la_constants_xdp
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
        ...
        ! =====================================================================
           ...
           if (upper) then
              ...
              if (ipiv(k) > 0) then
                 ...
                 ! multiply by the inverse of the diagonal block.
                 s = real(cone,KIND=xdp)/real(a(k,k),KIND=xdp)
                 call la_yxscal(nrhs,s,b(k,1),ldb)
                 k = k - 1
              else
                 ! 2 x 2 diagonal block
                 ! interchange rows k-1 and -ipiv(k).
              ...
              if (ipiv(k) > 0) then
                 ...
                 ! stored in column k of a.
                 if (k > 1) then
                    call la_ylacgv(nrhs,b(k,1),ldb)
                    call la_ygemv('CONJUGATE TRANSPOSE',k - 1,nrhs,-cone,b,ldb,a(1,k), &
                              1,cone,b(k,1),ldb)
                    call la_ylacgv(nrhs,b(k,1),ldb)
                 end if
                 ! interchange rows k and ipiv(k).
                 ...
                 if (k > 1) then
                    ...
                    call la_ygemv('CONJUGATE TRANSPOSE',k - 1,nrhs,-cone,b,ldb,a(1,k), &
                              1,cone,b(k,1),ldb)
                    call la_ylacgv(nrhs,b(k,1),ldb)
                    call la_ylacgv(nrhs,b(k + 1,1),ldb)
                    call la_ygemv('CONJUGATE TRANSPOSE',k - 1,nrhs,-cone,b,ldb,a(1,k + 1) &
                              ,1,cone,b(k + 1,1),ldb)
                    call la_ylacgv(nrhs,b(k + 1,1),ldb)
                 end if
                 ! interchange rows k and -ipiv(k).
              ...
              if (ipiv(k) > 0) then
                 ...
                 if (k < n) then
                    call la_ylacgv(nrhs,b(k,1),ldb)
                    call la_ygemv('CONJUGATE TRANSPOSE',n - k,nrhs,-cone,b(k + 1,1),ldb, &
                              a(k + 1,k),1,cone,b(k,1),ldb)
                    call la_ylacgv(nrhs,b(k,1),ldb)
                 end if
                 ! interchange rows k and ipiv(k).
           ...
           return
     end subroutine la_yhetrs
#endif
#ifdef LA_WITH_QP
     !> WHETRS: solves a system of linear equations A*X = B with a complex
     !> Hermitian matrix A using the factorization A = U*D*U**H or
     !> A = L*D*L**H computed by WHETRF.

     pure subroutine la_whetrs(uplo,n,nrhs,a,lda,ipiv,b,ldb,info)
        use la_constants_qp
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
     ...
     pure subroutine la_chetrs2(uplo,n,nrhs,a,lda,ipiv,b,ldb,work,info)
        use la_constants_sp
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
     ...
     pure subroutine la_zhetrs2(uplo,n,nrhs,a,lda,ipiv,b,ldb,work,info)
        ...
        ! compute (l**h \ b) -> b   [ l**h \ (d \ (l \p**t * b) ) ]
           ...
           return
     end subroutine la_zhetrs2
#ifdef LA_WITH_XDP
     !> YHETRS2: solves a system of linear equations A*X = B with a complex
     !> Hermitian matrix A using the factorization A = U*D*U**H or
     !> A = L*D*L**H computed by YHETRF and converted by YSYCONV.

     pure subroutine la_yhetrs2(uplo,n,nrhs,a,lda,ipiv,b,ldb,work,info)
        use la_constants_xdp
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
        ...
        ! compute (l**h \ b) -> b   [ l**h \ (d \ (l \p**t * b) ) ]
           ...
           return
     end subroutine la_yhetrs2
#endif
#ifdef LA_WITH_QP
     !> WHETRS2: solves a system of linear equations A*X = B with a complex
     !> Hermitian matrix A using the factorization A = U*D*U**H or
     !> A = L*D*L**H computed by WHETRF and converted by WSYCONV.

     pure subroutine la_whetrs2(uplo,n,nrhs,a,lda,ipiv,b,ldb,work,info)
        use la_constants_qp
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
     ...
     pure subroutine la_chptrs(uplo,n,nrhs,ap,ipiv,b,ldb,info)
        use la_constants_sp
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
        ...
        ! =====================================================================
           ...
           if (upper) then
              ...
              if (ipiv(k) > 0) then
                 ...
                 ! multiply by inv(u(k)), where u(k) is the transformation
                 ! stored in column k of a.
                 call la_cgeru(k - 1,nrhs,-cone,ap(kc),1,b(k,1),ldb,b(1,1),ldb)
                 ...
                 ! multiply by inv(u(k)), where u(k) is the transformation
                 ! stored in columns k-1 and k of a.
                 call la_cgeru(k - 2,nrhs,-cone,ap(kc),1,b(k,1),ldb,b(1,1),ldb)

                 call la_cgeru(k - 2,nrhs,-cone,ap(kc - (k - 1)),1,b(k - 1,1),ldb,b(1, &
                           1),ldb)
                 ! multiply by the inverse of the diagonal block.
                 akm1k = ap(kc + k - 2)
                 akm1 = ap(kc - 1)/akm1k
              ...
              if (ipiv(k) > 0) then
                 ...
                 if (k > 1) then
                    call la_clacgv(nrhs,b(k,1),ldb)
                    call la_cgemv('CONJUGATE TRANSPOSE',k - 1,nrhs,-cone,b,ldb,ap(kc), &
                              1,cone,b(k,1),ldb)
                    call la_clacgv(nrhs,b(k,1),ldb)
                 end if
                 ! interchange rows k and ipiv(k).
                 ...
                 if (k > 1) then
                    ...
                    call la_clacgv(nrhs,b(k,1),ldb)
                    call la_clacgv(nrhs,b(k + 1,1),ldb)
                    call la_cgemv('CONJUGATE TRANSPOSE',k - 1,nrhs,-cone,b,ldb,ap(kc + k), &
                               1,cone,b(k + 1,1),ldb)
                    call la_clacgv(nrhs,b(k + 1,1),ldb)
                 end if
                 ! interchange rows k and -ipiv(k).
              ...
              if (ipiv(k) > 0) then
                 ...
                 ! multiply by inv(l(k)), where l(k) is the transformation
                 ! stored in column k of a.
                 if (k < n) call la_cgeru(n - k,nrhs,-cone,ap(kc + 1),1,b(k,1),ldb,b(k + &
                           1,1),ldb)
                 ! multiply by the inverse of the diagonal block.
                 ...
                 ! stored in columns k and k+1 of a.
                 if (k < n - 1) then
                    call la_cgeru(n - k - 1,nrhs,-cone,ap(kc + 2),1,b(k,1),ldb,b(k + 2, &
                              1),ldb)
                    call la_cgeru(n - k - 1,nrhs,-cone,ap(kc + n - k + 2),1,b(k + 1,1),ldb,b( &
                              k + 2,1),ldb)
                 end if
                 ! multiply by the inverse of the diagonal block.
                 akm1k = ap(kc + 1)
           ...
           return
     end subroutine la_chptrs
     !> ZHPTRS: solves a system of linear equations A*X = B with a complex
     !> Hermitian matrix A stored in packed format using the factorization
     !> A = U*D*U**H or A = L*D*L**H computed by ZHPTRF.

     pure subroutine la_zhptrs(uplo,n,nrhs,ap,ipiv,b,ldb,info)
        use la_constants_dp
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
        ...
        ! =====================================================================
           ...
           if (upper) then
              ...
              if (ipiv(k) > 0) then
                 ...
                 ! multiply by inv(u(k)), where u(k) is the transformation
                 ! stored in column k of a.
                 call la_zgeru(k - 1,nrhs,-cone,ap(kc),1,b(k,1),ldb,b(1,1),ldb)
                 ...
                 ! multiply by inv(u(k)), where u(k) is the transformation
                 ! stored in columns k-1 and k of a.
                 call la_zgeru(k - 2,nrhs,-cone,ap(kc),1,b(k,1),ldb,b(1,1),ldb)

                 call la_zgeru(k - 2,nrhs,-cone,ap(kc - (k - 1)),1,b(k - 1,1),ldb,b(1, &
                           1),ldb)
                 ! multiply by the inverse of the diagonal block.
                 akm1k = ap(kc + k - 2)
                 akm1 = ap(kc - 1)/akm1k
              ...
              if (ipiv(k) > 0) then
                 ...
                 if (k > 1) then
                    call la_zlacgv(nrhs,b(k,1),ldb)
                    call la_zgemv('CONJUGATE TRANSPOSE',k - 1,nrhs,-cone,b,ldb,ap(kc), &
                              1,cone,b(k,1),ldb)
                    call la_zlacgv(nrhs,b(k,1),ldb)
                 end if
                 ! interchange rows k and ipiv(k).
                 ...
                 if (k > 1) then
                    ...
                    call la_zlacgv(nrhs,b(k,1),ldb)
                    call la_zlacgv(nrhs,b(k + 1,1),ldb)
                    call la_zgemv('CONJUGATE TRANSPOSE',k - 1,nrhs,-cone,b,ldb,ap(kc + k), &
                               1,cone,b(k + 1,1),ldb)
                    call la_zlacgv(nrhs,b(k + 1,1),ldb)
                 end if
                 ! interchange rows k and -ipiv(k).
              ...
              if (ipiv(k) > 0) then
                 ...
                 ! multiply by inv(l(k)), where l(k) is the transformation
                 ! stored in column k of a.
                 if (k < n) call la_zgeru(n - k,nrhs,-cone,ap(kc + 1),1,b(k,1),ldb,b(k + &
                           1,1),ldb)
                 ! multiply by the inverse of the diagonal block.
                 ...
                 ! stored in columns k and k+1 of a.
                 if (k < n - 1) then
                    call la_zgeru(n - k - 1,nrhs,-cone,ap(kc + 2),1,b(k,1),ldb,b(k + 2, &
                              1),ldb)
                    call la_zgeru(n - k - 1,nrhs,-cone,ap(kc + n - k + 2),1,b(k + 1,1),ldb,b( &
                              k + 2,1),ldb)
                 end if
                 ! multiply by the inverse of the diagonal block.
                 akm1k = ap(kc + 1)
           ...
           return
     end subroutine la_zhptrs
#ifdef LA_WITH_XDP
     !> YHPTRS: solves a system of linear equations A*X = B with a complex
     !> Hermitian matrix A stored in packed format using the factorization
     !> A = U*D*U**H or A = L*D*L**H computed by YHPTRF.

     pure subroutine la_yhptrs(uplo,n,nrhs,ap,ipiv,b,ldb,info)
        use la_constants_xdp
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
        ...
        ! =====================================================================
           ...
           if (upper) then
              ...
              if (ipiv(k) > 0) then
                 ...
                 ! multiply by inv(u(k)), where u(k) is the transformation
                 ! stored in column k of a.
                 call la_ygeru(k - 1,nrhs,-cone,ap(kc),1,b(k,1),ldb,b(1,1),ldb)

                 ! multiply by the inverse of the diagonal block.
                 s = real(cone,KIND=xdp)/real(ap(kc + k - 1),KIND=xdp)
                 call la_yxscal(nrhs,s,b(k,1),ldb)
                 k = k - 1
              else
                 ! 2 x 2 diagonal block
                 ...
                 ! multiply by inv(u(k)), where u(k) is the transformation
                 ! stored in columns k-1 and k of a.
                 call la_ygeru(k - 2,nrhs,-cone,ap(kc),1,b(k,1),ldb,b(1,1),ldb)

                 call la_ygeru(k - 2,nrhs,-cone,ap(kc - (k - 1)),1,b(k - 1,1),ldb,b(1, &
                           1),ldb)
                 ! multiply by the inverse of the diagonal block.
                 akm1k = ap(kc + k - 2)
                 akm1 = ap(kc - 1)/akm1k
              ...
              if (ipiv(k) > 0) then
                 ...
                 if (k > 1) then
                    call la_ylacgv(nrhs,b(k,1),ldb)
                    call la_ygemv('CONJUGATE TRANSPOSE',k - 1,nrhs,-cone,b,ldb,ap(kc), &
                              1,cone,b(k,1),ldb)
                    call la_ylacgv(nrhs,b(k,1),ldb)
                 end if
                 ! interchange rows k and ipiv(k).
                 ...
                 if (k > 1) then
                    ...
                    call la_ylacgv(nrhs,b(k,1),ldb)
                    call la_ylacgv(nrhs,b(k + 1,1),ldb)
                    call la_ygemv('CONJUGATE TRANSPOSE',k - 1,nrhs,-cone,b,ldb,ap(kc + k), &
                               1,cone,b(k + 1,1),ldb)
                    call la_ylacgv(nrhs,b(k + 1,1),ldb)
                 end if
                 ! interchange rows k and -ipiv(k).
              ...
              if (ipiv(k) > 0) then
                 ...
                 ! multiply by inv(l(k)), where l(k) is the transformation
                 ! stored in column k of a.
                 if (k < n) call la_ygeru(n - k,nrhs,-cone,ap(kc + 1),1,b(k,1),ldb,b(k + &
                           1,1),ldb)
                 ! multiply by the inverse of the diagonal block.
                 s = real(cone,KIND=xdp)/real(ap(kc),KIND=xdp)
                 call la_yxscal(nrhs,s,b(k,1),ldb)
                 kc = kc + n - k + 1
                 k = k + 1
              else
                 ...
                 ! stored in columns k and k+1 of a.
                 if (k < n - 1) then
                    call la_ygeru(n - k - 1,nrhs,-cone,ap(kc + 2),1,b(k,1),ldb,b(k + 2, &
                              1),ldb)
                    call la_ygeru(n - k - 1,nrhs,-cone,ap(kc + n - k + 2),1,b(k + 1,1),ldb,b( &
                              k + 2,1),ldb)
                 end if
                 ! multiply by the inverse of the diagonal block.
                 akm1k = ap(kc + 1)
           ...
           return
     end subroutine la_yhptrs
#endif
#ifdef LA_WITH_QP
     !> WHPTRS: solves a system of linear equations A*X = B with a complex
     !> Hermitian matrix A stored in packed format using the factorization
     !> A = U*D*U**H or A = L*D*L**H computed by WHPTRF.

     pure subroutine la_whptrs(uplo,n,nrhs,ap,ipiv,b,ldb,info)
        use la_constants_qp
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
        ...
        ! =====================================================================
           ...
           if (upper) then
              ...
              if (ipiv(k) > 0) then
                 ...
                 ! multiply by inv(u(k)), where u(k) is the transformation
                 ! stored in column k of a.
                 call la_wgeru(k - 1,nrhs,-cone,ap(kc),1,b(k,1),ldb,b(1,1),ldb)
                 ...
                 ! multiply by inv(u(k)), where u(k) is the transformation
                 ! stored in columns k-1 and k of a.
                 call la_wgeru(k - 2,nrhs,-cone,ap(kc),1,b(k,1),ldb,b(1,1),ldb)

                 call la_wgeru(k - 2,nrhs,-cone,ap(kc - (k - 1)),1,b(k - 1,1),ldb,b(1, &
                           1),ldb)
                 ! multiply by the inverse of the diagonal block.
                 akm1k = ap(kc + k - 2)
                 akm1 = ap(kc - 1)/akm1k
              ...
              if (ipiv(k) > 0) then
                 ...
                 if (k > 1) then
                    call la_wlacgv(nrhs,b(k,1),ldb)
                    call la_wgemv('CONJUGATE TRANSPOSE',k - 1,nrhs,-cone,b,ldb,ap(kc), &
                              1,cone,b(k,1),ldb)
                    call la_wlacgv(nrhs,b(k,1),ldb)
                 end if
                 ! interchange rows k and ipiv(k).
                 ...
                 if (k > 1) then
                    ...
                    call la_wlacgv(nrhs,b(k,1),ldb)
                    call la_wlacgv(nrhs,b(k + 1,1),ldb)
                    call la_wgemv('CONJUGATE TRANSPOSE',k - 1,nrhs,-cone,b,ldb,ap(kc + k), &
                               1,cone,b(k + 1,1),ldb)
                    call la_wlacgv(nrhs,b(k + 1,1),ldb)
                 end if
                 ! interchange rows k and -ipiv(k).
              ...
              if (ipiv(k) > 0) then
                 ...
                 ! multiply by inv(l(k)), where l(k) is the transformation
                 ! stored in column k of a.
                 if (k < n) call la_wgeru(n - k,nrhs,-cone,ap(kc + 1),1,b(k,1),ldb,b(k + &
                           1,1),ldb)
                 ! multiply by the inverse of the diagonal block.
                 s = real(cone,KIND=qp)/real(ap(kc),KIND=qp)
                 call la_wqscal(nrhs,s,b(k,1),ldb)
                 kc = kc + n - k + 1
                 k = k + 1
              else
                 ...
                 ! stored in columns k and k+1 of a.
                 if (k < n - 1) then
                    call la_wgeru(n - k - 1,nrhs,-cone,ap(kc + 2),1,b(k,1),ldb,b(k + 2, &
                              1),ldb)
                    call la_wgeru(n - k - 1,nrhs,-cone,ap(kc + n - k + 2),1,b(k + 1,1),ldb,b( &
                              k + 2,1),ldb)
                 end if
                 ! multiply by the inverse of the diagonal block.
                 akm1k = ap(kc + 1)
     ...
     pure subroutine la_checon(uplo,n,a,lda,ipiv,anorm,rcond,work,info)
        use la_constants_sp
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
        ...
        ! =====================================================================
           ...
           if (kase /= 0) then
              ...
              call la_chetrs(uplo,n,1,a,lda,ipiv,work,n,info)
              go to 30
           end if
           ! compute the estimate of the reciprocal condition number.
           if (ainvnm /= zero) rcond = (one/ainvnm)/anorm
           return
     end subroutine la_checon
     ...
     pure subroutine la_zhecon(uplo,n,a,lda,ipiv,anorm,rcond,work,info)
        use la_constants_dp
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
        ...
        ! =====================================================================
           ...
           if (kase /= 0) then
              ! multiply by inv(l*d*l**h) or inv(u*d*u**h).
              call la_zhetrs(uplo,n,1,a,lda,ipiv,work,n,info)
              go to 30
           end if
           ! compute the estimate of the reciprocal condition number.
           if (ainvnm /= zero) rcond = (one/ainvnm)/anorm
           return
     end subroutine la_zhecon
#ifdef LA_WITH_XDP
     !> YHECON: estimates the reciprocal of the condition number of a complex
     !> Hermitian matrix A using the factorization A = U*D*U**H or
     !> A = L*D*L**H computed by YHETRF.
     !> An estimate is obtained for norm(inv(A)), and the reciprocal of the
     !> condition number is computed as RCOND = 1 / (ANORM * norm(inv(A))).

     pure subroutine la_yhecon(uplo,n,a,lda,ipiv,anorm,rcond,work,info)
        use la_constants_xdp
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
        ...
        ! =====================================================================
           ...
           if (kase /= 0) then
              ! multiply by inv(l*d*l**h) or inv(u*d*u**h).
              call la_yhetrs(uplo,n,1,a,lda,ipiv,work,n,info)
              go to 30
           end if
           ! compute the estimate of the reciprocal condition number.
           if (ainvnm /= zero) rcond = (one/ainvnm)/anorm
           return
     end subroutine la_yhecon
#endif
#ifdef LA_WITH_QP
     ...
     pure subroutine la_whecon(uplo,n,a,lda,ipiv,anorm,rcond,work,info)
        use la_constants_qp
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
        ...
        ! =====================================================================
           ...
           if (kase /= 0) then
              ! multiply by inv(l*d*l**h) or inv(u*d*u**h).
              call la_whetrs(uplo,n,1,a,lda,ipiv,work,n,info)
              go to 30
           end if
           ! compute the estimate of the reciprocal condition number.
           if (ainvnm /= zero) rcond = (one/ainvnm)/anorm
           return
     end subroutine la_whecon
#endif
     ...
     pure subroutine la_cherfs(uplo,n,nrhs,a,lda,af,ldaf,ipiv,b,ldb,x,ldx,ferr, &
        ...
        ! =====================================================================
           ...
           loop_140: do j = 1,nrhs
              ...
              if (berr(j) > eps .and. two*berr(j) <= lstres .and. count <= itmax) then
                 ...
                 call la_chetrs(uplo,n,1,af,ldaf,ipiv,work,n,info)
           ...
           return
     end subroutine la_cherfs
     !> ZHERFS: improves the computed solution to a system of linear
     !> equations when the coefficient matrix is Hermitian indefinite, and
     !> provides error bounds and backward error estimates for the solution.

     pure subroutine la_zherfs(uplo,n,nrhs,a,lda,af,ldaf,ipiv,b,ldb,x,ldx,ferr, &
               berr,work,rwork,info)
        ...
        ! =====================================================================
           ...
           loop_140: do j = 1,nrhs
              ...
              if (berr(j) > eps .and. two*berr(j) <= lstres .and. count <= itmax) then
                 ! update solution and try again.
                 call la_zhetrs(uplo,n,1,af,ldaf,ipiv,work,n,info)
           ...
           return
     end subroutine la_zherfs
#ifdef LA_WITH_XDP
     !> YHERFS: improves the computed solution to a system of linear
     !> equations when the coefficient matrix is Hermitian indefinite, and
     !> provides error bounds and backward error estimates for the solution.

     pure subroutine la_yherfs(uplo,n,nrhs,a,lda,af,ldaf,ipiv,b,ldb,x,ldx,ferr, &
               berr,work,rwork,info)
        ...
        ! =====================================================================
           ...
           loop_140: do j = 1,nrhs
              ...
              call la_yhemv(uplo,n,-cone,a,lda,x(1,j),1,cone,work,1)
              ! compute componentwise relative backward error from formula
              ! max(i) ( abs(r(i)) / ( abs(a)*abs(x) + abs(b) )(i) )
              ! where abs(z) is the componentwise absolute value of the matrix
              ! or vector z.  if the i-th component of the denominator is less
              ...
              if (upper) then
                 do k = 1,n
                    ...
                    rwork(k) = rwork(k) + abs(real(a(k,k),KIND=xdp))*xk + s
                 end do
              else
                 do k = 1,n
                    s = zero
                    xk = cabs1(x(k,j))
                    rwork(k) = rwork(k) + abs(real(a(k,k),KIND=xdp))*xk
                    do i = k + 1,n
                       rwork(i) = rwork(i) + cabs1(a(i,k))*xk
                       s = s + cabs1(a(i,k))*cabs1(x(i,j))
                    end do
              ...
              if (berr(j) > eps .and. two*berr(j) <= lstres .and. count <= itmax) then
                 ! update solution and try again.
                 call la_yhetrs(uplo,n,1,af,ldaf,ipiv,work,n,info)
     ...
     pure subroutine la_wherfs(uplo,n,nrhs,a,lda,af,ldaf,ipiv,b,ldb,x,ldx,ferr, &
               berr,work,rwork,info)
        ...
        ! =====================================================================
           ...
           loop_140: do j = 1,nrhs
              ...
              if (berr(j) > eps .and. two*berr(j) <= lstres .and. count <= itmax) then
                 ! update solution and try again.
                 call la_whetrs(uplo,n,1,af,ldaf,ipiv,work,n,info)
     ...
     pure subroutine la_chpcon(uplo,n,ap,ipiv,anorm,rcond,work,info)
        use la_constants_sp
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
        ...
        ! =====================================================================
           ...
           if (kase /= 0) then
              ! multiply by inv(l*d*l**h) or inv(u*d*u**h).
              call la_chptrs(uplo,n,1,ap,ipiv,work,n,info)
              go to 30
           end if
           ! compute the estimate of the reciprocal condition number.
           if (ainvnm /= zero) rcond = (one/ainvnm)/anorm
           return
     end subroutine la_chpcon
     ...
     pure subroutine la_zhpcon(uplo,n,ap,ipiv,anorm,rcond,work,info)
        use la_constants_dp
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
        ...
        ! =====================================================================
           ...
           if (kase /= 0) then
              ! multiply by inv(l*d*l**h) or inv(u*d*u**h).
              call la_zhptrs(uplo,n,1,ap,ipiv,work,n,info)
              go to 30
           end if
           ! compute the estimate of the reciprocal condition number.
           if (ainvnm /= zero) rcond = (one/ainvnm)/anorm
           return
     end subroutine la_zhpcon
#ifdef LA_WITH_XDP
     !> YHPCON: estimates the reciprocal of the condition number of a complex
     !> Hermitian packed matrix A using the factorization A = U*D*U**H or
     !> A = L*D*L**H computed by YHPTRF.
     !> An estimate is obtained for norm(inv(A)), and the reciprocal of the
     !> condition number is computed as RCOND = 1 / (ANORM * norm(inv(A))).

     pure subroutine la_yhpcon(uplo,n,ap,ipiv,anorm,rcond,work,info)
        use la_constants_xdp
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
        ...
        ! =====================================================================
           ...
           if (kase /= 0) then
              ! multiply by inv(l*d*l**h) or inv(u*d*u**h).
              call la_yhptrs(uplo,n,1,ap,ipiv,work,n,info)
              go to 30
           end if
           ! compute the estimate of the reciprocal condition number.
           if (ainvnm /= zero) rcond = (one/ainvnm)/anorm
           return
     end subroutine la_yhpcon
#endif
#ifdef LA_WITH_QP
     ...
     pure subroutine la_whpcon(uplo,n,ap,ipiv,anorm,rcond,work,info)
        use la_constants_qp
        ! -- lapack computational routine --
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
        ...
        ! =====================================================================
           ...
           if (kase /= 0) then
              ! multiply by inv(l*d*l**h) or inv(u*d*u**h).
              call la_whptrs(uplo,n,1,ap,ipiv,work,n,info)
              go to 30
           end if
           ! compute the estimate of the reciprocal condition number.
           if (ainvnm /= zero) rcond = (one/ainvnm)/anorm
           return
     end subroutine la_whpcon
#endif
